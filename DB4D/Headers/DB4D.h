...
class ENTITY_API PrimKey : public XBOX::IRefCountable
{
	public:
		//typedef std::vector<XBOX::VValueSingle*> parts;
		typedef XBOX::VectorOfVValue parts;
		...
		XBOX::VError PutInto(XBOX::VStream* outStream);
		XBOX::VError GetFrom(XBOX::VStream* inStream);

		XBOX::VError GetFrom(void* fic, bool oldOne);

		///Returns the required number of bytes to write the instance into a VStream
		...
		const parts& GetValues() const
		{
			return fParts;
		}

		bool operator < (const PrimKey& other) const;

		bool IsValid() const;

		void AddPart(XBOX::VValueSingle* part);

		void GetAsString(XBOX::VString& outKey, bool autoQuote) const;
...
public:
	...
	virtual	XBOX::VError			DescribePermissionsForSchema (
		...
		/** @brief	Registers a task to the user. Returns false if the UID is not null but no matching user was found */
	virtual	bool					RegisterVTaskForRemoteUID(XBOX::VTask *inTask, const XBOX::VUUID& inRemoteUID, const XBOX::VUUID& inDB4DBaseContextUID) = 0;
	virtual bool					UnregisterVTaskForRemoteUID(XBOX::VTask *inTask, const XBOX::VUUID& inRemoteUID) = 0;
	...
	virtual	XBOX::VError			AppendJSContextRequiredScript( RIApplicationRef inApplicationRef, const XBOX::VFilePath& inPath) = 0;

			/**	@brief	Returns some informations about the JavaScript contexts using for the specified application */
	virtual	XBOX::VError			GetJSContextInformations( RIApplicationRef inApplicationRef, XBOX::VValueBag& outBag) = 0;
	...
	virtual bool					AcceptRestConnection(const XBOX::VString& inUserName, const XBOX::VString& inPassWord, const XBOX::VUUID& inSessionUUID, const XBOX::VString& inPeerAddress, bool passIsHashed) = 0;
			/** @brief	the application (usually 4D) should check is a REST connection
						is accepted for this user/password
			*/

	virtual void					CloseSomeRestConnection(const std::vector<XBOX::VUUID>& inSessionUUIDs) = 0;
			/** @brief	the application (usually 4D) should close REST session for specified uuids
			*/

	virtual bool					RetainOneRestLicense() = 0;
	...
	virtual void					ReleaseRestLicenses( sLONG inHowMany) = 0;
	...
	virtual XBOX::VProgressIndicator*	CreateProgressIndicator() = 0;
			/** @brief	the application should return a fresh progress indicator instance
			*/

	virtual XBOX::VFileSystemNamespace* RetainRuntimeFileSystemNamespace(RIApplicationRef inApplicationRef) = 0;
};
...
public:
	 ...
	 */	
	virtual XBOX::VError Initialize(const XBOX::VValueBag& inBag, const  XBOX::VFilePath& inBaseFolder) = 0;

	/**
	* \brief Fully initialize this instance from a JSON object.
	* \param inObject the JS object carrying ALL the init values.
	...
	* \see CheckValidity()
	*/
	virtual XBOX::VError Initialize(const XBOX::VJSObject& inObject) = 0;

	/**
	 * \brief Creates a clone of this instance
	 */
	virtual IBackupSettings* Clone()const = 0;

	/**
	* \brief Partially initialize some properties of this instance from a JSON object.
	* \details this method can be called using only a subset of initializations keys enabling to partially reset the instance
	* and preserve some values, e.g. after cloning a fully initilized instance that one wants to customize.
	* \param inObject the JS object carrying the init values.
	* \return false if the final object is invalid state after the new values have been taken into account
	* \return true if the object is valid after the new values have been taken into account
	* \see CheckValidity()
	*/
	virtual XBOX::VError FromJSObject(const XBOX::VJSObject& inObject) = 0;
	...
	*/
	virtual int GetMaxRetainedBackups(bool& outKeepAll)const = 0;
...
public:
	...
	virtual XBOX::VError Initialize(const XBOX::VFile& inTableDefFile) = 0;
	/**
	 ...
	 */
	virtual bool ParseJournal(XBOX::VFile& inJournalFile, XBOX::VJSONArray& outOperations, IDB4D_DataToolsIntf* inProgressLog) = 0;
	...
	/**
	 * Reads the journal path and UUID from an existing data file
	 * \param inDataFile the data file to read from
	 * \outJournalPath the resulting journal path
	 * \outDataLink the UUID linking to the data file
	 */
	virtual XBOX::VError ReadJournalInfoFromDataFile(XBOX::VFile& inDataFile, XBOX::VFilePath& outJournalPath, XBOX::VUUID& outDataLink) = 0;
	
	/**
	 * \brief Resets a data file's journal
	 * \param inDataFile the data file which journal must be reset
	 * \param outFormerJournalPath name of safe copy of the data file's journal, unmodified if no safe copy has been made
	 */
	virtual XBOX::VError ResetDataFileJournal(XBOX::VFile& inDataFile, XBOX::VFilePath& outFormerJournalPath, XBOX::VFilePath& outCurrentJournalPath) = 0;
...
public:
	 ...
	 * \return false if an error occured in the process
	 */
	virtual bool BackupClosedDatabase(XBOX::VFile& inModelFile,XBOX::VFile& inDataFile,const IBackupSettings& inBackupSettings,XBOX::VFilePath* outManifestFilePath,IDB4D_DataToolsIntf* inBackupEventLog) = 0;
	...
	*/
	virtual bool RestoreDataStore(XBOX::VFolder& inDamagedDataFolder, const XBOX::VFolder& inDataFolderToRestoreFrom, const XBOX::VFile* inCurrentJournal, const XBOX::VFile* inJournalToRestoreFrom, IDB4D_DataToolsIntf* inBackupEventLog,
		XBOX::VFilePath* outDamagedDFCopy, XBOX::VFilePath* outReplacedJournalCopy) = 0;
	 ...
	 * </pre>
	 */
	virtual bool RestoreDataFolderContent(const XBOX::VFilePath& inBackupManifestPath,const XBOX::VFilePath& inRestoreFolderPath,IDB4D_DataToolsIntf* inProgress) = 0;
...
typedef enum {  DB4D_Log_CreateRecord = 1,
				...
				DB4D_Log_CreateRecordWithPrimKey = 24,
				DB4D_Log_ModifyRecordWithPrimKey = 25,
				DB4D_Log_DeleteRecordWithPrimKey = 26,
				DB4D_Log_CreateBlobWithPrimKey = 27,
				DB4D_Log_ModifyBlobWithPrimKey = 28,
				DB4D_Log_DeleteBlobWithPrimKey = 29,
				DB4D_Log_StartCommit = 30,
				DB4D_Log_EndCommit = 31,
...
const sLONG DB4D_Index_OnObj = 5; // Index on object with multiple path indexed
...
const	sLONG DB4D_Index_BtreeObjVals = 8;
...
enum
{
	...
	kDB4DTaskKind_Mirror			= 'DB4M',
	kDB4DTaskKind_Listener			= 'DB4L',
};
...
enum { PrimKeyState_notVerified = 0, PrimKeyState_Valid, PrimKeyState_NonUnique, PrimKeyState_SomeNULLs, PrimKeyState_MissingPrimKeyDef };
...
typedef enum
{
	DB4D_RemoteCat_AllOK = 0,
	DB4D_RemoteCat_FileIsBad = 1,
	DB4D_RemoteCat_CannotConnect = 2,
	DB4D_RemoteCat_PrivilegeError = 3
} DB4D_RemoteCat_Problems;
...
typedef enum {
	...
	DB4D_SaveNewRecord_Trigger		= 1,
	DB4D_SaveExistingRecord_Trigger	= 2,
	DB4D_DeleteRecord_Trigger		= 3,
...
class DB4D_API DB4DTriggerManager
{
	public:
		...
		// IsTriggerExisting() purpose is rather to tell DB4D if a trigger needs to be called than to check for actual existence.
		// There is no need (return false) to call a trigger if none is defined or if there is one but it is disabled.
...
//	virtual XBOX::VError GetFrom(XBOX::VStream& inStream) = 0;
	...
	virtual XBOX::VJSONArray* ToJSON(CDB4DBaseContext* context, UniChar wildcharReplace) const = 0;
};
...
class ENTITY_API QueryParamElement
{
	public:
		...
		QueryParamElement(XBOX::VJSONArray* inArray);
...
public:
	...
	virtual DB4DNetworkManager* CreateRemoteTCPSession(const XBOX::VString& inServerName, uWORD inPortNum, VErrorDB4D& outerr, bool inSSL = false) = 0;
...
*/
	...
	*/
	...
	/** @brief	Release remaining contexts for an orphaned (i.e. dying) user. */
	virtual void	ReleaseContextsOfOrphanedUser(const XBOX::VUUID& inUserID) = 0;
	...
	virtual VErrorDB4D StartServer(const XBOX::VString& inServerName, uWORD inPortNum, bool inSSL = false) = 0;
	...
	virtual void PatchTableAndFieldNames(XBOX::VValueBag *inDefinition_bag,const std::vector<XBOX::VString>& existingTableNames, const std::vector<XBOX::VString>& existingFieldNames, std::map<XBOX::VString,XBOX::VString> &tables_map,std::map<XBOX::VString,XBOX::VString> &fields_map) const = 0;
	virtual void GetUniqueName( const std::vector<XBOX::VString>& inAlreadyUsedNames, const XBOX::VString &inName, XBOX::VString &outUniqueName, bool inWithEllipse, XBOX::VIndex inMaxLength) const = 0;
	...
	virtual	XBOX::VJSObject		CreateJSDatabaseForMaintenanceObject( const XBOX::VJSContext& inContext, CDB4DBaseContext *inBaseContext) = 0;
	...
	virtual	void				InitJSContextWithEntityModels(XBOX::VJSObject& globalObject, CDB4DBaseContext *inBaseContext, void* ProjectRef, std::vector<XBOX::VFile*>& outModelJSFiles) = 0;
	...
	*/
	virtual IHTTPRequestHandler*	AddRestRequestHandler( VErrorDB4D& outError, CDB4DBase *inBase, IHTTPServerProject *inHTTPServerProject, RIApplicationRef inApplicationRef, const XBOX::VString& inPattern, bool inEnabled, IHTTPRequestHandler **outAdminRequestHandler) = 0;

	virtual void ForceExpires4DRestSession(const XBOX::VUUID& inSessionID) = 0;
	virtual void GetAll4DRestSessions(std::vector<XBOX::VUUID>& outSessions) = 0;
	virtual XBOX::VJSONObject* RetainUserSessionRequestInfo(const XBOX::VUUID& inSessionID) = 0;
	virtual void SetUserSessionRequestInfo(const XBOX::VUUID& inSessionID, XBOX::VJSONObject* inRequestInfo) = 0;
...
public:
	...
	virtual Boolean CreateData( const XBOX::VFile& inDataFile, sLONG inParameters, XBOX::VFile *inJournalingFile = NULL, 
								CDB4DBaseContextPtr inContext = NULL, VErrorDB4D* outErr = nil, XBOX::FileAccess inAccess = XBOX::FA_READ_WRITE, CDB4DRawDataBase* fromData = nil ) const = 0;
...
//	virtual CNameSpace* GetNameSpace() const = 0;
	...
	virtual VErrorDB4D SetJournalFileInfos( XBOX::VString *inFilePath, XBOX::VUUID *inUUID,bool inResetSequenceNumber = false ) = 0;
//	virtual	VErrorDB4D ResetJournalFileContent() = 0;
	...
	virtual VErrorDB4D ActivateJournal(XBOX::VFolder *inFolder, XBOX::VUUID &inDataLink, bool inWriteOpenDataOperation = true) = 0;
	...
	virtual VErrorDB4D StartDataConversion() = 0;
	virtual void StopDataConversion(CDB4DBaseContext* context, XBOX::VProgressIndicator* progress, bool MustCheckPrimKeyValidity) = 0;
	...
	virtual VErrorDB4D IntegrateJournal(CDB4DJournalParser* inJournal, uLONG8 from = 0, uLONG8 upto = 0, uLONG8 *outCountIntegratedOperations = NULL, VDB4DProgressIndicator* InProgress = NULL, bool forMirror = false, sLONG8* lastAction = NULL) = 0;
	...
	*/
	...
	virtual bool GetCatalogErrors(XBOX::VErrorStack& outErrors) = 0;
	...
	*/
	...
	virtual bool SomeTablesCannotBeLogged(XBOX::VJSONObject* ioParams) = 0;

	virtual bool CheckPrimaryKeysValidity(CDB4DBaseContext* inContext, VDB4DProgressIndicator* progress, VErrorDB4D& outErr, XBOX::VJSONObject* ioParams, bool forceCheck = false) = 0;

	virtual	void GetPrimaryKeysValidityState( CDB4DBaseContext* inContext, XBOX::VJSONObject* ioParams) const = 0;


	virtual VErrorDB4D BuildCacheFileOnRemoteCatalog(XBOX::VFile* inRemoteConfigFile, DB4D_RemoteCat_Problems& outProblem) = 0;

	virtual XBOX::VJSONObject* RetainMeasures(XBOX::VJSONObject* options = nil) = 0;
	virtual void SetMeasureInterval(uLONG interval) = 0;
...
public:
	...
	virtual Boolean ShouldBuildQueryPlan()const = 0;
	virtual void	BuildQueryPlan(Boolean on)= 0;
...
//	virtual DB4D_Rel_AutoLoadState GetRelationAutoLoadState(const CDB4DField* inSource, const CDB4DField* inDest) const = 0;
	...
	virtual CUAGThreadPrivilege* GetThreadPrivileges() = 0;

	virtual void SetThreadPrivileges(CUAGThreadPrivilege* privileges) = 0;
...
public:
	...
	virtual VErrorDB4D AddField(const XBOX::VString& inName, DB4DFieldType inType, sLONG inSize, DB4DFieldAttributes inAttributes, 
								VDB4DProgressIndicator* InProgress = NULL, CDB4DBaseContextPtr inContext = NULL, bool forceAtTheEnd = false) = 0;
	...
	virtual VErrorDB4D DelayIndexes(bool includesPrimKey) = 0;
...
#if 0
	virtual VErrorDB4D GetModificationsSinceStamp(sLONG stamp, XBOX::VStream& outStream, uLONG& outLastStamp, sLONG& outNbRows, CDB4DBaseContextPtr inContext, std::vector<sLONG>& cols) = 0;
	...
	virtual VErrorDB4D SetPreventJournaling(bool preventJournaling, CDB4DBaseContextPtr inContext, VDB4DProgressIndicator* InProgress = nil) = 0;

	virtual bool GetPreventJournaling(CDB4DBaseContextPtr inContext) const = 0;

	virtual VErrorDB4D GetModificationsSinceStampWithPrimKey(sLONG8 stamp, XBOX::VStream& outStream, uLONG8& outLastStamp, sLONG& outNbRows, 
	...
	virtual XBOX::VJSONObject* BuildLockInfo() = 0;
	...
	virtual sWORD GetPrimaryKeyValidityState() = 0;
};
...
public:
	...
	virtual Boolean AddCriteria(CDB4DField* inField, const XBOX::VectorOfVString& objectPath, DB4DComparator inComparator, const XBOX::VValueSingle& inValue, Boolean inDiacritic = false) = 0;
...
class CDB4DComplexQuery : public XBOX::IRefCountable
{
	public:
		...
		virtual VErrorDB4D BeginWhere() = 0;
		virtual VErrorDB4D BeginOn() = 0;
		virtual VErrorDB4D EndOn() = 0;
...
public:
	...
	virtual void RemoveDataBaseContext(CDB4DBase* inTarget) = 0;
	...
	virtual	void	GetUser4DSessionID( XBOX::VUUID& outUUID) const = 0;
};
...
class DB4D_API CDB4DJournalData : public XBOX::IRefCountable
{
	public:
		...
		virtual PrimKey* GetPrimKey() const = 0;
		virtual sLONG GetFieldNumber() const = 0;

		virtual XBOX::VJSONObject* RetainDescription(XBOX::VError& outErr, bool withFullDetail, CDB4DBase* inBase) = 0;
...
class CDB4DRawDataBase : public XBOX::IRefCountable
{
	public:
		...
		virtual XBOX::VIntlMgr *CreateIntlMgr() = 0;
};
...
class CDB4DEntityModel : public XBOX::IRefCountable
{
	public:
		...
		virtual EntityAttributeScope GetScope() const = 0;
		...
		virtual bool PermissionMatch(DB4D_EM_Perm inPerm, CUAGSession* inSession, CUAGThreadPrivilege* inThreadPrivilege) const = 0;
...
class CDB4DEntityRecord : public XBOX::IRefCountable
{
	...
	//	virtual CDB4DEntityAttributeValue* GetAttributeValue(sLONG pos, VErrorDB4D& err) = 0;
		...
		virtual VErrorDB4D Save(uLONG stamp, bool allowOverrideStamp = false, bool refreshOnly = false) = 0;
...
const VErrorDB4D	VE_DB4D_LOG_NEEDS_A_VALID_PRIMARYKEY	= MAKE_VERROR(CDB4DManager::Component_Type, 1281);

const VErrorDB4D	VE_DB4D_PRIMARYKEY_CANNOT_BE_FOUND	= MAKE_VERROR(CDB4DManager::Component_Type, 1282);

const VErrorDB4D	VE_DB4D_PRIMARYKEY_IS_MISSING	= MAKE_VERROR(CDB4DManager::Component_Type, 1283);

const VErrorDB4D	VE_DB4D_PRIMARYKEY_IS_MALFORMED	= MAKE_VERROR(CDB4DManager::Component_Type, 1284);

const VErrorDB4D	VE_DB4D_PRIMARYKEY_IS_NOT_UNIQUE	= MAKE_VERROR(CDB4DManager::Component_Type, 1285);

const VErrorDB4D	VE_DB4D_PRIMARYKEY_IS_NULL	= MAKE_VERROR(CDB4DManager::Component_Type, 1286);

const VErrorDB4D	VE_DB4D_LOGFILE_ISBEFOREV14	= MAKE_VERROR(CDB4DManager::Component_Type, 1287);

const VErrorDB4D	VE_DB4D_CONSTRAINTS_CANNOT_REMOVED_WITH_LOG	= MAKE_VERROR(CDB4DManager::Component_Type, 1288);

const VErrorDB4D	VE_DB4D_MISSING_LOG_DESTINATION_FOLDER = MAKE_VERROR(CDB4DManager::Component_Type, 1289);

const VErrorDB4D	VE_DB4D_LOG_CANNOT_BE_OPENED = MAKE_VERROR(CDB4DManager::Component_Type, 1290);
...
namespace DB4DBagKeys
{
	...
	EXTERN_BAGKEY_WITH_DEFAULT_SCALAR( prevent_journaling, XBOX::VBoolean, bool);
...
//	EXTERN_BAGKEY_WITH_DEFAULT_SCALAR( fully_delete_records, XBOX::VBoolean, bool);		// sc 19/02/2008, doublon de "leave_tag_on_delete"
	...
	EXTERN_BAGKEY_WITH_DEFAULT(sql_schema_name, XBOX::VString);
	EXTERN_BAGKEY( color);
...
namespace d4
{
	...
	EXTERN_BAGKEY(matchTable);
	EXTERN_BAGKEY(matchColumn);
