...
namespace Helpers
{
	...
	XBOX::VError ThrowPathError(const XBOX::VError& inError, const XBOX::VFilePath& inPathSrc, const XBOX::VFilePath& inPathDst);
	XBOX::VError ThrowPathError(const XBOX::VError& inError, const XBOX::VFilePath& inPathSrc, const XBOX::VString inPathDst);
	...
	XBOX::VError CreateTimestampedName(const XBOX::VFilePath& inPath, XBOX::VString& outName, const XBOX::VTime* inTime = NULL, bool inUseMs = false);
...
namespace JournalUtils
{
	...
	VError ReadJournalInfo(XBOX::VFile& inDataFile, XBOX::VFilePath& outJournalPath, XBOX::VUUID& outDataLink)
	{
		VError error = VE_OK;
		VDBMgr* dbMgr = VDBMgr::GetManager();
		const VValueBag* extraProps = dbMgr->RetainExtraProperties(&inDataFile, NULL, NULL, error, NULL, NULL);
		if (error != VE_OK)
		{
			error = vThrowError(VE_FAILED_TO_RETRIEVE_JOURNAL_INFO);
		}
		else if (extraProps != NULL)
		{
			XBOX::VFolder* parent = inDataFile.RetainParentFolder();
			JournalUtils::GetJournalInfos(*extraProps, *parent, outJournalPath, outDataLink);
...
bool VBackupTool::BackupClosedDatabase(XBOX::VFile& inModelFile,XBOX::VFile& inDataFile,const IBackupSettings& inBackupConfig,XBOX::VFilePath* outManifestFilePath,IDB4D_DataToolsIntf* inBackupEventLog)
{
	...
	if(backupErr == VE_OK)
	{
		backupErr = JournalUtils::ReadJournalInfo(inDataFile, currentJournalPath, dataLink);
		if (backupErr == VE_OK)
		{
...
#if NewJournal
						backupErr = ThrowBaseError(VE_DB4D_NOTIMPLEMENTED);
#else
...
bool VBackupTool::RestoreDataStore(XBOX::VFolder& inDamagedDataFolder, const XBOX::VFolder& inDataFolderToRestoreFrom, const XBOX::VFile* inDamagedDFCurrentJournal, const XBOX::VFile* inJournalToRestoreFrom, 
	IDB4D_DataToolsIntf* inBackupEventLog,VFilePath* outDamagedDFCopy,VFilePath* outReplacedJournalCopy)
{
	XBOX::VError err = VE_OK;
	//StErrorContextInstaller errContext;
	XBOX::VFolder* copiedDamagedDataFolder = NULL;
	XBOX::VFile* copiedCurrentJournalFile = NULL;

	if (inDamagedDataFolder.Exists())
	{
		VString name;
		err = Helpers::CreateTimestampedName(inDamagedDataFolder.GetPath(), name);
		if (err == VE_OK)
		{
			err = inDamagedDataFolder.Rename(name, &copiedDamagedDataFolder);
			if (err != VE_OK)
			{
				err = Helpers::ThrowPathError(VE_CANNOT_CREATE_DATA_FOLDER_SAFETY_COPY, inDataFolderToRestoreFrom.GetPath());
			}
		}
	}
	if (err == VE_OK)
	{
		err = inDamagedDataFolder.CreateRecursive();
		if (err != VE_OK)
		{
			err = Helpers::ThrowPathError(VE_CANNOT_CREATE_DATA_FOLDER, inDamagedDataFolder.GetPath());
		}
	}

	if (err == VE_OK)
	{
		err = inDataFolderToRestoreFrom.CopyContentsTo(inDamagedDataFolder);
		if (err != VE_OK)
		{
			err = Helpers::ThrowPathError(VE_CANNOT_RESTORE_DATA_FOLDER, inDataFolderToRestoreFrom.GetPath(), inDamagedDataFolder.GetPath());
		}
		else
		{
			if (inDamagedDFCurrentJournal != NULL)
			{
				//Damaged data store uses a journal

				XBOX::VString relativeJournalPath;
				if (!inDamagedDFCurrentJournal->GetPath().GetRelativePath(inDamagedDataFolder.GetPath(), relativeJournalPath))
				{
					relativeJournalPath.Clear();
				}

				if (inJournalToRestoreFrom != NULL)
				{
					//And we need to replace it with inJournalToRestoreFrom
					if (!relativeJournalPath.IsEmpty())
					{
						//current journal is relative to (damaged) data folder which has been copied
						//so just copy inJournalToRestoreFrom over
						err = inJournalToRestoreFrom->CopyTo(inDamagedDataFolder,NULL);
					}
					else
					{
						//Journal is not relative to data folder, so make a safety copy and replace
						VString name;
						err = Helpers::CreateTimestampedName(inDamagedDFCurrentJournal->GetPath(), name);
						if (err == VE_OK)
						{
							err = inDamagedDFCurrentJournal->Rename(name, &copiedCurrentJournalFile);
							if (err != VE_OK)
							{
								err = Helpers::ThrowPathError(VE_CANNOT_CREATE_JOURNAL_SAFETY_COPY, inDamagedDFCurrentJournal->GetPath(), name);
							}
							else
							{
								err = inJournalToRestoreFrom->CopyTo(inDamagedDFCurrentJournal->GetPath(), NULL);
							}
						}
					}
					if (err != VE_OK)
					{
						err = Helpers::ThrowPathError(VE_CANNOT_RESTORE_JOURNAL, inJournalToRestoreFrom->GetPath(), inDamagedDFCurrentJournal->GetPath());
					}
				}
				else
				{
					//And no alternate journal is specified so ensure the current journal is properly positionned...
					if (!relativeJournalPath.IsEmpty())
					{
						//...particularly if it is inside the data folder
						VFilePath src;
						src.FromRelativePath(copiedDamagedDataFolder->GetPath(), relativeJournalPath);
						VFile srcFile(src);
						err = srcFile.CopyTo(*inDamagedDFCurrentJournal);//no overwrite because there should not be any journal at inDamagedDFCurrentJournal
						if (err != VE_OK)
						{
							err = Helpers::ThrowPathError(VE_CANNOT_RESTORE_JOURNAL, src, inDamagedDFCurrentJournal->GetPath());
						}
					}
				}
			}
	...
	if (err == VE_OK)
	{
		if (outDamagedDFCopy != NULL && copiedDamagedDataFolder != NULL)
			copiedDamagedDataFolder->GetPath(*outDamagedDFCopy);
		
		if (copiedCurrentJournalFile != NULL && outReplacedJournalCopy != NULL)
			copiedCurrentJournalFile->GetPath(*outReplacedJournalCopy);
	}
	else
	{
		VString name;
		
		if (copiedCurrentJournalFile != NULL)
		{
			//revert safety copy of inDamagedDataFolder's journal 
			inDamagedDFCurrentJournal->GetPath().GetFileName(name);
			inDamagedDFCurrentJournal->Delete();
			copiedCurrentJournalFile->Rename(name);
		}
		if (copiedDamagedDataFolder != NULL)
		{
			//revert safety copy of inDamagedDataFolder
			name.Clear();
			inDamagedDataFolder.Delete(true);
			inDamagedDataFolder.GetPath().GetFolderName(name);
			copiedDamagedDataFolder->Rename(name,NULL);
		}
	}
	XBOX::ReleaseRefCountable(&copiedCurrentJournalFile);
	XBOX::ReleaseRefCountable(&copiedDamagedDataFolder);
	return (err == VE_OK);
}
...
XBOX::VError VJournalTool::ReadJournalInfoFromDataFile(XBOX::VFile& inDataFile, XBOX::VFilePath& outJournalPath, XBOX::VUUID& outDataLink)
{
	return JournalUtils::ReadJournalInfo(inDataFile, outJournalPath, outDataLink);
}

XBOX::VError VJournalTool::ResetDataFileJournal(XBOX::VFile& inDataFile, XBOX::VFilePath& outFormerJournalPath, XBOX::VFilePath& outJournalPath)
{
	XBOX::VError err = VE_OK;
	XBOX::VFilePath curJournalPath;
	XBOX::VUUID journalUUID;

	err = JournalUtils::ReadJournalInfo(inDataFile, curJournalPath, journalUUID);
	if (curJournalPath.IsEmpty())
	{
		err = vThrowError(VE_JOURNAL_NOT_FOUND);
	}
	else if (journalUUID == XBOX::VUUID::sNullUUID)
	{
		err = vThrowError(VE_JOURNAL_NOT_FOUND);
	}
	else
	{
		XBOX::VFile journalFile(curJournalPath), *journalCopy = NULL;
		if (journalFile.Exists())
		{
			XBOX::VString temp,ext,name;
			Helpers::ComputeTimeStamp(NULL, temp, true);
			curJournalPath.GetFileName(name, false);
			curJournalPath.GetExtension(ext);
			name.AppendPrintf("_%S.%S", &temp,&ext);
			err = journalFile.Rename(name,&journalCopy);
		}
		if (err == VE_OK)
		{
			//Now create the stream
			err = journalFile.Create(FCR_Overwrite);
			if (err == VE_OK)
			{
				VFileStream *fileStream = new VFileStream(&journalFile);
				err = fileStream->OpenWriting();
				fileStream->SetBufferSize(0);
				if (err == VE_OK)
				{
					DB4DJournalHeader journalHeader;
					journalHeader.Init(journalUUID);
					err = journalHeader.WriteToStream(fileStream);
					if (err == VE_OK)
					{
						if (journalCopy != NULL)
						{ 
							journalCopy->GetPath(outFormerJournalPath);
						}
						outJournalPath = curJournalPath;
					}
					fileStream->CloseWriting();
				}
				delete fileStream;
			}
			if (err != VE_OK)
			{
				//err = vThrowError(VE_FAILED_TO_CREATE_JOURNAL);
				//Rollback renaming
				XBOX::VString temp;
				curJournalPath.GetFileName(temp, true);
				journalCopy->Rename(temp,NULL);
				journalCopy->Delete();
			}
			XBOX::ReleaseRefCountable(&journalCopy);
		}
	}
	return err;
}
...
namespace Helpers{
	...
	XBOX::VError ThrowPathError(const XBOX::VError& inError, const XBOX::VFilePath& inPathSrc, const XBOX::VFilePath& inPathDst)
	{
		StThrowError<> err(inError);
		XBOX::VString posixPath;
		inPathSrc.GetPosixPath(posixPath);
		err->SetString("pathsrc", posixPath);
		inPathDst.GetPosixPath(posixPath);
		err->SetString("pathdst", posixPath);
		return inError;
	}
	XBOX::VError ThrowPathError(const XBOX::VError& inError, const XBOX::VFilePath& inPathSrc, const XBOX::VString inPathDst)
	{
		...
		inPathSrc.GetPosixPath(posixPath);
		err->SetString("pathsrc", posixPath);
		err->SetString("pathdst", inPathDst);
		return inError;
	}
...
XBOX::VError CreateTimestampedName(const XBOX::VFilePath& inPath, XBOX::VString& outName, const XBOX::VTime* inTime, bool inUseMs)
{
	VString ts, name;
	VError err = VE_OK;

	Helpers::ComputeTimeStamp(inTime, ts,inUseMs);

	if (inPath.IsFile())
	{ 
		inPath.GetFileName(outName, false /*inWithExt*/);
		outName.AppendPrintf("_%S", &ts);
	}
	else if (inPath.IsFolder())
	{
		inPath.GetFolderName(outName, false /*inWithExt*/);
		outName.AppendPrintf("_%S", &ts);
	}
	else
	{
