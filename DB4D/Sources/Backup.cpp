...
XBOX::VError VBackupSettings::Initialize(const XBOX::VJSObject& inObject)
{
	...
	{
		error = XBOX::vThrowError(VE_BACKUP_CONFIGURATION_PROPERTY_NOT_FOUND, CVSTR("destination"));
	}
	if (error == VE_OK)
	{
		...
		{
			error = XBOX::vThrowError(VE_BACKUP_CONFIGURATION_PROPERTY_NOT_FOUND, CVSTR("backupRegistryFolder"));
		}
	}
	if (error == VE_OK)
	{
		...
		{
			error = XBOX::vThrowError(VE_BACKUP_CONFIGURATION_PROPERTY_NOT_FOUND, CVSTR("useUniqueNames"));
		}
	}
	return error;
}
...
XBOX::VError VBackupTool::_WriteManifestToRegistry(const IBackupSettings& inBackupSettings,const VFilePath& inManifestPath)
{
	...
	if(error == VE_OK ||error == VE_FILE_NOT_FOUND )
	{
		...
		//WAK0084452: Jan 27th 2014, O.R.: ensure no duplicate entries are kept
		VectorOfVString::iterator dup = find(registryItems.begin(), registryItems.end(), value);
		if (dup != registryItems.end()){
	...
	if( journalParser)
	{
		...
		if(err == VE_OK)
		{
			...
			for(uLONG8 current = 0; (current < total) && (err == VE_OK); current++)
			{
				...
				if (journalOp)
				{
					VJSObject obj(outOperations.GetContext());
					obj.MakeEmpty();
...
void VJournalTool::_ConvertJournalOperationToJSObject(CDB4DJournalData& inOp,XBOX::VJSObject& ioObject)
{
	...
	{
		VJSValue v(ioObject.GetContext());
		XBOX::VString action;
	...
	}
	{
		...
		{
			VJSArray opFields(ioObject.GetContext());
