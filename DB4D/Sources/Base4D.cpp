...
VError Base4D::CloseData(Boolean KeepFlushInfo)
{
	...
	if (err == VE_OK)
	{
		...
		if ( fLogStream )
		{
			WriteLog(DB4D_Log_CloseData, nil);
...
VError Base4D::CreateData( const VFile& inFile, sLONG inParameters, VIntlMgr* inIntlMgr, VFile *inJournalingFile, Boolean BuildResMap, FileAccess inAccess, const VUUID* inChosenID )
{
#if debuglr == 2
	...
	if (testAssert(!fIsRemote))
	{
		...
		if (inChosenID != nil)
		{
			inChosenID->ToBuffer(hbbloc.ID);
		}
		else
		{
			if (fStructure != nil)
			{
				fStructure->GetUUID().ToBuffer(hbbloc.ID);
			}
		}
			...
			if (err2 == VE_OK)
			{
				if (ixf->Exists())
				{
					...
					if (err2 == VE_OK)
					{
						...
						sLONG retour = 0;
						Boolean wasswapped;
						err2 = ReadBaseHeader(*f, &hbb, false, false, isAStruct ? nil : &(GetUUID()), &retour, wasswapped, true, true, true);
						if (err2 == VE_OK)
						{
		...
		if (err2  == VE_OK )
		{
			...
			if (err2 == VE_OK)
			{
				...
				if (err2 == VE_OK)
				{
					...
					if (err2 == VE_OK)
					{
						if (journalHeader.IsValid(inDataLink) )
						{
...
void Base4D::WriteDebugLogData(void* p, sLONG len)
{
	if (fDebugLog == nil)
	{
		if (fStructure == nil)
		{
			fDebugLog = new VFile(L"c:\\debugLogStruct.txt");
		}
		else
		{
			fDebugLog = new VFile(L"c:\\debugLogData.txt");
		}

		VError err = fDebugLog->Create(true);
		if (err != VE_OK)
		{
			delete fDebugLog;
			fDebugLog = nil;
...
VError Base4D::writelong(void* p, sLONG len, DataAddr4D ou, sLONG offset, VString* WhereFrom,sLONG TrueLen)
{
	...
	assert( ou != 0 || debug_OKToWriteIn0 );
#endif
...
VError Base4D::LoadDataSegments( const VFile& inFile, Boolean DO_NOT_Map_Data_And_Struct, FileAccess InAccess, bool ConvertToHigherVersion, bool allowConvertForReadOnly, bool allowOneVersionMoreRecent)
{
	...
	if (err == VE_OK) {
		if (firstSegFileDesc->GetMode() == FA_READ)
			fWriteProtected = true;
		Boolean wasswapped;
		err = ReadBaseHeader( *firstSegFileDesc, &hbbloc, true, DO_NOT_Map_Data_And_Struct, fStructure == nil ? nil : &(fStructure->GetUUID()), &specialretour, wasswapped, ConvertToHigherVersion, allowConvertForReadOnly, allowOneVersionMoreRecent);
		if ( err != VE_OK )
						...
						for( i = 1 ; (i < hbbloc.nbsegdata) && (err == VE_OK) ; ++i) 
						{
							...
							if (err == VE_OK || err == XBOX_LONG8(8679670649385386184)) 
							{
								...
								if (err == VE_OK)
								{
									...
									Boolean wasswapped2;
									err = ReadBaseHeader(*fdesc, &hbb, false, DO_NOT_Map_Data_And_Struct, fStructure == nil ? nil : &(fStructure->GetUUID()), nil, wasswapped2, ConvertToHigherVersion, allowConvertForReadOnly, allowOneVersionMoreRecent);
	...
	if (err == VE_OK) 
	{
		...
		else
		{
			if (outHeader->VersionNumber == XBOX_LONG8(0x0000000100000005) || outHeader->VersionNumber == 0)
			{
				...
				{
					StErrorContextInstaller errs(false); 
					StructID->ToBuffer(outHeader->ID);
...
VError Base4D::AddTable( Table *inTable, bool inWithNamesCheck, CDB4DBaseContext* inContext, bool inKeepUUID, bool inGenerateName, bool BuildDataTable, sLONG inPos, bool cantouch, const VUUID *inUUID)
{
	...
	if (fIsRemote)
	{
		assert(!inGenerateName);	// not supported
				...
				if (curcontext->GetBase() == this)
				{
					...
					if (trans != nil)
					{
						...
						if ( err == VE_OK )
						{
							...
							err = trans->GetOwner()->WriteLog(DB4D_Log_StartTrans);
							trans->KeepEntries();
						}
								...
								if (err == VE_OK)
								{
									...
									if (err == VE_OK)
									{
										if ( operationTag == kTagLogDB4D )
										{
											...
											assert(contextID < 0);
											contextID = -contextID;
											assert(contextID == curcontext->GetID());
...
VError Base4D::SetIntlMgr(VIntlMgr* inIntlMgr, VDB4DProgressIndicator* InProgress)
{
	...
	if (inIntlMgr != nil)
	{
		...
		{
			occupe();
			if (fIntlMgr == nil || inIntlMgr->GetDialectCode() != fIntlMgr->GetDialectCode() || inIntlMgr->GetCollator()->GetOptions() != fIntlMgr->GetCollator()->GetOptions())
			{
				mustrebuild = true;
...
void Base4D::GetIndices(VJSArray& outIndices)
{
	...
	{
		VJSObject objrec(outIndices.GetContextRef());
		objrec.MakeEmpty();
		IndexInfo* ind = *cur;
		...
		{
			VJSArray arr(outIndices.GetContextRef());
			...
			for (FieldRefArray::Iterator curx = frefs->First(), endx = frefs->End(); curx != endx; curx++)
			{
				...
				if (cri != nil)
				{
					VString ss;
					cri->GetName(ss);
					VJSValue jsval(outIndices.GetContextRef());
					jsval.SetString(ss);
					arr.PushValue(jsval);
				}
			...
			else
			{
				...
				if (ForceCreate)
				{
					if (OkToUpdate(err))
					{
						...
						AddCreateIndex(ind,InProgress, event);
						event = nil;
...
void Base4D::RebuildIndexByRef(IndexInfo* ind, CDB4DBaseContext* inContext, VDB4DProgressIndicator* InProgress, VSyncEvent* event)
{
	...
	else
	{
		if (OkToUpdate(err))
		{
			...
			//if (InProgress != nil) InProgress->Retain();
			AddRebuildIndex(ind,InProgress, event);
			event = nil;
			ClearUpdating();
		}
	}
...
void Base4D::ClearAllTableDependencies()
{
	TableArray::Iterator cur = fich.First(), end = fich.End();
	for (; cur != end; cur++)
	{
						...
						if (err == VE_OK)
						{
							...
							for (IndexMap::iterator cur = fAllIndexes.begin(), end = fAllIndexes.end(); cur != end && err == VE_OK; cur++)
							{
								...
								if (ind != nil)
								{
									...
									assert(istamp > 0 && istamp <= fIndexesStamp);
		...
		if (err == VE_OK)
		{
			...
			if (err == VE_OK)
			{
				...
				else if ((inParameters & DB4D_Open_With_Own_UsersAndGroups) != 0)
				{
					...
					fUAGDirectory = VDBMgr::GetUAGManager()->RetainDirectory(xmldata, inAccess, fStructFolder, &fBaseName, &err);
				}
			}
...
VError Base4D::CreateStructure( const VFile& inStructureFile, sLONG inParameters, VIntlMgr* inIntlMgr, FileAccess InAccess, const VUUID* inChosenID, VString* EntityFileExt, CUAGDirectory* inUAGDirectory, const VFile* inPermissionsFile)
{
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			fID = fStructure->fID;
			fID.ToBuffer(fStructure->hbbloc.ID);
			...
			if (err == VE_OK)
			{
				...
				else if ((inParameters & DB4D_Open_With_Own_UsersAndGroups) != 0)
				{
					...
					VError err2;
					VFile xmldata(*fStructFolder, fBaseName+L".RIADirectory");
					fUAGDirectory = VDBMgr::GetUAGManager()->RetainDirectory(xmldata, InAccess, fStructFolder, &fBaseName, &err2);
				}
			}
		}
		firststage->libere();
...
void Base4D::LoadAllTableDefsInData()
{
	VError err = VE_OK;
	if (fStructure != nil)
	{
		...
		for (sLONG i = 0; i < hbbloc.nbTableDef && enoughmem; i++)
		{
			...
			{
				e->libere();
				TableRegular* tt = new TableRegular(this, 0, false, false);
...
VError Base4D::StartWriteLog(DB4D_LogAction action, sLONG len, BaseTaskInfo* context, VStream* &outLogStream)
{
	if (context != nil)
...
VError Base4D::EndWriteLog(sLONG len)
{
	VError err = VE_OK;
	if (fLogStream != nil)
	{
...
VError Base4D::WriteLog(DB4D_LogAction action, BaseTaskInfo* context)
{
	VStream *log;
	VError err = StartWriteLog(action, 0, context, log);
	if (err == VE_OK)
		err = EndWriteLog(0);
	return err;
}
...
VError Base4D::IntegrateJournal(DB4DJournalParser* jparser, uLONG8 from, uLONG8 upto, uLONG8 *outCountIntegratedOperations, VDB4DProgressIndicator* InProgress)
{
	VError err = VE_OK;
	...
	{
		ContextMap contexts;
		BlobActionMapByBlobID blobactions;
		...
		for (uLONG8 curoper = from; curoper <= upto && err == VE_OK; curoper++)
		{
			if ( curoper%64 == 0 )
				VTask::Yield();
						...
						case DB4D_Log_DeleteBlob:
							{
								...
								if (tt != nil)
								{
									...
									else
									{
										try
										{
											blobactions[make_pair(tt->GetNum(), numblob)] = jd;
											jd->Retain();
								...
								if (context != nil)
								{
									...
									if (tt != nil)
									{
										...
										if (df != nil)
										{
											...
											if (action == DB4D_Log_DeleteRecord)
											{
												FicheInMem* fic = df->LoadRecord(numrec, err, DB4D_Keep_Lock_With_Record, context, false, false, nil, nil);
													...
													if (ficD != nil)
													{
														...
														for (sLONG i = 1; i <= nbfield; i++)
														{
															...
															if (p != nil)
															{
																...
																{
																	sLONG blobid = *((sLONG*)p);
																	blobactions.erase(make_pair(tt->GetNum(), blobid));
																}
															}
														}
													}
											...
											if (action == DB4D_Log_CreateRecord || action == DB4D_Log_ModifyRecord)
											{
												FicheInMem* fic;
												sLONG nbfield;
												jd->GetCountFields(nbfield);
												if (action == DB4D_Log_CreateRecord)
												{
													fic = df->NewRecord(err, context);
													if (fic != nil)
													{
														fic->SetRecordNumberPreAllocated(numrec);
													}
												}
												else
												{
													fic = df->LoadRecord(numrec, err, DB4D_Keep_Lock_With_Record, context, false, false, nil, nil);
												}
															...
															if (logcv == nil && (typ == VK_TEXT || typ == VK_TEXT_UTF8 || typ == VK_IMAGE || typ == VK_BLOB || typ == VK_BLOB_DB4D))
															{
																...
																else
																{
																	...
																	else
																	{
																		if (blobid >= 0)
																		{
																			BlobActionMapByBlobID::iterator found = blobactions.find(make_pair(tt->GetNum(), blobid));
			...
			if (err != VE_OK)
				ThrowBaseError(VE_DB4D_INTEGRATE_JOURNAL_FAILED_AT, ToString((sLONG8)from));
		...
		if (xcuroper != -1)
		{
			assert(xcuroper >= hbbloc.lastaction);
...
VError Base4D_NotOpened::AttachTo(Base4D* inBase, ToolLog *log, BaseTaskInfo* context)
{
	...
	if (errexec == VE_OK && fStruct == nil)
	{
		...
		AddTableName(1, L"Resources", log);
...
VError Base4D_NotOpened::AddTableName(sLONG tablenum, const VString& tablename, ToolLog* log)
{
	try
	{
	...
	else
	{
		...
		else
		{
			...
			if (fOwner->GetStruct() == nil) // dans le cas des tabledef qui sont dans la structure (les vrais)
				errexec = datas->AddTableName(numobj+1, tablename, fLog);
		}
		if (errexec == VE_OK)
		{
			if (fOwner->GetStruct() == nil) // dans le cas des tabledef qui sont dans la structure (les vrais)
