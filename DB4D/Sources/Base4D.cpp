...
//		special_pour_index.libere();
	...
	///WAK0084654, O.R. Oct 24th 2013: journal integration will fail if fCurrentLogOperation is initialized to -1
	fCurrentLogOperation = 0;
...
bool Base4D::CheckPrimaryKeysValidity(BaseTaskInfo* context, VProgressIndicator* progress, VError& outErr, VJSONObject* ioParams, bool forceCheck)
{
	outErr = VE_OK;
	bool allOK = true;
	for (TableArray::Iterator cur = fich.First(), end = fich.End(); cur != end && outErr == VE_OK; ++cur)
	{
		Table* tt = *cur;
		if (tt != nil)
		{
			...
			if (df != nil)
			{
				if (!df->CheckPrimaryKeyValid(context, progress, outErr, forceCheck))
				{
					...
					{
						if (ttobj != nil)
							ttobj->SetPropertyAsNumber("primaryKeyValidityState", PrimKeyState_NonUnique);
						outErr = VE_OK;
					}
					else if (outErr == VE_DB4D_PRIMARYKEY_IS_NULL)
					{
						if (ttobj != nil)
							ttobj->SetPropertyAsNumber("primaryKeyValidityState", PrimKeyState_SomeNULLs);
						outErr = VE_OK;
					}
					else if (outErr == VE_DB4D_PRIMARYKEY_IS_MISSING)
					{
						if (ttobj != nil)
							ttobj->SetPropertyAsNumber("primaryKeyValidityState", PrimKeyState_MissingPrimKeyDef);
						outErr = VE_OK;
					}
				}
			}
	...
	else
	{
		...
		else
		{
			...
			if (VDBMgr::GetManager()->IsRunningWakanda())
			{
				...
				fOwner->AddTableName(numobj+1, tablename, fLog);
			}
			else
			{
