...
VError Base4D::CreateData( const VFile& inFile, sLONG inParameters, VIntlMgr* inIntlMgr, VFile *inJournalingFile, Boolean BuildResMap, FileAccess inAccess, const VUUID* inChosenID, CDB4DRawDataBase* fromData )
{
	...
	if (testAssert(!fIsRemote))
	{
		...
		{
			VFolder* journalfolder = inJournalingFile->RetainParentFolder();
			if (journalfolder != nil)
			{
				StErrorContextInstaller errs(false);
				fLogPool.SetBaseFolder(journalfolder, true, true);
				journalfolder->Release();
			}
		}
#else
		if ( err == VE_OK && inJournalingFile )
...
VError Base4D::OpenData( const VFile& inFile, sLONG inParameters, Boolean BuildResMap, Boolean KeepFlushInfoIfClose, FileAccess inAccess)
{
	...
	VDBMgr::GetManager()->FlushCache(this, true);
	...
	gIndexBuilder->GoIndex();

	//LoadFieldIdsInRec();

#if NewJournal
	fLogPool.LoadFrom(&inFile, true);
#else
...
#if NewJournal
		if (fLogPool.IsLogActivated())
		{
			sLONG offset = ((char*)&hbbloc.lastaction) - ((char*)&hbbloc);
			sLONG8 curlogoper = fLogPool.GetCurrentOperation();
			seg0->writeat(&curlogoper, 8, offset);
			hbbloc.lastaction = curlogoper;
		}
...
#if NewJournal
	if (!fLogPool.IsLogValid())
	{
		VValueBag* errorstack = fLogPool.RetainErrorStack();
		if (errorstack != nil)
			VTask::GetErrorContext(true)->PushErrorsFromBag(*errorstack);
		err = ThrowError( VE_DB4D_CURRENT_JOURNAL_IS_INVALID, DBaction_TryingToUpdateDB);
		QuickReleaseRefCountable(errorstack);
		return false;
	}
#else
	if (!IsLogValid())
...
{ 
#if NewJournal
	ThrowBaseError(VE_DB4D_NOTIMPLEMENTED, noaction);
	return nil;
#else
	VStream* result;
	VTaskLock lock(&fLogMutex);
...
{
#if NewJournal
	return ThrowError(VE_DB4D_NOTIMPLEMENTED, noaction);
#else
	VError err;
	if ( inNewLog )
	{
...
{
#if NewJournal
	return fLogPool.StartWriteLog(action, len, context, outLogStream, SignificantAction, mutextAlreadyLocked);
#else
	if (context != nil)
...
{
#if NewJournal
	return fLogPool.EndWriteLog(len, keepMutexLocked);
#else
	VError err = VE_OK;
	if (fLogStream != nil)
	{
...
{
#if NewJournal
	return fLogPool.WriteLog(action, context, SignificantAction);
#else
	VStream *log;
																						...
																						if (typ == VK_IMAGE)
																						{
																							...
																							if (xVPictureRef != nil) 
																							{
																								...
																								if (blobid == -3)
																								{
																									...
																									*/
																									blob->SetForcePathIfEmpty(true);
																									cv->SetForcePathIfEmpty(true);
																								}
...
FicheInMem* Base4D::BuildRecordFromClient(VStream* from, BaseTaskInfo* inContext, Table* inTable, VError& outError)
{
	...
	if (fic == nil && outError == VE_OK)
	{
		...
		if (xnumrec == -3)
		{
			...
			// Store the ID created on client side for the new record
			if (fic != nil)
				fic->SetID(xid.GetBuffer());
		}
		else
		{
			// This can happen for example if the client retrieved the record by Req_CacheDisplaySelection (e.g. listbox display cache)
			// which doesn't load the record on server side (note this doesn't mean the client didn't also load the record separately from
			// the cached record, but the cached record doesn't know the other one's ID which is why he sent to the server an unknown ID for
			// an existing record ... in this case the following line will find the already present loaded record)
			fic = inTable->GetDF()->LoadRecord(xnumrec, outError, DB4D_Do_Not_Lock, inContext, true);
		}
	}
	else
		outError = from->GetLong(xnumrec);
				...
				*/
				...
				case DB4D_Index_OnObj:

					{
						...
						/*
						if (IndexInfoTyp == DB4D_Index_OnObj)
						{
							if (errexec == VE_OK && err == VE_OK)
							{
								if (fTypObj != obj_IndexInStructDef)
	...
	public:
		...
		VError SetRecInfo(RecInfoVector& arr, sLONG pos, DataAddr4D ou, sLONG len, uLONG8 stamp)
		{
			try
			{
				...
				//ACI0087621 June 13th 2014, O.R.: if the stamp is 0 it may imply that the record was previously recovered with a timestamp of 0
				if ( (stamp > recinf->fTimeStamp) || (stamp==0))
				{
					recinf->fAddr = ou;
					recinf->fLen = len;
					//if (stamp > recinf->fTimeStamp)
					//ACI0087621 June 13th 2014, O.R.: if the stamp is 0 then initialize it so that further Recover by tag don't simply strip the record away
					if (stamp==0)
						recinf->fTimeStamp = recoverStamp;
					else
						...
						if (err == VE_OK)
						{
							...
							if (info.fAddr != 0)
							{
								...
								if (ficD != nil)
								{
									...
									err = df->SaveRawRecord(curstack, j, ficD->GetDataBegining(), ficD->GetDataLen(), ficD->GetNbCrit(), context, ficD->GetTimeStamp());
									ficD->Release();
