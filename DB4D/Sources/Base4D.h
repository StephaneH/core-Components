...
class StructElemDef : public ObjCacheInTree
{
	public:
		...
		virtual void RecalcNbElemTab() { assert(false); nbelemtab = 0; /* should not be called*/ };
...
public:
	...
	VError CreateData( const VFile& inFile, sLONG inParameters, VIntlMgr* inIntlMgr, VFile *inJournalingFile = NULL, Boolean BuildResMap = false, 
						FileAccess inAccess = FA_READ_WRITE, const VUUID* inChosenID = nil );
...
//	inline CClass* GetBaseRef(void) { return BaseRef; };
	...
	VError WriteLog(DB4D_LogAction action, BaseTaskInfo* context = nil);
	VError StartWriteLog(DB4D_LogAction action, sLONG len, BaseTaskInfo* context, VStream* &outLogStream);
	VError EndWriteLog(sLONG len);
	...
	inline void StartDataConversion()
	{
	...
	inline void StopDataConversion()
	{
	...
	VError IntegrateJournal(DB4DJournalParser* jparser, uLONG8 from, uLONG8 upto, uLONG8 *outCountIntegratedOperations, VDB4DProgressIndicator* InProgress);
	...
	void* GetRiaServerProjectRef() const
	{
		return fRiaServerProjectRef;
	}
...
class Base4D_NotOpened : public ObjAlmostInCache
{
	...
	public:
		...
		VError AddTableName(sLONG tablenum, const VString& tablename, ToolLog* log);
