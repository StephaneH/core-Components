			...
			else
			{
				...
				else
				{
					...
					if (!b2)
					{
						...
						{
							assert(nbbit - autre->nbbit < kNbBitParSet);
							p[kNbLongParSet] = -1;
...
VError Bittab::LibereEspaceDisk(VDB4DProgressIndicator* InProgress)
{
	sLONG i,len;
	DataAddr4D ou;
	VError err = VE_OK;
...
//		assert(sel->qtfic == ( (curpage * kNbElemInSel) + curelem ));
