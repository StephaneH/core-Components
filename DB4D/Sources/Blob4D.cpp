...
VError Blob4D::WriteSaveToLog(BaseTaskInfo* context, Boolean newone)
{
	...
	sLONG len = calclen() + 4/*numblob*/  + sizeof(VUUIDBuffer)/*numtable*/;
	...
	VError err = db->StartWriteLog(newone ? DB4D_Log_CreateBlob : DB4D_Log_ModifyBlob, len, context, log);
	if (err == VE_OK)
	{
		if (log != nil)
		{
...
VError Blob4D::WriteDeleteToLog(BaseTaskInfo* context)
{
	...
	Base4D* db = fDataFile->GetDB();
	Table* tt = fDataFile->GetTable();
	sLONG len =  4/*numblob*/  + sizeof(VUUIDBuffer)/*numtable*/;
	sLONG blobid = fBlobID;
	VString pathToSave;
	if (fInOutsidePath && fPathIsRelative)
	{
	...
	VError err = db->StartWriteLog(DB4D_Log_DeleteBlob, len, context, log);
	if (err == VE_OK)
	{
		if (log != nil)
		{
...
VError Blob4D::CheckIfMustDeleteOldBlobID(BaseTaskInfo* context, Transaction* trans)
{
	...
	if (trans == nil)
	{
		...
		{
			err = fDataFile->DelBlobForTrans(fOldBlobID, context, true);
			fOldBlobID = -1;
			fOldWasInData = false;
		}
	}
	else
	{
		if (fOldBlobID != -1 && fOldWasInData)
		{
			err = trans->DelBlob(fDataFile->GetNum(), fOldBlobID);
			fOldBlobID = -1;
			fOldWasInData = false;
		}
	}
...
VError Blob4D::CheckIfMustDeleteOldPath(BaseTaskInfo* context, Transaction* trans)
{
	...
	if (trans == nil)
	{
		if (fOldPathIsValid)
		{
			if (IsPathRelative())
			{
				GetDF()->MarkOutsideBlobToDelete(fOldOutsidePath, fOldOutsidePath);
			}
		}
	}
	else
...
void Blob4D::MarkDeleteOutsidePath()
{
	if (fInOutsidePath && !fOutsidePath.IsEmpty())
	{
		...
		if (IsPathRelative())
			path = fComputedPath;
		GetDF()->MarkOutsideBlobToDelete(s, path );
	}
}
