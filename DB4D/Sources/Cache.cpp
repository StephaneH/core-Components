...
void BtreeCache::search(uBOOL *h, BTitemCache *v, sLONG level)
{
	...
	if (isegal)
	{
		...
		else
		{
			if (!remplace)
			{
				assert( false);	// "pourquoi on remplace dans cache"
				#if 0
							 ...
							 Boolean& BigObjectsNeedToBeFreedByOtherTask)
{
	...
	for (i=0;(i<nkeys) && (*tot<need);i++)
	{
		...
		if (obj!=nil)
		{
			...
			else
			{
				if (obj->okToLiberemem())
				{
					...
					tot2=obj->liberemem(allocationBlockNumber, (sLONG)need,false);
					//CHECKCACHE;
					if (tot2<0)
					{
						toutvide=false;
		...
		if (sousbt!=nil)
		{
			...
			{
				//CHECKCACHE;
				assert(subremaining[i+1] == 0);
				tabsous[i+1]=nil;
				//assert( (ioDebugcurcompteobj-20) <= VDBMgr::GetCacheManager()->CompteObj());
				//CHECKCACHEFIND(sousbt);
				delete sousbt;
			}
			else
	...
	assert(nkeys <= kNbElemInTreeCache);
#if VERSIONDEBUG
		...
		if (obj != nil)
		{
			if (fCheckCacheWithFlush)
			{
				if (!obj->occupe())
				{
					if (obj->modifieonly())
					{
						assert(VDBMgr::GetFlushManager()->FindObjInFlush(obj));
					}
					obj->libere();
				}
			}
...
{
	assert( gAlternateCppMem == &fCache);
...
BtreeCache* VDBCacheMgr::xNextFreeCachePage()
{
	...
	assert( bc != nil);	// "Grave : plus de FreeCachePage"
	return bc;
}
