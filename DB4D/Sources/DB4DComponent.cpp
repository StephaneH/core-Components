...
QueryParamElement::QueryParamElement(XBOX::VJSArray& inArray)
{
	...
	XBOX::JS4D::ProtectValue(fArray->GetContext(), *fArray);
}
...
void QueryParamElement::Dispose()
{
	...
	else if (fType == DB4D_QPE_array)
	{
		if (fArray != nil)
		{
			XBOX::JS4D::UnprotectValue(fArray->GetContext(), *fArray);
			delete fArray;
		}
...
void VDB4DMgr::SetAllDB4DGlobalProperties(VJSObject& globalObject, CDB4DBaseContext *inBaseContext, void* ProjectRef)
{
	BaseTaskInfo* context = ConvertContext(inBaseContext);
	fManager->SetAllDB4DGlobalProperties(globalObject, context, ProjectRef);
}
...
VDB4DQueryInfoCollector::VDB4DQueryInfoCollector(CDB4DBaseContext* inContext):
...
XBOX::VJSONObject* VDB4DQueryInfoCollector::CollectInfo(VProgressIndicator *inIndicator)
{
	...
	if(fBaseContext != nil)
	{
		...
		{
			XBOX::VString tmp;
			XBOX::VValueBag* plan = fBaseContext->GetLastQueryPlan();
			if(plan != nil)
			{
				plan->DumpXML(tmp,CVSTR(""),false);
				info->SetPropertyAsString(CVSTR("QueryPlan"),tmp);
			}
		}
	}
	return info;
