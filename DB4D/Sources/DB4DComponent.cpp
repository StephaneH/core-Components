...
{
	return VDB4DMgr::RetainInstance( inLocalizationManager);
}
...
QueryParamElement::QueryParamElement(XBOX::VJSONArray* inArray)
{
	fArray = nil;
	fScalar = nil;
	fSimpleDateScalar = nil;
	fType = DB4D_QPE_array;
	fJSONArray = RetainRefCountable(inArray);
}
...
void QueryParamElement::Dispose()
{
	...
	else if (fType == DB4D_QPE_array)
	{
		...
		QuickReleaseRefCountable(fJSONArray);
	}
}
...
#if DB4DasDLL
VDB4DMgr*	VDB4DMgr::sInstance = nil;
#endif
...
VDB4DMgr* VDB4DMgr::RetainInstance( VLocalizationManager* inLocalizationManager)
{
	VDB4DMgr *mgr = RetainRef( sInstance);
	if (mgr == nil)
	{
		mgr = new VDB4DMgr(inLocalizationManager);
	}
	else
	{
		if (mgr->GetManager()->GetDefaultLocalization() == nil && inLocalizationManager != nil)
		{
			mgr->GetManager()->SetDefaultLocalization(inLocalizationManager);
		}
	}
	return mgr;
}
...
DB4DNetworkManager* VDB4DMgr::CreateRemoteTCPSession(const VString& inServerName, uWORD inPortNum, VError& outerr, bool inSSL)
{
...
VErrorDB4D VDB4DMgr::OpenRemoteCatalogs(void* projectRef, const XBOX::VFolder* folderToScan, CDB4DBase* associate, bool stopOnFirstError)
{
	xbox_assert(associate != nil);
	Base4D* assocBase = dynamic_cast<VDB4DBase*>(associate)->GetBase();
	return fManager->OpenRemoteCatalogs(projectRef, folderToScan, assocBase, stopOnFirstError);
}
...
CDB4DBase* VDB4DMgr::OpenBase( const VFile& inStructureFile, sLONG inParameters, VError* outErr, FileAccess inAccess, VString* EntityFileExt, CUAGDirectory* inUAGDirectory, const VFile* inPermissionsFile, void* ProjectRef)
{
	CDB4DBase* id = nil;
	if (testAssert( fManager != nil)) {
		id = fManager->OpenBase( inStructureFile, inParameters, outErr, inAccess, EntityFileExt, inUAGDirectory, nil, nil, inPermissionsFile, ProjectRef);
	}
	return id;
}
...
void VDB4DMgr::ReleaseContextsOfOrphanedUser(const XBOX::VUUID& inUserID)
{
	// Note that most contexts are "naturaly" released by their owner task. This method is only here for specific cases
	// when a task dies and voluntarily leaves some contexts behind (typicaly, postponed contexts waiting for an incomming
	// request to revive the task ... which must not happen after the user is kicked from the server, see ACI0079316)

	// Release any remaining postponed contexts for the dying user
	fManager->ForgetAllPostPonedContextsForUser(inUserID);
}
	...
	if (outError == VE_OK)
	{
		...
		for (sLONG i = 1, nb = inStructureDB->CountTables(nil); i <= nb; i++)
		{
			...
			if (tt != nil)
			{
				...
				tt->GetName(tname);
				tt->GetUUID(xid);
				xdata->AddTableName(i, tname, &mylog, nil);
...
VError VDB4DMgr::StartServer(const VString& inServerName, uWORD inPortNum, bool inSSL)
{
...
void VDB4DMgr::PatchTableAndFieldNames(XBOX::VValueBag *inDefinition_bag,const std::vector<XBOX::VString>& existingTableNames, const std::vector<XBOX::VString>& existingFieldNames,std::map<XBOX::VString,XBOX::VString> &tables_map,std::map<XBOX::VString,XBOX::VString> &fields_map) const
{
	VString oldname, newname, copyname,oldManyName, newManyName;
	std::vector<XBOX::VString> vectorOfTableName, vectorOfFieldName;
	XBOX::VBagArray *tables_array = NULL;
	XBOX::VBagArray *relations_array = NULL;
	XBOX::VBagArray *bagarray_1 = NULL;
	
	tables_array = inDefinition_bag->RetainElements( DB4DBagKeys::table);
	
	//exisiting field name is used only when importing into a table. In this case, 
	// the xml definition must contain only one table. See xbox_assert in SCDocument.cpp PatchDefinitions().
	if (!existingFieldNames.empty())
		xbox_assert(tables_array->GetCount()==1);


	for (std::vector<XBOX::VString>::const_iterator it = existingTableNames.begin(); it != existingTableNames.end(); it++)
		vectorOfTableName.push_back(*it);

	for (std::vector<XBOX::VString>::const_iterator it = existingFieldNames.begin(); it != existingFieldNames.end(); it++)
		vectorOfFieldName.push_back(*it);

	//make sure the table names are valid
	for(XBOX::VIndex n=1;n<=tables_array->GetCount();n++)
	{
		XBOX::VValueBag *table_bag = tables_array->GetNth(n);

		//build array of field names
		XBOX::VBagArray *fields_array = table_bag->RetainElements( DB4DBagKeys::field);

		// on verifie que le nom de la table est valide et unique
		table_bag->GetString( DB4DBagKeys::name, oldname);
		GetUniqueName( vectorOfTableName, oldname, newname, true, kMaxTableNameLength);
		if(IsValidTableName( newname, VN_LegacyLanguageCompatible, NULL, NULL) != XBOX::VE_OK)
		{
			BuildValid4DTableName( newname, copyname);
			GetUniqueName( vectorOfTableName, copyname, newname, true, kMaxTableNameLength);
		}
		table_bag->SetString( DB4DBagKeys::name, newname);
		vectorOfTableName.push_back(newname);
		tables_map[oldname]=newname;
		
		//make sure field names are valid
		for(XBOX::VIndex field = 1 ; field <= fields_array->GetCount() ; field++)
		{
			XBOX::VValueBag *field_bag = fields_array->GetNth(field);
			field_bag->GetString( DB4DBagKeys::name, oldname);
			GetUniqueName( vectorOfFieldName, oldname, newname, true, kMaxFieldNameLength);
			if(IsValidFieldName( newname, VN_LegacyLanguageCompatible, NULL, NULL) != XBOX::VE_OK)
			{
				BuildValid4DFieldName( newname, copyname);
				GetUniqueName( vectorOfFieldName, copyname, newname, true, kMaxFieldNameLength);
			}
			field_bag->SetString(DB4DBagKeys::name, newname);
			vectorOfFieldName.push_back(newname);
			fields_map[oldname]=newname;


			//convert sub-table type to long int
			DB4DFieldType type;
			if( field_bag->GetLong(DB4DBagKeys::type , type ) )
			{
				if( type == DB4D_SubTable || type == DB4D_SubTableKey )
					field_bag->SetLong( DB4DBagKeys::type, DB4D_Integer32 );
			}
		}
		fields_array->Release();
		vectorOfFieldName.clear();
	}
	tables_array->Release();

	//fix link names
	relations_array = inDefinition_bag->RetainElements( DB4DBagKeys::relation);
	for(VIndex n=1;n<=relations_array->GetCount();n++)
	{
		// Remap names of tables and fields into relations.
		VValueBag *bag = relations_array->GetNth(n);
		bagarray_1 = bag->RetainElements( DB4DBagKeys::related_field);
		for(VIndex n1=1;n1<=bagarray_1->GetCount();n1++)
		{
			VValueBag *bag1 = bagarray_1->GetNth(n1);
			VValueBag *bagfield= bag1->RetainUniqueElement( DB4DBagKeys::field_ref);
			VValueBag *bagtable= bagfield->RetainUniqueElement( DB4DBagKeys::table_ref);

			bagfield->GetString( DB4DBagKeys::name,oldname);
			newname = fields_map[oldname];
			if (newname != "")
				bagfield->SetString( DB4DBagKeys::name,newname);

			bagtable->GetString( DB4DBagKeys::name,oldname);
			newname = tables_map[oldname];
			if (newname != "")
				bagtable->SetString( DB4DBagKeys::name,newname);
...
void VDB4DMgr::GetUniqueName(const std::vector<VString>& inAlreadyUsedNames, const VString &inName, VString &outUniqueName, bool inWithEllipse, VIndex inMaxLength) const
{
	return ::GetUniqueName(inAlreadyUsedNames, inName, outUniqueName, inWithEllipse, inMaxLength);
}
...
void VDB4DMgr::InitJSContextWithEntityModels(VJSObject& globalObject, CDB4DBaseContext *inBaseContext, void* ProjectRef, std::vector<XBOX::VFile*>& outModelJSFiles)
{
	BaseTaskInfo* context = ConvertContext(inBaseContext);
	fManager->InitJSContextWithEntityModels(globalObject, context, ProjectRef, outModelJSFiles);
}


void VDB4DMgr::ReleaseProjectInfo(void* ProjectRef)
{
	fManager->RemoveProjectInfo(ProjectRef);
}
...
VJSObject VDB4DMgr::CreateJSDatabaseForMaintenanceObject( const VJSContext& inContext, CDB4DBaseContext *inBaseContext)
{
	return VJSDatabaseForMaintenance::CreateInstance( inContext,ConvertContext(inBaseContext)->GetBase());
}
...
void buildFullDSDataRef(VFile* dataDS, fileVector& outFiles)
{
	...
	VFile* TableDefFile = new VFile(*parent, name+kDataTableDefExt);
	outFiles.push_back(dataDS);
	QuickReleaseRefCountable(TableDefFile);
...
IHTTPRequestHandler* VDB4DMgr::AddRestRequestHandler( VErrorDB4D& outError, CDB4DBase *inBase, IHTTPServerProject *inHTTPServerProject, RIApplicationRef inApplicationRef, const XBOX::VString& inPattern, bool inEnabled, IHTTPRequestHandler **outAdminRequestHandler)
{
	xbox_assert(outAdminRequestHandler != NULL);

	outError = VE_OK;
	*outAdminRequestHandler = NULL;
	...
	if (inHTTPServerProject != NULL)
	{
		...
		if (inBase != nil)
		{
			VString adminpattern = "admin";
			adminpattern.Insert( CVSTR( "(?i)/"), 1);
			adminpattern += CVSTR( "/.*");
			AdminRequestHandler* adminhandler = new AdminRequestHandler(inBase, adminpattern, inApplicationRef);
			if (adminhandler != NULL) {

				outError = inHTTPServerProject->AddHTTPRequestHandler(adminhandler);
				if (outError != VE_OK)

					QuickReleaseRefCountable(adminhandler);

				else

					*outAdminRequestHandler = adminhandler;				
...
void VDB4DMgr::ForceExpires4DRestSession(const XBOX::VUUID& inSessionID)
{
	fManager->ForceExpires4DRestSession(inSessionID);
}


void VDB4DMgr::GetAll4DRestSessions(std::vector<XBOX::VUUID>& outSessions)
{
	fManager->GetAll4DRestSessions(outSessions);
}

XBOX::VJSONObject* VDB4DMgr::RetainUserSessionRequestInfo(const XBOX::VUUID& inSessionID)
{
	return fManager->RetainUserSessionRequestInfo(inSessionID);
}

void VDB4DMgr::SetUserSessionRequestInfo(const XBOX::VUUID& inSessionID, XBOX::VJSONObject* inRequestInfo)
{
	fManager->SetUserSessionRequestInfo(inSessionID, inRequestInfo);
}
...
static BaseTaskInfo* GetBaseTaskInfo(CDB4DBaseContextPtr inContext, Base4D* base)
{
	...
	if (inContext != nil)
	{
		...
		xbox_assert(context->GetBase() == base);
	}
	return context;
}
...
void VDB4DBase::Close(VSyncEvent* waitForClose)
{
	...
	VDBMgr::GetManager()->GetRestContexts().DisposeAllContextsOnBase(this);
}
...
void VDB4DBase::CloseAndRelease(Boolean WaitUntilFullyClosed)
{
	...
	UnRegisterForLang();
#if USE_V8_ENGINE
	// for now, on V8 do not wait until previous base is fully closed (allows to change solution by studio)
	WaitUntilFullyClosed = false;
#endif
	...
	Release();  // attention this n'est plus valide apres cela
...
Boolean VDB4DBase::CreateData( const VFile& inDataFile, sLONG inParameters, VFile *inJournalingFile, CDB4DBaseContextPtr inContext, VError* outErr, FileAccess inAccess, CDB4DRawDataBase* fromData) const
{
	VError err = fBase->CreateData( inDataFile, inParameters, fBase->GetIntlMgr(), inJournalingFile, false, inAccess, nil, fromData);
	if (outErr != nil)
		*outErr = err;
	return err == VE_OK;
}
...
CDB4DIndex* VDB4DBase::FindAndRetainIndexByName(const VString& inIndexName, CDB4DBaseContextPtr inContext, Boolean MustBeValid, Boolean Occupy) const
{
	...
	if (inContext != nil)
	{
		...
		xbox_assert(context->GetBase() == fBase);
	}
...
VError VDB4DBase::SetJournalFileInfos( VString *inFilePath, VUUID *inUUID,bool inResetSequenceNumber )
{
	return fBase->SetJournalFileInfos( inFilePath, inUUID, inResetSequenceNumber);
}
...
VError VDB4DBase::ActivateJournal(VFolder *inFolder, VUUID &inDataLink, bool inWriteOpenDataOperation)
{
#if NewJournal
	JournalPool* logpool = fBase->GetLogPool();
	return logpool->SetBaseFolder(inFolder, true, true);
#else
	return ThrowBaseError(VE_DB4D_NOTIMPLEMENTED, noaction);
#endif
}
...
VError VDB4DBase::OpenJournal( VFile *inFile, VUUID &inDataLink, bool inWriteOpenDataOperation )
{
	...
	JournalPool* logpool = fBase->GetLogPool();
	VError err = logpool->SetBaseFolder(folder, true, true);
...
VError VDB4DBase::StartDataConversion()
{
	return fBase->StartDataConversion();
}


void VDB4DBase::StopDataConversion(CDB4DBaseContext* context, VProgressIndicator* progress, bool MustCheckPrimKeyValidity)
{
	fBase->StopDataConversion(ConvertContext(context), progress, MustCheckPrimKeyValidity);
}
											...
											VDB4DProgressIndicator* InProgress, DB4D_Way_of_Locking HowToLock, sLONG limit, VErrorDB4D *outErr)
{
	...
	// fBase->LockIndexes(); // will be done at another level when necessary
...
VError VDB4DBase::IntegrateJournal(CDB4DJournalParser* inJournal, uLONG8 from, uLONG8 upto, uLONG8 *outCountIntegratedOperations, VDB4DProgressIndicator* InProgress, bool forMirror, sLONG8* lastAction)
{
	VError err = VE_OK;
	...
	if (inJournal != nil)
	{
		...
		if (jparser != nil)
		{
			err = fBase->IntegrateJournal(jparser, from, upto, outCountIntegratedOperations, InProgress, forMirror, lastAction);
		}
		else
...
VErrorDB4D VDB4DBase::PrepareForBackup(CDB4DBaseContextPtr inContext)
{
	...
	else
	{
		...
		if (fBase->FlushAndLock(context))
		{
			fBase->WriteLog(DB4D_Log_StartBackup, nil, false);
		}
		else
...
bool VDB4DBase::GetCatalogErrors(XBOX::VErrorStack& outErrors)
{
	bool sommerrors = false;
	EntityModelCatalog* cat = fBase->GetEntityCatalog(true);
	if (cat != nil)
	{
		ModelErrorReporter* errReporter = cat->GetErrorReporter();
		if (errReporter != nil)
		{
			outErrors = errReporter->GetErrors();
			sommerrors = (outErrors.size() > 0);
		}
	}
	return sommerrors;
}
...
void VDB4DBase::InvalidateAutoCatalog()
{
	fBase->InvalidateAutoCatalog();
}


bool VDB4DBase::SomeTablesCannotBeLogged(VJSONObject* ioParams)
{
	return fBase->SomeTablesCannotBeLogged(ioParams);
}


bool VDB4DBase::CheckPrimaryKeysValidity(CDB4DBaseContext* inContext, VDB4DProgressIndicator* progress, VErrorDB4D& outErr, VJSONObject* ioParams, bool forceCheck)
{
	return fBase->CheckPrimaryKeysValidity(ConvertContext(inContext), progress, outErr, ioParams, forceCheck);
}


void VDB4DBase::GetPrimaryKeysValidityState( CDB4DBaseContext* inContext, VJSONObject* ioParams) const
{
	fBase->GetPrimaryKeysValidityState( ConvertContext( inContext), ioParams);
}


VJSONObject* VDB4DBase::RetainMeasures(VJSONObject* options)
{
	JSONPath jsonpath;
	JSONPath* xpath = nil;
	if (options != nil)
	...
	return fBase->RetainMeasures(options, xpath);
}


void VDB4DBase::SetMeasureInterval(uLONG interval)
{
	fBase->SetMeasureInterval(interval);
}


VError VDB4DBase::BuildCacheFileOnRemoteCatalog(VFile* inRemoteConfigFile, DB4D_RemoteCat_Problems& outProblem)
{
	VError err = VE_OK;
	outProblem = DB4D_RemoteCat_FileIsBad;
	VJSONValue val;
	err = val.LoadFromFile(inRemoteConfigFile);
	if (err == VE_OK && val.IsObject())
	{
		...
		{
			VFolder* parent = inRemoteConfigFile->RetainParentFolder();
			VString catname;
			inRemoteConfigFile->GetNameWithoutExtension(catname);
			VFile *cachefile = new VFile(*parent, catname+kWaRemoteModelExt);
			EntityModelCatalog* remoteCat = VDBMgr::GetManager()->OpenRemoteCatalog(catname, valAccess.GetObject(), fBase, cachefile, true /* store cache only*/, err);
			if (err == VE_OK)
			{
				outProblem = DB4D_RemoteCat_AllOK;
			}
			else
			{
				if (err == VE_HTTP_PROTOCOL_UNAUTHORIZED)
					outProblem = DB4D_RemoteCat_PrivilegeError;
				else
					outProblem = DB4D_RemoteCat_CannotConnect;
			}
...
CDB4DRecord *VDB4DSelection::LoadSelectedRecord( RecIDType inRecordIndex, DB4D_Way_of_Locking HowToLock, CDB4DBaseContextPtr inContext, Boolean WithSubTable, Boolean* outLockWasKeptInTrans)
{
	...
	else
	{
		...
		{
			context = ConvertContext(inContext);
			xbox_assert(context->GetBase() == fSel->GetParentFile()->GetDB());
		}
									...
									VErrorDB4D& outErr, Boolean* outCouldLock, Boolean* outLockWasKeptInTrans)
{
	...
	else
	{
		...
		{
			context = ConvertContext(inContext);
			xbox_assert(context->GetBase() == fTable->GetOwner());
		}
		RecIDType recordID = GetSelectedRecordID( inRecordIndex, inContext);
		if (recordID == -2)
...
RecIDType VDB4DSelection::GetRecordPos(RecIDType inRecordID, CDB4DBaseContextPtr inContext)
{
	...
	{
		context = ConvertContext(inContext);
		xbox_assert( (fSel->GetParentFile() == nil)  || (context->GetBase() == fSel->GetParentFile()->GetDB()) );
	}
	...
	else
	{
		...
		if (testAssert( query != nil)) {
			...
			if (err == VE_OK)
			{
				...
				VDB4DQueryInfoCollector queryInfoCollector(context, &rech, InProgress);
#endif
...
CDB4DQueryResult* VDB4DTable::ExecuteQuery(CDB4DQuery* inQuery, CDB4DBaseContext* inContext, CDB4DQueryOptions* inOptions, VDB4DProgressIndicator* inProgress, VError& outError)
{
	...
	if (context != nil && options->IsDescribeUndef())
		options->SetDescribeQuery(context->ShouldBuildQueryPath());
	...
	else
	{
		if (testAssert( query != nil)) 
		{
			...
			{
				/*VDB4DQueryInfoCollector* queryInfoCollector = NULL;
				if(inProgress != NULL)
				{
					queryInfoCollector = new VDB4DQueryInfoCollector(inContext,query,xresult);
					if(queryInfoCollector != NULL)
					{
				...
				*/
				...
				/*if(inProgress != nil && queryInfoCollector != nil)
				{
					inProgress->SetDefaultInfoCollector(nil);
					delete queryInfoCollector;
				}
				*/
				/*
				if (outError != VE_OK)
...
VJSONObject* VDB4DTable::BuildLockInfo()
{
	VError err = VE_OK;
	if (fTable->GetDF() == nil)
		return nil;
	else
		return fTable->GetDF()->BuildLockInfo(err);
}
...
VError VDB4DTable::AddField(const XBOX::VString& inName, DB4DFieldType inType, sLONG inSize, DB4DFieldAttributes inAttributes, 
							VDB4DProgressIndicator* InProgress, CDB4DBaseContextPtr inContext, bool forceAtTheEnd)
{
	VError err = VE_OK;
	...
	if (locker.CanWork())
	{
		fTable->AddField(inName, inType, inSize, inAttributes, err, inContext, InProgress, forceAtTheEnd);
		fTable->save();
	}
	else
		err = VE_DB4D_OBJECT_IS_LOCKED_BY_OTHER_CONTEXT;
...
VErrorDB4D VDB4DTable::DelayIndexes(bool includesPrimKey)
{
	if (fTable->IsRemote())
	{
	...
	{
		fTable->DelayIndexes(includesPrimKey);
		return VE_OK;
	}
}
...
#if AllowSyncOnRecID
VError VDB4DTable::GetModificationsSinceStamp(sLONG stamp, VStream& outStream, uLONG& outLastStamp, sLONG& outNbRows, CDB4DBaseContextPtr inContext, vector<sLONG>& cols)
{
	if (fTable->IsRemote())
	{
		return ThrowBaseError(VE_DB4D_NOTIMPLEMENTED);
...
VErrorDB4D VDB4DTable::SetPreventJournaling(bool x, CDB4DBaseContextPtr inContext, VDB4DProgressIndicator* InProgress)
{
	return fTable->SetPreventJournaling( inContext, x, InProgress);
}

bool VDB4DTable::GetPreventJournaling(CDB4DBaseContextPtr inContext) const
{
	return fTable->GetPreventJournaling();
}


VErrorDB4D VDB4DTable::GetModificationsSinceStampWithPrimKey(sLONG8 stamp, VStream& outStream, uLONG8& outLastStamp, sLONG& outNbRows, 
...
sWORD VDB4DTable::GetPrimaryKeyValidityState()
{
	return fTable->GetPrimaryKeyValidityState();
}
...
Boolean VDB4DQuery::AddCriteria(CDB4DField* inField, const VectorOfVString& objectPath, DB4DComparator inComparator, const XBOX::VValueSingle& inValue, Boolean inDiacritic)
{
	Field* f = dynamic_cast<VDB4DField*>(inField)->GetField();
	if (f != nil)
	{
		fQuery.AddSearchLineSimple(f, objectPath, inComparator, &inValue, inDiacritic);
		return true;
	}
	else return false;
}
...
{
	ObjectPath objpath;
	fQuery.AddSearchLineEm(inAttributePath, objpath, inComparator, &inValue, inDiacritic);
	return true;
}
...
VError VDB4DComplexQuery::BeginWhere()
{
	return fQuery.AddBeginWhere();
}


VError VDB4DComplexQuery::BeginOn()
{
	return fQuery.AddBeginOn();
}


VError VDB4DComplexQuery::EndOn()
{
	return fQuery.AddEndOn();
}
...
VDB4DRecord::VDB4DRecord( VDBMgr* manager, FicheInMem *inFiche, CDB4DBaseContextPtr inContext)
{
	...
	*/
	if (inContext != nil)
	{
		...
		fContext->Retain();
		xbox_assert(fRecord == nil || fContext->GetBase() == fRecord->GetOwner()->GetOwner());
	}
	else fContext = nil;
}
...
RecIDType VDB4DIndex::FindKey(const VValueSingle &inValToFind, VErrorDB4D& err, CDB4DBaseContext* inContext, VValueSingle* *outResult, CDB4DSelectionPtr Filter, const VCompareOptions& inOptions)
{
	...
	{
		context = ConvertContext(inContext);
		xbox_assert(context->GetBase() == fInd->GetDB());
	}
...
{
	xbox_assert(fNbLevel<kMaxPageLevelInPath);
...
void VDB4DContext::SetThreadPrivileges(CUAGThreadPrivilege* privileges)
{
	for (ContextByBaseMap::iterator cur = fContexts.begin(), end = fContexts.end(); cur != end; cur++)
	{
		cur->second.fContext->SetThreadPrivileges(privileges);
	}
}
...
void VDB4DContext::RemoveDataBaseContext(CDB4DBase* inTarget)
{
	...
	{
		found->second.fContext->Release();
		fContexts.erase(found);
	}
}
...
VDB4DJournalData::VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inRecord, const VUUID& inTableID, PrimKey* primkey)
: fExtraData( RetainRefCountable( inContextExtraData))
{
	xinit(globaloperation, inActionType, inContextID, inTimeStamp);
	fRecordNumber = inRecord;
...
VDB4DJournalData::VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const VValueBag *inContextExtraData, RecordHeader *inRecHeader, uLONG8 inTimeStamp, sLONG8 inPos, VStream* dataStream, const VUUID& inTableID, PrimKey* primkey)
: fExtraData( RetainRefCountable( inContextExtraData))
{
	xinit(globaloperation, inActionType, inContextID, inTimeStamp);
	fRecHeader = *inRecHeader;
...
VDB4DJournalData::VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inBlob, sLONG inBlobLen, const VUUID& inTableID, sLONG8 inPos, VStream* dataStream, PrimKey* primkey, sLONG fieldnum)
: fExtraData( RetainRefCountable( inContextExtraData))
{
	xinit(globaloperation, inActionType, inContextID, inTimeStamp);
	fDataPosInStream = inPos;
...
VDB4DJournalData::VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inRecord, const VUUID& inTableID, PrimKey* primkey, sLONG fieldnum)
: fExtraData( RetainRefCountable( inContextExtraData))
{
	xinit(globaloperation, inActionType, inContextID, inTimeStamp);
	fRecordNumber = inRecord;
...
VDB4DJournalData::~VDB4DJournalData()
{
	...
	QuickReleaseRefCountable( fExtraData);
...
{
	Boolean result = false;
	if ( fActionType == DB4D_Log_CreateRecord || fActionType == DB4D_Log_ModifyRecord || fActionType == DB4D_Log_CreateRecordWithPrimKey || fActionType == DB4D_Log_ModifyRecordWithPrimKey )
	{
		outDataLen = fRecHeader.GetLen();
		result = true;
	}
	else if (fActionType == DB4D_Log_CreateBlob || fActionType == DB4D_Log_ModifyBlob || fActionType == DB4D_Log_CreateBlobWithPrimKey || fActionType == DB4D_Log_ModifyBlobWithPrimKey)
	{
		outDataLen = fBlobLen;
		result = true;
	}
...
Boolean VDB4DJournalData::GetRecordNumber( sLONG &outRecordNumber ) const
{
	...
	if ( fActionType == DB4D_Log_CreateRecord || fActionType == DB4D_Log_ModifyRecord  || fActionType == DB4D_Log_CreateRecordWithPrimKey || fActionType == DB4D_Log_ModifyRecordWithPrimKey)
	{
		...
		result = true;
	} else if (fActionType == DB4D_Log_DeleteRecord || fActionType == DB4D_Log_DeleteRecordWithPrimKey)
	{
...
{
	Boolean result = false;
	if (fActionType == DB4D_Log_CreateBlob || fActionType == DB4D_Log_ModifyBlob || fActionType == DB4D_Log_DeleteBlob  || fActionType == DB4D_Log_CreateBlobWithPrimKey || fActionType == DB4D_Log_ModifyBlobWithPrimKey || fActionType == DB4D_Log_DeleteBlobWithPrimKey)
	{
...
PrimKey* VDB4DJournalData::GetPrimKey() const
{
	return fPrimKey;
}


sLONG VDB4DJournalData::GetFieldNumber() const
{
	return fFieldNum;
}
...
{
	Boolean result = false;
	if ( fActionType == DB4D_Log_CreateRecord || fActionType == DB4D_Log_ModifyRecord || fActionType == DB4D_Log_DeleteRecord || fActionType == DB4D_Log_DeleteRecordWithPrimKey
		|| fActionType == DB4D_Log_CreateBlob || fActionType == DB4D_Log_ModifyBlob || fActionType == DB4D_Log_DeleteBlob 
		|| fActionType == DB4D_Log_TruncateTable  || fActionType == DB4D_Log_SaveSeqNum  || fActionType == DB4D_Log_CreateRecordWithPrimKey || fActionType == DB4D_Log_ModifyRecordWithPrimKey
		|| fActionType == DB4D_Log_CreateBlobWithPrimKey || fActionType == DB4D_Log_ModifyBlobWithPrimKey || fActionType == DB4D_Log_DeleteBlobWithPrimKey)
	{
		outID = fTableID;
		result = true;
	}
...
Boolean VDB4DJournalData::ReadDataFromPos()
{
	if (fDataPtr == nil)
	{
		...
		else
		{
			...
			sLONG dataSize;
			sLONG offset = 0;
			if (fActionType == DB4D_Log_CreateBlob || fActionType == DB4D_Log_ModifyBlob  || fActionType == DB4D_Log_CreateBlobWithPrimKey || fActionType == DB4D_Log_ModifyBlobWithPrimKey)
			{
				dataSize = fBlobLen/*+4*/;
				offset = 4;
...
VJSONObject* VDB4DJournalData::RetainDescription(VError& outErr, bool withFullDetail, CDB4DBase* inBase)
{
	VJSONObject* res = new VJSONObject();
	VString s;

	res->SetPropertyAsNumber("operationType", fActionType);
	Helpers::LocalizeJournalOperation(fActionType, s);
	res->SetPropertyAsString("operationName", s);

	res->SetPropertyAsNumber("operationNumber", fGlobalOperation);
	
	sLONG8 contextid;
	if (GetContextID(contextid))
		res->SetPropertyAsNumber("contextID", contextid);


	VTime tstamp;
	tstamp.FromStamp(fTimeStamp);
	tstamp.GetXMLString(s, XSO_Time_UTC);
	res->SetPropertyAsString("timeStamp", s);

	sLONG len;
	if (GetDataLen(len))
		res->SetPropertyAsNumber("dataLen", len);

	sLONG recnum;
	VString path;
	if (GetRecordNumber(recnum))
		res->SetPropertyAsNumber("recordNumber", recnum);
	if (GetBlobNumber(recnum, path))
		res->SetPropertyAsNumber("blobNumber", recnum);
	if (!path.IsEmpty())
		res->SetPropertyAsString("blobPath", path);

	if (fPrimKey != nil)
	{
		//res->SetProperty("primKey", fPrimKey->GetJsonValue());
	}

	if (fFieldNum != -1)
		res->SetPropertyAsNumber("fieldNumber", fFieldNum);

	sLONG8 seqnum;
	if (GetSequenceNumber(seqnum))
		res->SetPropertyAsNumber("sequenceNumber", seqnum);

	VUUID id;
	if (GetTableID(id))
	{
		id.GetString(s);
		res->SetPropertyAsString("tableID", s);

		if (inBase != nil)
		{
			Base4D* db = dynamic_cast<VDB4DBase*>(inBase)->GetBase();
		Table* tt = db->FindAndRetainTableRef(id);
		if (tt != nil)
		{
			tt->GetName(s);
			res->SetPropertyAsString("tableName", s);
		}
	}
	}
	...
	{
		id.GetString(s);
		res->SetPropertyAsString("userID", s);
	}
	return res;
}
...
VIntlMgr *VDB4DRawDataBase::CreateIntlMgr()
{
	...
	VIntlMgr *newIntl = NULL;

	if (header != NULL)
	{
		newIntl = VIntlMgr::Create(header->dialect, header->collatorOptions);
	}
...
{
	VIntlMgr *newIntl = CreateIntlMgr();
	VIntlMgr *curIntl = VTask::GetCurrentIntlManager();
	curIntl->Retain();

	if (newIntl != NULL)
	{
		VTask::SetCurrentIntlManager(newIntl);
		newIntl->Release();
	}
