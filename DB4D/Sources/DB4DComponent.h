...
public:
	...
	virtual DB4DNetworkManager* CreateRemoteTCPSession(const VString& inServerName, uWORD inPortNum, VError& outerr, bool inSSL = false);
	...
	virtual VErrorDB4D OpenRemoteCatalogs(void* projectRef, const XBOX::VFolder* folderToScan, CDB4DBase* associate, bool stopOnFirstError);
...
/*
	...
	*/
	...
	virtual CDB4DBase* OpenBase( const VFile& inStructureFile, sLONG inParameters, VError* outErr = nil, FileAccess inAccess = FA_READ_WRITE, VString* EntityFileExt = nil, 
								CUAGDirectory* inUAGDirectory = nil, const VFile* inPermissionsFile = nil, void* ProjectRef = nil);
	...
	virtual void	ReleaseContextsOfOrphanedUser(const XBOX::VUUID& inUserID);
	...
	virtual VError StartServer(const VString& inServerName, uWORD inPortNum, bool inSSL = false);
	...
	virtual void PatchTableAndFieldNames(XBOX::VValueBag *inDefinition_bag,const std::vector<XBOX::VString>& existingTableNames, const std::vector<XBOX::VString>& existingFieldNames,std::map<XBOX::VString,XBOX::VString> &tables_map,std::map<XBOX::VString,XBOX::VString> &fields_map) const;
	virtual	VErrorDB4D	BuildValid4DTableName( const VString& inName, VString& outValidName) const;
	virtual void GetUniqueName(const std::vector<VString>& inAlreadyUsedNames, const VString &inName, VString &outUniqueName, bool inWithEllipse, VIndex inMaxLength) const;
	...
	virtual	VJSObject			CreateJSDatabaseForMaintenanceObject( const VJSContext& inContext, CDB4DBaseContext *inBaseContext);
	...
	virtual	void				InitJSContextWithEntityModels(VJSObject& globalObject, CDB4DBaseContext *inBaseContext, void* ProjectRef, std::vector<XBOX::VFile*>& outModelJSFiles);
	virtual	void				ReleaseProjectInfo(void* ProjectRef);
	...
	virtual IHTTPRequestHandler*	AddRestRequestHandler( VErrorDB4D& outError, CDB4DBase *inBase, IHTTPServerProject *inHTTPServerProject, RIApplicationRef inApplicationRef, const XBOX::VString& inPattern, bool inEnabled, IHTTPRequestHandler **outAdminRequestHandler);

	virtual void ForceExpires4DRestSession(const XBOX::VUUID& inSessionID);
	virtual void GetAll4DRestSessions(std::vector<XBOX::VUUID>& outSessions);
	virtual XBOX::VJSONObject* RetainUserSessionRequestInfo(const XBOX::VUUID& inSessionID);
	virtual void SetUserSessionRequestInfo(const XBOX::VUUID& inSessionID, XBOX::VJSONObject* inRequestInfo);
	...
	#if DB4DasDLL
	static	VDB4DMgr*	RetainInstance( XBOX::VLocalizationManager* inLocalizationManager);
	#endif

protected:
	#if DB4DasDLL
	static	VDB4DMgr*	sInstance;
	#endif
...
public:
	...
	virtual Boolean CreateData( const VFile& inDataFile, sLONG inParameters, VFile *inJournalingFile = NULL, CDB4DBaseContextPtr inContext = NULL, VErrorDB4D* outErr = nil, FileAccess inAccess = FA_READ_WRITE, CDB4DRawDataBase* fromData = nil) const;
...
//	virtual CNameSpace* GetNameSpace() const;
	...
	virtual VError SetJournalFileInfos( VString *inFilePath, VUUID *inUUID,bool inResetSequenceNumber = false );
//	virtual VError ResetJournalFileContent();
	...
	virtual VErrorDB4D ActivateJournal(VFolder *inFolder, VUUID &inDataLink, bool inWriteOpenDataOperation = true);
	...
	virtual VError StartDataConversion();
	virtual void StopDataConversion(CDB4DBaseContext* context, VProgressIndicator* progress, bool MustCheckPrimKeyValidity);
	...
	virtual VError IntegrateJournal(CDB4DJournalParser* inJournal, uLONG8 from = 0, uLONG8 upto = 0, uLONG8 *outCountIntegratedOperations = NULL, VDB4DProgressIndicator* InProgress = NULL, bool forMirror = false, sLONG8* lastAction = NULL);
	...
	virtual bool GetCatalogErrors(XBOX::VErrorStack& outErrors);

	virtual void InvalidateAutoCatalog();

	virtual bool SomeTablesCannotBeLogged(VJSONObject* ioParams);

	virtual bool CheckPrimaryKeysValidity(CDB4DBaseContext* inContext, VDB4DProgressIndicator* progress, VErrorDB4D& outErr, VJSONObject* ioParams, bool forceCheck = false);

	virtual	void GetPrimaryKeysValidityState( CDB4DBaseContext* inContext, VJSONObject* ioParams) const;

	virtual VError BuildCacheFileOnRemoteCatalog(VFile* inRemoteConfigFile, DB4D_RemoteCat_Problems& outProblem);

	virtual VJSONObject* RetainMeasures(VJSONObject* options = nil);
	virtual void SetMeasureInterval(uLONG interval);
...
public:
	...
	virtual VError AddField(const XBOX::VString& inName, DB4DFieldType inType, sLONG inSize, DB4DFieldAttributes inAttributes, 
							VDB4DProgressIndicator* InProgress = NULL, CDB4DBaseContextPtr inContext = NULL, bool forceAtTheEnd = false);
	...
	virtual VErrorDB4D DelayIndexes(bool includesPrimKey);
...
#if AllowSyncOnRecID
	virtual VError GetModificationsSinceStamp(sLONG stamp, VStream& outStream, uLONG& outLastStamp, sLONG& outNbRows, CDB4DBaseContextPtr inContext, vector<sLONG>& cols);
	...
	virtual VErrorDB4D SetPreventJournaling(bool preventJournaling, CDB4DBaseContextPtr inContext, VDB4DProgressIndicator* InProgress = nil);

	virtual bool GetPreventJournaling(CDB4DBaseContextPtr inContext) const;

	virtual VErrorDB4D GetModificationsSinceStampWithPrimKey(sLONG8 stamp, VStream& outStream, uLONG8& outLastStamp, sLONG& outNbRows, 
	...
	virtual VJSONObject* BuildLockInfo();
	...
	virtual sWORD GetPrimaryKeyValidityState();
...
public:
	...
	virtual Boolean AddCriteria(CDB4DField* inField, const VectorOfVString& objectPath, DB4DComparator inComparator, const XBOX::VValueSingle& inValue, Boolean inDiacritic = false);
...
class VDB4DComplexQuery : public CDB4DComplexQuery
{
	public:
		...
		virtual VError BeginWhere();
		virtual VError BeginOn();
		virtual VError EndOn();
...
class VDB4DContext : public CDB4DContext
{
	public:
		...
		virtual void RemoveDataBaseContext(CDB4DBase* inTarget);
		...
		*/
		...
		virtual void SetThreadPrivileges(CUAGThreadPrivilege* privileges);

		// inherited from CDB4DContext
		virtual	void	GetUser4DSessionID( VUUID& outUUID) const	{ outUUID = fUser4DSessionID;}
				void	SetUser4DSessionID( const VUUID& inUUID)	{ fUser4DSessionID = inUUID;}
...
class VDB4DJournalData : public CDB4DJournalData
{
	public:
		...
		VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, RecordHeader *inRecHeader, uLONG8 inTimeStamp, sLONG8 inPos, VStream* dataStream, const VUUID& inTableID, PrimKey* primkey);
		VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inRecord, const VUUID& inTableID, PrimKey* primkey);
		VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inBlob, sLONG inBlobLen, const VUUID& inTableID, sLONG8 inPos, VStream* dataStream, PrimKey* primkey, sLONG fieldnum);
		VDB4DJournalData(sLONG8 globaloperation, uLONG inActionType, sLONG8 inContextID, const XBOX::VValueBag *inContextExtraData, uLONG8 inTimeStamp, sLONG inRecord, const VUUID& inTableID, PrimKey* primkey, sLONG fieldnum);
		...
		virtual PrimKey* GetPrimKey() const;
		virtual sLONG GetFieldNumber() const;

		virtual VJSONObject* RetainDescription(VError& outErr, bool withFullDetail, CDB4DBase* inBase);
...
class VDB4DRawDataBase : public CDB4DRawDataBase
{
	public:
		...
		virtual VIntlMgr *CreateIntlMgr();
		VIntlMgr *SwitchIntlMgr();

		Base4D_NotOpened* GetBase()
		{
			return fBase;
		}
