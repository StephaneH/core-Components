...
void InitOptionalModule (VString NameModule)
{
	VString Suffix;
#if VERSIONDEBUG
	Suffix.AppendCString("Debug");
#elif WITH_ASSERT
	Suffix.AppendCString("Beta");
#endif
	
#if VERSIONWIN
	VFilePath path = VProcess::Get()->GetExecutableFolderPath().ToSubFile( NameModule + Suffix + ".dll");
#elif VERSION_LINUX
	VFilePath path = VProcess::Get()->GetExecutableFolderPath().ToSubFolder("Libs").ToSubFile(NameModule + Suffix + ".so");
#elif VERSIONMAC
	VFilePath path = VProcess::Get()->GetExecutableFolderPath().ToParent().ToSubFolder( CVSTR( "Frameworks")).ToSubFolder(NameModule + Suffix + ".framework");
#endif
	
#if VERSIONMAC
	VFolder OptionalModule(path);
#else
	VFile OptionalModule(path);
#endif
	
	if (OptionalModule.Exists())
	{
		XBOX::VLibrary* OptionalModuleLib = new XBOX::VLibrary(path);
		
		if ( OptionalModuleLib != nil)
		{
			if (OptionalModuleLib->Load())
			{
				initCodeFunc initCode = (initCodeFunc) OptionalModuleLib->GetProcAddress("Init" + NameModule + "Component");
				if (initCode == nil)
					initCode = (initCodeFunc) OptionalModuleLib->GetProcAddress("_Init" + NameModule + "Component@0");
				if (initCode != nil)
					initCode();
				else
					xbox_assert(false);
			...
			//delete OptionalModuleLib; pas de liberation de la lib ici
		}
	}
	return ;
}

void ContextKeep::Init(CDB4DBase* base, uLONG timeout)
{
	fApplicationAquired = false;
	fContext = base->NewContext(nil, nil);
	BaseTaskInfo* context = ConvertContext(fContext);
	fJSDelegate = new DB4DJSRuntimeDelegate(context);
	fJSGlobalContext = VJSGlobalContext::Create( fJSDelegate);
	context->SetJSContext(fJSGlobalContext);
	fBase = base; // not retained on purpose
	fTimeout = timeout;
	fNextFree = -4;
}


void ContextKeep::Dispose(IHTTPResponse* inResponse, ContextStash& inStash)
{
	if (fApplicationAquired)
	{
		IDB4D_ApplicationIntf *applicationIntf = VDBMgr::GetManager()->GetApplicationInterface();
		applicationIntf->ReleaseJSContext( fRIAAppRef, fJSGlobalContext, inResponse);
	}
	else
	{
		fContext->SetJSContext(NULL);
		fContext->Release();
		QuickReleaseRefCountable(fJSGlobalContext);
		if (fJSDelegate != nil)
			delete fJSDelegate;
		//		CleanUpForReUse(inStash);
	}
}


void ContextKeep::Aquired(CDB4DBase* base, VJSGlobalContext* inJSGlobalContext, RIApplicationRef inRIAAppRef, uLONG timeout)
{
	fBase = base; // not retained on purpose
	fRIAAppRef = inRIAAppRef;
	fJSGlobalContext = inJSGlobalContext;
	fApplicationAquired = true;
	fTimeout = timeout;
}


void ContextKeep::CleanUpForReUse(ContextStash& inStash)
{
	xbox_assert(fApplicationAquired == false);
	xbox_assert(fPosInStack != -1);
	fContext->CleanUpForReuse();
	inStash.FreeKeep(this);
}
...
ContextStash::~ContextStash()
{
	DisposeAllContexts();
}


void ContextStash::FreeKeep(ContextKeep* contextKeeper)
{
	VTaskLock lock(&fMutex);
	//contextKeeper->fNextFree = fNextFree;  // contextKeeper may be a copy of what is in fStack (coming from fStash)
	fStack[contextKeeper->fPosInStack]->fNextFree = fNextFree;
	fNextFree = contextKeeper->fPosInStack;
}

void ContextStash::DisposeAllContextsOnBase(CDB4DBase* base)
{
	VTaskLock lock(&fMutex);
	vector< VUUID > fRefToDelete;
	for (contextcollection::iterator cur = fStash.begin(), end = fStash.end(); cur != end; ++cur)
	{
		if (cur->second.fBase == base)
		{
			fRefToDelete.push_back(cur->first);
			cur->second.Dispose(nil, *this);
		}
	}

	for (vector<VUUID>::iterator cur = fRefToDelete.begin(), end = fRefToDelete.end(); cur != end; ++cur)
	{
		fStash.erase(*cur);
	}

	if (fNextFree != -1)
		fStack[fNextFree]->fNextFree = -2;

	contextstack newstack;
	newstack.reserve(fStack.size());
	for (contextstack::iterator cur = fStack.begin(), end = fStack.end(); cur != end; ++cur)
	{
		if ((*cur)->fBase == base)
		{
			(*cur)->Dispose(nil, *this);
			delete (*cur);
		}
		else
			newstack.push_back(*cur);
	}
	fStack.swap(newstack);
	sLONG curpos = 0;
	fNextFree = -1;
	for (contextstack::iterator cur = fStack.begin(), end = fStack.end(); cur != end; ++cur, ++curpos)
	{
		(*cur)->fPosInStack = curpos;
		if ((*cur)->fNextFree != -1)
		{
			(*cur)->fNextFree = fNextFree;
			fNextFree = curpos;
		}
	}
...
void ContextStash::DisposeAllContexts()
{
	VTaskLock lock(&fMutex);
	for (contextcollection::iterator cur = fStash.begin(), end = fStash.end(); cur != end; ++cur)
	{
...
void ContextStash::Dispose(ContextKeep* contextKeeper, IHTTPResponse* inResponse)
{
	{
		VTaskLock lock(&fMutex);
		//contextKeeper->Dispose(inResponse);
		fStash.erase(contextKeeper->fID);
	}
}


ContextKeep* ContextStash::GetContextKeep(const VUUID& inID)
{
	ContextKeep* result = nil;
	VTaskLock lock(&fMutex);
	CheckExpired();
	contextcollection::iterator found = fStash.find(inID);
	if (found != fStash.end())
	{
		result = &(found->second);
		result->ResetTimer();
...
/*
ContextKeep* ContextStash::CreateContextKeep(CDB4DBase* base, uLONG timeout)
{
	ContextKeep* result;
	VUUID id(true);
	{
	...
	result->Init(base, timeout);
	return result;
}
*/


ContextKeep* ContextStash::CreateContextKeep(CDB4DBase* base, VJSGlobalContext* inJSGlobalContext, RIApplicationRef inRIAAppRef, uLONG timeout)
{
	ContextKeep* result;
	VUUID id(true);
	{
	...
	result->Aquired(base, inJSGlobalContext, inRIAAppRef, timeout);
	return result;
}


void ContextStash::Add(ContextKeep& contextKeeper, uLONG timeout)
{
	VUUID id(true);
	{
		VTaskLock lock(&fMutex);
		contextKeeper.fID = id;
		contextKeeper.fTimeout = timeout;
		ContextKeep* newone = &fStash[id];
		*newone = contextKeeper;
		newone->fJSDelegate = nil;
		newone->fContext->Retain();
		newone->fJSGlobalContext->Retain();
	}
}


void ContextStash::CheckExpired()
{
	uLONG curtime = VSystem::GetCurrentTime();
	if ( DiffTime(curtime, fLastCheck) > 60*1000)
	{
		VTaskLock lock(&fMutex);
		vector< VUUID > fRefToDelete;
		for (contextcollection::iterator cur = fStash.begin(), end = fStash.end(); cur != end; ++cur)
...
ContextKeep* ContextStash::GetNewContextKeep(CDB4DBase* base, uLONG timeout)
{
	ContextKeep* result = nil;
	{
		VTaskLock lock(&fMutex);
		if (fNextFree != -1)
		{
			result = fStack[fNextFree];
			fNextFree = result->fNextFree;
			result->fNextFree = -3;
		}
		else
		{
			result = new ContextKeep();
			result->Init(base, timeout);
			result->fPosInStack = (sLONG)fStack.size();
...
#if debuglr
void ContextStash::debugDump(VString& outStr)
{
	outStr.Clear();
	outStr = "FirstFree : " + ToString(fNextFree) + "\n";

	sLONG pos = 0;
	for (contextstack::iterator cur = fStack.begin(), end = fStack.end(); cur != end; ++cur,++pos)
	{
		ContextKeep* keeper = *cur;
		
		VString s = "pos : " + ToString(pos);
		s += " , posInStack : " + ToString(keeper->fPosInStack);
		s += " , nextFree : " + ToString(keeper->fNextFree);
		/*
		VString aquiredstr;
		aquiredstr.FromBoolean(keeper->fApplicationAquired);
		s += " , aquired : " + aquiredstr;
		*/
		VString contextstr;
		contextstr.FromHexLong((uLONG8)ConvertContext(keeper->fContext));
		s += " , context : " + contextstr;
...
#endif
{
	fLastReleaseExpiredUAGSessions = 0;
...
VDBMgr::~VDBMgr()
{
	if (fCronTask != nil) {
		...
		fCronTask->Kill();
		fCronSync.Unlock();
		QuickReleaseRefCountable(fCronTask);
	}
	...
	if (fMirrorTask != nil)
	{
		fMirrorTask->WakeUp();
		fMirrorTask->Kill();
		QuickReleaseRefCountable(fMirrorTask);
	}
	...
	*/
	...
	VErrorBase::UnregisterLocalizer(kBACKUP_RESTORE_SIGNATURE);
	VErrorBase::UnregisterLocalizer(CDB4DManager::Component_Type);
...
XBOX::VError VDBMgr::GetJournalInfo(const XBOX::VFilePath& inDataFilePath,XBOX::VFilePath& outJournalPath,XBOX::VUUID& outDataLink)
{
	...
	//O.R. Nov 7th 2013: this API is now only used in Wakanda and for compatibility 
	//purposes only (retrieving journal infos from waExtra files)
	if(testAssert(GetManager()->IsRunningWakanda()))
	{
				...
				if (error == VE_OK)
				{
					...
					if ( journal_bag != NULL )
					{
						...
						if( !logFilePath.IsEmpty() )
						{
							...
							{
								//WAK0084579 O.R., resolve against a POSIX relative path AND against the DataFolder, no the data file
								XBOX::VFilePath parent;
								inDataFilePath.GetParent(parent);
								outJournalPath.FromRelativePath(parent,logFilePath, FPS_POSIX);
							}
							else
							{
...
CDB4DBase* VDBMgr::OpenBase( const VFile& inStructureFile, sLONG inParameters, VError* outErr, FileAccess inAccess, VString* EntityFileExt, 
							CUAGDirectory* inUAGDirectory, const VString* inHtmlContent, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles, const VFile* inPermissionsFile, void* ProjectRef )
{
	return xOpenCreateBase( inStructureFile, false, inParameters, nil, outErr, inAccess, nil,  EntityFileExt, inUAGDirectory, inHtmlContent, outIncludedFiles, inPermissionsFile, ProjectRef );
}
...
CDB4DBase* VDBMgr::xOpenCreateBase( const VFile& inStructureFile, Boolean inCreate, sLONG inParameters, VIntlMgr* inIntlMgr, VError* outErr, FileAccess inAccess, 
									 ...
									 const VFile* inPermissionsFile, void* ProjectRef)
{
//	ClearError();
	VError err = VE_OK;
	
	Base4D *basd = new Base4D(fFlushMgr, inAccess != FA_READ);
	basd->SetRiaServerProjectRef(ProjectRef);

	IDB4D_ApplicationIntf *applicationIntf = VDBMgr::GetManager()->GetApplicationInterface();
	if (applicationIntf != nil && ProjectRef != nil)
	{
		VFileSystemNamespace* fsnamespace = applicationIntf->RetainRuntimeFileSystemNamespace((RIApplicationRef) ProjectRef);
		if (fsnamespace != nil)
		{
			basd->SetFileSystemNamespace(fsnamespace);
			fsnamespace->Release();
		}
	}
...
#endif
{
	...
	*/
	...
	xbox_assert(fDefaultLocalization != nil);
	/*
	...
	VErrorBase::RegisterLocalizer(kBACKUP_RESTORE_SIGNATURE, fDefaultLocalization);
...
//	Register_DB4DArrayOfValuesCreator('cons', DBAVConsCreator);
	...
	*/
	...
	fCronTask = new VDBCronTask();
	fCronTask->SetName( CVSTR( "DB4D CRON"));
	fCronTask->Run();


	InitOptionalModule ("RemoteEntity");
	InitOptionalModule ("SQLEntity");
...
void VDBMgr::KeepPostPonedContext(const VString& sessionID, CDB4DContext* inContext, const VUUID& inRemoteUID)
{
	VTaskLock lock(&fAllContextsMutex);
	inContext->Retain();
	fPostPonedContexts[sessionID] = make_pair(inContext, inRemoteUID.GetBuffer());
}


CDB4DContext* VDBMgr::StealPostPonedContext(const VString& sessionID, VUUID& outUserID)
{
	VTaskLock lock(&fAllContextsMutex);
	...
	if (found != fPostPonedContexts.end())
	{
		...
		outUserID.FromBuffer(found->second.second);
...
void VDBMgr::ForgetAllPostPonedContextsForUser(const VUUID& inUserID)
{
	VTaskLock lock(&fAllContextsMutex);
	mapOfContextBySessionID::iterator iter = fPostPonedContexts.begin();
	while (iter != fPostPonedContexts.end())
	{
		// Note that (before C++11) map's erase() doesn't return an iterator and invalidates the erased iterator
		// so we have to work on a copy and increment iter *before* erasing
		mapOfContextBySessionID::iterator cur = iter++;
		if (cur->second.second == inUserID.GetBuffer())
		{
			cur->second.first->Release();
			fPostPonedContexts.erase(cur);
		}
	}
}
...
VError VDBMgr::StartServer(const VString& inServerName, uWORD inPortNum, bool inSSL)
{
	if (inPortNum == 0)
...
{
	StErrorContextInstaller errs(false);
	xbox_assert(inRequestID >= kRangeReqDB4DWithBaseID && inRequestID < kMaxRangeReqDB4DWithBaseID);
	...
	switch(inRequestID-kRangeReqDB4D)
	{
		...
		case Req_SetTablePreventJournaling:
			ExecSetTablePreventJournaling( inRequest, inContext);
			break;
...
{
	xbox_assert(inRequestID >= kRangeReqDB4D && inRequestID < kMaxRangeReqDB4D);
...
void VDBMgr::ExecSetTablePreventJournaling( IRequestReply *inRequest, CDB4DBaseContext *inContext)
{
	VError err = VE_OK;
	Boolean alreadysend = false;
	Base4D *base = inRequest->RetainBaseParam( err);
	if (err == VE_OK && base != nil)
	{
		...
		if (err == VE_OK && tt != nil)
		{
			...
			if (err == VE_OK)
			{
				...
				if (locker.CanWork())
				{
					err = tt->SetPreventJournaling( inContext, keepStamp, progress);
				}
				else
				{
					err = ThrowBaseError( VE_DB4D_OBJECT_IS_LOCKED_BY_OTHER_CONTEXT, DBaction_ChangingTableProperties);
...
void VDBMgr::RemoveKeptDatasetsOnBase(Base4D* inBase)
{
	vector<VUUIDBuffer> toDelete;
	...
	for (mapOfDataSet::iterator cur = fKeptDataSets.begin(), end = fKeptDataSets.end(); cur != end; ++cur)
	{
		...
		Base4D* assocBase = ds->GetModel()->GetCatalog()->GetAssocBase();
		if (assocBase == inBase)
			toDelete.push_back(cur->first);
	}

	for (vector<VUUIDBuffer>::iterator cur = toDelete.begin(), end = toDelete.end(); cur != end; ++cur)
	{
		mapOfDataSet::iterator found = fKeptDataSets.find(*cur);
...
{
	bool result = true;
	Base4D* assocBase = inDataSet->GetModel()->GetCatalog()->GetAssocBase();
	assocBase->occupe();
	if (assocBase->IsClosing())
		result = false;
	else
	{
		VTaskLock lock(&fKeptDataSetsMutex);
...
void VDBMgr::ReleaseExpiredUAGSessions()
{
	uLONG curtime = VSystem::GetCurrentTime();
	if ( DiffTime(curtime, fLastReleaseExpiredUAGSessions) > 10*1000)
	{
		if (fApplicationIntf != nil)
			fApplicationIntf->ReleaseExpiredUAGSessions();
		fLastReleaseExpiredUAGSessions = curtime;
	}
}


void VDBMgr::InitJSContextWithEntityModels(VJSObject& globalObject, BaseTaskInfo* context, void* ProjectRef, std::vector<XBOX::VFile*>& outModelJSFiles)
{
	VJSContext jscontext(globalObject.GetContext());
	jscontext.GetGlobalObjectPrivateInstance()->SetSpecific( 'db4x', ProjectRef, VJSSpecifics::DestructorVoid);

	context->GetBase()->GetGoodEntityCatalog(true)->EvaluateModelScript(globalObject, context, ProjectRef, outModelJSFiles);
	context->GetBase()->EvaluateOutsideCatalogModelScript(globalObject, context, ProjectRef, outModelJSFiles);

	VJSDatabase::PutAllModelsInGlobalObject(globalObject, context->GetBase(), context);
	{
		VTaskLock lock(&fProjectInfosMutex);
		ProjectMap::iterator found = fProjectInfos.find(ProjectRef);
		if (found != fProjectInfos.end())
		{
			found->second.SetCatalogsAsGlobalProperties(globalObject, context);
		}
	}
}


void VDBMgr::AddOutsideCatalog(const VString& name, EntityModelCatalog* cat, void* ProjectRef)
{
	VTaskLock lock(&fProjectInfosMutex);
	fProjectInfos[ProjectRef].AddCatalog(name, cat);
}


void VDBMgr::RemoveProjectInfo(void* ProjectRef)
{
	if (ProjectRef != nil)
	{
		vector<EntityModelCatalog*> cats;
		{
			VTaskLock lock(&fProjectInfosMutex);
			ProjectMap::iterator found = fProjectInfos.find(ProjectRef);
			if (found != fProjectInfos.end())
			{
				found->second.PutCatalogsInto(cats);
				fProjectInfos.erase(found);
			}
		}
		for (vector<EntityModelCatalog*>::iterator cur = cats.begin(), end = cats.end(); cur != end; ++cur)
		{
...
VError VDBMgr::OpenRemoteCatalogs(void* projectRef, const VFolder* folderToScan, Base4D* associate, bool stopOnFirstError)
{
	VError err = VE_OK;
	...
	while (iter.IsValid() && cont)
	{
		...
		{
			StErrorContextInstaller errs(stopOnFirstError, false);
			VJSONValue val;
			...
			if (err == VE_OK && val.IsObject())
			{
				...
				if (valAccess.IsObject())
				{
					...
					if (!merge)
					{
						...
						VFile *cachefile = new VFile(*folderToScan, catname+kWaRemoteModelExt);
						EntityModelCatalog* remoteCat = OpenRemoteCatalog(catname, valAccess.GetObject(), associate, cachefile, true /* store cache only*/, err);
						if (err == VE_OK)
						{
							AddOutsideCatalog(catname, remoteCat, projectRef);
						}
						cachefile->Release();
					}
				}
...
EntityModelCatalog* VDBMgr::OpenRemoteCatalog(const VString& inCatName, VJSONObject* inAccessInfo, Base4D* associate, VFile* inCacheFile, bool useAsCacheOnly, VError& outErr)
{
	outErr = VE_OK;

	sLONG kTypeOfCatalog = kRemoteCatalogFactory;
	bool sql = false;
	if (inAccessInfo->GetPropertyAsBool(kSQL, &sql))
	{
		if (sql)
			kTypeOfCatalog = kSQLCatalogFactory;
	}

	VError err = VE_OK;

	EntityModelCatalog* remotecat = EntityModelCatalog::NewCatalog(kTypeOfCatalog, associate, inAccessInfo, outErr);
	...
	if (remotecat != nil)
	{
		...
		{
			VString localJsPath;
			if (inAccessInfo->GetPropertyAsString(kLocalJSPath, localJsPath) && !localJsPath.IsEmpty())
			{
				VFolder* ff = RetainRefCountable(associate->GetStructFolder());
				localJS = new VFile(*ff, localJsPath, FPS_POSIX);
				ReleaseRefCountable( &ff);
			}
		}
		if (localJS == nil && inCacheFile != nil)
		{
			VFolder* parent = inCacheFile->RetainParentFolder();
			if (parent != nil)
			{
				VString filename;
				inCacheFile->GetNameWithoutExtension(filename);
				localJS = new VFile(*parent, filename+".js");
			}
		}
		...
		{
			if (localJS->Exists())
				remotecat->SetModelJSFile(localJS);
			VFolder* parent = localJS->RetainParentFolder();
			if (parent != nil)
			{
				remotecat->SetModelFolder(parent);
				parent->Release();
			}
			VString modeljsname;
			EntityModelCatalog::GetNewJSModelName(inCatName, modeljsname);
			remotecat->SetModelJSName(modeljsname);
			err = remotecat->addJSExtensions(associate, localJS, nil);
			if (err != VE_OK)
				outErr = err;
		}

		QuickReleaseRefCountable(localJS);

		if (err == VE_OK)
		{
			VFolder* ff = RetainRefCountable(associate->GetStructFolder());
			VFile* permfile = new VFile(*ff, "Permissions.waPerm");
			QuickReleaseRefCountable(ff);
			CUAGDirectory* dir = associate->GetUAGDirectory();
			if (dir != nil)
			{
				err = remotecat->LoadEntityPermissions(*permfile, dir, true, nil);
			}
			QuickReleaseRefCountable(permfile);
		}
	}

	if (remotecat != nil && inCacheFile != nil)
	{
		...
		if (!useAsCacheOnly)
		{
			...
			VJSONValue val(inAccessInfo);
			VString accessstr;
			val.Stringify(accessstr);
			VJSONImporter imp(accessstr);
			imp.JSONObjectToBag(*subbag);
			bag.AddElement(kremoteAccess, subbag);
			subbag->SetBool("____objectunic", true);
			QuickReleaseRefCountable(subbag);
		}
		{
			StErrorContextInstaller errs(false);
			VJSONValue oldval;
			VError err2 = oldval.LoadFromFile(inCacheFile);
			if (err2 == VE_OK)
			{
				if (oldval.IsObject())
				{
					VJSONObject* oldobj = oldval.GetObject();
					VJSONValue valextra = oldobj->GetProperty("extraProperties");
					if (valextra.IsObject())
					{
						VValueBag* subbag = new VValueBag();
						VString extrastr;
						valextra.Stringify(extrastr);
						VJSONImporter imp(extrastr);
						imp.JSONObjectToBag(*subbag);
						bag.AddElement("extraProperties", subbag);
						subbag->SetBool("____objectunic", true);
						QuickReleaseRefCountable(subbag);
						okextra = true;
					}
		...
		if (!okextra)
		{
			VValueBag* subbag = new VValueBag();
			bag.AddElement("extraProperties", subbag);
			subbag->SetString("version", "2");
			subbag->SetBool("____objectunic", true);
			
			VValueBag* subbag2 = new VValueBag();
			subbag->AddElement("model", subbag2);
			subbag2->SetString("scriptMode", "manual");
			subbag2->SetBool("____objectunic", true);
			QuickReleaseRefCountable(subbag2);

			VValueBag* subbag3 = new VValueBag();
			subbag->AddElement("classes", subbag3);
			subbag3->SetBool("____objectunic", true);
			QuickReleaseRefCountable(subbag3);

			QuickReleaseRefCountable(subbag);
		}
		err = remotecat->SaveEntityModels(bag, true, false);
		if (err == VE_OK)
		{
			bag.SetBool("isRemote", true);
			if (inCacheFile->Exists())
				inCacheFile->Delete();
			VString jsonstring;
			bag.GetJSONString(jsonstring);
			err = inCacheFile->SetContentAsString( jsonstring, VTC_UTF_8);
...
void VDBMgr::CronSleepFor(sLONG nbmilliseconds)
{
	fCronSync.ResetAndLock();
	fCronSync.Wait(nbmilliseconds);
}


void VDBMgr::CronWakeUp()
{
	fCronSync.Unlock();
}


VError VDBMgr::ReleaseWhatNeedsToBeReleasedOnCatalogs()
{
	VError err = VE_OK;
	vector<Base4D*> bases;
	{
		VTaskLock lock(&fBasesMutex);
		bases.reserve(fBases.GetCount());
		for (VArrayOf<base4dinfo>::Iterator cur = fBases.First(), end = fBases.End(); cur != end; ++cur)
		{
			if (cur->base != nil && !cur->isClosing)
				bases.push_back(RetainRefCountable(cur->base));
		}
	}
	for (vector<Base4D*>::iterator cur = bases.begin(), end = bases.end(); cur != end; ++cur)
	{
		Base4D* base = *cur;
		base->ReleaseWhatNeedsToBeReleasedOnCatalogs();
		base->Release();
	}
	...
	{
		VTaskLock lock(&fProjectInfosMutex);
		for (ProjectMap::iterator cur = fProjectInfos.begin(), end = fProjectInfos.end(); cur != end; ++cur)
		{
			cur->second.PutCatalogsInto(cats);
		}
	}

	for (vector<EntityModelCatalog*>::iterator cur = cats.begin(), end = cats.end(); cur != end; ++cur)
	{
		(*cur)->ReleaseWhatNeedsToBeReleased();
		(*cur)->Release();
	}
...
void VDBMgr::RegisterRestHandler(RestRequestHandler* restHandler)
{
	VTaskLock lock(&fRestHandlersMutex);
	fRestHandlers.insert(restHandler);
}


void VDBMgr::UnRegisterRestHandler(RestRequestHandler* restHandler)
{
	VTaskLock lock(&fRestHandlersMutex);
	fRestHandlers.erase(restHandler);
}


void VDBMgr::ForceExpires4DRestSession(const VUUID& inSessionID)
{
	VTaskLock lock(&fRestHandlersMutex);
	for (set<RestRequestHandler*>::iterator cur = fRestHandlers.begin(), end = fRestHandlers.end(); cur != end; ++cur)
	{
		RestRequestHandler* resthandler = *cur;
		resthandler->ForceExpireUserSession(inSessionID);
	}
}


void VDBMgr::GetAll4DRestSessions(vector<VUUID>& outSessions)
{
	outSessions.clear();
	VTaskLock lock(&fRestHandlersMutex);
	for (set<RestRequestHandler*>::iterator cur = fRestHandlers.begin(), end = fRestHandlers.end(); cur != end; ++cur)
	{
		RestRequestHandler* resthandler = *cur;
		resthandler->GetAllUserSessionIDs(outSessions);
	}
}


VJSONObject* VDBMgr::RetainUserSessionRequestInfo(const VUUID& inSessionID)
{
	VTaskLock lock(&fRestHandlersMutex);
	for (set<RestRequestHandler*>::iterator cur = fRestHandlers.begin(), end = fRestHandlers.end(); cur != end; ++cur)
	{
		RestRequestHandler* resthandler = *cur;
		VJSONObject* reqinfo = resthandler->RetainUserSessionRequestInfo(inSessionID);
		if (reqinfo != nil)
			return reqinfo;
	}
	return nil;
...
void VDBMgr::SetUserSessionRequestInfo(const VUUID& inSessionID, VJSONObject* inRequestInfo)
{
	VTaskLock lock(&fRestHandlersMutex);
	for (set<RestRequestHandler*>::iterator cur = fRestHandlers.begin(), end = fRestHandlers.end(); cur != end; ++cur)
	{
		RestRequestHandler* resthandler = *cur;
		resthandler->SetUserSessionRequestInfo(inSessionID, inRequestInfo);
	}
}


void VDBMgr::PurgeRemoteUserSessions()
{
	VError err = VE_OK;
	MapUserSessionPurgeInfo remoteUserSessionsToBePurged;
	{
		VTaskLock lock(&fRemoteUserSessionsToBePurgedMutex);
		fRemoteUserSessionsToBePurged.swap(remoteUserSessionsToBePurged);
	}

	for (MapUserSessionPurgeInfo::iterator cur = remoteUserSessionsToBePurged.begin(), end = remoteUserSessionsToBePurged.end(); cur != end; ++cur)
	{
		UserSessionPurgeInfo* purgeinfo = &(cur->second);
		VString ids;
		for (vector<VUUID>::iterator curid = purgeinfo->fIDs.begin(), endid = purgeinfo->fIDs.end(); curid != endid; ++curid)
		{
			if (!ids.IsEmpty())
				ids +=",";
			VString s;
		...
		VXMLHttpRequest xhr;
		VString url = purgeinfo->fBaseURL+"/$info/sessionInfo/release/"+ids;
		err = xhr.Open("GET", url, false);
		xhr.SetRequestHeader("username-4D", purgeinfo->fUserName);
		xhr.SetRequestHeader("NoSession-4D", "true");
		if (purgeinfo->fPassword.IsEmpty())
		{
			xhr.SetRequestHeader("password-4D", purgeinfo->fPassword);
		}
		else
		{
			VString hashedpass;
			MD5 checksum;
			StStringConverter<UniChar> buffer( purgeinfo->fPassword, VTC_UTF_16_BIGENDIAN);
			VChecksumMD5::GetChecksumFromBytes( buffer.GetCPointer(), buffer.GetSize() + sizeof( UniChar), checksum);
			VChecksumMD5::EncodeChecksumBase64( checksum, hashedpass);
			xhr.SetRequestHeader("hashed-password-4D", hashedpass);
		}
		VString body;
		VError err2 = xhr.Send(body, &err);
		VLong status;
		xhr.GetStatus(&status);
	}
}


void VDBMgr::AddRemoteUserSessionToBePurged(const VString& inBaseURL, const VUUID& inSessionID, const VString& inUserName, const VString& inPassWord, bool withSSL)
{
	VTaskLock lock(&fRemoteUserSessionsToBePurgedMutex);
	UserSessionPurgeInfo* purgeinfo = &(fRemoteUserSessionsToBePurged[inBaseURL]);
	purgeinfo->fIDs.push_back(inSessionID);
	purgeinfo->fBaseURL = inBaseURL;
	purgeinfo->fPassword = inPassWord;
	purgeinfo->fUserName = inUserName;
	purgeinfo->fWithSSL = withSSL;
}
...
{
	for (CatalogMap::iterator cur = fCatalogMap.begin(), end = fCatalogMap.end(); cur != end; ++cur)
	{
		cur->second->Release();
	}
}
...
void ProjectInfo::AddCatalog(const VString& name, EntityModelCatalog* cat)
{
	//VTaskLock lock(&fMutex);
	CopyRefCountable(&(fCatalogMap[name]), cat);
}


void ProjectInfo::SetCatalogsAsGlobalProperties(VJSObject& globalObject, BaseTaskInfo* context)
{
	//VTaskLock lock(&fMutex);
	for (CatalogMap::iterator cur = fCatalogMap.begin(), end = fCatalogMap.end(); cur != end; ++cur)
	{
		globalObject.SetProperty(cur->first, VJSEntityModelCatalog::CreateInstance(globalObject.GetContext(), cur->second), JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete);
	}
}


void ProjectInfo::PutCatalogsInto(vector<EntityModelCatalog*>& outCatalogs)
{
	//VTaskLock lock(&fMutex);
	for (CatalogMap::iterator cur = fCatalogMap.begin(), end = fCatalogMap.end(); cur != end; ++cur)
	{
		outCatalogs.push_back(RetainRefCountable(cur->second));
	}
}
