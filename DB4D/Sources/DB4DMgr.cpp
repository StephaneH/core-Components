...
{
	assert(fApplicationAquired == false);
	assert(fPosInStack != -1);
...
{
	VJSContext jscontext(globalObject.GetContextRef());
		...
		if (ext == "waRemoteModel")
		{
			...
			if (err == VE_OK && val.IsObject())
			{
				...
				if (valAccess.IsObject())
				{
					...
					EntityModelCatalog* remoteCat = OpenRemoteCatalog(catname, valAccess.GetObject(), associate, /*cacheFile*/file, false /* store cache only*/, err);
					if (err == VE_OK)
					{
...
EntityModelCatalog* VDBMgr::OpenRemoteCatalog(const VString& inCatName, VJSONObject* inAccessInfo, Base4D* associate, VFile* inCacheFile, bool useAsCacheOnly, VError& outErr)
{
	...
	if (remotecat != nil)
	{
		...
		if (localJS != nil)
		{
			err = remotecat->addJSExtensions(associate, localJS, false);
			if (err != VE_OK)
				outErr = err;
		}
...
void ProjectInfo::SetCatalogsAsGlobalProperties(VJSObject& globalObject, BaseTaskInfo* context)
{
	...
	for (CatalogMap::iterator cur = fCatalogMap.begin(), end = fCatalogMap.end(); cur != end; ++cur)
	{
		globalObject.SetProperty(cur->first, VJSEntityModelCatalog::CreateInstance(globalObject.GetContextRef(), cur->second), JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete);
	}
}
