...
typedef map<VString, pair<CDB4DContext*, VUUIDBuffer> > mapOfContextBySessionID;
...
class ContextKeep
{
	public:
		...
		void Init(CDB4DBase* base, uLONG timeout = 0);
		void Aquired(CDB4DBase* base, VJSGlobalContext* inJSGlobalContext, RIApplicationRef inRIAAppRef, uLONG timeout = 0);
		void Dispose(IHTTPResponse* inResponse, ContextStash& inStash);
		...
		bool Expired(uLONG curtime) const
		{
			if (fTimeout == 0)
				return false;
			else
		...
		uLONG GetExpirationTime() const
		{
			return fStartingTime + fTimeout;
		}

		void CleanUpForReUse(ContextStash& inStash);

		CDB4DBaseContext* fContext;
		VJSGlobalContext* fJSGlobalContext;
		DB4DJSRuntimeDelegate* fJSDelegate;
		RIApplicationRef fRIAAppRef;
		CDB4DBase* fBase;
		VUUID fID;
		sLONG fNextFree;
		sLONG fPosInStack;
		bool fApplicationAquired;
		uLONG fTimeout;
...
class ContextStash
{
	public:
		ContextStash()
		{
			fLastCheck = VSystem::GetCurrentTime();
			fNextFree = -1;
		}

		~ContextStash();

		void DisposeAllContextsOnBase(CDB4DBase* base);
		void DisposeAllContexts();
		void Dispose(ContextKeep* contextKeeper, IHTTPResponse* inResponse);
		ContextKeep* GetContextKeep(const VUUID& inID);
//		ContextKeep* CreateContextKeep(CDB4DBase* base, uLONG timeout = 0);
		ContextKeep* CreateContextKeep(CDB4DBase* base, VJSGlobalContext* inJSGlobalContext, RIApplicationRef inRIAAppRef, uLONG timeout = 0);
		void Add(ContextKeep& contextKeeper, uLONG timeout = 0);

		void CheckExpired();

		ContextKeep* GetNewContextKeep(CDB4DBase* base, uLONG timeout = 0);
		void FreeKeep(ContextKeep* contextKeeper);

#if debuglr
		void debugDump(VString& outStr);
#endif

	protected:
		typedef map<VUUID, ContextKeep> contextcollection;
		typedef vector<ContextKeep*> contextstack;
		contextcollection fStash;
		contextstack fStack;
...
typedef map<VString, EntityModelCatalog*> CatalogMap;
...
class ProjectInfo
{
	public:
		...
		void AddCatalog(const VString& name, EntityModelCatalog* cat);

		void SetCatalogsAsGlobalProperties(VJSObject& globalObject, BaseTaskInfo* context);

		void PutCatalogsInto(vector<EntityModelCatalog*>& outCatalogs);

	protected:
		CatalogMap fCatalogMap;
		//VCriticalSection fMutex;
};

typedef map<void*, ProjectInfo> ProjectMap;


class MirrorRequesterTask;
...
public:
	...
	*/
	...
	CDB4DBase* OpenBase( const VFile& inStructureFile, sLONG inParameters, VError* outErr = nil, FileAccess inAccess = FA_READ_WRITE, VString* EntityFileExt = nil, 
							CUAGDirectory* inUAGDirectory = nil, const VString* inHtmlContent = nil, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles = nil, const VFile* inPermissions = nil, void* ProjectRef = nil);
	...
	VError OpenRemoteCatalogs(void* projectRef, const VFolder* folderToScan, Base4D* associate, bool stopOnFirstError);
	EntityModelCatalog* OpenRemoteCatalog(const VString& inCatName, VJSONObject* inAccessInfo, Base4D* associate, VFile* inCacheFile, bool useAsCacheOnly, VError& outErr);
	...
	*/
	...
	void ExecSetTablePreventJournaling( IRequestReply *inRequest, CDB4DBaseContext *inContext);
	...
	VError StartServer(const VString& inServerName, uWORD inPortNum, bool inSSL = false);
	...
	void KeepPostPonedContext(const VString& sessionID, CDB4DContext* inContext, const VUUID& inRemoteUID);
	CDB4DContext* StealPostPonedContext(const VString& sessionID, VUUID& outUserID);
	void ForgetPostPonedContext(CDB4DContext* inContext);
	void ForgetAllPostPonedContextsForUser(const VUUID& inUserID);
	...
	void RemoveKeptDatasetsOnBase(Base4D* inBase);
...
*/
	...
	ContextStash& GetRestContexts()
	{
		return fRestContextStash;
	}

	void ReleaseExpiredUAGSessions();

	void InitJSContextWithEntityModels(VJSObject& globalObject, BaseTaskInfo* context, void* ProjectRef, std::vector<XBOX::VFile*>& outModelJSFiles);

	void AddOutsideCatalog(const VString& name, EntityModelCatalog* cat, void* ProjectRef);

	void RemoveProjectInfo(void* ProjectRef);

	void CronSleepFor(sLONG nbmilliseconds);

	void CronWakeUp();

	VError ReleaseWhatNeedsToBeReleasedOnCatalogs();

	void SetMirrorTask(MirrorRequesterTask* mirrortask)
	{
		fMirrorTask = mirrortask;
	}

	MirrorRequesterTask* GetMirrorTask()
	{
		return fMirrorTask;
	}

	void RegisterRestHandler(RestRequestHandler* restHandler);
	void UnRegisterRestHandler(RestRequestHandler* restHandler);

	void ForceExpires4DRestSession(const VUUID& inSessionID);
	void GetAll4DRestSessions(vector<VUUID>& outSessions);
	...
	void PurgeRemoteUserSessions();
	void AddRemoteUserSessionToBePurged(const VString& inBaseURL, const VUUID& inSessionID, const VString& inUserName, const VString& inPassWord, bool withSSL);

	class UserSessionPurgeInfo
	{
		public:
			VString fBaseURL;
			vector<VUUID> fIDs;
	...
	typedef map<VString, UserSessionPurgeInfo> MapUserSessionPurgeInfo;
...
private:
	...
	CDB4DBase* xOpenCreateBase( const VFile& inStructureFile, Boolean inCreate, sLONG inParameters, VIntlMgr* inIntlMgr, VError* outErr = nil, 
								...
								CUAGDirectory* inUAGDirectory = nil, const VString* inXmlContent = nil, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles = nil, const VFile* inPermissionsFile = nil, void* ProjectRef = nil);
...
//	CLanguage* sLanguage;
	...
	VDBCronTask* fCronTask;

	VString fRestPrefix;

	ContextStash fRestContextStash;
	uLONG fLastReleaseExpiredUAGSessions;

	ProjectMap fProjectInfos;
	VCriticalSection fProjectInfosMutex;

	mutable vxSyncEvent fCronSync;

	MirrorRequesterTask* fMirrorTask;

	set<RestRequestHandler*> fRestHandlers;
	VCriticalSection fRestHandlersMutex;

	MapUserSessionPurgeInfo fRemoteUserSessionsToBePurged;
	VCriticalSection fRemoteUserSessionsToBePurgedMutex;
