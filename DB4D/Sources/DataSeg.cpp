...
VError SegData::readat(void* p, sLONG len, DataAddr4D ou)
{
	VError err;
	{
		...
		fDiskReadBytes.AddMeasure(len);
		fDiskReadCounts.AddMeasure(1);
		db->HasRead(1, len);
	}
...
VError SegData::writeat(void* p, sLONG len, DataAddr4D ou, sLONG TrueLen)
{
	...
	if (TrueLen == -1 || fWriteBuffer == (void*)-1)
	{
		...
		fDiskWriteBytes.AddMeasure(len);
		fDiskWriteCounts.AddMeasure(1);
		db->HasWritten(1, len);
	}
	else
	{
...
VError SegData::FlushBuffer()
{
	...
	if (fWriteBuffer != nil &&  fWriteBuffer != (void*)-1 && fWriteBufferOffset > 0)
	{
		...
		fDiskWriteBytes.AddMeasure(fWriteBufferOffset);
		fDiskWriteCounts.AddMeasure(1);
		db->HasWritten(1, fWriteBufferOffset);
		fWriteBufferOffset = 0;
	}
	else
		err = VE_OK;
...
VJSONObject* SegData::RetainMeasures(VJSONObject* options, JSONPath* path)
{
...
void SegData::SetMeasureInterval(uLONG interval)
{
	fDiskReadBytes.SetInterval(interval);
	fDiskWriteBytes.SetInterval(interval);
	fDiskReadCounts.SetInterval(interval);
	fDiskWriteCounts.SetInterval(interval);
}
