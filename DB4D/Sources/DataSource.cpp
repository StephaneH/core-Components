...
DB4DArrayOfValues* DBAVVCreator(sLONG signature, VStream *inStream, XBOX::VError& outError)
{
	...
	xbox_assert(signature == kDBAVSIG);
...
VError AutoSeqNumberSimple::LibereEspaceDisk(OccupableStack* curstack, VDB4DProgressIndicator* InProgress)
{
	VError err = VE_OK;

	if (testAssert(fIsValid))
	{
		xbox_assert(getaddr()>0);
...
VError AutoSeqNumberNoHole::LibereEspaceDisk(OccupableStack* curstack, VDB4DProgressIndicator* InProgress)
{
	VError err = VE_OK;
...
void VStringUTF8::_AdjusteUTF8Buffer() const
{
	if (fUTF8Buffer == nil || IsDirty())
	{
		...
		xbox_assert(nbbytesproduced == nbbytes);
		xbox_assert(nbbytesconsumed == fLength * 2);
...
VBlobTextUTF8::VBlobTextUTF8( sLONG * /*inNumBlob*/, BlobTextUTF8 *inBlob)
{
	...
	if (inBlob != nil)
	{
		xbox_assert( inBlob->GetRefCount() > 0);
		inBlob->Retain();
...
const VBlobObj::InfoType	VBlobObj::sInfo;

const VValueInfo *VBlobObj::GetValueInfo() const
{
	return &sInfo;
}


void* VBlobObjInfo::ByteSwapPtr( void *inBackStore, bool inFromNative) const
{
	sLONG lenblob = *(sLONG*)inBackStore;
	if (!inFromNative)
		ByteSwap(&lenblob);
...
VBlobObj::VBlobObj( sLONG *inNumBlob, BlobWithPtr *inBlob):VBlob4DWithPtr(inNumBlob, inBlob)
{
	fJsonObj = nil;
	fJsonObjIsDirty = true;
	ReloadObj();
}

VBlobObj::VBlobObj( const VBlobObj& inBlob):VBlob4DWithPtr(dynamic_cast<const VBlob4DWithPtr&>(inBlob))
{
	fJsonObj = RetainRefCountable(inBlob.fJsonObj);
	fJsonObjIsDirty = true;
}

VBlobObj::~VBlobObj()
{
	QuickReleaseRefCountable(fJsonObj);
}


Boolean VBlobObj::FromValueSameKind( const VValue* inValue)
{
	Boolean isOk = VBlob4DWithPtr::FromValueSameKind(inValue);
	if (isOk)
	{
		const VBlobObj* other = dynamic_cast<const VBlobObj*>(inValue);
		if (other != nil)
		{
			fJsonObjIsDirty = true;
			CopyRefCountable(&fJsonObj, other->fJsonObj);
		}
	}
...
void VBlobObj::FromValue( const VValueSingle& inValue)
{
...
void* VBlobObj::WriteToPtr(void* pData, Boolean pRefOnly, VSize inMax) const
{
	WriteObj();
	return VBlob4DWithPtr::WriteToPtr(pData, pRefOnly, inMax);
}


VSize VBlobObj::GetSpace(VSize inMax) const
{
	WriteObj();
	return VBlob4DWithPtr::GetSpace(inMax);
}


VError VBlobObj::Flush( void *inDataPtr, void *InContext) const
{
	VError err = WriteObj();
	if (err == VE_OK)
	{
		err = VBlob4DWithPtr::Flush(inDataPtr, InContext);
	}
	return err;
}


VBlobObj *VBlobObj::Clone() const
{
	return new VBlobObj( *this);
}


VValue* VBlobObj::FullyClone(bool ForAPush) const
{
	VValue* result = nil;
	WriteObj();
	...
	if (newblob != nil)
	{
		if (ForAPush)
		{
			...
			newblob->Detach();
		result = new VBlobObj(nil, (BlobWithPtr*)newblob);
		newblob->Release();
	}
...
VError VBlobObj::ReadFromStream( VStream* ioStream, sLONG inParam)
{
	VError err = VBlob4DWithPtr::ReadFromStream(ioStream, inParam);
	if (err == VE_OK)
		err = ReloadObj();
	else
		Clear();
...
VError VBlobObj::WriteToStream( VStream* ioStream, sLONG inParam) const
{
	VError err = WriteObj();
	if (err == VE_OK)
		err = VBlob4DWithPtr::WriteToStream(ioStream, inParam);
	return err;
}
...
void VBlobObj::Clear()
{
	VBlob4DWithPtr::Clear();
	ReleaseRefCountable(&fJsonObj);
	fJsonObjIsDirty = true;
}


VError VBlobObj::ReloadObj()
{
	VError err = VE_OK;
	ReleaseRefCountable(&fJsonObj);
	VSize len = GetSize();
	...
	{
		VJSONValue val;
		VJSONBinaryImporter imp(fBlob->GetDataPtr(), len);
		err = imp.GetValue(val);
		/*
		VString s;
		s.FromBlock(fBlob->GetDataPtr(), len, VTC_UTF_16);
		err = val.ParseFromString(s, true);
		*/
		if (val.IsObject() && err == VE_OK)
		{
			fJsonObj = RetainRefCountable(val.GetObject());
		}
	}
	fJsonObjIsDirty = false;
	return err;
}


VError VBlobObj::WriteObj() const
{
	VBlobObj* xthis = const_cast<VBlobObj*>(this);
	VError err = VE_OK;
	if (fBlob != nil && fJsonObjIsDirty)
	{
		xthis->VBlob4DWithPtr::Clear();
		if (fJsonObj != nil)
		{
			VJSONValue val(fJsonObj);
			VJSONBinaryWriter exp;
			VMemoryBuffer<> buff;
			err = exp.StreamValue(val, buff);
			if (err == VE_OK)
				err = xthis->PutData(buff.GetDataPtr(), buff.GetDataSize(), 0);
			/*
			VString s;
			err = VJSONValue(fJsonObj).Stringify(s, JSON_AllowDates);
			if (err == VE_OK)
			{
				err = xthis->PutData(s.GetCPointer(), s.GetLength() * sizeof(UniChar), 0);
			}
			*/
		}
		xthis->fJsonObjIsDirty = false;
	}
	return err;
}
...
VSubTableDB4D::~VSubTableDB4D()
{
	...
	for (i=1; i<=nb; i++)
	{
		if (fSubRecs[i] != nil)
		{
			...
			//xbox_assert(fSubRecs[i]->GetRefCount() == 1);
			fSubRecs[i]->Release();
		}
	}
	...
	for (i=1; i<=nb; i++)
	{
		if (fDeletedSubRecs[i] != nil)
		{
			...
			//xbox_assert(fDeletedSubRecs[i]->GetRefCount() == 1);
...
void VSubTableDB4D::Detach(Boolean inMayEmpty)
{
	sLONG i,nb = fSubRecs.GetCount();
	for (i=1; i<=nb; i++)
	{
...
ValPtr CreVBlobObj(DataTable *df, sLONG maxlen,void* from, uBOOL init, uBOOL formodif, BaseTaskInfo* context, creVValue_ParamType parameters)
{
	VBlobObj *cv = nil; 
	BlobWithPtr *bt = nil;
	VError err = VE_OK;
...
ValPtr CreVBlobPicture(DataTable *df, sLONG maxlen,void* from, uBOOL init, uBOOL formodif, BaseTaskInfo* context, creVValue_ParamType parameters)
{
	...
	if (bt != nil && sVPictureRef != nil) {
			...
			if (isnull && (parameters & creVValue_forquery) != 0)
				cv->SetNull(true);
			else if (bt->IsNull())
				cv->SetNull(true);
			bt->Release();
...
sLONG InitChampVar()
{
	...
	RegisterCV(VK_BLOB_OBJ,&CreVBlobObj);
