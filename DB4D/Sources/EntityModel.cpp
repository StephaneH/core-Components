	...
	else
	{
		if (fType == eav_composition)
		{
			if (fSel != nil)
			{
				...
				outJSObject = fSel->ToJsArray(fOwner->getContext(), outJSObject.GetContext(), atts, nil, nil, false, false, 0, fSel->GetLength(fOwner->getContext()), err);
				outJSObject = arr;
			}
			if (err == VE_OK)
			{
					...
					case eav_composition:
						{
							...
							if (sel != nil && submodel != nil)
							{
								...
								if (eai != nil && eai->fAttribute != nil)
								{
									...
									VJSArray subarr(sel->ToJsArray(context, outObj.GetContext(), *cur->fSousSelection, eai->fSousSelection, subSortingAttributes, false, true, eai->fSkip, countelem, err));
								...
								else
								{
									...
									{
										EntityAttributeSortedSelection subatts(submodel);
										VJSArray subarr(sel->ToJsArray(context, outObj.GetContext(), subatts, nil, nil, false, true, 0, sel->GetLength(context), err));
									}
									else
									{
										VJSObject subObj( outObj, attribute->GetName());
...
VError EntityModelCatalog::SecondPassLoadEntityModels(ModelErrorReporter* errorReporter)
{
	VError err = VE_OK;

	err = do_SecondPassLoadEntityModels(errorReporter);
	return err;
}


VError _addOneAttribute( const VJSValue& attVal, VValueBag* dataClassBag, const VString* attname, bool& outEmpty)
{
	bool res, exists;
	sLONG ll;
	VString s;
...
VError ModelJSTransformer::TransformJSIntoBag(VFile& inJSFile, const VString& modelRoot, VValueBag& bagEntities, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles, ModelErrorReporter* errorReporter)
{
	VError err = VE_OK;
			...
			if (dataClassVal.IsObject() && !dataClassVal.IsFunction())
			{
				...
				else
				{
					...
					while (curatt.IsValid())
					{
						...
						{
							bool empty = true;
							_addOneAttribute(curatt.GetProperty(), dataClassBag, &attname, empty);
...
VError EntityModelCatalog::addJSExtensions(Base4D* associate, VFile* fileJS, ModelErrorReporter* errorReporter)
{
	...
	if (fileJS != nil)
	{
		...
		err = jstrans.TransformJSIntoBag(*fileJS, "model", bagEntities, nil, errorReporter);
#if debuglr
		VString modelstr;
...
VError EntityModelCatalog::LoadEntityModels(const VValueBag& bagEntities, ModelErrorReporter* errorReporter, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles)
{
	VError err = VE_OK;

	{
		...
		if (fileJs != nil)
		{
			err = jstrans.TransformJSIntoBag(*fileJs, "model", (VValueBag&) bagEntities, outIncludedFiles, errorReporter);
			if (err == VE_OK || errorReporter != nil)
			{
				if (errorReporter != nil)
				{
		...
		if (err == VE_OK)
		{
			...
			if (okbag)
			{
				...
				*/
				...
				if (err == VE_OK)
				{
					err = LoadEntityModels(bagEntities, errorReporter, outIncludedFiles);
					if (err == VE_OK)
					{
						VString fileext;
