...
bool EntityAttribute::PermissionMatch(DB4D_EM_Perm inPerm, CUAGSession* inSession) const
{
	return permissionMatch(inPerm, inSession);
}
...
bool EntityAttribute::permissionMatch(DB4D_EM_Perm inPerm, BaseTaskInfo* context) const
{
	if (context != nil)
	{
		return permissionMatch(inPerm, context->GetCurrentUserSession());
	}
	else
		return false;
}
...
VError EntityAttribute::FromBag(const VValueBag* bag, EntityModelCatalog* catalog, bool fieldmustexits, bool devMode, bool limitedChange)
{
	VError err = VE_OK;
...
VError EntityModel::FromBag(const VValueBag* inBag, EntityModelCatalog* catalog, bool devMode, bool addToClassOnly, bool allowAttributeChange)
{
	VError err = VE_OK;
			...
			for (VIndex i = 1; i <= nbatts && err == VE_OK; i++)
			{
				...
				if (attributeBag != nil)
				{
					...
					else
					{
						...
						if (err == VE_OK)
							err = entAtt->FromBag(attributeBag, catalog, false, devMode, allowAttributeChange && (dejaAtt != nil));
...
EntityRecord* EntityModel::findEntityWithAttribute(const EntityAttribute* inAtt, const VValueSingle* value, BaseTaskInfo* context, VError& err, DB4D_Way_of_Locking HowToLock)
{
	...
	VCompareOptions options;
	options.SetDiacritical(false);
	xquery.AddSearchLineEmSimple(inAtt, DB4D_Equal, value, options, 0, false);
...
bool EntityModel::permissionMatch(DB4D_EM_Perm inPerm, CUAGSession* inSession) const
{
	if (inPerm >= DB4D_EM_Read_Perm && inPerm <= DB4D_EM_Promote_Perm)
	{
		...
		{
			if (inSession->BelongsTo(*id))
				return true;
			else
			{
				if (inPerm == DB4D_EM_Describe_Perm)
					return permissionMatch(DB4D_EM_Read_Perm, inSession);
				else if (inPerm == DB4D_EM_Read_Perm)
					return permissionMatch(DB4D_EM_Update_Perm, inSession) || permissionMatch(DB4D_EM_Delete_Perm, inSession);
				else
					return false;
			}
		}
...
EntityAttributeValue* EntityRecord::do_getAttributeValue(const EntityAttribute* inAttribute, VError& err, BaseTaskInfo* context, bool restrictValue)
{
	...
	if (inAttribute == nil)
		return nil;
	assert(fModel != nil && fModel->isExtendedFrom(inAttribute->GetOwner()));
					...
					*/
					{
						...
						{
							VJSContext jscontext(context->GetJSContext());
							VJSObject therecord(jscontext, VJSEntitySelectionIterator::CreateInstance(jscontext, new EntitySelectionIterator(this, context)));
							VJSValue result(jscontext);
							VJSObject* objfunc = nil;
							VJSObject localObjFunc(jscontext);
							...
							{
								cv = result.CreateVValue(nil, inAttribute->isSimpleDate());
							}
...
VError EntityRecord::CallSaveEvents(BaseTaskInfo* context)
{
	...
	if (err == VE_OK)
	{
		for (EntityAttributeValueCollection::iterator cur = fValues.begin(), end = fValues.end(); cur != end && err == VE_OK; cur++)
		{
			...
			if (val != nil)
			{
				...
				{
					err = att->CallDBEvent(dbev_save, this, context);
				}
			}
		}
	}
...
EntityModel* EntityModelCatalog::BuildEntityModel(const VValueBag* bag, VError& err, bool devMode)
{
	...
	err = model->FromBag(bag, this, devMode, false, false);
	if (err != VE_OK)
...
VError ModelJSTransformer::TransformJSIntoBag(VFile& inJSFile, const VString& modelRoot, VValueBag& bagEntities, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles, bool devMode)
{
	VError err = VE_OK;
...
VError EntityModelCatalog::LoadEntityModels(const VValueBag& bagEntities, bool devMode, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles)
{
...
#endif
	{
		...
		context = new BaseTaskInfo(fAssociatedBase, nil, nil, nil);
		xJSDelegate = new DB4DJSRuntimeDelegate(context);
		xJSContext = VJSGlobalContext::Create( xJSDelegate);
		context->SetJSContext(xJSContext);
		...
		VFile* fileJs = fAssociatedBase->RetainCatalogJSFile();
		if (fileJs != nil)
		{
			if (fileJs->Exists())
			{
		...
		if (err == VE_OK)
		{
			...
			if (outsideCatalogs != nil)
			{
				...
				for (VIndex i = 1; i <= nbcatalogs && err == VE_OK; i++)
				{
					...
					{
						StErrorContextInstaller errs(false, false);
						fAssociatedBase->AddOutsideCatalog(oneCatRef);
					}
				}
			}
			...
			if (outsideSQLCatalogs != nil)
			{
				...
				for (VIndex i = 1; i <= nbcatalogs && err == VE_OK; i++)
				{
					...
					{
						StErrorContextInstaller errs(false, false);
						fAssociatedBase->AddOutsideSQLCatalog(oneCatRef);
					}
				}
			}
		}
...
VError EntityModelCatalog::addJSExtensions(Base4D* associate, VFile* fileJS, bool devMode)
{
	VError err = VE_OK;
	...
	if (fileJS != nil)
	{
		...
		err = jstrans.TransformJSIntoBag(*fileJS, "model", bagEntities, nil, devMode);
#if debuglr
		VString modelstr;
			...
			if (entities != nil)
			{
				...
				for (VIndex i = 1; i <= nbentities && err == VE_OK; i++)
				{
					...
					if (OneEntity != nil)
					{
						...
						if (em != nil)
						{
							err = em->FromBag(OneEntity, this, devMode, true, true);
						}
						else
						{
							VString extendFrom;
							...
							else
							{
								...
								em = NewModel();
								err = em->FromBag(OneEntity, this, devMode, false, false);
								em->Retain();
...
VError EntityModelCatalog::LoadEntityModels(const VValueBag& bagEntities, bool devMode, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles)
{
...

	{
		...
		if (fileJs != nil)
		{
			err = jstrans.TransformJSIntoBag(*fileJs, "model", (VValueBag&) bagEntities, outIncludedFiles, devMode);
			if (err == VE_OK || devMode)
			{
				if (devMode)
				{
		...
		if (err == VE_OK)
		{
			...
			*/
			...
			{
				SubPathRefSet already;
				err =cur->second->ResolveRelatedEntities(already, this, devMode, jstrans.GetBaseContext());
			}
			...
			*/
			...
			for (EntityModelMap::iterator cur = fEntityModels.begin(), end = fEntityModels.end(); cur != end && err == VE_OK; cur++)
			{
				err =cur->second->ResolveQueryStatements(this, devMode, jstrans.GetBaseContext());
			}
		}
		...
		if (err == VE_OK)
		{
			...
			if (outsideCatalogs != nil)
			{
				...
				for (VIndex i = 1; i <= nbcatalogs && err == VE_OK; i++)
				{
					...
					if (oneCatRef != nil)
					{
						StErrorContextInstaller errs(!devMode, false);
		...
		if (err == VE_OK)
			err = do_LoadEntityModels(bagEntities, devMode, outIncludedFiles, jstrans.GetBaseContext());
