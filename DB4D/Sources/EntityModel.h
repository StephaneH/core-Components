...
const VErrorDB4D	VE_DB4D_REMOTE_CATALOG_INVALID = MAKE_VERROR(CDB4DManager::Component_Type, 1612);
...
{ 
	...
	dbev_restget,
	dbev_clientrefresh,
	dbev_transactioncommitted,
	dbev_firstEvent = dbev_save,
	dbev_lastEvent = dbev_transactioncommitted
} DBEventKind;
...
class ENTITY_API EntityCollection  : public CDB4DEntityCollection
{
	public:
		...
		virtual VError ToJSON(VJSONObject* into, BaseTaskInfo* context, UniChar wildcharReplace = 0)
		{
...
class ENTITY_API EntityMethod : public CDB4DEntityMethod
{
	public:
		...
		virtual bool PermissionMatch(DB4D_EM_Perm inPerm, CUAGSession* inSession, CUAGThreadPrivilege* inThreadPrivilege) const;
		bool permissionMatch(DB4D_EM_Perm inPerm, CUAGSession* inSession, CUAGThreadPrivilege* inThreadPrivilege) const;


		EntityAttributeScope getScope() const
		{
			return fScope;
		}

		virtual EntityAttributeScope GetScope() const
		{
			return fScope;
		}
...
class ENTITY_API DBEvent
{
	public:
		...
		VError Call(EntityRecord* inRec, BaseTaskInfo* context, const EntityAttribute* inAtt, const EntityModel* inDataClass, EntityCollection* *outSel, sLONG eventpos) const;
...
class ENTITY_API EntityAttribute : public CDB4DEntityAttribute
{
	public:
		...
		*/
		...
		VError AssociateJSEvents(EntityModelCatalog* catalog, ModelErrorReporter* errorReporter);
		...
		bool NeedValidationOnRemove() const;
...
class ENTITY_API EntityModel : public CDB4DEntityModel
{
	public:
		...
		virtual EntityCollection* BuildCollectionFromJSON(const VJSONObject* from, VError& outErr)
		{
			outErr = ThrowBaseError(VE_DB4D_NOTIMPLEMENTED);
			return nil;
		}
...
class ENTITY_API EntityAttributeValue : public CDB4DEntityAttributeValue
{
	public:
		...
		VError Save(Transaction* &trans, BaseTaskInfo* context, bool refreshOnly);
		...
		VError ValidateRemove(BaseTaskInfo* context);
...
class ENTITY_API EntityRecord : public CDB4DEntityRecord
{
	public:
		...
		virtual PrimKey* RetainRelatedAttributeKey(const EntityAttribute* inAttribute, VError& err) = 0;

		virtual bool ContainsBlobData(const EntityAttribute* inAttribute, BaseTaskInfo* context) = 0;

		virtual VError Save(Transaction* *trans, BaseTaskInfo* context, uLONG stamp, bool allowOverrideStamp = false, bool refreshOnly = false) = 0;
		...
		virtual VError CallSaveEvents(BaseTaskInfo* context, bool refreshOnly);
		...
		virtual VError ValidateRemove(BaseTaskInfo* context);
		virtual VErrorDB4D ValidateRemove();
		...
		static void GetPrimKeyAsString(const VectorOfVValue& values, VString& outKey, bool autoQuotes);
...
class ENTITY_API EntityModelCatalog : public IDebugRefCountable
{
	...
	public:
		...
		*/
		...
		virtual VError do_SecondPassLoadEntityModels(ModelErrorReporter* errorReporter, bool dataWasAlreadyThere)
		{
			return VE_OK;
		}
		...
		EntityModelCatalog* AddSubProject(const VFile* inProjectFile, VError& outError);
		void SetModelFile(const VFile* inProjectFile);
		void SetModelJSName(const VString& name)
		{
			fModelJSName = name;
		}

		const VString& GetModelJSName() const
		{
			return fModelJSName;
		}

		void SetModelJSFile(const VFile* inJsFile)
		{
			QuickReleaseRefCountable(fJSFile);
			fJSFile = new VFile(*inJsFile);
		}

		void SetModelFolder(const VFolder* inFolder)
		{
			QuickReleaseRefCountable(fModelFolder);
			fModelFolder = new VFolder(*inFolder);
		}

		VError EvaluateModelScript(const VJSObject& globalObject, BaseTaskInfo* context, void* ProjectRef, std::vector<XBOX::VFile*> &outModelJSFiles);
		...
		static void GetNewJSModelName(const VString& hint, VString& outName);
		static void ReleaseJSModelName(const VString& modelname);

		EntityCollection* BuildCollectionFromJSON(const VJSONObject* from, VError& outErr);
	...
	protected:
		...
		typedef map<VString, EntityModelCatalog*> SubProjectMap;
		SubProjectMap fSubProjects;
		VCriticalSection fSubProjectsMutex;
		VFile* fModelFile;
		VFile* fJSFile;
		VFolder* fModelFolder;
		VString fModelJSName;

		static set<VString> sCatalogNames;
		static VCriticalSection sCatalogNamesMutex;
