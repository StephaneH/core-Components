...
FicheInMem* FicheInMem::CloneForPush(BaseTaskInfo* Context, VError& err) const
{
	...
	if (lacopy != nil)
	{
		...
		if (!fReadOnly)
		{
			...
			if (numrec != -3)
			{
				asscrit->MarkRecordAsPushed(numrec);
				if (Context != nil)
...
void FicheInMem::RestoreFromPop()
{
	...
	if (!fIsRemote)
	{
		...
		{
			assert((!isnew) && fWasClonedForPush > 0);
			VError err;
	...
	if (!fReadOnly)
	{
		...
		if (numrec != -3)
		{
			asscrit->UnMarkRecordAsPushed(numrec);
			if (context != nil)
...
void FicheInMem::Touch(sLONG inFieldID)
{
	...
	if (inFieldID > 0 && inFieldID <= maxnbcrit)
	{
		...
		if (cv != nil)
		{
			...
			{
				assert( ((VString*)cv)->IsStringValid() );
			}
#endif
		...
		if (primkeyfields.GetCount() > 0)
		{
			...
			if (err == VE_OK)
			{
				...
				if (action == Sync_Delete)
				{
					...
					timestamp = assocData->GetTimeStamp();
...
VError FicheInMem::stockcrit(BaseTaskInfo* context)
{
	VError err;
	...
	if (err == VE_OK)
	{
		...
		if (deps2.GetCount() > 0 && asscrit->GetOwner()->CheckNot_NullEnabled() && MustCheckIntegrity())
		{
			...
			{
				startofprimkey = deps.GetCount()+1;
				for (i=1; i<=nb; i++)
				{
					sLONG n = deps2[i];
					ValPtr cv = GetNthField(-n, err);
					...
					if (deps2.GetCount() > 1 || deps.GetCount() == 0)
					{
						if (!deps.Add(deps2[i]))
						{
		...
		if (deps.GetCount() > 0 && err == VE_OK && MustCheckIntegrity())
		{
			if (!asscrit->CheckUniqueness(this, deps, context, startofprimkey))
			{
...
VError FicheOnDisk::WriteSaveToLog(BaseTaskInfo* context, Boolean newone)
{
	VStream* log;
	Base4D* db = DF->GetDB();
	sLONG len = fDataLen + fNbCrit*sizeof(ChampHeader) + sizeof(RecordHeaderOnDisk) + sizeof(VUUIDBuffer);
	Table* tt = DF->GetTable();

	VError err = db->StartWriteLog(newone ? DB4D_Log_CreateRecord : DB4D_Log_ModifyRecord, len, context, log);
	if (err == VE_OK)
	{
		if (log != nil)
		{
...
VError FicheOnDisk::WriteDeleteToLog(BaseTaskInfo* context)
{
	VError err = VE_OK;
	VStream* log;
	Base4D* db = DF->GetDB();
