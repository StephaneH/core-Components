...
class FicheOnDisk : public ObjInCacheMem, public IObjToFlush
{
	public:
		...
		/*
		void use(void) { nbuse++; };
		void unuse(void) { xbox_assert(nbuse>0); nbuse--; };
		...
		*/
		...
		void SetTimeStamp(uLONG8 timestamp) { TimeStamp = timestamp; }
		...
		VError WriteSaveToLog(BaseTaskInfo* context, Boolean newone, FicheInMem* fic, PrimKey* oldPrimKey);
		VError WriteDeleteToLog(BaseTaskInfo* context, FicheInMem* fic);
		...
		{
			return RetainRefCountable(fPrimKey);
		}

		void SetPrimKey(PrimKey* primkey)
		{
...
class FicheInMem : public VObject, public IDebugRefCountable
{
	public:
		...
		virtual VError stockcrit(BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress);
		...
		PrimKey* RetainPrimKey(VError& outErr, bool oldOne);

		void SetUsedAsCacheInQuery()
		{
			fIsUsedAsCacheInQuery = true;
		}

		bool IsUsedAsCacheInQuery() const
		{
			return fIsUsedAsCacheInQuery;
		}

		bool IsAssocDataNull() const
		{
			return assocData == nil;
		}
	...
	protected:
		...
		bool fAddressAllreadyTaken, fIsUsedAsCacheInQuery;
};
...
class FicheInMemSystem : public FicheInMem
{
	public:
		...
		//virtual FicheInMem* CopyMem(void);
		//virtual FicheInMem* Duplique(void);
		virtual VError stockcrit(BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress) { return VE_DB4D_TABLEISLOCKED; };
