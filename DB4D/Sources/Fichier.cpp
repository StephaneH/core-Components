...
void Field::ResetAttProps()
{
	cri->SetAutoSeq(false);
	cri->SetAutoGenerate(false);
	cri->SetNot_Null(false);
	cri->SetNeverNull(false);
	cri->SetUnique(false);
	cri->SetOutsideData(false);
	cri->SetStyledText(false);
}
...
void Field::RebuildAllRelatedIndex(CDB4DBaseContext* inContext, sLONG NeverNullChange, VProgressIndicator* progress)
{
	...
	for (i=nb; i>0; i--)
	{
		...
		{
			// sc 02/09/2013 ACI0083001, when the index is rebuild, it must remain in the indexes dependencies
			ind->Retain();
			IndexDep.Add(ind);
...
VError Field::LoadFromBagWithLoader( const VValueBag& inBag, VBagLoader *inLoader, void* inContext)
{
	...
	if (err == VE_OK)
	{
		...
		if ((typ == VK_BLOB) || (typ == VK_BLOB_DB4D) || (typ == VK_IMAGE) || (typ == VK_BLOB_OBJ))
...
void Field::ClearAllDependencies(bool includingRelations)
{
...
IndexInfo* Field::FindAndRetainIndexObj(uBOOL sortable, Boolean MustBeValid, BaseTaskInfo* context) const
{
	sLONG i;
	sLONG nb;
	IndexInfo *ind, *ind2;
	...
	for (i = 1; i <= nb; i++)
	{
		...
		if (ind != nil)
		{
			if (ind->MatchType(DB4D_Index_OnObj))
			{
				if (sortable)
				{
					if (ind->MayBeSorted())
...
void Field::MeasureQueryCount(sLONG nb) const
{
	VTaskLock lock(&fMeasureMutex);
	fQueryCount.AddMeasure(nb);
}

void Field::MeasureSortCount(sLONG nb) const
{
	VTaskLock lock(&fMeasureMutex);
	fSortCount.AddMeasure(nb);
}

VJSONObject* Field::RetainMeasures(VJSONObject* options, JSONPath* path)
{
	VTaskLock lock(&fMeasureMutex);
	VJSONObject* result = nil;

	JSONPathMarker xpath(path);
	if (xpath.AcceptJsonPathPart(MeasureCollection::kQueryCount))
		jsonProp(&result, MeasureCollection::kQueryCount, fQueryCount.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kSortCount))
		jsonProp(&result, MeasureCollection::kSortCount, fSortCount.RetainMeasures(options));
...
void InitCritere()
{
	...
	RegisterCri( VK_BLOB_OBJ, CritereBlobObj::NewCritere);
...
VError Table::WriteSeqNumToLog(BaseTaskInfo* context, sLONG8 curnum)
{
	...
	if (ShouldBeLogged())
	{
		...
		err = db->StartWriteLog(DB4D_Log_SaveSeqNum, len, context, log, true);
		if (err == VE_OK)
		{
			if (log != nil)
			{
...
Boolean Table::CheckUniqueness(FicheInMem* fic, const NumFieldArray& deps, BaseTaskInfo* context, const NumFieldArray& primkeydeps, XBOX::VProgressIndicator* inProgress, vector<IndexInfo*>& outUniqIndexes)
{
	VError err = VE_OK;;
	...
	vector<IndexInfo*> inds;
	Transaction* trans = nil;
	if (context != nil)
		trans = context->GetCurrentTransaction();
	...
	if (testAssert(!fIsRemote))
	{
		...
		{
			sLONG primkeyfieldnum = 0;
			if (primkeydeps.GetCount() == 1)
				primkeyfieldnum = primkeydeps[1];
			for (sLONG i = 0, nb = deps.GetCount(); i <= nb && err == VE_OK; ++i)
			{
				IndexInfo* ind = nil;

				if (i == 0)
				{
					sLONG nb = primkeydeps.GetCount();
					if (nb > 1)
					{
						IndexInfo* ind = FindAndRetainIndex(primkeydeps, false, false, context);
						if (ind == nil)
						{
							err = ThrowError(VE_DB4D_INVALIDINDEX, noaction);
						}
					}
					else if (nb == 1)
					{
						sLONG numfield = primkeydeps[1];
						ind = FindAndRetainIndexSimple(numfield, false, false, context);
						if (ind == nil)
						{
							err = ThrowError(VE_DB4D_INVALIDINDEX, noaction);
						}
				...
				{
					sLONG numfield = deps[i];
					if (numfield != primkeyfieldnum)
					{
						ind = FindAndRetainIndexSimple(numfield, false, false, context);
						if (ind == nil)
						{
				...
				if (ind != nil)
				{
					if (ind->NeedUpdate(fic, context))
					{
						...
						if (ind->AskForValid(context, false))
						{
							...
							bool ok = false;
							if (ind->PlaceGlobalUniqKey(curstack, fic, context, true))
							{
								BTitemIndex* val = ind->BuildKey(fic, err, false);
								VCompareOptions options;
								val->StartUse();
								val->SetInd(ind);
								options.SetLike(false);
								options.SetDiacritical(false);
								options.SetIntlManager(ind->GetDB()->GetIntlMgr());
								sLONG alreadyrecnum = ind->FindKey(val, context, options, nil, nil);
								if (alreadyrecnum == -1 || alreadyrecnum == fic->GetNum())
									ok = true;
								val->Unuse();
							}
							...
							{
								err = ind->ThrowError(VE_DB4D_DUPLICATED_KEY, DBaction_InsertingKeyIntoIndex);
							}
							ind->Close();
							ind->ReleaseValid();
						}
			...
			if (err != VE_OK)
			{
				for (vector<IndexInfo*>::iterator cur = inds.begin(), end = inds.end(); cur != end; ++cur)
				{
					IndexInfo* ind = *cur;
					ind->Open(index_write);
					ind->ReleaseGlobalUniqKey(curstack, fic, context, true);
					ind->Close();
					ind->Release();
				}
				inds.clear();
...
void Table::UnRegisterForLang(void)
{
	...
	if (fRegistered)
	{
		...
		if (!Owner->IsNotifying() && !Owner->IsWakandaRunningMode())
		{
			if (VDBMgr::GetCommandManager() != nil)
			{
				VUUID UID_base = Owner->GetUUID();
...
void Table::ReleaseAllFields()
{
	...
	for (mapOfFieldsBySmallID::iterator cur = fFieldsByID.begin(), end = fFieldsByID.end(); cur != end; ++cur)
	{
		cur->second->Release();
	}
...
void Table::DelayIndexes(bool includesPrimKey)
{
	VTaskLock lock(&fAllIndexDelayRequestCountMutex);
	{
		...
		for (IndexArray::Iterator cur = IndexDep.First(), end = IndexDep.End(); cur != end; cur++)
		{
			...
			if (ind != nil)
			{
				if (!ind->IsOnPrimKey() || includesPrimKey)
				{
					ind->DelayIndex();
				}
			}
...
//	SelectionRef = nil;
	...
	fRemotePrimaryKeyValidityState = -1;
}
...
void Table::ResetAttProps()
{
	occupe();
	for (sLONG i = 1, nb = tc.GetCount(); i <= nb; ++i)
	{
		Field* cri = tc[i];
		if (cri != nil)
		{
...
IndexInfo* Table::FindAndRetainIndexObj(sLONG nc, uBOOL sortable, Boolean MustBeValid, BaseTaskInfo* context) const
{
	sLONG i;
	sLONG nb;
	IndexInfo *ind, *ind2;
...
VError Table::CheckReferentialIntegrity(FicheInMem *fic, BaseTaskInfo* context, Boolean OnModify,XBOX::VProgressIndicator* inProgress)
{
	VError err = VE_OK;
	...
	if (testAssert(!fIsRemote))
	{
		...
		if (Owner->ReferentialIntegrityEnabled() && AtLeastOneReferentialIntegrity())
		{
			...
			for (i=1; i <= nb; i++)
			{
				Relation* rel = rels[i];
				xbox_assert(rel != nil);
				xbox_assert(rel->WithReferentialIntegrity());
				...
				if (okcheck)
				{
					if (rel->AutoDeleteRelatedRecords())
					{
						//Cascading deletes will take time on large data sets so monitor them
						...
						if (err == VE_OK)
						{
							...
							{
								Selection* sel;
								err = rel->ActivateOneToMany(fic, sel, context, OnModify, false, DB4D_Keep_Lock_With_Transaction,actualProgress);
								if (err == VE_OK)
								{
									if (sel != nil)
									{
												...
												if (rec != nil)
												{
													if (err == VE_OK)
													{
														...
														if (err == VE_OK)
														{
															...
															err = sel->GetParentFile()->SaveRecord(rec, context,false,inProgress);
										...
										else
										{
											...
											if (sel->DeleteRecords(context, nil, rel,actualProgress) != VE_OK)
											{
...
VError Table::CheckReferentialIntegrityForForeignKey(FicheInMem *fic, BaseTaskInfo* context, Boolean OnModify,XBOX::VProgressIndicator* inProgress)
{
	VError err = VE_OK;
	...
	if (testAssert(!fIsRemote))
	{
		...
		if (Owner->ReferentialIntegrityEnabled() && AtLeastOneReferentialIntegrityForeign())
		{
			...
			sLONG i,nb = rels.GetCount();
			VProgressIndicator* actualProgress = inProgress;
			if (nb > 0)
			{
				//If some referential integrity constraints must be checked then monitor the check using a progress indicator

				actualProgress = GetCachedProgressIndicator(context, inProgress);
				if (actualProgress != NULL)
				{
					actualProgress->BeginSession(nb,CVSTR("Checking referential integrity"),false,NULL);
				}
			}
			for (i=1; i <= nb; i++)
			{
				...
				if (actualProgress!= NULL)
					actualProgress->Increment();
			}
			if (actualProgress != NULL)
			{
				actualProgress->EndSession();
				if(inProgress == NULL)
				{
					context->SetCachedProgressIndicator(actualProgress);
				}
				...
				if (ind->AskForValid(context, false)) 
				{
					...
					if (curelem == -2 || curelem>=n || ind->GetMaxElemEnCreation()<=n)
					{
						...
						if (err == VE_OK)
						{
							if (ind->IsUniqueKey())
							{
								if (context != nil && context->GetCurrentTransaction() != nil)
								{
									if (!ind->PlaceGlobalUniqKey(curstack, fic, context, false))
									{
										err = ind->ThrowError(VE_DB4D_DUPLICATED_KEY, DBaction_InsertingKeyIntoIndex);
									}
								}
...
VError Table::UpdateBlobs(FicheInMem *fic, BaseTaskInfo* context, PrimKey* oldPrimKey)
{
	sLONG i,nb,n;
	VError err;
	ValPtr cv;
		...
		for (i=1;(i<=nb) && (err==VE_OK);i++)
		{
			...
			if (fic->IsModif(n))
			{
				...
				if (cv != nil)
				{
					...
					{
						if (oldPrimKey != nil)
							cv->AssociateRecord(oldPrimKey, n);
						...
						if (key == nil)
						{
							...
							VError err2 = VE_OK;
							key = fic->RetainPrimKey(err2, true);
							cv->AssociateRecord(key, n);
							QuickReleaseRefCountable(key);
						}
						else
							cv->AssociateRecord(key, n);
						*/
			...
			if (cv != nil)
			{
				...
				if (typblob == VK_TEXT || typblob == VK_STRING || typblob == VK_STRING_UTF8 || typblob == VK_TEXT_UTF8)
				{
					...
					if (testAssert(blobtext != nil))
					{
						...
						if (theBlob != nil)
						{
							if (theBlob->NeedDuplicate() || GetCurrentTransaction(context) != nil)
							{
								BlobText* newblob = (BlobText*)theBlob->Clone(false);
								newblob->SetNewInTrans(theBlob->IsNewInTrans());
								blobtext->SetBlob4D(newblob);
								theBlob->Release();
								theBlob = newblob;
							}
					...
					if (typblob == VK_IMAGE)
					{
						...
						if ( testAssert(blobhandle != nil)) 
						{
							...
							if (theBlob != nil)
							{
								if (theBlob->NeedDuplicate() || GetCurrentTransaction(context) != nil)
								{
									BlobWithPtr* newblob = (BlobWithPtr*)theBlob->Clone(false);
									newblob->SetNewInTrans(theBlob->IsNewInTrans());
									blobhandle->SetBlob4D(newblob);
									theBlob->Release();
									theBlob = newblob;
								}
...
void Table::ClearAllDependencies(bool includingRelations)
{
	if (includingRelations)
	{
		ClearRelationNto1Dep();
	...
	for (; cur != end; cur++)
	{
		Field* cri = *cur;
		if (cri != nil)
			cri->ClearAllDependencies(includingRelations);
	}
...
VError Table::SetPrimaryKey(const NumFieldArray& xPrimKey, VProgressIndicator* InProgress, Boolean CanReplaceExistingOne, CDB4DBaseContext* inContext, VString* inPrimaryKeyName)
{
	VError err = VE_OK;
	...
	if (fIsRemote)
	{
		...
		else
		{
			...
			req->PutNumFieldArrayParam( xPrimKey);
	...
	else
	{
		...
		if (xPrimKey.GetCount() > 0)
		{
			occupe();

			if (HasSyncInfo() || ShouldBeLogged())
			...
			{
				NumFieldArray::ConstIterator cur = xPrimKey.First(), end = xPrimKey.End();
				for (;cur != end && err == VE_OK; cur++)
				{
					sLONG numcrit = *cur;
					if (numcrit<=0 || numcrit>tc.GetCount())
			...
			if (err == VE_OK && DF != nil)
			{
				...
				if (DF->LockTable(context))
				{
					Selection* all = DF->AllRecords(context, err);
					if (all != nil)
					{
						if (InProgress != nil)
						{
							...
							gs(1005,35,session_title);
							session_title.Format(&bag);
							InProgress->BeginSession(all->GetQTfic(), session_title, false);
						}

						SelectionIterator iter(all);
						RecIDType cur = iter.FirstRecord();
						sLONG curval = 0;
						while (cur != -1 && err == VE_OK)
						{
							if ((InProgress != nil) && !InProgress->Progress(curval))
							{
								err = ThrowError(VE_DB4D_ACTIONCANCELEDBYUSER, noaction);
							}
							else
							{
								FicheInMem* rec = DF->LoadRecord(cur, err, DB4D_Do_Not_Lock, context);
								if (rec != nil)
								{
									rec->SetBackToNew(); // permet de recalculer les autoseq si la valeur du champ est nulle
									...
									{
										sLONG numcrit = *cur;
										ValPtr cv = rec->GetNthField(numcrit, err);
										if (cv != nil)
										{
											if (cv->IsNull())
												keyvalid = false;
									...
									if (keyvalid)
									{
										...
										{
											rec->ValidateAutoSeqToken(context);
											err = DF->SaveRecord(rec, context, rec->GetModificationStamp() == 0 ? 1 : rec->GetModificationStamp());
										}
									}
									else
									{
										DF->SetPrimKeyState(PrimKeyState_SomeNULLs);
										//err = ThrowError(VE_DB4D_PRIMARYKEY_IS_NULL, noaction);
										err = VE_DB4D_PRIMARYKEY_IS_NULL;
									}
									rec->Release();
								}
							}
					...
					if (err == VE_OK && DF->CheckForNonUniqueField(xPrimKey, InProgress, err, inContext))
					{
						err = ThrowError(VE_DB4D_DUPLICATED_KEY, DBaction_SettingPrimaryKey);
						DF->SetPrimKeyState(PrimKeyState_NonUnique);
					}
					...
					*/
					...
					if (err == VE_OK)
					{
						...
						for (NumFieldArray::ConstIterator cur = xPrimKey.First(), end = xPrimKey.End(); cur != end && err == VE_OK; cur++)
						{
							fields.Add(tc[*cur]);
						}
				...
				if (err == VE_OK)
				{
					DF->SetPrimKeyState(PrimKeyState_Valid);
				}
			}
		}
		else
		{
			occupe();
			if (PrimaryKey.GetCount() > 0)		// check if there are still foreign key to it
			{										// sc 12/09/2013 ACI0083944, do nothing if the primary key of the table is empty
				if (HasSyncInfo() || ShouldBeLogged())
				...
				if ( (err == VE_OK) && (DF != nil) )
				{
					DF->SetPrimKeyState(PrimKeyState_notVerified);
				}
			}
			libere();
		}

		if (err == VE_OK && (PrimaryKey.GetCount() > 0 || xPrimKey.GetCount() > 0))
		{
			if (xPrimKey.GetCount() == 1)
			{
				Field* cri = RetainField(xPrimKey[1]);
				if (cri != nil)
				{
					cri->GetCritere()->SetUnique(true);
					cri->Touch();	// sc 02/09/2013
					cri->Release();
				}
			}
			...
			if (inPrimaryKeyName != nil)
				...
				fPrimaryKeyName.Clear();
			if (!PrimaryKey.CopyFrom(xPrimKey))
				err = ThrowError(memfull, DBaction_SettingPrimaryKey);
			AddConstraint();
			AddConstraintCols();
			libere();
...
sWORD Table::GetPrimaryKeyValidityState() const
{
	sWORD state = -1;
	
	if (DF != nil)
		state = DF->LogInfoValidityState();
	else if (fIsRemote)
		state = fRemotePrimaryKeyValidityState;
...
bool Table::CanBeLogged() const
{
	return Owner->LogIsOn() && false; // on purpose LogIsOn locks a mutex
}
...
void Table::AssociateField(sLONG smallID, Field* field)
{
	xbox_assert(fFieldsByID.find(smallID) == fFieldsByID.end());
	fFieldsByID[smallID] = RetainRefCountable(field);
}


VJSONObject* Table::AddToJSON(const VString& TableArrayName, VJSONObject* parent)
{
	VJSONObject* obj = nil;
	if (parent != nil)
	{
		...
		VString s;
		GetName(s);
		obj->SetPropertyAsString("name", s);
		VUUID id;
		GetUUID(id);
		if (!id.IsNull())
		{
			id.GetString(s);
			obj->SetPropertyAsString("ID", s);
		}
		VJSONArray* arr = nil;
		VJSONValue val = parent->GetProperty(TableArrayName);
		if (val.IsNull() || val.IsUndefined())
		{
			arr = new VJSONArray();
			parent->SetProperty(TableArrayName, VJSONValue(arr));
...
VError TableRegular::Update(VStream* dataget)
{
	...
	if (err == VE_OK)
	{
		...
		// sc 27/09/2013 ACI0083824, read the primary key validity state
		if (err == VE_OK)
		{
			sWORD state = -1;
			err = dataget->GetWord( state);
			if (err == VE_OK)
				fRemotePrimaryKeyValidityState = state;
		}
	}
...
VError TableRegular::SendToClient(VStream* datasend)
{
	...
	if (err == VE_OK)
	{
		...
		// sc 27/09/2013 ACI0083824, send the primary key validity state
		if (err == VE_OK)
			err = datasend->PutWord( GetPrimaryKeyValidityState());
	}
	...
	if (testAssert(!fIsRemote))
	{
		...
		if (tabdef != nil && err == VE_OK )
		{
			...
			if (data != nil)
			{
				...
				if (fStamp == 0)
					fStamp = 1; // sc 30/07/2014 ACI0070801
	...
	else
	{
		if (Owner->OkToUpdate(err))
		{
			...
			if (fld != nil)
			{
				...
				if (inFieldDefinition.GetLong( DB4DBagKeys::blob_switch_size, limitlen) && (cri.typ == VK_IMAGE || cri.typ == VK_BLOB || cri.typ == VK_BLOB_DB4D || cri.typ == VK_BLOB_OBJ))
				{
					cri.LimitingLength = limitlen;
					if (cri.LimitingLength < 0)
						cri.LimitingLength = 0;
...
VError TableRegular::LoadFromBagWithLoader( const VValueBag& inBag, VBagLoader *inLoader, void* inContext)
{
	...
	if (err == VE_OK)
	{
		XBOX::VString schemaName = DB4DBagKeys::sql_schema_name.Get( &inBag);
		CDB4DSchema *schema = Owner->RetainSchema( schemaName);
		
		if (schema == NULL)
			FID.fSchemaID = 1;
		else
			FID.fSchemaID = schema->GetID();
		ReleaseRefCountable( &schema);
	}

	if (err == VE_OK)
	{
		FID.fPreventLog = DB4DBagKeys::prevent_journaling.Get( &inBag) ? 1 : 0;
	}
...
VError TableRegular::SaveToBag(VValueBag& ioBag, VString& outKind) const
{
	...
	if (schema != NULL)
	{
		DB4DBagKeys::sql_schema_name.Set(&ioBag, schema->GetName());
		ReleaseRefCountable(&schema);
	}


	DB4DBagKeys::prevent_journaling.Set( &ioBag, FID.fPreventLog == 1);
...
sLONG TableRegular::AddField(const VString &name, sLONG fieldtyp, sLONG fieldlen, DB4DFieldAttributes inAttributes, VError& err, 
							 CDB4DBaseContext* inContext, VProgressIndicator* progress, bool forceAtTheEnd)
{
	CritereDISK cd;
	sLONG nc = -1;
	VUUID x;
		...
		if (ok)
		{
			...
			*/
			...
			if (err == VE_OK)
			{
				...
				err = db->StartWriteLog(DB4D_Log_TruncateTable, len, context, log, true);
				if (err == VE_OK)
				{
					if (log != nil)
					{
...
VError TableRegular::SetPreventJournaling( CDB4DBaseContext* inContext, bool x, VDB4DProgressIndicator* inProgress)
{
	VError err = VE_OK;

	if (fIsRemote)
	{
		IRequest *req = GetOwner()->CreateRequest( inContext, Req_SetTablePreventJournaling + kRangeReqDB4D);
		if (req == NULL)
		{
			err = ThrowBaseError( memfull, DBaction_ChangingTableProperties);
		}
...
bool TableRegular::GetPreventJournaling() const
{
	return FID.fPreventLog != 0;
}

void TableRegular::MarkRecordAsPushed(sLONG numrec, BaseTaskInfo* context)
{
	if (fIsRemote)
	{
		//VDBMgr::GetManager()->AddMarkRecordAsPushed(GetNum(), numrec);
		context->AddMarkRecordAsPushed(GetNum(), numrec);
	}
	else
	{
		if (DF != nil)
...
void TableRegular::UnMarkRecordAsPushed(sLONG numrec, BaseTaskInfo* context)
{
	if (fIsRemote)
	{
		//VDBMgr::GetManager()->AddUnMarkRecordAsPushed(GetNum(), numrec);
		context->AddUnMarkRecordAsPushed(GetNum(), numrec);
	}
	else
	{
		if (DF != nil)
	...
	if (err == VE_OK)
	{
		...
		if (err == VE_OK)
		{
			...
			do 
			{
				...
				if (err == VE_OK)
				{
					FicheInMem* fic = job.ReadOneRecord(context, err);
					if (fic != nil && err == VE_OK)
					{
						err = DF->SaveRecord(fic, context);
					}
					QuickReleaseRefCountable(fic);
				}
				++curfic;
			...
			if (options.DelayIndexes)
			{
				AwakeIndexes(inProgress, nil);
			}
			if (options.ChangeIntegrityRules)
				Owner->StopDataConversion(context, inProgress, true);
		}
	}
	job.StopJob();
...
bool TableRegular::CanBeLogged() const
{
#if debuglr
	return Owner->LogIsOn() && (FID.fPreventLog == 0);
#else
	return Owner->LogIsOn() && (FID.fPreventLog == 0);
#endif
}
...
VError TableRegular::CheckIndexesOnPrimKeys()
{
	VError err = VE_OK;
	if (GetDF() != nil)
	{
		bool primkeyIsSingleField = true;
		if (PrimaryKey.GetCount() > 1)
		{
			FieldNuplet fields(PrimaryKey.GetCount(), true);
			sLONG i = 1;
			primkeyIsSingleField = false;
			for (NumFieldArray::Iterator cur = PrimaryKey.First(), end = PrimaryKey.End(); cur != end; ++cur, ++i)
			{
				Field* cri = tc[*cur];
				fields.SetNthField(i, cri);
			}
			Owner->CreIndexOnFields(&fields, DB4D_Index_Btree, true, nil, nil, nil, nil, false, nil); // will not recreate the index, if it already exists
		}
		...
		for (FieldArray::Iterator cur = tc.First(), end = tc.End(); cur != end && err == VE_OK; ++cur)
		{
			...
			if (cri != nil)
			{
				if (!cri->IsIndexed())
				{
					if ((cri->IsPrimKey() && primkeyIsSingleField) || cri->GetUnique())
						err = Owner->CreIndexOnField(cri, DB4D_Index_Btree, true, nil);
				}
			}
		}
	}
...
TableOfTable::TableOfTable(Base4D* owner):TableSystem(owner, -1, L"_USER_TABLES", VUUID((VUUIDBuffer&)"Sys_DB4D_000001"), L"_USER_TABLE")
{
	...
	id_Rest_Available = System_AddField(DB4D_Boolean, L"REST_AVAILABLE");
	id_Log = System_AddField(DB4D_Boolean, L"LOGGED");
}
...
TableOfField::TableOfField(Base4D* owner):TableSystem(owner, -2, L"_USER_COLUMNS", VUUID((VUUIDBuffer&)"Sys_DB4D_000002"), L"_USER_COLUMN")
{
	...
	id_Autoincrement = System_AddField(DB4D_Boolean, L"AUTOINCREMENT");
	id_Autogenerate = System_AddField(DB4D_Boolean, L"AUTOGENERATE");
	id_Rest_Available = System_AddField(DB4D_Boolean, L"REST_AVAILABLE");
	id_Is_Unique = System_AddField(DB4D_Boolean, L"UNIQUENESS");
}
...
TableOfIndexes::TableOfIndexes(Base4D* owner):TableSystem(owner, -3, L"_USER_INDEXES", VUUID((VUUIDBuffer&)"Sys_DB4D_000003"), L"_USER_INDEXES")
{
	...
	id_Keyword = System_AddField(DB4D_Boolean, L"KEYWORD");
	id_Table_Name = System_AddField(DB4D_StrFix, L"TABLE_NAME");
	id_Uniq = System_AddField(DB4D_Boolean, L"UNIQUENESS");
	id_Table_ID = System_AddField(DB4D_Integer64, L"TABLE_ID");
	id_Index_ID = System_AddField(DB4D_StrFix, L"INDEX_ID");
}
				...
				if (qt > 0)	// do nothing if 0
				{
					...
					if (cols.SetCount(nbcol, nil) && RelPaths.SetCount(nbcol, nil))
					{
						...
						if (err == VE_OK && !TousDejaPris)
						{
							...
							IProgressInfoCollector* collector = nil;
#if WITH_RTM_DETAILLED_ACTIVITY_INFO
							VDB4DSelectionInfoCollector selInfoCollector(context, sel, &collection, VDB4DBasicInfoCollector::kSelectionToArray);
							collector = &selInfoCollector;
#endif 
							if (InProgress != nil)
							{
								XBOX::VString session_title;
								gs(1005,25, session_title);
								InProgress->BeginSession(qt, session_title, collector);
							}
																	...
																	Boolean AddToSel, Boolean CreateAlways, Selection* &outSel, Bittab* &outLockSet, VDB4DProgressIndicator* InProgress)
{
	...
	if (NotDeleted(err))
	{
		...
		if (cols.SetCount(nbcol, nil))
		{
			...
			IProgressInfoCollector* collector = nil;
			#if WITH_RTM_DETAILLED_ACTIVITY_INFO
			VDB4DSelectionInfoCollector selInfoCollector(context, sel, &collection, VDB4DBasicInfoCollector::kArrayToSelection);
			collector = &selInfoCollector;
			#endif 
			if (InProgress != nil)
			{
				...
				InProgress->BeginSession(nbraw, session_title, false, collector);
			}
			...
			for (rawnumber = 0; rawnumber < nbraw; rawnumber++)
			{
				...
				else
				{
					...
					if (err != VE_OK)
						break;
					err = SaveRecord(fic, context,0,false,InProgress);
					if (err == VE_OK)
					{
						if (newone && AddToSel)
						{
...
VError DataTable::ExecuteQuery(SearchTab* query, QueryOptions* options, QueryResult& outResult, BaseTaskInfo* context, VDB4DProgressIndicator* inProgress)
{
	...
	Boolean oldBuildQueryPath = false,oldBuildQueryPlan=false;

	if (context != nil)
	{
		oldBuildQueryPath = context->ShouldBuildQueryPath();
		oldBuildQueryPath = context->ShouldBuildQueryPlan();
		if (options->ShouldDescribeQuery())
		{
			context->BuildQueryPlan(true);
			context->BuildQueryPath(true);
		}
	}
	...
	if (outError == VE_OK)
	{
		VDB4DQueryInfoCollector queryInfoCollector(context,&rech);
		if (inProgress != NULL)
		{
			inProgress->SetDefaultInfoCollector(&queryInfoCollector);
		}
		...
		if (inProgress != NULL)
		{
			inProgress->SetDefaultInfoCollector(nil);
		}
	}
	...
	if (context != nil)
	{
		context->BuildQueryPath(oldBuildQueryPath);
		context->BuildQueryPlan(oldBuildQueryPlan);
	}
...
void DataTable::MeasureQuery(const VString& refstr, sLONG nb, sLONG8 time)
{
	VTaskLock lock(&fMeasureMutex);
	QueryMeasure* qm = &(fQueryMeasures[refstr]);
	qm->fCount.AddMeasure(nb);
	qm->fTime.AddMeasure(time);
}



VJSONObject* DataTable::RetainMeasures(VJSONObject* options, JSONPath* path)
{
	VTaskLock lock(&fMeasureMutex);
	VJSONObject* result = nil;

	JSONPathMarker xpath(path);
	if (xpath.AcceptJsonPathPart(MeasureCollection::kDiskReadBytes))
		jsonProp(&result, MeasureCollection::kDiskReadBytes, fDiskReadBytes.RetainMeasures(options));
	
	if (xpath.AcceptJsonPathPart(MeasureCollection::kDiskWriteBytes))
		jsonProp(&result, MeasureCollection::kDiskWriteBytes, fDiskWriteBytes.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kDiskReadCount))
		jsonProp(&result, MeasureCollection::kDiskReadCount, fDiskReadCount.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kDiskWriteCount))
		jsonProp(&result, MeasureCollection::kDiskWriteCount, fDiskWriteCount.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kCacheHitBytes))
		jsonProp(&result, MeasureCollection::kCacheHitBytes, fCacheHitBytes.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kCacheMissBytes))
		jsonProp(&result, MeasureCollection::kCacheMissBytes, fCacheMissBytes.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kCacheHits))
		jsonProp(&result, MeasureCollection::kCacheHits, fCacheHitCount.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kCacheMisses))
		jsonProp(&result, MeasureCollection::kCacheMisses, fCacheMissCount.RetainMeasures(options));


	VString recordpart("records");
	if (xpath.AcceptJsonPathPart(recordpart))
	{
		VJSONObject * records = nil;
		JSONPathMarker xpath2(path);

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskReadBytes))
			jsonProp(&records, MeasureCollection::kDiskReadBytes, fRecordsDiskReadBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskWriteBytes))
			jsonProp(&records, MeasureCollection::kDiskWriteBytes, fRecordsDiskWriteBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskReadCount))
			jsonProp(&records, MeasureCollection::kDiskReadCount, fRecordsDiskReadCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskWriteCount))
			jsonProp(&records, MeasureCollection::kDiskWriteCount, fRecordsDiskWriteCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheHitBytes))
			jsonProp(&records, MeasureCollection::kCacheHitBytes, fRecordCacheHitBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheMissBytes))
			jsonProp(&records, MeasureCollection::kCacheMissBytes, fRecordCacheMissBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheHits))
			jsonProp(&records, MeasureCollection::kCacheHits, fRecordCacheHitCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheMisses))
			jsonProp(&records, MeasureCollection::kCacheMisses, fRecordCacheMissCount.RetainMeasures(options));

		jsonProp(&result, recordpart, RetainRefCountable(records));
		QuickReleaseRefCountable(records);
	}


	VString blobpart("blobs");
	if (xpath.AcceptJsonPathPart(blobpart))
	{
		VJSONObject * blobs = nil;
		JSONPathMarker xpath2(path);

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskReadBytes))
			jsonProp(&blobs, MeasureCollection::kDiskReadBytes, fBlobsDiskReadBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskWriteBytes))
			jsonProp(&blobs, MeasureCollection::kDiskWriteBytes, fBlobsDiskWriteBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskReadCount))
			jsonProp(&blobs, MeasureCollection::kDiskReadCount, fBlobsDiskReadCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kDiskWriteCount))
			jsonProp(&blobs, MeasureCollection::kDiskWriteCount, fBlobsDiskWriteCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kOutsideBlobsDiskReadBytes))
			jsonProp(&blobs, MeasureCollection::kOutsideBlobsDiskReadBytes, fOutsideBlobsDiskReadBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kOutsideBlobsDiskWriteBytes))
			jsonProp(&blobs, MeasureCollection::kOutsideBlobsDiskWriteBytes, fOutsideBlobsDiskWriteBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheHitBytes))
			jsonProp(&blobs, MeasureCollection::kCacheHitBytes, fBlobsCacheHitBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheMissBytes))
			jsonProp(&blobs, MeasureCollection::kCacheMissBytes, fBlobsCacheMissBytes.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheHits))
			jsonProp(&blobs, MeasureCollection::kCacheHits, fBlobsCacheHitCount.RetainMeasures(options));

		if (xpath2.AcceptJsonPathPart(MeasureCollection::kCacheMisses))
			jsonProp(&blobs, MeasureCollection::kCacheMisses, fBlobsCacheMissCount.RetainMeasures(options));

		jsonProp(&result, blobpart, RetainRefCountable(blobs));
		QuickReleaseRefCountable(blobs);
	}

	for (sLONG i = 1, nb = crit->GetNbCrit(); i <= nb; ++i)
	{
		Field* cri = crit->RetainField(i);
		if (cri != nil)
		{
			VString critname;
			cri->GetName(critname);
			if (!critname.IsEmpty())
			{
				if (xpath.AcceptJsonPathPart(critname))
				{
					jsonProp(&result, critname, cri->RetainMeasures(options, path));
				}
			}
		}
	}

	VString querypart("queries");
	if (xpath.AcceptJsonPathPart(querypart))
	{
		if (!fQueryMeasures.empty())
		{
			VJSONArray* qmarr = new VJSONArray();
			for (QueryMeasureMap::iterator cur = fQueryMeasures.begin(), end = fQueryMeasures.end(); cur != end; ++cur)
			{
				VJSONObject* qmobj = new VJSONObject();
				qmobj->SetPropertyAsString(MeasureCollection::kQueryStatment, cur->first);
				QueryMeasure* qm = &(cur->second);
				jsonProp(&qmobj, MeasureCollection::kQueryCount, qm->fCount.RetainMeasures(options));
				jsonProp(&qmobj, MeasureCollection::kTime, qm->fTime.RetainMeasures(options));
				qmarr->Push(VJSONValue(qmobj));
				qmobj->Release();
			}

			if (result == nil)
				result = new VJSONObject();
			result->SetProperty(querypart, VJSONValue(qmarr));
			qmarr->Release();
		}
	}
	...
	MeasureCollection fRecordCacheHitBytes;
	MeasureCollection fRecordCacheHitCount;

	MeasureCollection fBlobsCacheHitBytes;
	MeasureCollection fBlobsCacheHitCount;
	*/

	return result;
}

void DataTable::SetMeasureInterval(uLONG interval)
{

	fDiskReadBytes.SetInterval(interval);
	fDiskReadCount.SetInterval(interval);
	fDiskWriteBytes.SetInterval(interval);
	fDiskWriteCount.SetInterval(interval);

	fRecordsDiskReadBytes.SetInterval(interval);
	fRecordsDiskReadCount.SetInterval(interval);
	fRecordsDiskWriteBytes.SetInterval(interval);
	fRecordsDiskWriteCount.SetInterval(interval);

	fBlobsDiskReadBytes.SetInterval(interval);
	fBlobsDiskReadCount.SetInterval(interval);
	fBlobsDiskWriteBytes.SetInterval(interval);
	fBlobsDiskWriteCount.SetInterval(interval);

	fOutsideBlobsDiskReadBytes.SetInterval(interval);
	fOutsideBlobsDiskWriteBytes.SetInterval(interval);
	...
	fRecordCacheHitBytes.SetInterval(interval);
	fRecordCacheHitCount.SetInterval(interval);

	fBlobsCacheHitBytes.SetInterval(interval);
	fBlobsCacheHitCount.SetInterval(interval);
...
Blob4D* DataTableRegular::LoadBlobFromOutsideCache(const void* from, CreBlob_Code Code, VError& err, BaseTaskInfo* context)
{
	...
	if (result == nil)
	{
		...
		if (err == VE_OK)
		{
			...
			sLONG lenblob = result->GetDataLength();
			db->MissCache(1,lenblob);

			VTaskLock lock(&fMeasureMutex);
			fCacheMissBytes.AddMeasure(lenblob);
			fBlobsCacheMissBytes.AddMeasure(lenblob);
			fCacheMissCount.AddMeasure(1);
			fBlobsCacheMissCount.AddMeasure(1);
		}
	}
	else
	{
		sLONG lenblob = result->GetDataLength();
		db->HitCache(1, lenblob);
		VTaskLock lock(&fMeasureMutex);
		fCacheHitBytes.AddMeasure(lenblob);
		fBlobsCacheHitBytes.AddMeasure(lenblob);
		fCacheHitCount.AddMeasure(1);
		fBlobsCacheHitCount.AddMeasure(1);
...
Blob4D* DataTableRegular::LoadBlob(sLONG n, CreBlob_Code Code, uBOOL formodif, VError& err, BaseTaskInfo* context)
{
	...
	if (theBlob != nil)
	{
		sLONG lenblob = theBlob->GetDataLength() + kSizeDataBaseObjectHeader;
		if (hitcache)
			db->HitCache(1, lenblob);
		else
			db->MissCache(1, lenblob);
...
VError DataTableRegular::SaveRawBlob(OccupableStack* curstack, DataBaseObjectType inType,sLONG numblob, void* rawdata, sLONG lendata, BaseTaskInfo* context)
{
	...
	if (ou != -1)
	{
		...
		{
			Blob4D* blob;
			if (inType == DBOH_BlobText)
			{
				blob = new BlobText(this);
			}
			else
			{
				blob = new BlobWithPtr(this);
			}
			err = blob->GetFrom(rawdata, lendata);
			if (err == VE_OK)
			{
				blob->setaddr(ou, false);
				blob->SetAntelen(len);
				blob->SetNum(numblob);
				blob->setmodif(true, db, context);
			}
			else
			{
				sLONG xdebug = 1;
...
VError DataTableRegular::SaveBlobForTrans(Blob4D *inBlob, BaseTaskInfo* context)
{
	...
	else if (NotDeleted(err))
	{
		...
		else
		{
			...
			{
				xbox_assert(false);
				//xbox_assert(inBlob->IsDansCache());
				//oldblob->libere();
			}
			else
			{
			...
			else
			{
				...
				else
				{
					...
					if (ou>0)
					{
						...
						{
							n = n; // break here;
							xbox_assert(xoldaddr == inBlob->getaddr());
						}
	...
	db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Saving Blob: "+ToString(n));
#endif
	...
	if (NotDeleted(err, DBaction_SavingBlob))
	{
		...
		if (trans != nil)
		{
			if (inBlob->IsEmpty())
			{
				if (forcePathIfEmpty)
				{
					sLONG xdebug = 1; // put a break here, should not occur
				}
				else
				{
...
#if debug_blobOrphans
		if (trans != nil)
			db->WriteDebugLogStr("transaction : ");
		db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Saving Blob: "+ToString(n));
#endif
...
VError DataTableRegular::SaveRawRecord(OccupableStack* curstack, sLONG numrec, void* rawdata, sLONG lendata, sLONG nbfields, BaseTaskInfo* context, uLONG8 timestamp)
{
	VError err = VE_OK;
	...
	if (ou != -1)
	{
		...
		if (err == VE_OK)
		{
			FicheOnDisk* ficD = FicheOnDisk::BuildRecordFromLog(err, this, numrec, rawdata, lendata, nbfields, false);
			ficD->SetTimeStamp(timestamp);
			ficD->setaddr(ou, false);
			ficD->SetAntelen(len);
...
VError DataTableRegular::SaveRecordInTrans(FicheOnDisk *ficD, BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress)
{
	VError err = VE_OK;
	Boolean newone = false;
	sLONG n;
	...
	if (NotDeleted(err))
	{
		...
		if (db->OkToUpdate(err))
		{
			...
			if (err == VE_OK)
			{
				...
				ficD->WriteSaveToLog(context, newone, nil, nil);
		...
		LibereDelete();
#if debug_blobOrphans
		db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Saving record: "+ToString(n));
		ficD->debugBlobOrphans();
#endif
	}
...
VError DataTableRegular::SaveRecord(FicheInMem *fic, BaseTaskInfo* context, uLONG stamp, bool allowOverrideStamp,XBOX::VProgressIndicator* inProgress)
{
	...
	PrimKey* oldPrimkey = nil;

	bool mustgetprimkey = false;
	...
	{
		mustgetprimkey = true;
		oldPrimkey = fic->RetainPrimKey(err, true);
		if (!CheckPrimaryKeyValid(context, inProgress, err, false))
			err = ThrowError(err, noaction);
	}
	db->ReleaseLogMutex();

	if ((err == VE_OK) && NotDeleted(err))
	{
		StartDataModif(context);
#if trackModif
		...
		if (err == VE_OK && db->OkToUpdate(err))
		{
			...
			{
				bool primkeymodified = false;
				FieldArray primkey;
				crit->RetainPrimaryKey(primkey);
				for (FieldArray::Iterator cur = primkey.First(), end = primkey.End(); cur != end; ++cur)
				{
					Field* cri = *cur;
					if (fic->IsModif(cri))
						primkeymodified = true;
					cri->Release();
				}
				if (primkeymodified)
					UpdatingAPrimKey();
			...
			if (fic->SetSaving())
			{
								...
								if (err == VE_OK)
								{
									...
									if (n < 0)
									{
										if (DFD.nbfic < kMaxRecordsInTable)
										{
											...
											xbox_assert(oldnbfic == DFD.nbfic);
										}
										else
				...
				if (err == VE_OK)
				{
					...
					{
						err = crit->CheckReferentialIntegrity(fic, context, true,inProgress);
					}

					if (err == VE_OK && fic->MustCheckIntegrity())  // puis verification de l'integrite referentielle pour les foreign keys
						err = crit->CheckReferentialIntegrityForForeignKey(fic, context, !newone,inProgress);
					...
					{						
						...
						if (err == VE_OK)
						{
							...
							err = fic->stockcrit(context, inProgress);
							...
							if (err == VE_OK)
								err = crit->UpdateBlobs(fic, context, oldPrimkey);
						...
						if (err==VE_OK)
						{
							...
							if (enTransaction)
							{
								if (oldPrimkey != nil)
								{
									ficD->SetPrimKey(oldPrimkey);
								}
								else
								{
									if (mustgetprimkey || crit->HasSyncInfo())
									{
										oldPrimkey = fic->RetainPrimKey(err, !newone);
										ficD->SetPrimKey(oldPrimkey);
									}
								}
								/*
						...
						if (err==VE_OK)
						{
							...
							{
								ficD->SetAntelen(len);
								ficD->WriteSaveToLog(context, newone, fic, oldPrimkey);
								ficD->setmodif(true, db, context);
							}
	...
	QuickReleaseRefCountable(oldPrimkey);
...
RecIDType DataTableRegular::FindRecordID(PrimKey* primkey, VError& err, BaseTaskInfo* Context)
{
	RecIDType result = -1;
	err = VE_OK;

	if (primkey == nil)
		err = ThrowError(VE_DB4D_PRIMARYKEY_IS_MISSING, noaction);
	else
	{
		if (crit->HasPrimKey())
		{
			...
			PrimKey::parts& primkeyvalues = primkey->GetValues();

			if (primkeyvalues.size() != primkeyfields.GetCount())
				err = ThrowError(VE_DB4D_PRIMARYKEY_IS_MALFORMED, noaction);
			else
			{
				SearchTab query(crit);
				PrimKey::parts::iterator curval = primkeyvalues.begin();
				bool first = true;
				for (FieldArray::Iterator cur = primkeyfields.First(), end = primkeyfields.End(); cur != end && err == VE_OK; ++cur, ++curval)
				{
					const VValueSingle* cv = *curval;
					if (cv == nil)
						err = ThrowError(VE_DB4D_PRIMARYKEY_IS_MALFORMED, noaction);
					else
					{
						if (first)
							...
							query.AddSearchLineBoolOper(DB4D_And);
						query.AddSearchLineSimple(*cur, DB4D_Equal, cv);
					}
				}
				...
				if (err == VE_OK)
				{
					...
					Selection* sel = nil;
					if (err == VE_OK)
						sel = xquery.Perform((Bittab*)nil, nil, Context, err, DB4D_Do_Not_Lock);
					if (err == VE_OK)
					{
						if (sel != nil && !sel->IsEmpty())
						{
...
FicheInMem* DataTableRegular::LoadRecord(PrimKey* primkey, VError& err, DB4D_Way_of_Locking HowToLock, BaseTaskInfo* Context, Boolean WithSubRecords, 
										 Boolean BitLockOnly, ReadAheadBuffer* buffer, Boolean* outEnoughMem)
{
	FicheInMem* result = nil;
	RecIDType numrec = FindRecordID(primkey, err, Context);
	if (err == VE_OK)
	{
		if (numrec>= 0)
		{
			result = LoadRecord(numrec, err, HowToLock, Context, WithSubRecords, BitLockOnly, buffer, outEnoughMem);
		}
		else
			err = ThrowError(VE_DB4D_PRIMARYKEY_CANNOT_BE_FOUND, noaction);
	}
		...
		if ( trans==nil || !trans->IsRecordDeleted(GetNum(), n)) 
		{
			if (n>=0 && n<DFD.nbfic)
			{
				...
				if (fic == nil)
				{
					...
					{
						{
							sLONG lenrec = ficD->GetDataLen() + ficD->GetChampHeaderSize() + kSizeRecordHeader;
							if (hitcache)
								db->HitCache(1, lenrec);
							else
								db->MissCache(1, lenrec);
							VTaskLock lock(&fMeasureMutex);
							if (hitcache)
							{
								fCacheHitBytes.AddMeasure(lenrec);
								fRecordCacheHitBytes.AddMeasure(lenrec);
								fCacheHitCount.AddMeasure(1);
								fRecordCacheHitCount.AddMeasure(1);
							}
							else
							{
								fCacheMissBytes.AddMeasure(lenrec);
								fRecordCacheMissBytes.AddMeasure(lenrec);
								fCacheMissCount.AddMeasure(1);
								fRecordCacheMissCount.AddMeasure(1);
							}
						}
#if debug_blobOrphans
						if (crit->GetNum() == 20)
						{
							db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Loading record: "+ToString(n));
							ficD->debugBlobOrphans();
						}
#endif
						fic=new FicheInMem(Context, crit,ficD);
...
VError DataTableRegular::WriteDeleteBlobToLog(BaseTaskInfo* context, const VString& path)
{
	VStream* log;
	VError err = VE_OK;
	if (crit != nil)
	{
		if (crit->CanBeLogged())
		{
			...
			if (primkey != nil)
			{
				...
				sLONG blobid = -2;
				len	= len + 4 + path.GetLength() * sizeof(UniChar);
				...
				if (err == VE_OK)
				{
					if (log != nil)
					{
						...
						if (err == VE_OK)
						{
							sLONG lenPath = path.GetLength();
							err = log->PutLong(lenPath);
							err = log->PutWords(path.GetCPointer(), lenPath);
							if (err == VE_OK)
							{
								VUUID xid;
								crit->GetUUID(xid);
				...
				err = crit->ThrowError(VE_DB4D_LOG_NEEDS_A_VALID_PRIMARYKEY, noaction);

		}
		GetDB()->ReleaseLogMutex();
	}
...
VError DataTableRegular::WriteDeleteBlobToLog(BaseTaskInfo* context, sLONG n, PrimKey* primkey, sLONG fieldnum)
{
	VStream* log;
	VError err = VE_OK;
	if (crit != nil)
...
VError DataTableRegular::WriteDeleteToLog(BaseTaskInfo* context, sLONG n, PrimKey* primkey)
{
	VError err = VE_OK;
	if (crit != nil)
	{
...
VError DataTableRegular::DelRecordForTrans(sLONG n, BaseTaskInfo* context, PrimKey* primkey,XBOX::VProgressIndicator* inProgress)
{
	//CheckUseCount();
	VError err = VE_OK;
	...
	if (NotDeleted(err))
	{
		...
		if (db->OkToUpdate(err))
		{
			...
			WriteDeleteToLog(context, n, primkey);
...
#if debug_blobOrphans
			db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Deleting Record: "+ToString(n));
			if (ficD != nil)
			{
				ficD->debugBlobOrphans();
				ficD->Release();
...
VError DataTableRegular::DelRecord(FicheInMem *rec, BaseTaskInfo* context, uLONG stamp,XBOX::VProgressIndicator* inProgress)
{
	sLONG n;
	VError err = VE_OK;
	FicheOnDisk *ficD = nil;
		...
		if (db->OkToUpdate(err))
		{
			...
			else
			{
				...
				if (n>=0)
				{
					...
					{
						err = crit->CheckReferentialIntegrity(rec, context, false,inProgress);
					}
					...
					if (err == VE_OK)
					{
						...
						if (err == VE_OK)
						{
							...
							{
								PrimKey* primkey = nil;
								if (crit->IsLoggedWithPrimKey())
								{
									primkey = rec->RetainPrimKey(err, true);
								}
								trans->DelRecord(GetNum(), n, primkey);
								QuickReleaseRefCountable(primkey);
							}
							else
							{
							...
							if (!EnTransaction)
								ficD->WriteDeleteToLog(context, rec);
						}
						if (!EnTransaction)
						{
							if (MustFullyDeleteRecords())
...
VError DataTableRegular::DelBlob(Blob4D *inBlob, BaseTaskInfo* context)
{
	...
	if (NotDeleted(err))
	{
		...
		if (trans != nil)
		{
			...
			else
			{
				...
				if (n>=0)
				{
					...
					if (crit->IsLoggedWithPrimKey())
					{
						primkey = inBlob->GetAssociatedPrimKey();
						numfield = inBlob->GetAssociatedFieldNumber();
					}
					err = trans->DelBlob(GetNum(), n, primkey, numfield);
					if (err == VE_OK)
					{
						//inBlob->WriteDeleteToLog(context);
						inBlob->SetNotExisting();  // le blob a normalement ete duplique avant (soit dans VBobText::Flush, soit dans CreVBLobWithPtr ou CreVBlobPicture juste apres le loadblob, soit dans DelRecord::DeleteBlobs)
					}
				}
			}
		}
		...
		if (trans != nil)
			db->WriteDebugLogStr("transaction : ");
		db->WriteDebugLogLn("Table "+ToString(crit->GetNum())+" , Deleting Blob: "+ToString(n));
#endif
	}
...
VError DataTableRegular::DelBlobForTrans(sLONG n, BaseTaskInfo* context, bool alterModifState, PrimKey* primkey, sLONG fieldnum, bool keepBlobOccupiedSpace)
{
	VError err = VE_OK;
	...
	if (NotDeleted(err))
	{
		WriteDeleteBlobToLog(context, n, primkey, fieldnum);
...
VJSONObject* DataTableRegular::BuildLockInfo(VError& err)
{
	...
	if (dblocker != nil && dblocker->GetOwner() != nil)
	{
		VJSONObject* obj = dblocker->GetOwner()->BuildLockInfo();
		if (obj != nil)
		{
			result->SetProperty("database", VJSONValue(obj));
			obj->Release();
		}
	}

	if (fWholeTableIsLockedBy != nil && fWholeTableIsLockedBy->GetOwner() != nil)
	{
		VJSONObject* obj = fWholeTableIsLockedBy->GetOwner()->BuildLockInfo();
		if (obj != nil)
		{
			result->SetProperty("table", VJSONValue(obj));
			obj->Release();
		}
	}
	...
	err = LockRec.GetLockerInfo(arr, curstack);
	if (err == VE_OK)
		result->SetProperty("records", VJSONValue(arr));
						...
						if ((obj==nil) || obj->GetOwner() == nil)
						{
							if (HowToLock != DB4D_Do_Not_Lock)	// on nous demande juste de tester le verrou sans en mettre un
							{
								...
								else
								{
									...
									{
										xbox_assert(fAllLocks.IncCount(Context));
										xbox_assert(db->IncLockCount(Context));
										fTotalLocks++;
									}
									if (newlock != nil)
										newlock->Retain();
			...
			*/
			...
			if ( obj == thelock /* || (obj != nil && obj->GetOwner() == nil)*/ )
			{
				...
				if (obj != nil)
				{
					obj->Release();
					fTotalLocks--;
					xbox_assert(fAllLocks.DecCount(Context));
					xbox_assert(db->DecLockCount(Context));
				}
...
VError DataTableRegular::LibereEspaceDisk(OccupableStack* curstack, VDB4DProgressIndicator* InProgress)
{
	...
	err2 = BlobTabAddr.LibereEspaceDisk(curstack, InProgress);
	if (err2 != VE_OK)
	{
		if (PremiereErreur)
			PremiereErreur = false;
	...
	err2 = FicTabAddr.LibereEspaceDisk(curstack, InProgress);
	if (err2 != VE_OK)
	{
		if (PremiereErreur)
			PremiereErreur = false;
...
VError DataTableRegular::Truncate(BaseTaskInfo* context, VDB4DProgressIndicator* InProgress, Boolean ForADeleteSelection, IndexArray& indexdeps)
{
	...
	xbox_assert(GetCurrentTransaction(context) == nil);
...
#if AllowSyncOnRecID
VError DataTableRegular::GetModificationsSinceStamp(sLONG stamp, VStream& outStream, uLONG& outLastStamp, sLONG& outNbRows, BaseTaskInfo* context, vector<sLONG>& cols)
{
	VError err = VE_OK;
...
VError DataTableRegular::GetModificationsSinceStamp(sLONG8 stamp, VStream& outStream, uLONG8& outLastStamp, sLONG& outNbRows, BaseTaskInfo* context, vector<sLONG>& cols,
				...
				while (numrec != -1 && err == VE_OK)
				{
					...
					if (rec != nil)
					{
						...
						if (err == VE_OK)
						{
							...
							err = formatter->PutTimeStamp(Timestamp, false);
					...
					while (numrec != -1 && err == VE_OK)
					{
						...
						if (err == VE_OK)
						{
							...
							if ( !bLoadFailed )
							{
								...
								err = formatter->PutTimeStamp(*cvtimestamp, action != DB4D_SyncAction_Update);
...
void DataTableRegular::MarkOutsideBlobToDelete(const VString& inID, const VString& inPath, BaseTaskInfo* context)
{
	UnCacheOutsideBlob(inID);
	if (!inPath.IsEmpty())
	{
		if (context != nil)
			WriteDeleteBlobToLog(context, inPath);
...
uBYTE DataTableRegular::LogInfoValidityState()
{
	uBYTE validitystate = PrimKeyState_Valid;
	if (crit != nil)
	{
		if (crit->ShouldBeLogged())
		{
			if (crit->HasPrimKey())
			{
				if (DFD.nbfic == 0)
				{
					DFD.fPrimKeyState = PrimKeyState_Valid;
				}
				else
				{
					validitystate = DFD.fPrimKeyState;
				}
				
			}
			else
				validitystate = PrimKeyState_MissingPrimKeyDef;
		}
	}
...
bool DataTableRegular::CheckPrimaryKeyValid(BaseTaskInfo* context, VProgressIndicator* progress, VError& outErr, bool forceCheck)
{
	bool result = false;
	outErr = VE_OK;
	if (DFD.fPrimKeyState == PrimKeyState_Valid && !forceCheck)
	{
		result = true;
	}
	else
	{
		if (crit != nil)
		{
			if (crit->HasPrimKey())
			{
				...
				{
					result = true;
					SetPrimKeyState(PrimKeyState_Valid);
				}
				else
				{
					NumFieldArray fields;
					...
					{
						Field* cri = *cur;
						fields.Add(cri->GetPosInRec());
						QuickReleaseRefCountable(cri);
					}

					Selection* all = AllRecords(context, outErr);
					if (all != nil)
					{
						if (progress != nil)
						{
							...
							gs(1005,35,session_title);
							session_title.Format(&bag);
							progress->BeginSession(all->GetQTfic(), session_title, false);
						}
						
						SelectionIterator iter(all);
						RecIDType cur = iter.FirstRecord();
						sLONG curval = 0;
						while (cur != -1 && outErr == VE_OK)
						{
							if (progress != nil && !progress->Progress(curval))
							{
								outErr = ThrowError(VE_DB4D_ACTIONCANCELEDBYUSER, noaction);
							}
							else
							{
								FicheInMem* rec = LoadRecord(cur, outErr, DB4D_Do_Not_Lock, context);
								if (rec != nil)
								{
									rec->SetBackToNew(); // permet de recalculer les autoseq si la valeur du champ est nulle
									PrimKey* key = rec->RetainPrimKey(outErr, true);
									rec->ClearNew();
									if ((key != nil) && key->IsValid())
									{
										...
										{
											rec->ValidateAutoSeqToken(context);
											outErr = SaveRecord(rec, context, rec->GetModificationStamp() == 0 ? 1 : rec->GetModificationStamp());
										}
									}
									else
									{
										SetPrimKeyState(PrimKeyState_SomeNULLs);
										//outErr = ThrowError(VE_DB4D_PRIMARYKEY_IS_NULL, noaction);
										outErr = VE_DB4D_PRIMARYKEY_IS_NULL;
									}
									key->Release();
									rec->Release();
								}
					...
					if (outErr == VE_OK)
					{
						if (CheckForNonUniqueField(fields, progress, outErr, context))
						{
							SetPrimKeyState(PrimKeyState_NonUnique);
							//outErr = ThrowError(VE_DB4D_PRIMARYKEY_IS_NOT_UNIQUE, noaction);
							outErr = VE_DB4D_PRIMARYKEY_IS_NOT_UNIQUE;
						}
						else
						{
							SetPrimKeyState(PrimKeyState_Valid);
			...
			{
				result = !(crit->ShouldBeLogged());
				SetPrimKeyState(PrimKeyState_notVerified);
				if (!result)
				{
					//outErr = ThrowError(VE_DB4D_PRIMARYKEY_IS_MISSING, noaction);
					outErr = VE_DB4D_PRIMARYKEY_IS_MISSING;
				}
			}
		}
	}
...
void DataTableRegular::SetPrimKeyState(uBYTE primkeystate)
{
	if (DFD.fPrimKeyState != primkeystate)
	{
		DFD.fPrimKeyState = primkeystate;
		setmodif(true, GetDB(), nil);
		if (crit != nil)
			crit->Touch();
	}
...
{ 
	...
	else
	{
		...
		if (fic != nil)
		{
			sLONG nbcrit = crit->GetNbCrit();
			for (sLONG i = 1; i <= nbcrit; ++i)
			{
				fic->Touch(i);
			}
...
Boolean DataTableRegular::Debug_CheckBlobAddrMatching(DataAddr4D addrToCheck, sLONG blobID)
{
	if (blobID>=0 && addrToCheck != 0)
	{
		...
		{
			err = err; // break here
			xbox_assert(danstable == addrToCheck);
			return true;
		}
	}
	return false;
										...
										Boolean WithSubRecords, Boolean BitLockOnly, ReadAheadBuffer* buffer, Boolean* outEnoughMem)
{
	...
	if (tt != nil)
	{
		...
		if (err == VE_OK)
		{
			VBoolean* srestavailable = new VBoolean(!tt->GetHideInRest());
			if (srestavailable == nil)
				err = ThrowError(memfull, DBaction_LoadingRecord);
			else
			{
				err = fic->SetNthField(tcrit->id_Rest_Available, srestavailable);
				if (err != VE_OK)
					delete srestavailable;
			}
		}

		if (err == VE_OK)
		{
			VBoolean* slogenabled = new VBoolean(!tt->GetPreventJournaling());
			if (slogenabled == nil)
				err = ThrowError(memfull, DBaction_LoadingRecord);
			else
			{
				err = fic->SetNthField(tcrit->id_Log, slogenabled);
				if (err != VE_OK)
					delete slogenabled;
			}
		}
		...
		if (n>=0 && n<(sLONG)fFields.size())
		{
			...
			if (ntable > 0)
			{
				...
				if (ff != nil)
				{
					...
					if (err == VE_OK)
					{
						VBoolean* sautoincrement = new VBoolean(ff->GetAutoSeq());
						if (sautoincrement == nil)
							err = ThrowError(memfull, DBaction_LoadingRecord);
						else
						{
							err = fic->SetNthField(tcrit->id_Autoincrement, sautoincrement);
							if (err != VE_OK)
								delete sautoincrement;
						}
					}

					if (err == VE_OK)
					{
						VBoolean* sautogenerate = new VBoolean(ff->GetAutoGenerate());
						if (sautogenerate == nil)
							err = ThrowError(memfull, DBaction_LoadingRecord);
						else
						{
							err = fic->SetNthField(tcrit->id_Autogenerate, sautogenerate);
							if (err != VE_OK)
								delete sautogenerate;
						}
					}

					if (err == VE_OK)
					{
						VBoolean* srestavailable = new VBoolean(!ff->GetHideInRest());
						if (srestavailable == nil)
							err = ThrowError(memfull, DBaction_LoadingRecord);
						else
						{
					...
					if (err == VE_OK)
					{
						VBoolean* sunique = new VBoolean(ff->GetUnique());
						if (sunique == nil)
							err = ThrowError(memfull, DBaction_LoadingRecord);
						else
						{
							err = fic->SetNthField(tcrit->id_Is_Unique, sunique);
							if (err != VE_OK)
								delete sunique;
						}
					}
	...
	if (err == VE_OK)
	{
		...
		if (found != fIndexRefs.end())
		{
			...
			if (ind != nil)
			{
				...
				if (err == VE_OK)
				{
					VBoolean* skeyword = new VBoolean(ind->GetTyp() == DB4D_Index_OnKeyWords);
					if (skeyword == nil)
						err = ThrowError(memfull, DBaction_LoadingRecord);
					if (err == VE_OK)
					{
						err = fic->SetNthField(tcrit->id_Keyword, skeyword);
						if (err != VE_OK)
							delete skeyword;
					}
				}
...
VError DataFile_NotOpened::CheckOneBlob(DataAddr4D ou, sLONG len, sLONG numblob, ToolLog* log)
{
	...
	if (bd->IsAddrValid(ou))
	{
		...
		if (err != VE_OK)
		{
			BlobProblem pb(TOP_BlobPhysicalDataIsInvalid, fNumTable, numblob, fNumTableDef);
			errexec = log->Add(pb);
		}
		...
		if (err == VE_OK && errexec == VE_OK)
		{
			...
			if (err != VE_OK)
			{
				BlobProblem pb(TOP_BlobTagIsInvalid, fNumTable, numblob, fNumTableDef);
				errexec = log->Add(pb);
			}
		}
		...
		if (err == VE_OK && errexec == VE_OK)
		{
			if (lll>0)
			{
				...
				if (p!=nil)
				{
					...
					if (err == VE_OK && errexec == VE_OK)
					{
						...
						if (err != VE_OK)
						{
							BlobProblem pb(TOP_BlobCheckSumIsInvalid, fNumTable, numblob, fNumTableDef);
							errexec = log->Add(pb);
						}
					}
					...
					if (err == VE_OK && errexec == VE_OK)
					{
						...
						else
						{
							...
							{
								BlobProblem pb(TOP_BlobLengthDoesNotMatchTabAddr, fNumTable, numblob, fNumTableDef);
								errexec = log->Add(pb);
			...
			{
				BlobProblem pb(TOP_BlobLengthIsInvalid, fNumTable, numblob, fNumTableDef);
				errexec = log->Add(pb);
			}
		}
	}
	...
	{
		BlobProblem pb(TOP_BlobAddrIsInvalid, fNumTable, numblob, fNumTableDef);
		errexec = log->Add(pb);
		fBlobMapIsValid = false;
		err = VE_DB4D_CANNOTLOADBLOB;
	}
		...
		if (err == VE_OK)
		{
			if (nbblobmax > kNbElemTabAddr)
			{
				...
				for (sLONG i=0; i<nbparents && errexec == VE_OK; i++)
				{
					...
					{
						DataTable_BlobTabAddrProblem pb(TOP_LenghOfAddressTableIsInvalid, fNumTable, mastermultiplicateur + i, fNumTableDef);
						errexec = log->Add(pb);
						fBlobMapIsValid = false;
					}
					if (errexec == VE_OK)
				...
				for (sLONG i=0; i<nbblobstocheck && errexec == VE_OK; i++)
				{
					...
					if (errexec == VE_OK)
					{
						...
						if (addr <= 0)
						{
							...
							{
								BlobProblem pb(TOP_BlobAddrIsInvalid, fNumTable, mastermultiplicateur + i, fNumTableDef);
								errexec = log->Add(pb);
								fBlobMapIsValid = false;
							}
						}
			...
			if (errexec == VE_OK)
			{
				if (DFD.addrBlobtabaddr != 0)
				{
					...
					if (log->IsCompacting())
					{
						...
						{
							VString tname;
							if (bd->ExistUUID(DFD.TableDefID, tuuid_Table, &tname, nil, nil, nil))
							{
								Table* tt = target->FindAndRetainTableRef(tname);
								if (tt != nil)
								{
									df = RetainRefCountable(tt->GetDF());
									tt->Release();
								}
							}
						}
						...
						if (df != nil)
						{
							...
							if (df != nil)
							{
								VError locerr = df->ResizeTableBlobs(curstack, DFD.nbBlob);
								if (locerr == -2)
								{
									log->SetDataTableBeCompacted(nil);
									df->Release();
									df = nil;
								}
								else
...
VError DataFile_NotOpened::CheckOneRecord(DataAddr4D ou, sLONG len, sLONG numrec, ToolLog* log)
{
		...
		if (err==VE_OK)
		{
			...
			{		
				...
				if (lenx >= 0)
				{
					if (nbc >= 0 && nbc <= kMaxFields)
					{
						...
						if (p!=nil)
						{
							...
							if (err==VE_OK)
							{
									...
									*/
									{
										...
										for (; cur != end && errexec == VE_OK; cur++)
										{
											...
											{
												cansaverecord = false;
												FieldProblem pb(TOP_FieldDataOffsetIsInvalid, fNumTable, numrec, (cur-start)+1, fNumTableDef);
												errexec = log->Add(pb);
											}
											else
											{
												...
												{
													cansaverecord = false;
													FieldProblem pb(TOP_FieldDataLengthIsInvalid, fNumTable, numrec, (cur-start)+1, fNumTableDef);
													errexec = log->Add(pb);
												}
												else
												{
													...
													switch (cur->typ)
													{
															...
															if (lenfield != 0)
															{
																if (lenfield == 4)
																{
																	...
																	if (blobnum >=0 && blobnum < DFD.nbBlob)
																	{
																		if (fBlobsDeleted->isOn(blobnum))
																		{
																			...
																			FieldProblem pb(TOP_FieldBlobIsDeleted, fNumTable, numrec, (cur-start)+1, fNumTableDef);
																			errexec = log->Add(pb);
																		}
																		else
																		{
																			...
																			{
																				mustchangeblobnum = true;
																				FieldProblem pb(TOP_FieldBlobIsInvalid, fNumTable, numrec, (cur-start)+1, fNumTableDef);
																				errexec = log->Add(pb);
																			}
																			else
																			{
																				...
																				{
																					mustchangeblobnum = true;
																					FieldProblem pb(TOP_FieldBlobRefIsAlreadyUsed, fNumTable, numrec, (cur-start)+1, fNumTableDef);
																					errexec = log->Add(pb);
																				}
																				else
																				{
																	...
																	else
																	{
																		...
																		{
																			mustchangeblobnum = true;
																			FieldProblem pb(TOP_FieldBlobNumIsInvalid, fNumTable, numrec, (cur-start)+1, fNumTableDef);
																			errexec = log->Add(pb);
																		}
																	}
																...
																else
																{
																	if (lenfield >= 4)
																	{
																		...
																		{
																			lentext = - lentext - 10;
																			if (cur->typ == VK_BLOB || cur->typ == VK_BLOB_DB4D || cur->typ == VK_IMAGE || cur->typ == VK_BLOB_OBJ)
																			{
																			...
																			if ((pathlen*2 + 4 + 4) == lenfield)
																			{
																				...
																				if (isrelatif)
																				{
																					...
																					if (dfile != nil)
																					{
																						...
																						if (!blobfile.Exists())
																						{
																							FieldProblem pb(TOP_FieldExternalBlobIsMissing, fNumTable, numrec, (cur-start)+1, fNumTableDef);
														...
														default:
															{
																FieldProblem pb(TOP_FieldDataTypeIsInvalid, fNumTable, numrec, (cur-start)+1, fNumTableDef);
																errexec = log->Add(pb);
																if (len == 1)
																	cur->typ = VK_BYTE;
																else if (len == 2)
										...
										{
											cansaverecord = false;
											RecordProblem pb(TOP_RecordLengthDoesNotMatchTabAddr, fNumTable, numrec, fNumTableDef);
											errexec = log->Add(pb);
										}
										...
										if (log->IsCompacting() && cansaverecord)
										{
											...
											if (df != nil)
											{
												err = df->SaveRawRecord(curstack, numrec, p, lenx, nbc, nil, tag.GetTimeStamp());
											}
										}
								...
								{
									RecordProblem pb(TOP_RecordCheckSumIsInvalid, fNumTable, numrec, fNumTableDef);
									errexec = log->Add(pb);
								}
							}
							else
							{
								RecordProblem pb(TOP_RecordPhysicalDataIsInvalid, fNumTable, numrec, fNumTableDef);
								errexec = log->Add(pb);
							}
					...
					{
						RecordProblem pb(TOP_RecordNumberOfFieldsIsInvalid, fNumTable, numrec, fNumTableDef);
						errexec = log->Add(pb);
					}
				}
				else
				{
					RecordProblem pb(TOP_RecordLengthIsInvalid, fNumTable, numrec, fNumTableDef);
					errexec = log->Add(pb);
				}
			...
			{
				RecordProblem pb(TOP_RecordTagIsInvalid, fNumTable, numrec, fNumTableDef);
				errexec = log->Add(pb);
			}
		}
		else
	...
	{
		RecordProblem pb(TOP_RecordAddrIsInvalid, fNumTable, numrec, fNumTableDef);
		errexec = log->Add(pb);
		fRecordMapIsValid = false;
	}
		...
		if (err == VE_OK)
		{
			if (nbficmax > kNbElemTabAddr)
			{
				...
				for (sLONG i=0; i<nbparents && errexec == VE_OK; i++)
				{
					...
					{
						DataTable_RecTabAddrProblem pb(TOP_LenghOfAddressTableIsInvalid, fNumTable, mastermultiplicateur + i, fNumTableDef);
						errexec = log->Add(pb);
						fRecordMapIsValid = false;
					}
					if (errexec == VE_OK)
				...
				for (sLONG i=0; i<nbfictocheck && errexec == VE_OK; i++)
				{
					...
					if (errexec == VE_OK)
					{
						...
						if (addr <= 0)
						{
							...
							{
								RecordProblem pb(TOP_RecordAddrIsInvalid, fNumTable, mastermultiplicateur + i, fNumTableDef);
								errexec = log->Add(pb);
								fRecordMapIsValid = false;
							}
						}
				...
				if (DFD.addrtabaddr != 0)
				{
					...
					if (log->IsCompacting())
					{
						...
						if (df != nil)
						{
							...
							if (df != nil)
							{
								VError locerr = df->ResizeTableRecs(curstack, DFD.nbfic);
								if (locerr == -2)
								{
									log->SetDataTableBeCompacted(nil);
									df->Release();
								...
								if (errexec == VE_OK)
								{
									if (fBlobsOrphan->Compte() > 0)
									{
										...
										if (log->IsCompacting() && errexec == VE_OK)
										{
											...
											{
												errexec = df->DelBlobForTrans(n, nil, true, nil, -1, true);
...
VError ReplicationOutputJSONFormatter::PutTimeStamp ( VValueSingle& inStamp, bool inIsLastAttribute )
{
	VString			vstr;
