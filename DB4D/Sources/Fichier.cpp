...
Boolean Table::CheckUniqueness(FicheInMem* fic, const NumFieldArray& deps, BaseTaskInfo* context, sLONG beginprimkey,XBOX::VProgressIndicator* inProgress)
{
	VError err = VE_OK;;
	...
	if (testAssert(!fIsRemote))
	{
		...
		else
		{
			...
			if (err == VE_OK)
			{
				Selection *sel = query.Perform((Bittab*)nil, inProgress, context, err, DB4D_Do_Not_Lock);
	...
	if (GetDF() != nil)
	{
		for (FieldArray::Iterator cur = tc.First(), end = tc.End(); cur != end && err == VE_OK; ++cur)
		{
			...
			if (cri != nil)
			{
				if (!cri->IsIndexed())
				{
					if (cri->IsPrimKey() || cri->GetUnique())
...
VError DataTable::ExecuteQuery(SearchTab* query, QueryOptions* options, QueryResult& outResult, BaseTaskInfo* context, VDB4DProgressIndicator* inProgress)
{
	...
	VDB4DQueryInfoCollector queryInfoCollector(context);
...
VError DataTableRegular::DelBlobForTrans(sLONG n, BaseTaskInfo* context, bool alterModifState, PrimKey* primkey, sLONG fieldnum)
{
	VError err = VE_OK;
								...
								if (errexec == VE_OK)
								{
									if (fBlobsOrphan->Compte() > 0)
									{
										...
										if (log->IsCompacting() && errexec == VE_OK)
										{
											...
											{
												errexec = df->DelBlobForTrans(n, nil, true, nil, -1);
