...
class Critere : public Obj4D, public IObjCounter
{
	public:
		...
		virtual Boolean IsObjectIndexable(void) { return(false); };
...
//		virtual CClass* GetLangKindClass(void) = 0;
		...
		virtual ValueKind GetTypeForObjectPathIndex() const
		{
			return VK_STRING;
		}
...
class Field : public Obj4D, public IObjCounter, public IDebugRefCountable, public IBaggable
{
	public:
		...
		Boolean IsObjectIndexable(void) { return(cri->IsObjectIndexable()); };
		...
		void ClearAllDependencies(bool includingRelations);
		...
		IndexInfo* FindAndRetainIndexObj(uBOOL sortable, Boolean MustBeValid, BaseTaskInfo* context = nil) const;
		...
		ValueKind GetTypeForObjectPathIndex() const
		{
			return cri->GetTypeForObjectPathIndex();
		}
		...
		sLONG GetSmallIDInRec() const
		{
			return fSmallIDInRec;
		}

		void SetSmallIDInRec(sLONG inID)
		{
			fSmallIDInRec = inID;
		}

		void MeasureQueryCount(sLONG nb = 1) const;
		void MeasureSortCount(sLONG nb = 1) const;

		VJSONObject* RetainMeasures(VJSONObject* options, JSONPath* path);
	...
	protected:
		...
		sLONG fStamp;
		sLONG fSmallIDInRec;
		mutable MeasureCollection fQueryCount;
		mutable MeasureCollection fSortCount;
...
class CritereBlobObj : public CritereBlob
{
public:
	//		virtual VVariableKind GetLangKind(void);
	//		virtual CClass* GetLangKindClass(void);

	virtual Boolean IsObjectIndexable(void) { return(true); };
	//virtual Boolean IsIndexable(void) { return(true); };

	CritereBlobObj(CritereDISK* crd) :CritereBlob(crd) { CRD->typ = VK_BLOB_OBJ; };

	static Critere *NewCritere(CritereDISK* crd) {return new CritereBlobObj(crd);}
};
...
class Table : public Obj4D, public IObjCounter, public IDebugRefCountable, public IBaggable
{
	public:
		...
		IndexInfo* FindAndRetainIndexObj(sLONG nc, uBOOL sortable = false, Boolean MustBeValid = true, BaseTaskInfo* context = nil)  const;
		...
		virtual VError CheckIndexesOnPrimKeys()
		{
			return VE_OK;
		}
		...
		VError UpdateBlobs(FicheInMem *fic, BaseTaskInfo* context, PrimKey* oldPrimKey);
...
//		CClass* GetSelectionRef(void) { return SelectionRef; };
		...
		*/
		...
		Boolean CheckUniqueness(FicheInMem* fic, const NumFieldArray& deps, BaseTaskInfo* context, const NumFieldArray& primkeydeps, XBOX::VProgressIndicator* inProgress, vector<IndexInfo*>& outUniqIndexes);
		...
		VError CheckReferentialIntegrity(FicheInMem *fic, BaseTaskInfo* context, Boolean OnModify,XBOX::VProgressIndicator* inProgress);
		VError CheckReferentialIntegrityForForeignKey(FicheInMem *fic, BaseTaskInfo* context, Boolean OnModify,XBOX::VProgressIndicator* inProgress);
		...
		sWORD GetPrimaryKeyValidityState() const;
		...
		void DelayIndexes(bool includesPrimKey);
		...
		virtual sLONG AddField(const VString &name, sLONG fieldtyp, sLONG fieldlen, DB4DFieldAttributes inAttributes, VError& err, CDB4DBaseContext* inContext, VProgressIndicator* progress = nil, bool forceAtTheEnd = false) = 0;
		...
		virtual VError SetPreventJournaling( CDB4DBaseContext* inContext, bool x, VDB4DProgressIndicator* inProgress = nil)
		{
			return ThrowError( VE_DB4D_TABLEISLOCKED, DBaction_ChangingTableProperties);
		}

		virtual bool GetPreventJournaling() const
		{
			return true;
		}

		virtual bool CanBeLogged() const;

		virtual bool ShouldBeLogged() const
		{
			return false;
		}

		virtual bool IsLoggedWithPrimKey() const
		{
			return false;
		}

		virtual void MarkRecordAsPushed(sLONG numrec, BaseTaskInfo* context)
		{
		}

		virtual void UnMarkRecordAsPushed(sLONG numrec, BaseTaskInfo* context)
		{
		}
		...
		void AssociateField(sLONG smallID, Field* field);

		VJSONObject* AddToJSON(const VString& TableArrayName, VJSONObject* parent);
	...
	protected:
		typedef map<sLONG, Field*> mapOfFieldsBySmallID;

		VString fPrimaryKeyName;
		FieldArray tc;
		mapOfFieldsBySmallID fFieldsByID;
		sLONG nbuse;
		sLONG nres;
		sLONG numfile;
		DataTable *DF;
		...
		*/
		...
		sWORD fRemotePrimaryKeyValidityState;
...
class TableRegular : public Table
{
	public:
		...
		// L.E. pourquoi en virtual ? (pb de maskage)
		virtual sLONG AddField(const VString &name, sLONG fieldtyp, sLONG fieldlen, DB4DFieldAttributes inAttributes, VError& err, CDB4DBaseContext* inContext, VProgressIndicator* progress = nil, bool forceAtTheEnd = false);
		...
		virtual bool IsLoggedWithPrimKey() const
		{
			return (FID.fPreventLog == 0) && HasPrimKey();
		}
...
public:
	...
	sLONG id_Autoincrement;
	sLONG id_Autogenerate;
	sLONG id_Rest_Available;
	sLONG id_Is_Unique;
...
{
	public:
		MeasureCollection fCount;
		MeasureCollection fTime;
};

typedef unordered_map_VString<QueryMeasure> QueryMeasureMap;
...
{ 
	...
	public:
		...
		*/
		...
		virtual VError SaveRecord(FicheInMem *fic, BaseTaskInfo* context, uLONG stamp = 0, bool allowOverrideStamp = false,XBOX::VProgressIndicator* inProgress = NULL) = 0;
		virtual VError SaveRecordInTrans(FicheOnDisk *ficD, BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress = NULL) = 0;
		...
		virtual RecIDType FindRecordID(PrimKey* primkey, VError& err, BaseTaskInfo* Context = nil)
		{
			err = ThrowBaseError(VE_DB4D_NOTIMPLEMENTED);
			return -1;
		}

		virtual FicheInMem* LoadRecord(PrimKey* primkey, VError& err, DB4D_Way_of_Locking HowToLock = DB4D_Do_Not_Lock, BaseTaskInfo* Context = nil, Boolean WithSubRecords = false, 
		...
		virtual VError DelRecord(FicheInMem *rec, BaseTaskInfo* context, uLONG stamp = 0xFFFFFFFF,XBOX::VProgressIndicator* inProgress = NULL) = 0;
		virtual VError DelRecordForTrans(sLONG n, BaseTaskInfo* context, PrimKey* primkey,XBOX::VProgressIndicator* inProgress = NULL) = 0;
		...
		virtual VError DelBlobForTrans(sLONG n, BaseTaskInfo* context, bool alterModifState, PrimKey* primkey, sLONG fieldnum, bool keepBlobOccupiedSpace = false) = 0;
		...
		virtual VJSONObject* BuildLockInfo(VError& err)
		{
			err = VE_OK;
			return nil;
		}
		...
		virtual VError SaveRawRecord(OccupableStack* curstack, sLONG numrec, void* rawdata, sLONG lendata, sLONG nbfields, BaseTaskInfo* context, uLONG8 timestamp)
		{
			return ThrowError(VE_DB4D_TABLEISLOCKED, noaction);
		}
...
#if AllowSyncOnRecID
		virtual VError GetModificationsSinceStamp(sLONG stamp, VStream& outStream, uLONG& outLastStamp, sLONG& outNbRows, BaseTaskInfo* context, vector<sLONG>& cols)
		{
			return ThrowError(VE_DB4D_TABLEISLOCKED, noaction);
		}
		...
		*/
		...
		virtual VError GetModificationsSinceStamp(sLONG8 stamp, VStream& outStream, uLONG8& outLastStamp, sLONG& outNbRows, BaseTaskInfo* context, vector<sLONG>& cols,
		...
		virtual void MarkOutsideBlobToDelete(const VString& inID, const VString& inPath, BaseTaskInfo* context)
		{
		}
		...
		virtual bool CheckPrimaryKeyValid(BaseTaskInfo* context, VProgressIndicator* progress, VError& outErr, bool forceCheck = false)
		{
			outErr = VE_OK;
			return true;
		}

		virtual void SetPrimKeyState(uBYTE primkeystate)
		{
		}


		virtual uBYTE LogInfoValidityState()
		{
			return false;
		}

		void MeasureSavedRecord(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fDiskWriteBytes.AddMeasure(nbbytes);
			fRecordsDiskWriteBytes.AddMeasure(nbbytes);
			fDiskWriteCount.AddMeasure(1);
			fRecordsDiskWriteCount.AddMeasure(1);
		}

		void MeasureSavedBlob(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fDiskWriteBytes.AddMeasure(nbbytes);
			fBlobsDiskWriteBytes.AddMeasure(nbbytes);
			fDiskWriteCount.AddMeasure(1);
			fBlobsDiskWriteCount.AddMeasure(1);
		}

		void MeasureSavedOutsideBlob(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fOutsideBlobsDiskWriteBytes.AddMeasure(nbbytes);
			fDiskWriteCount.AddMeasure(1);
			fBlobsDiskWriteCount.AddMeasure(1);
		}

		void MeasureLoadedRecord(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fDiskReadBytes.AddMeasure(nbbytes);
			fRecordsDiskReadBytes.AddMeasure(nbbytes);
			fDiskReadCount.AddMeasure(1);
			fRecordsDiskReadCount.AddMeasure(1);
		}

		void MeasureLoadedBlob(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fDiskReadBytes.AddMeasure(nbbytes);
			fBlobsDiskReadBytes.AddMeasure(nbbytes);
			fDiskReadCount.AddMeasure(1);
			fBlobsDiskReadCount.AddMeasure(1);
		}

		void MeasureLoadedOutsideBlob(sLONG8 nbbytes)
		{
			VTaskLock lock(&fMeasureMutex);
			fOutsideBlobsDiskReadBytes.AddMeasure(nbbytes);
			fDiskReadCount.AddMeasure(1);
			fBlobsDiskReadCount.AddMeasure(1);
		}

		void MeasureQuery(const VString& refstr, sLONG nb, sLONG8 time);
	...
	protected:
		...
		MeasureCollection fRecordsDiskReadBytes;
		MeasureCollection fRecordsDiskReadCount;
		MeasureCollection fRecordsDiskWriteBytes;
		MeasureCollection fRecordsDiskWriteCount;

		MeasureCollection fBlobsDiskReadBytes;
		MeasureCollection fBlobsDiskReadCount;
		MeasureCollection fBlobsDiskWriteBytes;
		MeasureCollection fBlobsDiskWriteCount;

		MeasureCollection fOutsideBlobsDiskReadBytes;
		MeasureCollection fOutsideBlobsDiskWriteBytes;
		...
		MeasureCollection fRecordCacheMissBytes;
		MeasureCollection fRecordCacheMissCount;

		MeasureCollection fBlobsCacheHitBytes;
		MeasureCollection fBlobsCacheHitCount;
		MeasureCollection fBlobsCacheMissBytes;
		MeasureCollection fBlobsCacheMissCount;

		QueryMeasureMap fQueryMeasures;
...
{ 
	...
	public:
		...
		virtual VError SaveRecord(FicheInMem *fic, BaseTaskInfo* context, uLONG stamp = 0, bool allowOverrideStamp = false,XBOX::VProgressIndicator* inProgress = NULL);
		virtual VError SaveRecordInTrans(FicheOnDisk *ficD, BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress = NULL);
		virtual FicheInMem* LoadRecord(sLONG n, VError& err, DB4D_Way_of_Locking HowToLock = DB4D_Do_Not_Lock, BaseTaskInfo* Context = nil, Boolean WithSubRecords = false, 
										Boolean BitLockOnly = false, ReadAheadBuffer* buffer = nil, Boolean* outEnoughMem = nil);
		virtual FicheOnDisk* LoadNotFullRecord(sLONG n, VError& err, DB4D_Way_of_Locking HowToLock = DB4D_Do_Not_Lock, BaseTaskInfo* Context = nil, Boolean BitLockOnly = false, 
												ReadAheadBuffer* buffer = nil, Boolean* CouldLock = nil, Boolean* outEnoughMem = nil);
		...
		virtual VError DelRecord(FicheInMem *rec, BaseTaskInfo* context, uLONG stamp = 0xFFFFFFFF,XBOX::VProgressIndicator* inProgress = NULL);
		virtual VError DelRecordForTrans(sLONG n, BaseTaskInfo* context, PrimKey* primkey,XBOX::VProgressIndicator* inProgress = NULL);
		virtual VError QuickDelRecord(sLONG n, BaseTaskInfo* context);

		VError WriteDeleteToLog(BaseTaskInfo* context, sLONG n, PrimKey* primkey);
		VError WriteDeleteBlobToLog(BaseTaskInfo* context, sLONG n, PrimKey* primkey, sLONG fieldnum);
		VError WriteDeleteBlobToLog(BaseTaskInfo* context, const VString& path);
		...
		virtual VError DelBlobForTrans(sLONG n, BaseTaskInfo* context, bool alterModifState, PrimKey* primkey, sLONG fieldnum, bool keepBlobOccupiedSpace = false);
		...
		*/
		...
		void UpdatingAPrimKey()
		{
			VInterlocked::AtomicAdd(&fNbPrimKeyModif, 1);
		}

		sLONG GetNbPrimKeyUpdates() const
		{
			return fNbPrimKeyModif;
		}
...
{ 
	public:
		...
		virtual VError SaveRecord(FicheInMem *fic, BaseTaskInfo* context, uLONG stamp = 0, bool allowOverrideStamp = false,XBOX::VProgressIndicator* inProgress = NULL) { return ThrowError(VE_DB4D_TABLEISLOCKED, noaction); };
		virtual VError SaveRecordInTrans(FicheOnDisk *ficD, BaseTaskInfo* context,XBOX::VProgressIndicator* inProgress = NULL) { return ThrowError(VE_DB4D_TABLEISLOCKED, noaction); };
		...
		virtual VError DelRecord(FicheInMem *rec, BaseTaskInfo* context, uLONG stamp = 0xFFFFFFFF,XBOX::VProgressIndicator* inProgress = NULL) { return ThrowError(VE_DB4D_TABLEISLOCKED, noaction); };
		virtual VError DelRecordForTrans(sLONG n, BaseTaskInfo* context, PrimKey* primkey,XBOX::VProgressIndicator* inProgress = NULL) { return ThrowError(VE_DB4D_TABLEISLOCKED, noaction); };
		...
		virtual VError DelBlobForTrans(sLONG n, BaseTaskInfo* context, bool alterModifState, PrimKey* primkey, sLONG fieldnum, bool keepBlobOccupiedSpace = false) { return ThrowError(VE_DB4D_TABLEISLOCKED, noaction); };
...
class IReplicationOutputFormatter : public VObject
{
	public:
		...
		virtual VError PutTimeStamp ( VValueSingle& inStamp, bool inIsLastAttribute ) = 0;
...
class ReplicationOutputBinaryFormatter : public IReplicationOutputFormatter
{
	public:
		...
		virtual ~ReplicationOutputBinaryFormatter ( );
		...
		virtual VError PutTimeStamp ( VValueSingle& inStamp, bool inIsLastAttribute ) { return inStamp. WriteToStream ( &fStream ); }
...
class ReplicationOutputJSONFormatter : public IReplicationOutputFormatter
{
	public:
		...
		virtual ~ReplicationOutputJSONFormatter ( );
		...
		virtual VError PutTimeStamp ( VValueSingle& inStamp, bool inIsLastAttribute );
...
class IReplicationInputFormatter : public VObject
{
	public:
...
class ReplicationInputBinaryFormatter : public IReplicationInputFormatter
{
	public:
		...
		virtual ~ReplicationInputBinaryFormatter ( );
...
class ReplicationInputJSONFormatter : public IReplicationInputFormatter
{
	public:
		...
		virtual ~ReplicationInputJSONFormatter ( );
