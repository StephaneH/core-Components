								...
								if (sourceRec != nil)
								{
									...
									for (sLONG j = 1; j <= nbfields && err == VE_OK; j++)
									{
										...
										if (cri != nil)
										{
											...
											bool relative = true;
											VString path;
											if (typ == VK_TEXT || typ == VK_IMAGE || typ == VK_BLOB || typ == VK_BLOB_DB4D || typ == VK_BLOB_OBJ)
											{
												okcopy = false;
												...
												if (p != nil && *p != -1)
												{
													...
													else if (typ == VK_BLOB_OBJ)
													{
														VBlobObj* blobhandle = dynamic_cast<VBlobObj*>(cv);
														xbox_assert(blobhandle != nil);
														bloblen = (sLONG)blobhandle->GetSize();
														if (blobhandle->IsOutsidePath())
														{
															blobhandle->GetTrueOutsidePath(path, &relative);
