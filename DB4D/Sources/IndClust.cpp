...
ClusterSel::ClusterSel(IndexInfo* ind)
{
...
VError ClusterSel::AddSel(OccupableStack* curstack, Selection* sel, BaseTaskInfo* context)
{
	...
	if (err == VE_OK)
	{
		...
		{
			//sel->ForbidWrite();
			err = sel->TransformIntoCluster(ou, fInd);
			if (err == VE_OK)
			{
	...
	else
	{
		if (nclust<0)
		{
			...
			if (sel!=nil)
			{
				...
				{
					sLONG xdebug = 1; // break here #index
					sel->WriteUnLock();
				}
				sel->Free(curstack, true);
			}
...
IndexHeaderBTreeCluster::IndexHeaderBTreeCluster(IndexInfo *xentete) : IndexHeaderBTree(xentete), cls(xentete)
{ 
...
sLONG IndexHeaderBTreeObjVals::GetLen(void)
{
	return(IndexHeaderBTreeCluster::GetLen() + sizeof(sLONG));
}

VError IndexHeaderBTreeObjVals::PutInto(VStream& buf)
{
	VError err;

	err = IndexHeaderBTreeCluster::PutInto(buf);
	if (err==VE_OK) 
		err=buf.PutLong(fNextPathID);

	return(err);
}


VError IndexHeaderBTreeObjVals::GetFrom(VStream& buf)
{
	VError err;

	err = IndexHeaderBTreeCluster::GetFrom(buf);
	if (err == VE_OK)
		err = buf.GetLong(fNextPathID);
...
{
	return new IndexHeaderBTreeObjVals(xentete);
}
