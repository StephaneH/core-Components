...
VError IndexHeaderBTree::ChargeFirstPage(OccupableStack* curstack, BaseTaskInfo* context, uBOOL doiscreer)
{
	...
	if (firstpage != nil)
	{
		if (hitcache)
		{
			entete->GetHeader()->HitCache(1, firstpage->GetSize());
			entete->GetDB()->HitCache(1, firstpage->GetSize());
		}
		else
		{
			entete->GetHeader()->MissCache(1, firstpage->GetSize());
			entete->GetDB()->MissCache(1, firstpage->GetSize());
		}
	}
...
Bittab* IndexHeaderBTree::ParseAll(OccupableStack* curstack, BTitemIndex* val1, BaseTaskInfo* context, VError& err, const VCompareOptions& inOptions, VDB4DProgressIndicator* InProgress)
{
	...
	if (b!=nil)
	{
		sLONG nb = entete->GetMaxNB(context);
		err = b->aggrandit(nb);
		err = ChargeFirstPage(curstack, context, false);
		if (firstpage!=nil && err == VE_OK)
		{
	...
	if (b!=nil)
	{
		...
		if (err==VE_OK)
		{
			...
			if (firstpage!=nil && err == VE_OK)
			{
				...
				if (entete->IsUniqueKey() && val1 == val2 && !inOptions.IsBeginsWith())
				{
					if (!inOptions.IsLike())
						findOnlyOne = true;
					else
					{
						if (val1 != nil)
						{
							if (entete->GetReducedType() == DB4D_Index_OnOneField)
							{
								sLONG datakind = ((IndexInfoFromField*)entete)->GetDataKind();
...
Boolean IndexBuilderTask::DoRun()
{
	...
	if ( GetState() < TS_DYING ) 
	{
		...
		{
			VIndexInfoProvider	indexInfoCollector;
			...
			if (progress != nil)
			{
				indexInfoCollector.SetIndexInfo(aa.Ind);
				progress->SetDefaultInfoCollector(&indexInfoCollector);
				curaction++;
			...
			if (progress != nil)
			{
				progress->SetDefaultInfoCollector(nil);
			}
