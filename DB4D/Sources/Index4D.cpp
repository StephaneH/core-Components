...
void debug_addpage(BTreePageIndex* p)
{
	...
	xbox_assert(debug_nbpage<30000);
...
void debug_checkpages()
{
	...
	for (i = 0; i < debug_nbpage; i++)
	{
		...
		xbox_assert(sousBT->GetParent()->FindPosOfChild(sousBT) == sousBT->GetPosInParent());
	}
}
...
void IndexHeader::MeasureRead(sLONG nbread, VSize howManyBytes)
{
	VTaskLock lock(&fMeasureMutex);
	fDiskReadBytes.AddMeasure(howManyBytes);
	fDiskReadCount.AddMeasure(nbread);
}

void IndexHeader::MeasureWrite(sLONG nbwrite, VSize howManyBytes)
{
	VTaskLock lock(&fMeasureMutex);
	fDiskWriteBytes.AddMeasure(howManyBytes);
	fDiskWriteCount.AddMeasure(nbwrite);
}

void IndexHeader::HitCache(sLONG nbhit, VSize howManyBytes)
{
	VTaskLock lock(&fMeasureMutex);
	fCacheHitBytes.AddMeasure(howManyBytes);
	fCacheHitCount.AddMeasure(nbhit);
}

void IndexHeader::MissCache(sLONG nbmiss, VSize howManyBytes)
{
	VTaskLock lock(&fMeasureMutex);
	fCacheMissBytes.AddMeasure(howManyBytes);
	fCacheMissCount.AddMeasure(nbmiss);
}


void IndexHeader::MeasureInsertKey(sLONG nb)
{
	VTaskLock lock(&fMeasureMutex);
	fInsertKeyCount.AddMeasure(nb);
}

void IndexHeader::MeasureDeleteKey(sLONG nb)
{
	VTaskLock lock(&fMeasureMutex);
	fDeleteKeyCount.AddMeasure(nb);
}

void IndexHeader::MeasureFindKey(sLONG nb)
{
	VTaskLock lock(&fMeasureMutex);
	fFindKeyCount.AddMeasure(nb);
}

void IndexHeader::MeasureSort(sLONG nb)
{
	VTaskLock lock(&fMeasureMutex);
...
{
	VError err = taddr.LibereEspaceDisk(curstack, InProgress);
	if (err != VE_OK)
		...
		if (IHD.AddrNULLs <=0 || (adjuste(len+kSizeDataBaseObjectHeader) != adjuste(oldlen+kSizeDataBaseObjectHeader)))
		{
			...
			if (ou>0 && err == VE_OK)
			{
				if (sel->IsCluster())
					...
					sel->TransformIntoCluster(ou, assoc);
				sel->ChangeAddr(ou, bd, context);
		...
		if (IHD.AddrNULLs > 0)
		{
			...
			if (err==VE_OK)
			{
				...
				if (testAssert(typsel==sel_bitsel))
				{
					...
					{
						fNulls->SetSelAddr(IHD.AddrNULLs, assoc);
...
void IndexHeader::SetMeasureInterval(uLONG interval)
{
...
VJSONObject* IndexHeader::RetainMeasures(VJSONObject* options, JSONPath* path)
{
	VTaskLock lock(&fMeasureMutex);
	...
	if (xpath.AcceptJsonPathPart(MeasureCollection::kQueryCount))
		jsonProp(&result, MeasureCollection::kQueryCount, fFindKeyCount.RetainMeasures(options));
	...
	if (xpath.AcceptJsonPathPart(MeasureCollection::kInsertKeyCount))
		jsonProp(&result, MeasureCollection::kInsertKeyCount, fInsertKeyCount.RetainMeasures(options));

	if (xpath.AcceptJsonPathPart(MeasureCollection::kDeleteKeyCount))
		jsonProp(&result, MeasureCollection::kDeleteKeyCount, fDeleteKeyCount.RetainMeasures(options));
...
VError IndexHeaderBTree::CalculateColumnFormulas(OccupableStack* curstack, ColumnFormulas* formules, BaseTaskInfo* context, Bittab* filtre, VDB4DProgressIndicator* InProgress)
{
	...
	if (curtrans != nil && curtrans->ConcernIndex(GetAssoc()))
	{
		XBOX::VString session_title;
		gs(1005,27,session_title);
		IndexKeyIterator iter(GetAssoc(), filtre, context, InProgress, session_title, curstack);

		while (iter.NextKey(context, err))
		{
			err = formules->ExecuteOnOneBtreeKey(GetAssoc(), iter.GetKey(), stop, context);
			if (stop || (err != VE_OK))
				break;
		}
	}
...
VError IndexHeaderBTree::CalculateMin(OccupableStack* curstack, BaseTaskInfo* context, Bittab* filtre, VDB4DProgressIndicator* InProgress, VValueSingle* &result)
{
	...
	if (curtrans != nil && curtrans->ConcernIndex(GetAssoc()))
	{
		...
		{
			result = GetAssoc()->CreateVValueWithKey(iter.GetKey(), err);
		}
	}
	else 
	{ 
										   ...
										   VDB4DProgressIndicator* InProgress, VCompareOptions& inOptions)
{
	...
	if (curtrans != nil && curtrans->ConcernIndex(GetAssoc()))
	{
		...
		while (iter.NextKey(context, err))
		{
			...
			if (curkey != nil)
			{
				if (GetAssoc()->CompareKeys(iter.GetKey(), curkey, inOptions) != CR_EQUAL)
				{
					GetAssoc()->FreeKey(curkey, prealloc);
					curkey = GetAssoc()->CopyKey(iter.GetKey(), prealloc);
					add = true;
				}
			}
			else
			...
			if (add)
			{
				ValPtr cv = GetAssoc()->CreateVValueWithKey(curkey, err);
				if (err == VE_OK)
				{
									 ...
									 VDB4DProgressIndicator* InProgress, Boolean TestUnicite, Bittab* inWafSelbits, WafSelection* outWafSel)
{
	...
	if (bb!=nil)
	{
		...
		VCompareOptions options = GetAssoc()->GetStrictOptions();
		...
		if (sel2 != nil)
		{
			...
			if (trans != nil && trans->ConcernIndex(GetAssoc()))
			{
				XBOX::VString session_title;
				gs(1005,8,session_title);	// Indexed Sort: %curValue of %maxValue Keys
				IndexKeyIterator iter(GetAssoc(), bb, context, InProgress, session_title, curstack);
				BTitemIndex* curkey = nil;
				...
				while (iter.NextKey(context, err, &curfic))
				{
					if (TestUnicite)
					{
						if (curkey != nil)
						{
							if (GetAssoc()->CompareKeys(curkey, iter.GetKey(), options) == CR_EQUAL)
							{
								err = VE_DB4D_DUPLICATED_KEY;
								break;
							}
							GetAssoc()->FreeKey(curkey);
							curkey = nil;
						}
						curkey = GetAssoc()->CopyKey(iter.GetKey());
						if (curkey == nil)
						{
							err = ThrowError(memfull, DBaction_SortingOnIndex);
							break;
...
VError IndexHeaderBTree::ChargeFirstPage(OccupableStack* curstack, BaseTaskInfo* context, uBOOL doiscreer)
{
	...
	if (firstpage==nil)
	{
		...
		if ((IHD.nbpage==0) || (HBT.FirstPage==-1)) 
		{
			if (doiscreer)
			{
				firstpage = BTreePageIndex::AllocatePage(curstack, GetAssoc(), context, err);
				if (firstpage != nil)
				{
					HBT.FirstPage=firstpage->GetNum();
					//setmodif(true);
					taddr.setmodif(true, GetAssoc()->GetDB(), context);
					// GetAssoc()->ModifyIt();
				}
			}
		}
		else
		{
			...
			if (firstpage == nil)
				firstpage = BTreePageIndex::LoadPage(curstack, HBT.FirstPage, GetAssoc(), context, err);
			else
			{
	...
	if (firstpage != nil)
	{
		if (hitcache)
		{
			GetAssoc()->GetHeader()->HitCache(1, firstpage->GetSize());
			GetAssoc()->GetDB()->HitCache(1, firstpage->GetSize());
		}
		else
		{
			GetAssoc()->GetHeader()->MissCache(1, firstpage->GetSize());
			GetAssoc()->GetDB()->MissCache(1, firstpage->GetSize());
		}
	}
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			else
			{
				if (h)
				{
					pag = BTreePageIndex::AllocatePage(curstack, GetAssoc(), context, err);
					if (pag != nil)
					{
		...
		if (firstpage!=nil)
		{
...
#if debugIndexPage
			{
				...
				for (; cur != end; cur++)
				{
					...
					if (p != firstpage)
					{
						xbox_assert(p->GetNKeys() >= HalfPage);
						//xbox_assert(p->GetCountOccupe() == 0);
					}
					/*
					else
						xbox_assert(p->GetCountOccupe() == 1);
						*/
				}
			}
#endif
...
Bittab* IndexHeaderBTree::ParseAll(OccupableStack* curstack, BTitemIndex* val1, BaseTaskInfo* context, VError& err, const VCompareOptions& inOptions, VDB4DProgressIndicator* InProgress)
{
	...
	if (b!=nil)
	{
		sLONG nb = GetAssoc()->GetMaxNB(context);
		err = b->aggrandit(nb);
		err = ChargeFirstPage(curstack, context, false);
		if (firstpage!=nil && err == VE_OK)
		{
									...
									VDB4DProgressIndicator* InProgress, const VCompareOptions& inOptions, Bittab* dejasel, BTitemIndex** outVal)
{
	...
	if (b!=nil)
	{
		...
		if (err==VE_OK)
		{
			...
			if (firstpage!=nil && err == VE_OK)
			{
				...
				if (GetAssoc()->IsUniqueKey() && val1 == val2 && !inOptions.IsBeginsWith())
				{
					if (!inOptions.IsLike())
						findOnlyOne = true;
					else
					{
						if (val1 != nil)
						{
							if (GetAssoc()->GetReducedType() == DB4D_Index_OnOneField)
							{
								sLONG datakind = ((IndexInfoFromField*)GetAssoc())->GetDataKind();
								if (datakind == VK_TEXT || datakind == VK_STRING)
								{
									sLONG* p1 = (sLONG*)&(val1->data);
									sLONG len = *p1;
									UniChar* pchar = (UniChar*)(p1+1);
									if (std::find( pchar, pchar+len, GetWildChar(context)) == pchar+len)
										findOnlyOne = true;
								}
							}
						}
				...
				err = firstpage->Fourche(curstack, b, val1, xstrict1, val2, xstrict2, context, &progress, inOptions, outVal, findOnlyOne);
				firstpage->Free(curstack, true);
			}
#if 0 && VERSIONDEBUG
			// ##special test##
	...
	if (checktwice && err == 0 && b != nil && outVal == nil)
	{
		...
		err = -3;
		Bittab* b2 = Fourche(curstack, val1, xstrict1, val2, xstrict2, context, err, InProgress, inOptions, dejasel, outVal);
...
sLONG IndexHeaderBTree::FindKey(OccupableStack* curstack, BTitemIndex* val, BaseTaskInfo* context, const VCompareOptions& inOptions, Bittab* dejasel, BTitemIndex** outVal)
{
	...
	{
		trans->RemoveDeleteKeysFromFourche(GetAssoc(), val, false, val, false, inOptions, &dejasel2);
	}
	...
	if (err == VE_OK)
	{
		if (trans != nil)
		{
			...
			xbox_assert(b != nil);
			err = trans->Fourche(GetAssoc(), val, false, val, false, inOptions, dejasel, b, xtransval);
...
void IndexHeaderBTree::dumpToLog(BaseTaskInfo* context)
{
	...
	if (firstpage != nil && err == VE_OK)
	{
		firstpage->dumpToLog(context, curstack);
		firstpage->Free(curstack, true);
	}
	Free(curstack, true);
}
	...
	if (v != nil)
	{
		...
		if (p != -1 && chargesous)
		{
			...
			if (sous != nil)
			{
				if (hitcache)
				{
					entete->GetHeader()->HitCache(1, sous->GetSize());
					entete->GetDB()->HitCache(1, sous->GetSize());
				}
				else
				{
					entete->GetHeader()->MissCache(1, sous->GetSize());
					entete->GetDB()->MissCache(1, sous->GetSize());
				}
			}
		}
		else
...
void BTreePageIndex::Reorganize()
{
	...
	{
		xbox_assert(btp->CurrentSizeInMem > btp->TrueDataSize);
		char* dest = (char*) GetFastMem(fDataSize, false, 'btp ');
		xbox_assert(dest != nil);
		if (dest != nil)
		{
			char* p2 = dest;
...
VError BTreePageIndex::AddKey(void* p, sLONG qui, BTreePageIndex* sousBT)
{
	...
	if (fit)
	{
		sLONG nb = btp->nkeys;
		sLONG n = nb;
		xbox_assert(nb < kNbKeyParPageIndex);
		btp->nkeys = nb + 1;
...
VError BTreePageIndex::MoveAndAddItems(sLONG from, sLONG howmany, BTreePageIndex* into)
{
	...
	xbox_assert(into->btp->nkeys+howmany <= kNbKeyParPageIndex);
...
VError BTreePageIndex::loadobj(DataAddr4D xaddr, sLONG len)
{
	...
	xbox_assert(db->IsAddrValid(getaddr()));
	
	oldlen=len-kSizeDataBaseObjectHeader;
	
	xbox_assert(oldlen>0 && oldlen<=1000000);
	...
	entete->GetHeader()->MeasureRead(1, oldlen+kSizeDataBaseObjectHeader);
	...
	if (err == VE_OK)
	{
		xbox_assert(btp->nkeys>0 && btp->nkeys<=kNbKeyParPageIndex);
		xbox_assert(btp->CurrentSizeInMem <= oldlen);
...
VError BTreePageIndex::savepage(OccupableStack* curstack, BaseTaskInfo* context)
{
	...
	for (sLONG i = 0; i < nb; i++)
	{
		...
		xbox_assert(rescomp != CR_SMALLER);
	}
#endif
	...
	*/
	{
		...
		entete->GetHeader()->MeasureWrite(1, tot+kSizeDataBaseObjectHeader);
...
#if debug
	xbox_assert(n>=0 && n < ((IndexHeaderBTree*)(xentete->GetHeader()))->GetNbPage() );
#endif
...
void BTreePageIndex::LibereEspaceMem(OccupableStack* curstack)
{
	...
	xbox_assert(nb>=0 && nb<= kNbKeyParPageIndex);
...
bool gTempNewIndexFourche = true;

VError BTreePageIndex::SelectAllKeys(OccupableStack* curstack, Bittab* b, BaseTaskInfo* context, ProgressEncapsuleur* InProgress, BTitemIndex** outVal)
{
	VError err = VE_OK;
	VTask::Yield();
	if (!InProgress->Increment())
	...
	if (err == VE_OK)
	{
		...
		{
			BTreePageIndex* sousBT=GetSousPage(curstack, nb, err, context);
			if (sousBT!=nil)
			{
				err = sousBT->SelectAllKeys(curstack, b, context, InProgress, outVal);
				sousBT->Free(curstack, true);
...
VError BTreePageIndex::Fourche(OccupableStack* curstack, Bittab* b, BTitemIndex* val1, uBOOL xstrict1, BTitemIndex* val2, uBOOL xstrict2, 
								BaseTaskInfo* context, ProgressEncapsuleur* InProgress, const VCompareOptions& inOptions, BTitemIndex** outVal, bool findOnlyOne)
{
	BTreePageIndex *sousBT;
			...
			if (findOnlyOne)
			{
				...
				while (cont)
				{
					if ((right-left) <= 1)
					{
						bool found = false;
						cont = false;
						rescomp = CompareKeys(left, val1, inOptions);
						if (rescomp == CR_EQUAL)
						{
							found = true;
						}
						...
						if (found)
						{
							if (IsCluster)
							{
								entete->GetClusterSel(curstack)->AddSelToBittab(curstack, GetQui(left), b, context, err);
							}
							else
							{
								err = b->Set(GetQui(left), false);
							}
							if (outVal != nil)
							{
								canCont = false;
						...
						{
							sousBT=GetSousPage(curstack, left, err, context);
							if (sousBT!=nil)
							{
								err = sousBT->Fourche(curstack, b, val1, xstrict1, val2, xstrict2, context, InProgress, inOptions, outVal, findOnlyOne);
								sousBT->Free(curstack, true);
							}
						}
					}
					...
					{
						sLONG middle = left + ((right - left + 1) / 2);
						rescomp = CompareKeys( middle, val1, inOptions);
...
void BTreePageIndex::checkWrongRecordRef(sLONG limit, BTitemIndex* &curval)
{
	...
	for (sLONG i = 0; i < nb; i++)
	{
		...
		if (curval != nil)
		{
			...
			if (result == CR_EQUAL)
				xbox_assert(curval->GetQui() < x->qui);
			else
				xbox_assert(result == CR_SMALLER);
		}
		curval = x;
		//xbox_assert(! df->isRecordDeleted(x->qui));
		xbox_assert(x->GetQui() < limit);
	}
...
void BTreePageIndex::checkPosSousBT(BTreePageIndex* sousBT)
{
	...
	if (testAssert(found != -1))
	{
		...
		if (found != 0)
		{
			xbox_assert(CompareKeys(found-1, sousBTMin, options) != CR_BIGGER);
		}
		if (found != nb)
		{
			xbox_assert(CompareKeys(found, sousBTMax, options) != CR_SMALLER);
		}
	}
}
...
void BTreePageIndex::dumpToLog(BaseTaskInfo* context, OccupableStack* curstack)
{
	VError err = VE_OK;
	sLONG nb = btp->nkeys;

	BTreePageIndex* sousBT = GetSousPage(curstack, 0, err, context);
	if (sousBT != nil)
	{
		sousBT->dumpToLog(context, curstack);
		sousBT->Free(curstack, true);
	}
	...
	for (sLONG i = 0; i < nb && err == VE_OK; i++)
	{
		...
		VString s;
		cv->GetString(s);
		entete->GetDB()->WriteDebugLogLn(s+" : "+ToString(l));
...
bool BTTransUniqCleIndex::operator < (const BTTransUniqCleIndex& other) const
{
	bool result;

	VCompareOptions options;
	options.SetIntlManager(fCle->GetInd()->GetDB()->GetIntlMgr());
	options.SetDiacritical(false);

	sLONG comp = fCle->GetInd()->CompareKeys(fCle, other.fCle, options);
...
{
	LockValidity(); 
	xbox_assert(fValidityRequestCount > 0);
	fValidityRequestCount++;
	UnLockValidity(); 
}
...
{
	xbox_assert(RequiredElemsInPage<=MaxElemsInPage);
	sLONG level = 1;
...
VError IndexInfo::GenereFullIndex(VDB4DProgressIndicator* InProgress, sLONG MaxElemsInPage, sLONG RequiredElemsInPage)
{
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			{
				VString iname;
				IdentifyIndex(iname, true, true, false);
				XBOX::VString session_title;
...
bool IndexInfo::PlaceGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context, bool temporary)
{
	bool okAdded = false;
	VError err = VE_OK;
	...
	BTitemIndex* val = BuildKey(rec, err, false);
	val->StartUse();
	val->SetInd(this);
	xbox_assert(val != nil);
	BTTransUniqCleIndex uniqkey(val, trans, temporary);
	mapBTUniqKeys::iterator found = fBTUniqueKeysToBeProcessed.find(uniqkey);
	if (found == fBTUniqueKeysToBeProcessed.end())
	{
		fBTUniqueKeysToBeProcessed.insert(uniqkey);
		if (trans != nil && !temporary)
			trans->AddUniqKeyToRelease(val);
		okAdded = true;
	}
	else
	{
		if (found->fOwner == trans)
		{
			if (found->fIsTemp)
				found->fIsTemp = temporary;
			okAdded = true;
			if (!temporary && trans != nil)
			{
				trans->AddUniqKeyToRelease(val);
			}
		}
...
bool IndexInfo::IsGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	bool isThere = false;
	VError err = VE_OK;
	BTitemIndex* val = BuildKey(rec, err, false);
	...
	val->SetInd(this);
	xbox_assert(val != nil);
	BTTransUniqCleIndex uniqkey(val, nil, true);
	mapBTUniqKeys::iterator found = fBTUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fBTUniqueKeysToBeProcessed.end())
	{
		isThere = true;
...
void IndexInfo::ReleaseGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context, bool tempOnly)
{
	VError err = VE_OK;
	BTitemIndex* val = BuildKey(rec, err, false);
	val->SetInd(this);
	...
	BTTransUniqCleIndex uniqkey(val, trans, false);
	mapBTUniqKeys::iterator found = fBTUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fBTUniqueKeysToBeProcessed.end())
	{
		if (found->fOwner == trans)
		{
			...
			{
				if (found->fIsTemp)
					fBTUniqueKeysToBeProcessed.erase(found);
			}
			else
				fBTUniqueKeysToBeProcessed.erase(found);
		}
...
void IndexInfo::ReleaseGlobalUniqKey(const BTitemIndex* val, Transaction* trans)
{
	BTTransUniqCleIndex uniqkey(val, trans, false);
	mapBTUniqKeys::iterator found = fBTUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fBTUniqueKeysToBeProcessed.end())
...
void IndexInfoFromField::IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName, bool simple) const
{
	if (simple)
	{
		crit->GetName(outString);
	}
	else
	{
...
Boolean IndexInfoFromField::NeedToRebuildIndex(VIntlMgr* NewIntlMgr, bool stringAlgoHasChanged, bool keywordAlgoHasChanged)
{
	Boolean result = false;
	if (crit != nil && stringAlgoHasChanged)
	{
...
bool IndexInfoFromField::IsOnPrimKey()
{
	bool prim = false;
	if (crit != nil)
	{
...
IndexInfo* CreIndexInfoFromFieldObj(void)
{
	IndexInfoFromFieldObj *ind;

	ind = new IndexInfoFromFieldObj;
	return(ind);
}
...
void IndexInfoFromField::GetDebugInfo(VString& outText) const
{
	VString tablename;
	VString fieldname;
	if (fic != nil)
		fic->GetName(tablename);
	if (crit != nil)
		crit->GetName(fieldname);
	outText = "IndexInfoFromField : Table : "+tablename+" , Field : "+fieldname;
}
...
void IndexInfoFromFieldLexico::IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName, bool simple) const
{
	if (simple)
	{
		crit->GetName(outString);
...
{
	xbox_assert(false); // should never be called
	err = VE_DB4D_NOTIMPLEMENTED;
	return nil;
}
...
VSize ObjAttKey::CalculateLength(const void* p)
{
	VSize len = sizeof(sLONG)+sizeof(sWORD); // pathid + value type

	uBYTE* p2 = (uBYTE*)p;
	p2 = p2 + sizeof(sLONG);
	JSONType jtyp = (JSONType)*((sWORD*)p2);
	p2 = p2 + sizeof(sWORD);
	switch (jtyp)
	{
	case xbox::JSON_true:
	case xbox::JSON_false:
		len += sizeof(sWORD);
		break;

	case xbox::JSON_string:
		len += VDBMgr::GetStringRef()->GetValueInfo()->GetSizeOfValueDataPtr(p2);
		break;

	case xbox::JSON_number:
		len += sizeof(Real);
		break;
...
VSize ObjAttKey::CalculateLength(sLONG pathid, const VJSONValue& jval)
{
	sLONG len;
	BuildFromJSONVal(pathid, jval, false, nil, &len);
	return (VSize)len;
}

void* ObjAttKey::PutInto(void* into, sLONG pathid, const VJSONValue& jval)
{
	return BuildFromJSONVal(pathid, jval, false, into, nil);
}




void* ObjAttKey::BuildFromJSONVal(sLONG pathid, const VJSONValue& jval, bool includeIndexKey, void* into, sLONG* lenOnly)
{
	BTitemIndex* res = nil;
	VError err = VE_OK;
	sLONG len = sizeof(sLONG)+sizeof(sWORD); // pathid + value type
	Real rr = 0;
	sLONG8 tt;
	sWORD boolInt = 0;
	VString sValue;
	sWORD jvaltype = (sWORD)jval.GetType();
	switch (jval.GetType())
	{
		case JSON_number:
			{
		...
		case JSON_date:
			{
				...
				VTime dd;
				jval.GetTime(dd);
				tt = dd.GetMilliseconds();
			}
			break;

		case JSON_string:
			{
				jval.GetString(sValue);
				len += sValue.GetSpace(kMaxBytesForKeyStrings);
			}
			break;
		...
		case JSON_false:
			{
				jvaltype = (sWORD)JSON_true;
				if (jval.GetType() == JSON_true)
					boolInt = 1;
				len += sizeof(sWORD);
			}
			break;
		...
		case JSON_null:
			jvaltype = (sWORD)JSON_null;
			break;

		case JSON_undefined:
			jvaltype = (sWORD)JSON_undefined;
			break;

		default:
			jvaltype = (sWORD)JSON_object;
			break;
	}
	...
	else
	{
		...
		if (result == nil)
		{
			...
			{
				VSize len2 = sizeof(BTitemIndex)-sizeof(void*)+len;
				result = GetFastMem(len2, true, 'iKe5');
				res = new (result)BTitemIndex();
				p = (uBYTE*)&res->data;
			}
			else
			{
				result = GetFastMem(len, true, 'oKey');
				p = (uBYTE*)result;
			}
		}
		else
		...
		*((sWORD*)p) = jvaltype;
		...
		case JSON_string:
			sValue.WriteToPtr(p, false, kMaxBytesForKeyStrings);
			break;
...
sLONG ObjAttKey::Compare(const void* px1, const void* px2, const VCompareOptions& inOptions)
{
	const uBYTE* p1 = (uBYTE*)px1;
	const uBYTE* p2 = (uBYTE*)px2;
	sLONG pathid1 = *((sLONG*)p1);
	sLONG pathid2 = *((sLONG*)p2);
	if (pathid1 == pathid2)
	{
		bool jval1inf = false;
		bool jval2inf = false;
		...
		p1 += sizeof(sLONG);
		p2 += sizeof(sLONG);
		sWORD jtyp1 = (JSONType)*((sWORD*)p1);
		sWORD jtyp2 = (JSONType)*((sWORD*)p2);
		if (jtyp1 >= 200)
		{
			jtyp1 = jtyp1 - 200;
			jval1inf = true;
		...
		if (jtyp1 == jtyp2)
		{
			...
			else
			{
				...
				Real r1, r2;
				sLONG8 t1, t2;
				sWORD boolInt1, boolInt2;
				VString sValue1, sValue2;
				switch (jtyp1)
				{
				case xbox::JSON_true:
				case xbox::JSON_false:
				...
				case xbox::JSON_date:
					...
					if (t1 == t2)
						...
						return t1 < t2 ? CR_SMALLER : CR_BIGGER;
					break;
				...
				case xbox::JSON_number:
					...
					if (fabs(r1 - r2) < inOptions.GetEpsilon())
						...
						return r1 < r2 ? CR_SMALLER : CR_BIGGER;
					break;
...
ObjAttKey::ObjAttKey(sLONG pathid, const VJSONValue& jval)
{
	FromJSONVal(pathid, jval);
}


void ObjAttKey::FromJSONVal(sLONG pathid, const VJSONValue& jval)
{
	fData = BuildFromJSONVal(pathid, jval, false, nil, nil);
}
...
IndexInfoFromFieldPath::IndexInfoFromFieldPath(IndexInfoFromFieldObj* parent, Field* inCrit, Table* inTable) :IndexInfoFromFieldLexico()
{
	fParent = parent;
	crit = RetainRefCountable(inCrit);
	fic = RetainRefCountable(inTable);
	bd = parent->GetDB();
	fDataKind = VK_STRING;
}


VError IndexInfoFromFieldPath::Save()
{
	return fParent->Save();
}
...
IndexInfoFromFieldObj::IndexInfoFromFieldObj(Base4D* db, sLONG xnumfile, sLONG xnumfield, sLONG xtypindex) :IndexInfoFromField(db, xnumfile, xnumfield, DB4D_Index_BtreeObjVals, false, false)
{
	InfoTyp = DB4D_Index_OnObj;
	if (crit != nil)
		fDataKind = VK_BLOB_OBJ;
	else
		fDataKind = -1;

	fPathEntete = new IndexInfoFromFieldPath(this, crit, fic);
	fPathHeader = new IndexHeaderBTreeCluster(fPathEntete);
	fPathEntete->SetHeader(fPathHeader);
};


IndexInfoFromFieldObj::~IndexInfoFromFieldObj()
{
	//QuickReleaseRefCountable(fPathHeader);
	QuickReleaseRefCountable(fPathEntete);
}


void IndexInfoFromFieldObj::IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName, bool simple) const
{
	if (simple)
	{
		crit->GetName(outString);
		outString += " (Object Path)";
	}
	else
	{
		gs(1004, 35, outString);	// Object Index 
...
VError IndexInfoFromFieldObj::ValidateIndexInfo()
{
	VError err = VE_OK;
...
VError IndexInfoFromFieldObj::PutInto(VStream& buf, Base4D* xdb, Boolean WithHeader)
{
	VError err;

	err = IndexInfoFromField::PutInto(buf, xdb);

	if (err == VE_OK && WithHeader)
	{
		err = fPathHeader->PutInto(buf);
	}
...
VError IndexInfoFromFieldObj::GetFrom(VStream& buf, Base4D *xdb, Boolean WithHeader, Boolean oldformat)
{
	VError err;

	SetDB(xdb);

	err = IndexInfoFromField::GetFrom(buf, xdb, WithHeader);
	if (err == VE_OK)
	{
		if (WithHeader)
		{
			...
			//fPathHeader = CreateIndexHeader(DB4D_Index_BtreeWithCluster, this, -1);
			if (fPathHeader == nil)
			{
				err = ThrowError(memfull, DBaction_LoadingIndexHeader);
			}
			...
			{
				err = fPathHeader->GetFrom(buf);
				fPathHeader->SetAssoc(fPathEntete);
			}
		}
		else
		{
...
void IndexInfoFromFieldObj::GetDebugString(VString& s, sLONG numinstance)
{
	s.Clear();
	if (crit != nil)
	{
...
sLONG IndexInfoFromFieldObj::CalculateDefaultSizeKey()
{
	return 50;
}


BTitemIndex* IndexInfoFromFieldObj::NextPossibleLimit(BTitemIndex* key)
{
	BTitemIndex* res = nil;

	const uBYTE* p1 = (uBYTE*)&(key->data);
	sLONG pathid = *((sLONG*)p1);
	p1 += sizeof(sLONG);
	sWORD jvaltype = (JSONType)*((sWORD*)p1);
	jvaltype = jvaltype + 100;
...
BTitemIndex* IndexInfoFromFieldObj::PreviousPossibleLimit(BTitemIndex* key)
{
	BTitemIndex* res = nil;
...
BTitemIndex* IndexInfoFromFieldObj::BuildKeyWithJSONVal(sLONG pathid, const VJSONValue& jval)
{
	BTitemIndex* res = (BTitemIndex*)(ObjAttKey::BuildFromJSONVal(pathid, jval, true, nil, nil));
	/*
	VError err = VE_OK;
	sLONG len = sizeof(sLONG)+sizeof(sWORD); // pathid + value type
	Real rr = 0;
...
BTitemIndex* IndexInfoFromFieldObj::BuildKeyFromVValue(const ValPtr cv, VError &err)
{
	xbox_assert(false); // should never be called
	err = VE_DB4D_NOTIMPLEMENTED;
	return nil;
...
ValPtr IndexInfoFromFieldObj::CreateVValueWithKey(const BTitemIndex* key, VError &err)
{
	err = VE_OK;
	VString* cv;

	uBYTE* p2 = (uBYTE*)&(key->data);
	sLONG pathid = *((sLONG*)p2);
	p2 = p2 + sizeof(sLONG);
	JSONType jtyp = (JSONType)*((sWORD*)p2);
	p2 = p2 + sizeof(sWORD);
	...
	case xbox::JSON_date:
		styp = "date";
		break;
	case xbox::JSON_object:
		styp = "object";
		break;
	case xbox::JSON_array:
		styp = "array";
		break;
	case xbox::JSON_undefined:
		styp = "undefined";
		break;
	default:
		styp = "other";
	...
	styp += "(" + ToString((sLONG)jtyp) + ")";

	cv = new VString(ToString(pathid));
	(*cv) += " , " + styp;
	VString suite;
	...
	case xbox::JSON_string:
		suite.LoadFromPtr(p2, false);
		break;

	case xbox::JSON_number:
		suite.FromReal(*(Real*)p2);
		break;

	case xbox::JSON_date:
	{
		VTime dd;
		dd.FromMilliseconds(*(sLONG8*)p2);
		dd.GetString(suite);
		break;
	}
...
void IndexInfoFromFieldObj::SwapByteKeyData(BTitemIndex* key)
{
	uBYTE* p1 = (uBYTE*)&(key->data);
	ByteSwap((sLONG*)p1);
	p1 += sizeof(sLONG);
	ByteSwap((sWORD*)p1);
	JSONType jtyp = (JSONType)*((sWORD*)p1);
	p1 += sizeof(sWORD);
	...
	case xbox::JSON_string:
		VString::sInfo.ByteSwapPtr(p1, false);
		break;
...
BTitemIndex* IndexInfoFromFieldObj::BuildKey(FicheInMem *rec, VError &err, Boolean OldOne)
{
	xbox_assert(false); // should never be called
	err = VE_DB4D_NOTIMPLEMENTED;
	return nil;
...
VSize IndexInfoFromFieldObj::CalculatePureKeyLength(const void* p)
{
	return ObjAttKey::CalculateLength(p);
}


BTitemIndex* IndexInfoFromFieldObj::CreateIndexKey(const void* p)
{
	BTitemIndex* val;

	VSize len = CalculatePureKeyLength(p);
	VSize len2 = sizeof(BTitemIndex)-sizeof(void*)+len;
	void* p2 = GetFastMem(len2, true, 'iKe6');
	if (p2 != nil)
	{
...
sLONG IndexInfoFromFieldObj::CalulateDataPtrKeyLength(void* data)
{
	VSize res = sizeof(BTitemIndex)-sizeof(BTreePageIndex*)-sizeof(void*);

	res += CalculatePureKeyLength(data);

	return (sLONG)res;
}


sLONG IndexInfoFromFieldObj::CalulateKeyLength(const BTitemIndex* u)
{
	VSize res = sizeof(BTitemIndex)-sizeof(BTreePageIndex*)-sizeof(void*);
	res += CalculatePureKeyLength(&(u->data));
	
	return (sLONG)res;
}


sLONG IndexInfoFromFieldObj::CalulateFullKeyLengthInMem(const BTitemIndex* u)
{
	return CalulateKeyLength(u) + sizeof(BTreePageIndex*);
}


sLONG IndexInfoFromFieldObj::CompareKeys(const BTitemIndex *val1, const BTitemIndex *val2, const VCompareOptions& inOptions)
{
	if (val1 == nil)
	{
		if (val2 == nil)
	...
	else
	{
		...
		{
			return ObjAttKey::Compare(&(val1->data), &(val2->data), inOptions);
			/*
			const uBYTE* p1 = (uBYTE*)&(val1->data);
			const uBYTE* p2 = (uBYTE*)&(val2->data);
			sLONG pathid1 = *((sLONG*)p1);
			sLONG pathid2 = *((sLONG*)p2);
			if (pathid1 == pathid2)
			{
...
sLONG IndexInfoFromFieldObj::FindPathID(OccupableStack* curstack, const VString& path, BaseTaskInfo* context, const VCompareOptions& options)
{
	VError err = VE_OK;
	BTitemIndex* pathval = fPathEntete->BuildKeyFromVValue((ValPtr)&path, err);
	sLONG pathid = fPathHeader->FindKey(curstack, pathval, context, options, nil, nil);
	if (pathid == -1)
	{
		IndexKeyArray tempkey(fPathEntete);
		tempkey.AddKey(pathval);
		pathid = ((IndexHeaderBTreeObjVals*)header)->GetNextPathID();
		err = fPathHeader->PlaceCle(curstack, pathval, pathid, tempkey, context);
		header->setmodif(true, bd, context);
	}
	else
	{
		fPathEntete->FreeKey(pathval);
	}
	return pathid;
}


VError IndexInfoFromFieldObj::PlaceCle(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	VError err = VE_OK;
	if (fIsRemote)
		err = ThrowError(VE_DB4D_INTERNAL_ERROR_ILLEGAL_ON_REMOTE, noaction);
	else
	{
		...
		{
			ValueKind vkind = xf->GetValueKind();
			if (vkind == VK_BLOB_OBJ)
			{
				VBlobObj* cvobj = dynamic_cast<VBlobObj*>(xf);
				if (testAssert(cvobj != nil))
				{
					ObjectNodeCollection paths;
					VCompareOptions options;
					options.SetIntlManager(GetContextIntl(context));
					options.SetDiacritical(true);

					VJSONValue jval;
					cvobj->GetJSONValue(jval, true);
					ObjectParser parser(jval, paths);
					
					ReducedObjectNodeCollection reducedPaths;
					reducedPaths.reserve(paths.size());

					bool needUpdatePathHeader = false;
					for (ObjectNodeCollection::iterator cur = paths.begin(), end = paths.end(); cur != end && err == VE_OK; ++cur)
					{
						BTitemIndex* pathval = fPathEntete->BuildKeyFromVValue(cur->GetPath(), err);
						sLONG pathid = fPathHeader->FindKey(curstack, pathval, context, options, nil, nil);
						if (pathid == -1)
						{
							needUpdatePathHeader = true;
							IndexKeyArray tempkey(fPathEntete);
							tempkey.AddKey(pathval);
							pathid = ((IndexHeaderBTreeObjVals*)header)->GetNextPathID();
							err = fPathHeader->PlaceCle(curstack, pathval, pathid, tempkey, context);
							if (err == VE_OK)
							{
								reducedPaths.push_back(ReducedObjectNode(cur->GetValue(), pathid));
							}
						}
						else
						{
					...
					if (err == VE_OK)
					{
						for (ReducedObjectNodeCollection::iterator cur = reducedPaths.begin(), end = reducedPaths.end(); cur != end && err == VE_OK; ++cur)
						{
							BTitemIndex* objval = BuildKeyWithJSONVal(cur->GetPathID(), cur->GetValue());
							Transaction* trans = GetCurrentTransaction(context);
							IndexKeyArray tempkey(this);
							if (trans != nil)
							{
								err = trans->PlaceCle(this, objval, rec->GetAssoc()->getnumfic());
							}
							else
							{
								tempkey.AddKey(objval);
								err = header->PlaceCle(curstack, objval, rec->GetAssoc()->getnumfic(), tempkey, context);
							}
						}

					}

					if (needUpdatePathHeader)
						header->setmodif(true, bd, context);
				}
				else
					err = VE_DB4D_WRONGTYPE;
...
VError IndexInfoFromFieldObj::DetruireCle(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	VError err = VE_OK;
	if (fIsRemote)
		err = ThrowError(VE_DB4D_INTERNAL_ERROR_ILLEGAL_ON_REMOTE, noaction);
				...
				if (testAssert(cvobj != nil))
				{
					...
					if (err == VE_OK)
					{
						for (ReducedObjectNodeCollection::iterator cur = reducedPaths.begin(), end = reducedPaths.end(); cur != end && err == VE_OK; ++cur)
						{
							...
							if (trans != nil)
							{
								err = trans->DetruireCle(this, objval, rec->GetAssoc()->getnumfic());
							}
							else
							{
								tempkey.AddKey(objval);
								err = header->DetruireCle(curstack, objval, rec->GetAssoc()->getnumfic(), tempkey, context);
							}
						}
...
Bittab* IndexInfoFromFieldObj::FourcheOnPath(const ObjectPath& path, bool checkfornull, VValueSingle* val1, uBOOL xstrict1, VValueSingle* val2, uBOOL xstrict2, BaseTaskInfo* context, VError& err,
	VDB4DProgressIndicator* InProgress, const VCompareOptions& inOptions, Bittab* dejasel)
{
	Bittab* b = nil;
	...
	{
		BTitemIndex* cle1 = nil;
		BTitemIndex* cle2 = nil;
		if (AskForValid(context))
		{
			sLONG debugToLog = 0;
			...
			if (debugToLog == 1)
			{
				fPathHeader->dumpToLog(context);
				header->dumpToLog(context);
				GetDB()->WriteDebugFlush();
			}
#endif
			...
			VString spath;
			path.ToString(spath);
			BTitemIndex* pathval = fPathEntete->BuildKeyFromVValue(&spath, err);
			sLONG pathid = fPathHeader->FindKey(curstack, pathval, context, options, nil, nil);
			...
			if (pathid != -1)
			{
				if (checkfornull)
				{
					cle1 = BuildKeyWithJSONVal(pathid, VJSONValue::sNull);
					cle2 = BuildKeyWithJSONVal(pathid, VJSONValue::sUndefined);
					b = header->Fourche(curstack, cle1, true, cle2, false, context, err, InProgress, inOptions, dejasel, nil);
				}
				else
				{
					if (val1 != nil)
					{
						VJSONValue jval;
						val1->GetJSONValue(jval, true);
						cle1 = BuildKeyWithJSONVal(pathid, jval);
						if (val2 == nil)
						{
							cle2 = NextPossibleLimit(cle1);
						}
						else
						{
							VJSONValue jval2;
							val1->GetJSONValue(jval2, true);
							cle2 = BuildKeyWithJSONVal(pathid, jval2);
						}
					}
					else
					{
						...
						{
							VJSONValue jval;
							val2->GetJSONValue(jval, true);
							cle2 = BuildKeyWithJSONVal(pathid, jval);
							cle1 = PreviousPossibleLimit(cle2);
						}
					}
					b = header->Fourche(curstack, cle1, xstrict1, cle2, xstrict2, context, err, InProgress, inOptions, dejasel, nil);
				}
			...
			{
				b = new Bittab;
				err = b->aggrandit(GetMaxNB(context));
			}
		...
		{
			xbox_assert(b != nil);
			err = trans->Fourche(this, cle1, xstrict1, cle2, xstrict2, inOptions, dejasel, b, nil);
		}
...
VError IndexInfoFromFieldObj::GenereIndex(BaseTaskInfo* context, VDB4DProgressIndicator* InProgress)
{
	DataTable *FD;
	sLONG i, nb;
	VError err = VE_OK;
...
bool IndexInfoFromFieldObj::MustBeRebuild()
{
	Boolean result = false;
	if (crit != nil)
	{
...
void IndexInfoFromFieldObj::CreateHeader()
{
	xbox_assert(!fIsRemote);
	if (header == nil)
	{
...
{
	xbox_assert( this->fields == NULL);
	xbox_assert( this->frefs == NULL);
	VError err;
...
void IndexInfoFromMultipleField::IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName, bool simple) const
{
	if (simple)
	{
		sLONG i,nb;
...
Boolean IndexInfoFromMultipleField::NeedToRebuildIndex(VIntlMgr* NewIntlMgr, bool stringAlgoHasChanged, bool keywordAlgoHasChanged)
{
	Boolean result = false;
	sLONG i, nb = frefs->GetCount();
	if (stringAlgoHasChanged)
	{
		for (i = 1; i<=nb; i++)
		{
...
bool IndexInfoFromMultipleField::IsOnPrimKey()
{
	bool prim = false;
	Table* tt = (*frefs)[0].fic;
	FieldArray primkey;
	tt->RetainPrimaryKey(primkey);
	if (primkey.GetCount() == frefs->GetCount())
	{
		prim = true;
		FieldRefArray::Iterator curIndPart = frefs->First();
		for (FieldArray::Iterator cur = primkey.First(), end = primkey.End(); cur != end; ++cur, ++curIndPart)
		{
			if (*cur != curIndPart->crit)
				prim = false;
			(*cur)->Release();
		}
	}
...
void IndexInfoFromMultipleField::GetDebugInfo(VString& outText) const
{
	VString s;
	IdentifyIndex(s, true, true, false);
	outText = "IndexInfoFromMultipleField : "+s;
}
...
IndexInfoFromField* CreateIndexInfoField(Base4D* owner, sLONG nf, sLONG nc, sLONG typindex, Boolean UniqueKeys, sLONG datakind)
{
	...
	switch (datakind) 
	{
		...
		case VK_BLOB_OBJ:
			ind = new IndexInfoFromFieldObj(owner, nf, nc, typindex);
			break;
		default:
...
void IndexAction::SetAction(sLONG xindexcommande, Base4D *xdb, sLONG n, VDB4DProgressIndicator* InProgress)
{
	...
	fProgress = InProgress;
	Ind = nil;
	fIndexRequestOriginatorInfo = nil;
}
...
void IndexAction::SetAction(sLONG xindexcommande, IndexInfo* ind, VDB4DProgressIndicator* InProgress, const VJSONObject* inRequestOriginatorInfo)
{
	indexcommande=xindexcommande;
	ind->Retain();
	Ind=ind;
	...
	fProgress = InProgress;
	fIndexRequestOriginatorInfo = nil;
	if (inRequestOriginatorInfo != nil)
	{
		inRequestOriginatorInfo->Retain();
		fIndexRequestOriginatorInfo = inRequestOriginatorInfo;
	}
}
...
void IndexAction::Dispose(Boolean ProgressAlso)
{
	...
	if (fIndexRequestOriginatorInfo != nil)
		fIndexRequestOriginatorInfo->Release();
...
void AddRebuildIndex(IndexInfo* Ind, VDB4DProgressIndicator* InProgress, VSyncEvent* event, const VJSONObject* inRequestOriginatorInfo)
{
	IndexAction aa;

	aa.SetAction(indexrebuild, Ind, InProgress, inRequestOriginatorInfo);
...
void AddCreateIndex(IndexInfo* Ind, VDB4DProgressIndicator* InProgress, VSyncEvent* event, const VJSONObject* inRequestOriginatorInfo)
{
	IndexAction aa;
	...
	aa.SetAction(indexajoute, Ind, InProgress, inRequestOriginatorInfo);
	if (Ind->GetTargetTable()->GetDF() == nil)
	{
		sLONG xdebug = 1;
	}
...
void InitIndex(VDBMgr* db4d)
{
	...
	RegisterIndex(DB4D_Index_OnObj, &CreIndexInfoFromFieldObj);
	...
	RegisterHeader(DB4D_Index_BtreeObjVals, CreIndexHeaderBTreeObjVals);
#if 0
...
Boolean IndexBuilderTask::DoRun()
{
	...
	if ( GetState() < TS_DYING ) 
	{
		...
		if (OKIndex() && GetNextIndexAction(aa, startingstamp, nbnewactions))
		{
			VIndexInfoProvider	indexInfoCollector(aa);
			...
			if (progress == nil)
			{
				...
				else
				{
					...
					if (progress == nil)
					{
						progress = new XBOX::VProgressIndicator();
					}
				}
			...
			if (progress != nil)
			{
				progress->SetProgressInfoCollector(&indexInfoCollector);
				curaction++;
			...
			if (progress != nil)
			{
				progress->SetProgressInfoCollector(nil);
			}
...
VError Index_NonOpened::SetHeaderType(sLONG IndexHeaderType, ToolLog* log)
{
	...
	if (IndexHeaderType == DB4D_Index_Btree || IndexHeaderType == DB4D_Index_BtreeWithCluster || IndexHeaderType == DB4D_Index_AutoType || IndexHeaderType == DB4D_Index_BtreeObjVals)
	{
		fHeaderIsValid = true;
	}
	else
			...
			case DB4D_Index_BtreeObjVals:
				{
					...
					if (errexec == VE_OK && fHeaderIsValid)
					{
						if (fIndexHeaderType == DB4D_Index_BtreeWithCluster || fIndexHeaderType == DB4D_Index_BtreeObjVals)
						{
...
VError Index_NonOpened::SwapByteKeyData(Boolean needswap, BTitemIndex* item, sLONG maxlen, Boolean& ok, sLONG numpage, ToolLog* log, CompareResult& comp)
{
	...
	if (fFieldTypes[0] == VK_BLOB_OBJ)
	{
		...
		{
			VCompareOptions options;
			options.SetIntlManager(VIntlMgr::GetDefaultMgr());
			options.SetDiacritical(true);
			comp = (CompareResult)ObjAttKey::Compare(p2, p, options);
		}
		keylen = ObjAttKey::CalculateLength(p);
	}
	else
	{
							...
							if (adjusteindex(taglen+kSizeDataBaseObjectHeader) == adjusteindex(len))
							{
								...
								if (btp != nil)
								{
													...
													if (errexec == VE_OK)
													{
														...
														else
														{
															...
															else
															{
																...
																{
																	if (fType != DB4D_Index_OnKeyWords && fType != DB4D_Index_OnObj)
																	{
									...
									if (errexec == VE_OK && log->IsCompacting())
									{
										...
										sLONG lenInMem = BTreePageIndex::CalculateDefaultLen(log->GetTargetIndexCompact());

										void* p = GetFastMem(lenInMem, true, 'iPag');		
										BTreePageIndex* res = new (p)BTreePageIndex(log->GetTargetIndexCompact(), lenInMem, numpage);
										
										res->SetBTP(btp);
										res->setaddr(pageaddr, false);

										res->setmodif(true, log->GetTargetIndexCompact()->GetDB(), nil);
										btp = nil;
			...
			if (df != nil)
			{
				if (errexec == VE_OK)
				{
					if (OpenCheckKeysProgressSession( log, &errexec))
					{
						...
						if (errexec == VE_OK && !log->IsCompacting())
						{
							if (fRecordsInKeys != nil && fType != DB4D_Index_OnKeyWords && fType != DB4D_Index_OnObj)
							{
								if (df->NbFicIsValid())
								{
									Bittab allrecs;
