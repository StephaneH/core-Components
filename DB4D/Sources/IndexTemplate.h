...
class BtreePage : public ObjInCacheMem, public IObjToFlush
{
	public:
		...
		BtreePage(IndexInfo* xentete, sLONG xnum, Boolean iscluster, sWORD DefaultAccess = ObjCache::PageIndexAccess);
...
#endif
			assert(false); // cette methode doit absolument etre specialisee
			return nil;
		}
...
class IndexHeaderBTreeScalar : public IndexHeaderBTreeRoot
{
	public:
		...
		inline IndexHeaderBTreeScalar(IndexInfo *xentete, Boolean iscluster):IndexHeaderBTreeRoot(xentete) 
		{ 
			firstpage = nil;
		...
		virtual VError LibereEspaceDisk(OccupableStack* curstack, VDB4DProgressIndicator* InProgress)
		{
			VError err = VE_OK;
		...
		VError PlaceCle(OccupableStack* curstack, FullCleIndex<Type, MaxCles>& Key, sLONG xqui, BaseTaskInfo* context);
		VError DetruireCle(OccupableStack* curstack, FullCleIndex<Type, MaxCles>& Key, sLONG xqui, BaseTaskInfo* context);
...
class IndexInfoScalar : public IndexInfoFromField
{
	public:
		...
		inline sLONG CompareKeys(const FullCleIndex<Type, MaxCles>& val, const TransCleIndex<Type>& val2, const VCompareOptions& inOptions)
		{
			if (val2.cle == val.cle)
				return CR_EQUAL;
			else
		...
		inline sLONG CompareKeys(const TransCleIndex<Type>& val, const FullCleIndex<Type, MaxCles>& val2, const VCompareOptions& inOptions)
		{
			if (val2.cle == val.cle)
				return CR_EQUAL;
			else
...
template <>
inline sLONG IndexInfoScalar<Real, kNbKeysForScalar>::CompareKeys(const FullCleIndex<Real, kNbKeysForScalar>& val, const TransCleIndex<Real>& val2, const VCompareOptions& inOptions)
{
...
template <>
inline sLONG IndexInfoScalar<Real, kNbKeysForScalar>::CompareKeys(const TransCleIndex<Real>& val, const FullCleIndex<Real, kNbKeysForScalar>& val2, const VCompareOptions& inOptions)
{
	if (fabs(val2.cle - val.cle) < inOptions.GetEpsilon())
		return CR_EQUAL;
	else
