...
VError BtreePage<Type, MaxCles>::loadobj(DataAddr4D xaddr, sLONG len)
{
	...
	if (testAssert((size_t)datalen == sizeof(btp)))
	{
		...
		entete->GetHeader()->MeasureRead(1, datalen+kSizeDataBaseObjectHeader);
		if (err == VE_OK)
		...
		xbox_assert( (btp.nkeys>0 && btp.nkeys <= MaxCles) || err != VE_OK) ;
...
void BtreePage<Type, MaxCles>::MoveAndAddItems(sLONG from, sLONG howmany, BtreePage<Type, MaxCles>* into)
{
	...
	xbox_assert(into->btp.nkeys+howmany <= MaxCles);
...
{
	xbox_assert(false); // ne devrait plus jamais passer par la
#if 0
	BtreePage<Type, MaxCles> *sousBT;
...
void BtreePage<Type, MaxCles>::debug_checkKeyOrder(CleIndex<Type>* &curkey)
{
	for (sLONG i = 0; i < btp.nkeys; i++)
	{
		...
		if (curkey != nil)
		{
			...
			xbox_assert(comp != CR_BIGGER);
			if (comp == CR_BIGGER)
			{
				(void)comp; // put a break here
			}
		}
		curkey = x;
	}
...
void BtreePage<Type, MaxCles>::checkPosSousBT(BtreePage<Type, MaxCles>* sousBT)
{
	...
	if (testAssert(found != -1))
	{
		...
		if (found != 0)
		{
			xbox_assert(CompareKeysStrict(found-1, sousBTMin) != CR_BIGGER);
		}
		if (found != nb)
		{
			xbox_assert(CompareKeysStrict(found, sousBTMax) != CR_SMALLER);
		}
	}
}
...
template <class Type, sLONG MaxCles>
VError IndexHeaderBTreeScalar<Type,MaxCles>::PlaceCleScalar(OccupableStack* curstack, FullCleIndex<Type, MaxCles>& Key, sLONG xqui, BaseTaskInfo* context)
{
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			else
			{
				if (h)
				{
					pag = BtreePage<Type, MaxCles>::AllocatePage(curstack, GetAssoc(), context, err);
					if (pag != nil)
					{
...
template <class Type, sLONG MaxCles>
VError IndexHeaderBTreeScalar<Type,MaxCles>::DetruireCleScalar(OccupableStack* curstack, FullCleIndex<Type, MaxCles>& Key, sLONG xqui, BaseTaskInfo* context)
{
	uBOOL h;
	VError err;
	uBOOL locdoisdel;
...
VError IndexHeaderBTreeScalar<Type,MaxCles>::ChargeFirstPage(OccupableStack* curstack, BaseTaskInfo* context, uBOOL doiscreer)
{
	...
	if (firstpage==nil)
	{
		...
		if ((IHD.nbpage==0) || (HBT.FirstPage==-1)) 
		{
			if (doiscreer)
			{
				firstpage = BtreePage<Type, MaxCles>::AllocatePage(curstack, GetAssoc(), context, err);
				if (firstpage != nil)
				{
					HBT.FirstPage=firstpage->GetNum();
					taddr.setmodif(true, GetAssoc()->GetDB(), context);
		...
		else
		{
			VTaskLock lock(&fLoadPageMutex);
			if (firstpage == nil)
				firstpage = BtreePage<Type, MaxCles>::LoadPage(curstack, HBT.FirstPage, GetAssoc(), context, err);
			else
			{
														 ...
														 VDB4DProgressIndicator* InProgress, Boolean TestUnicite, Bittab* inWafSelbits, WafSelection* outWafSel)
{
	...
	if (bb != nil)
	{
		...
		if (sel2 != nil)
		{
			...
			if (TestUnicite || (trans != nil && trans->ConcernIndex(GetAssoc())))
			{
				XBOX::VString session_title;
				gs(1005,8,session_title);	// Indexed Sort: %curValue of %maxValue Keys
				IndexIterator<Type, MaxCles> iter((IndexInfoScalar<Type, MaxCles>*)GetAssoc(), bb, context, InProgress, session_title, curstack);
		...
		if (err==VE_OK)
		{
			...
			if (firstpage!=nil && err == VE_OK)
			{
				...
				else
				{
					...
					gs(1005,28,session_title);
					ProgressEncapsuleur progress(InProgress, IHD.nbpage, session_title, true);
					xbox_assert(false); // ne devrait plus passer par la
...
VError IndexHeaderBTreeScalar<Type,MaxCles>::CalculateColumnFormulas(OccupableStack* curstack, ColumnFormulas* formules, BaseTaskInfo* context, Bittab* filtre, VDB4DProgressIndicator* InProgress)
{
	...
	if (firstpage!=nil && err == VE_OK)
	{
		...
		if (okoptime)
		{
			...
			sLONG count = 0;
			sLONG datakind = ((IndexInfoScalar<Type, MaxCles>*)GetAssoc())->GetDataKind();
			if (datakind == VK_REAL)
			{
				Real resultsum = 0;
...
template <class Type, sLONG MaxCles>
bool IndexInfoScalar<Type, MaxCles>::PlaceGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context, bool temporary)
{
	bool okAdded = false;
	VError err = VE_OK;
	...
	TransUniqCleIndex<Type> uniqkey(val.cle, trans, temporary);
	typename mapUniqKeys::iterator found = fUniqueKeysToBeProcessed.find(uniqkey);
	if (found == fUniqueKeysToBeProcessed.end())
	{
		fUniqueKeysToBeProcessed.insert(uniqkey);
		okAdded = true;
		if (trans != nil && !temporary)
		{
			BTitemIndex* valx = BuildKey(rec, err, false);
			valx->SetInd(this);
			valx->StartUse();
			trans->AddUniqKeyToRelease(valx);
			valx->Unuse();
		}
	}
	else
...
template <class Type, sLONG MaxCles>
bool IndexInfoScalar<Type, MaxCles>::IsGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	bool isThere = false;
	VError err = VE_OK;
	FullCleIndex<Type, MaxCles> val;
	BuildFullCle(rec, val, err, false);
	TransUniqCleIndex<Type> uniqkey(val.cle, nil, true);
	typename mapUniqKeys::iterator found = fUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fUniqueKeysToBeProcessed.end())
	{
		isThere = true;
...
template <class Type, sLONG MaxCles>
void IndexInfoScalar<Type, MaxCles>::ReleaseGlobalUniqKey(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context, bool tempOnly)
{
	VError err = VE_OK;
	Transaction* trans = nil;
	if (context != nil)
	...
	FullCleIndex<Type, MaxCles> val;
	BuildFullCle(rec, val, err, false);
	TransUniqCleIndex<Type> uniqkey(val.cle, trans, false);
	typename mapUniqKeys::iterator found = fUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fUniqueKeysToBeProcessed.end())
	{
		if (found->fOwner == trans)
		{
			...
			{
				if (found->fIsTemp)
					fUniqueKeysToBeProcessed.erase(found);
			}
			else
				fUniqueKeysToBeProcessed.erase(found);
		}
...
template <class Type, sLONG MaxCles>
void IndexInfoScalar<Type, MaxCles>::ReleaseGlobalUniqKey(const BTitemIndex* val, Transaction* trans)
{
	FullCleIndex<Type, MaxCles> xval(val);
	TransUniqCleIndex<Type> uniqkey(xval.cle, trans, false);
	typename mapUniqKeys::iterator found = fUniqueKeysToBeProcessed.find(uniqkey);
	if (found != fUniqueKeysToBeProcessed.end())
	{
		if (found->fOwner == trans)
...
VError IndexInfoScalar<Type,MaxCles>::PlaceCle(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			if (trans == nil)
			{
				err = GetHeaderScalar()->PlaceCleScalar(curstack, val, rec->GetAssoc()->getnumfic(), context);
			}
			else
			{
...
VError IndexInfoScalar<Type,MaxCles>::DetruireCle(OccupableStack* curstack, FicheInMem *rec, BaseTaskInfo* context)
{
	...
	else
	{
		...
		if (err == VE_OK)
		{
			...
			if (trans == nil)
			{
				err = GetHeaderScalar()->DetruireCleScalar(curstack, val, rec->GetAssoc()->getnumfic(), context);
			}
			else
			{
	...
	else
	{
		...
		for (i=0;i<nb && err == VE_OK;i++)
		{
			...
			if (rec!=nil)
			{
				...
				{
					Open(index_write);
					err = GetHeaderScalar()->PlaceCleScalar(curstack, val, i, context);
					EnCreation(i);
					Close();
				}
				rec->Release();
		...
		if (vals != nil)
		{
			if (val1.IsNull())
			{
				if (!val2.IsNull())
				{
					...
					{
						if (CompareKeysScalar(*cur, val2, inOptions) == CR_BIGGER)
							upperbound = true;
						else
						{
			...
			else
			{
				...
				if (fDataKind == VK_REAL && cur != end)
				{
					...
					{
						if (CompareKeysScalar(*cur2, val1, inOptions) != CR_EQUAL)
						{
							lowerbound = true;
						}
						else
				...
				while (cur != end && !upperbound && err == VE_OK)
				{
					if ((!val2.IsNull()) && CompareKeysScalar(*cur, val2, inOptions) == CR_BIGGER)
						upperbound = true;
					else
					{
						sLONG numfic = cur->qui;
		...
		if (vals != nil)
		{
			if (val1.IsNull())
			{
				if (!val2.IsNull())
				{
					...
					while (cur != end && !upperbound && err == VE_OK)
					{
						sLONG comp2 = CompareKeysScalar(*cur, val2, inOptions);
			...
			else
			{
				...
				{
					bool ok = true;
					if (xstrict1 && CompareKeysScalar(*cur, val1, inOptions) == CR_EQUAL)
						ok = false;
	...
	//		if (!cur->IsNull())
			{
				...
				GetHeaderScalar()->PlaceCleScalar(curstack, key, key.qui, context);
			}
		}
...
VError IndexInfoScalar<Type,MaxCles>::DetruireAllCleForTrans(void* xvals, BaseTaskInfo* context)
{
	...
	//		if (!cur->IsNull())
			{
				FullCleIndex<Type, MaxCles> key(*cur);
				GetHeaderScalar()->DetruireCleScalar(curstack, key, key.qui, context);
			}
		}
	...
	{
		{
			...
			if (level > 1)
			{
				...
				else
				{
					...
					if (sousBT != nil)
					{
						xbox_assert(sousBT->GetNKeys() >= (MaxElemsInPage / 2));
						sousBT->Free(curstack, true);
					}
				}
