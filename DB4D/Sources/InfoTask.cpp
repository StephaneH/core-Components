...
BaseTaskInfo::~BaseTaskInfo()
{
	...
	if (bd != nil)
	{
		...
		assert( gCppMem->CheckVObject( bd));
...
VError BaseTaskInfo::WriteLog(DB4D_LogAction action)
{
	VError err = bd->WriteLog(action, this);
...
VError BaseTaskInfo::WriteCreationToLog()
{
	...
	if (bd != nil && !fIsCreationWrittenToLog)
	{
		...
		if (extra != NULL)
		{
			...
			if (err == VE_OK)
				err = bd->StartWriteLog(DB4D_Log_CreateContextWithExtra, (sLONG)tempStream.GetDataSize(), this, log);
			if (err == VE_OK)
			{
				if (log != nil)
				{
		...
		{
			err = bd->StartWriteLog(DB4D_Log_CreateContextWithUserUUID, sizeof(VUUIDBuffer), this, log);
			if (err == VE_OK)
			{
				if (log != nil)
				{
...
VError BaseTaskInfo::WriteDeletionToLog()
{
	...
	if (bd != nil && fIsCreationWrittenToLog)
	{
		err = bd->WriteLog(DB4D_Log_CloseContext, this);
	}
	return err;
}
...
#if 0
	assert(sel != nil);
	assert(sel->GetParentFile()->GetDB() == bd);
...
{
	assert(inTableToExclude->GetOwner() == bd);
	//occupe();
	fAutoRelStamp++;
...
{
	assert(inTableToInclude->GetOwner() == bd);
	//occupe();
...
{
	assert(inTableToCheck->GetOwner() == bd);
	Boolean res = false;
	//occupe();
			...
			if (!stop)
			{
				...
				else
				{
					if (found->first.GetOwner() == obj.GetOwner())
					{
						found->second++;
						ok =true;
						assert(found->first.fLockerType == obj.fLockerType);
					}
					else
					{
						if (found->first.IsForReadOnly() && ForReadOnly)
...
VError BaseTaskInfo::AddStructLocker(const StructObjRef& obj)
{
	...
	try
	{
		assert(fStructLockers.find(obj) == fStructLockers.end());
...
void BaseTaskInfo::SetJSMethod(const JSEntityMethodReference& methRef, const VJSObject& inObjFunc)
{
	if (fJSContext != nil)
	{
		...
		JS4D::ProtectValue(jscontext, inObjFunc);
...
void BaseTaskInfo::FreeAllJSMethods()
{
	for (MapOfJSMethods::iterator cur = fJSMethods.begin(), end = fJSMethods.end(); cur != end; ++cur)
	{
		...
		JS4D::UnprotectValue(jscontext, *funcobj);
	}
	fJSMethods.clear();
}
