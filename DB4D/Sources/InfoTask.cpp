...
VError BaseTaskInfo::PutRecIn(FicheInMem* fic)
{
	...
	xbox_assert(!fic->IsUsedAsCacheInQuery());
	xbox_assert(!fic->IsAssocDataNull());

#if WITH_ASSERT
	if (fic->IsUsedAsCacheInQuery() || fic->IsAssocDataNull())
	{
		// force crash
		sLONG* p = nil;
		*p = 1;
...
{
	VTaskLock lock(&fServerKeptRecordsMutex);
	return HasRemoteTransPending() || !fClientReleasedRecIDs.empty() || !fMarkedRecordsAsPushed.empty();
}
