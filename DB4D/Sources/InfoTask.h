...
class JSEntityMethodReference
{
	public:
				...
				if (fMethod == other.fMethod)
				{
					if (fAttribute == other.fAttribute)
					{
						if (fEvent == other.fEvent)
						{
							...
							{
								if (fCalculatedScript == other.fCalculatedScript)
								{
									result = fEventPos < other.fEventPos;
								}
								else
		...
		JSEntityMethodReference(const EntityModel* inDataClass, DBEventKind inEventKind, const EntityAttribute* inAttribute, sLONG eventpos) // for dataClass or attributes events 
		{
			init();
			fDataClass = inDataClass;
			fAttribute = inAttribute;
		...
		{
			init();
			fMethod = (const EntityMethod*)((size_t)ref); // will not be used as a pointer
		}
...
{
	public:
		JSEntityMethod():fFuncObj((VJSContext*)nil)
		{
		}
...
class DB4DJSRuntimeDelegate : public VObject, public IJSRuntimeDelegate
{
	public:
...
class ENTITY_API BaseTaskInfo : /*public ObjInCacheMemory,*/ public CDB4DBaseContext
{
	public:
		...
		void SetCachedProgressIndicator(XBOX::VProgressIndicator* inProgressToCache);
		XBOX::VProgressIndicator* GetCachedProgressIndicator();
		...
		*/
		...
		virtual CUAGThreadPrivilege* GetThreadPrivileges()
		{
			return fThreadPrivileges;
		}

		virtual void SetThreadPrivileges(CUAGThreadPrivilege* privileges)
		{
			CopyRefCountable(&fThreadPrivileges, privileges);
		}
		...
		virtual void	BuildQueryPlan(Boolean on);
		virtual void	BuildQueryPath(Boolean on);
		virtual Boolean ShouldBuildQueryPlan()const;
		virtual Boolean ShouldBuildQueryPath()const;
		...
		void StealMarkedRecordsAsPushed(vectorPushedRecs& outRecs)
		{
			outRecs.clear();
	...
	protected:
		...
		CUAGThreadPrivilege* fThreadPrivileges;
		...
		Boolean fBuildQueryPath;
		Boolean fBuildQueryPlan;
		...
		XBOX::VProgressIndicator* fCachedProgressIndicator;
...
inline VProgressIndicator* GetCachedProgressIndicator(BaseTaskInfo* context, VProgressIndicator* existingProgress)
{
	if (existingProgress == nil)
	{
		if (context != nil)
		{
			return context->GetCachedProgressIndicator();
		}
		else
			return nil;
	}
	...
	public:
		...
		inline void xInit(CDB4DBaseContextPtr inContext, Base4D* base, ObjLocker* dejalocker = nil)
		{
			...
			else
			{
				...
				{
					xbox_assert(fContext->GetBase() == base);
					fContext->Retain("ObjLocker");
				}
			}
		};
