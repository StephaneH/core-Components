	...
	if (okperm(context, fOwner, DB4D_EM_Delete_Perm))
	{
		...
		if (!fOwner->HasDeleteEvent())
		{
			if (!target->HasSyncInfo() && !(db->ReferentialIntegrityEnabled() && target->AtLeastOneReferentialIntegrity()) )
			{
				...
				if (!DeleteDejaFait)
				{
					if (!target->AtLeastOneBlob() && GetCurrentTransaction(context) == nil && !db->IsThereATrigger(DB4D_DeleteRecord_Trigger, target))
					{
...
EntityCollection* LocalEntityModel::executeQuery( SearchTab* querysearch, BaseTaskInfo* context, EntityCollection* Filter, 
									 ...
									 sLONG limit, EntityCollection* outLockSet, VErrorDB4D *outErr)
{
	VError err = VE_OK;
	Selection *sel = nil;
	if (okperm(context, fPerms[DB4D_EM_Read_Perm]) || okperm(context, fPerms[DB4D_EM_Update_Perm]) || okperm(context, fPerms[DB4D_EM_Delete_Perm]))
									   ...
									   sLONG limit, EntityCollection* outLockSet, VError *outErr)
{
	...
	if (err == VE_OK)
	{
		...
		{
			CopyRefCountable(&sel, sel->SortCollection(orderby, context, err, InProgress));
		}
	}
...
EntityCollection* LocalEntityModel::executeQuery( const VString& queryString, VJSONArray* params, BaseTaskInfo* context, EntityCollection* filter, 
												 ...
												 sLONG limit, EntityCollection* outLockSet, VError *outErr)
{
	assert(false); // shold not be called;
...
EntityRecord* LocalEntityModel::newEntity(BaseTaskInfo* inContext) const
{
	VError err;
	...
	if (fBaseEm != nil)
	{
		...
		if (fMainTable != nil)
		{
			for (EntityAttributeCollection::iterator cur = fAttributes.begin(), end = fAttributes.end(); cur != end && err == VE_OK; ++cur)
			{
				...
				Field* cri = fMainTable->FindAndRetainFieldRef(entAtt->GetName());
				if (cri != nil)
				{
...
VError LocalEntityModel::MatchWithTable()
{
	...
	if (fBaseEm == nil)
	{
		fMainTable = fDB->FindOrCreateTableRef(fName, err, fDB4DUUID);
		if (fMainTable != nil)
		{
			for (EntityAttributeCollection::iterator cur = fAttributes.begin(), end = fAttributes.end(); cur != end && err == VE_OK; ++cur)
			{
...
VError LocalEntityModel::BuildIndexes()
{
	VError err = VE_OK;
	if (fMainTable != nil)
	{
		...
		{
			db4dEntityAttribute* entAtt = (db4dEntityAttribute*)*cur;
			err = entAtt->BuildIndexes(fMainTable);
		}
...
VError db4dEntityAttribute::MatchField(Table* tt)
{
	...
	if (typ != 0 && err == VE_OK)
	{
		//if (typ == VK_STRING)
		//typ = VK_TEXT;
		Field* cri = tt->FindAndRetainFieldRef(fName);
		if (cri == nil)
		{
...
VError db4dEntityAttribute::BuildIndexes(Table* tt)
{
	VError err = VE_OK;
...
VErrorDB4D LocalEntityRecord::Save(uLONG stamp, bool allowOverrideStamp)
{
...
VError LocalEntityRecord::Save(Transaction* *trans, BaseTaskInfo* context, uLONG stamp, bool allowOverrideStamp)
{
	VError err = VE_OK;
	...
	if (!fAlreadySaving)
	{
		...
		if (okperm(context, xGroupID))
		{
			...
			if (err == VE_OK)
			{
				if (locmodel->HasPrimKey() && !locmodel->MatchPrimKeyWithDataSource())
				{
...
bool LocalEntityRecord::do_GetPrimKeyValueAsObject(VJSObject& outObj)
{
	...
	if (primatts->empty())
	{
		...
		VJSValue jsval2(outObj.GetContextRef());
...
VError LocalEntityModelCatalog::do_LoadEntityModels(const VValueBag& bagEntities, bool devMode, unordered_map_VString<VRefPtr<VFile> >* outIncludedFiles, BaseTaskInfo* context)
{
	VError err = VE_OK;
...
VError LocalEntityModelCatalog::do_SecondPassLoadEntityModels()
{
	Base4D* db = GetDB();
	VError err = VE_OK;
