...
static bool gDebugDisplayRech = false;
...
VError RechTokenSimpleComp::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	Field* cri = bd->RetainField(numfile, numfield);
	if (cri != nil)
	{
		if (canMeasure)
			cri->MeasureQueryCount();
		VString s;
		...
		BuildCompOperString(comparaison, s);
		outRef += s;
		outRef += "?";
	}
...
RechTokenSimpleComp* RechTokenEMSimpleComp::CopyIntoRechTokenSimpleComp()
{
	if (fCopyForLocalModel == nil)
	{
		...
		fCopyForLocalModel->fObjPath = fObjPath;
...
VJSONObject* RechTokenEMSimpleComp::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace)
{
	VJSONObject* result = RechToken::ToJSON(context, wildcharReplace);

	result->SetPropertyAsString("attName", fAtt->GetName());
	result->SetPropertyAsString("emName", /*fAtt->GetOwner()->GetName()*/ fAtt->GetOwner()->GetBaseModelName());
	result->SetPropertyAsNumber("comparaison", comparaison);
	result->SetPropertyAsNumber("instance", numinstance);
	result->SetPropertyAsBool("checkForNull", fCheckForNull);
	VJSONValue val;
	if (ch == nil)
		val.SetNull();
	else
	{
		if (wildcharReplace != 0 && ch->GetValueKind() == VK_STRING && (comparaison == DB4D_Like || comparaison == DB4D_NotLike))
		{
			VString s;
			ch->GetString(s);
			UniChar wild = '*';
			if (fOptions.GetIntlManager() != nil)
				wild = fOptions.GetIntlManager()->GetWildChar();
			s.ExchangeAll(wild, wildcharReplace);
			s.GetJSONValue(val);
		}
		else
			ch->GetJSONValue(val);
	...
	fOptions.ToJSON(result);

	return result;
}


VError RechTokenEMSimpleComp::FromJSON(BaseTaskInfo* context, VJSONObject* obj)
{
	VString attname;
	VString modelname;
	VError err = VE_OK;
	...
	obj->GetPropertyAsString("attName", attname);
	obj->GetPropertyAsString("emName", modelname);

	EntityModel* em = base->FindEntity(modelname, false);
	if (em != nil)
	{
		fAtt = em->getAttribute(attname);
		if (fAtt == nil)
			err = ThrowBaseError(VE_DB4D_ENTITY_ATTRIBUTE_NOT_FOUND);
	}
	else
		err = ThrowBaseError(VE_DB4D_ENTITY_ATTRIBUTE_NOT_FOUND);

	obj->GetPropertyAsNumber("comparaison", &comparaison);
	obj->GetPropertyAsNumber("instance", &numinstance);
	bool b;
	obj->GetPropertyAsBool("checkForNull", &b);
	fCheckForNull = b ? 1 : 0;
	fOptions.FromJSON(obj);

	ch = nil;
	if (fAtt != nil)
	{
		fSimpleDate = fAtt->isSimpleDate();
		SetCompOptionWithOperator(fOptions, comparaison);
		ValueKind datakind = fAtt->GetDataKind();
		VJSONValue val = obj->GetProperty("value");
		VValueSingle* ValueToCompare = val.CreateVValue();
		if (!fObjPath.IsEmpty())
			datakind = VK_STRING;
		if (ValueToCompare == nil)
		{
			if (datakind == VK_EMPTY)
				...
				ch = (VValueSingle*)VValue::NewValueFromValueKind(datakind);
		}
		else
		{
			if (datakind == VK_EMPTY)
				ch = ValueToCompare->Clone();
			else
			{
				if (datakind == VK_IMAGE)
					datakind = VK_STRING;
				ch = ValueToCompare->ConvertTo(datakind);
			}
		}

		if (comparaison == DB4D_Regex_Not_Match || comparaison == DB4D_Regex_Match)
		{
			VString s;
			ch->GetString(s);
			VError err;
			fRegMatcher = VRegexMatcher::Create(s, &err);
		}
...
VError RechTokenEMSimpleComp::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	if (fAtt != nil)
	{
		EntityModel* em = fAtt->GetOwner();
		if (canMeasure)
		{
			LocalEntityModel* locEm = dynamic_cast<LocalEntityModel*>(em);
			if (locEm != nil)
			{
				db4dEntityAttribute* att = (db4dEntityAttribute*)fAtt;
...
VError RechTokenRecordExist::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	outRef += "record exists";
	return VE_OK;
}
...
VJSONObject* RechTokenEntityRecordExist::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace )
{
	VJSONObject* result = RechToken::ToJSON(context, wildcharReplace);

	result->SetPropertyAsString("emName", /*fModel->GetName()*/ fModel->GetBaseModelName());
	result->SetPropertyAsNumber("instance", numinstance);
	result->SetPropertyAsBool("checkExists", fCheckIfExists);

	return result;
}


VError RechTokenEntityRecordExist::FromJSON(BaseTaskInfo* context, VJSONObject* obj)
{
	VError err = VE_OK;
	...
	obj->GetPropertyAsString("emName", modelname);
	bool b;
	obj->GetPropertyAsBool("checkExists", &b);
	fCheckIfExists = b ? 1 : 0;
...
VError RechTokenEntityRecordExist::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	outRef += "entity exists";
	return VE_OK;
}
...
VError RechTokenEmComp::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	const EntityAttributeInstance* attinst = fAttPath.FirstPart();
	bool first = true;
	while (attinst != nil)
	{
		const EntityAttribute* xatt = attinst->fAtt;

		EntityModel* em = xatt->GetOwner();
		if (canMeasure)
		{
			LocalEntityModel* locEm = dynamic_cast<LocalEntityModel*>(em);
			if (locEm != nil)
			{
				db4dEntityAttribute* att = (db4dEntityAttribute*)xatt;
				Field* cri = att->RetainField(true);
				if (cri != nil)
				{
					cri->MeasureQueryCount();
...
VError RechTokenArrayComp::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	Field* cri = bd->RetainField(numfile, numfield);
	if (cri != nil)
	{
...
VJSONObject* RechTokenEmArrayComp::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace)
{
	VJSONObject* result = RechToken::ToJSON(context, wildcharReplace);
	...
	if (values != nil)
	{
		VJSONArray* arr = values->ToJSON(context, wildcharReplace);
		result->SetProperty("value", VJSONValue(arr));
...
VError RechTokenEmArrayComp::FromJSON(BaseTaskInfo* context, VJSONObject* obj)
{
	VError err = VE_OK;
	Base4D* base = context->GetBase();
	VString attname,modelname;
	obj->GetPropertyAsString("attName", attname);
	obj->GetPropertyAsString("emName", modelname);
	...
	if (fAtt != nil)
	{
		fExpectedType = fAtt->ComputeScalarType();
		fSimpleDate = fAtt->isSimpleDate();
		SetCompOptionWithOperator(fOptions, comparaison);
		fIsDataDirect = false;
		...
		if (val.IsArray())
		{
			values = GenerateConstArrayOfValues(val.GetArray(), fExpectedType, fOptions);
		}
	}
...
VError RechTokenEmArrayComp::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	if (fAtt != nil)
	{
		EntityModel* em = fAtt->GetOwner();
...
VError RechTokenJoin::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	Field* cri = bd->RetainField(numfile, numfield);
	if (cri != nil)
	{
	...
	cri = bd->RetainField(numfileOther, numfieldOther);
	if (cri != nil)
	{
		if (canMeasure)
			cri->MeasureQueryCount();
...
VError RechTokenEmJoin::BuildRefString(VString& outRef, bool canMeasure, Base4D* bd)
{
	if (fAtt != nil)
	{
		EntityModel* em = fAtt->GetOwner();
	...
	if (fAttOther != nil)
	{
		EntityModel* em = fAttOther->GetOwner();
		if (canMeasure)
		{
			LocalEntityModel* locEm = dynamic_cast<LocalEntityModel*>(em);
			if (locEm != nil)
			{
				db4dEntityAttribute* att = (db4dEntityAttribute*)fAttOther;
				Field* cri = att->RetainField(true);
				if (cri != nil)
				{
					cri->MeasureQueryCount();
...
VJSONObject* RechTokenEmJoin::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace)
{
	VJSONObject* result = RechToken::ToJSON(context, wildcharReplace);

	result->SetPropertyAsString("attLeftName", fAtt->GetName());
	result->SetPropertyAsString("emLeftName", /*fAtt->GetOwner()->GetName()*/ fAtt->GetOwner()->GetBaseModelName());
	result->SetPropertyAsString("emRootBaseLeftName", fRootBaseEm->GetName());
	result->SetPropertyAsNumber("leftInstance", numinstance);
	result->SetPropertyAsNumber("comparaison", comparaison);
	result->SetPropertyAsString("attRightName", fAttOther->GetName());
	result->SetPropertyAsString("emRightName", /*fAttOther->GetOwner()->GetName()*/ fAttOther->GetOwner()->GetBaseModelName());
	result->SetPropertyAsString("emRootBaseRightName", fRootBaseEmOther->GetName());
	result->SetPropertyAsNumber("rightInstance", numinstanceOther);
	fOptions.ToJSON(result);
...
VError RechTokenEmJoin::FromJSON(BaseTaskInfo* context, VJSONObject* obj)
{
	VError err = VE_OK;

	Base4D* base = context->GetBase();
	VString attname, attothername, emname, emothername, emroobasename, emotherrootbasename;

	fAttOther = nil;
	fAtt = nil;

	obj->GetPropertyAsString("attLeftName", attname);
	obj->GetPropertyAsString("emLeftName", emname);
	obj->GetPropertyAsString("emRootBaseLeftName", emroobasename);
	obj->GetPropertyAsNumber("leftInstance", &numinstance);
	obj->GetPropertyAsNumber("comparaison", &comparaison);
	obj->GetPropertyAsString("attRightName", attothername);
	obj->GetPropertyAsString("emRightName", emothername);
	obj->GetPropertyAsString("emRootBaseRightName", emotherrootbasename);
	obj->GetPropertyAsNumber("rightInstance", &numinstanceOther);

	fOptions.FromJSON(obj);

	EntityModel* em = base->FindEntity(emname, false);
	if (em != nil)
	{
		fAtt = em->getAttribute(attname);
		if (fAtt == nil)
	...
	em = base->FindEntity(emothername, false);
	if (em != nil)
	{
		fAttOther = em->getAttribute(attothername);
		if (fAttOther == nil)
			err = ThrowBaseError(VE_DB4D_ENTITY_ATTRIBUTE_NOT_FOUND);
	}
	else
	...
	fRootBaseEm = base->FindEntity(emroobasename, false);

	fRootBaseEmOther = base->FindEntity(emotherrootbasename, false);
...
VJSONObject* RechTokenEmSel::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace)
{
	VJSONObject* result = RechToken::ToJSON(context, wildcharReplace);
	//VString emname;
	if (fSel != nil)
	{
		fSel->ToJSON(result, context, wildcharReplace);
		/*
		EntityModel* em = fSel->GetModel();
		if (em != nil)
		{
			emname = em->GetName();
		}
	...
	//result->SetPropertyAsString("emName", /*fAtt->GetOwner()->GetName()*/ emname);
	result->SetPropertyAsNumber("instance", fNumInstance);
	return result;
}


VError RechTokenEmSel::FromJSON(BaseTaskInfo* context, VJSONObject* obj)
{
	VError err = VE_OK;
	
	Base4D* base = context->GetBase();
	EntityModelCatalog* cat = base->GetEntityCatalog(false);
	fSel = cat->BuildCollectionFromJSON(obj, err);
	fNumInstance = 0;
	obj->GetPropertyAsNumber("instance", &fNumInstance);
...
void SearchTab::AddSearchLineEm(AttributePath& attpath, ObjectPath& objpath, sLONG comp, const VString& inParamToCompare, const VCompareOptions& inOptions, sLONG numinstance)
{
...
void SearchTab::AddSearchLineEm(AttributePath& attpath, ObjectPath& objpath, sLONG comp, const VValueSingle* ValueToCompare, const VCompareOptions& inOptions, bool checkfornull, sLONG numinstance)
{
	...
	SetCompOptionWithOperator(rt->fOptions, comp);
	ValueKind datakind = attpath.LastPart()->fAtt->GetDataKind();
	if (datakind == VK_EMPTY || datakind == VK_BLOB_OBJ)
...
void SearchTab::AddSearchLineEm(AttributePath& attpath, ObjectPath& objpath, sLONG comp, const VValueSingle* ValueToCompare, Boolean isDiacritic, bool checkfornull, sLONG numinstance)
{
	VCompareOptions options;
	options.SetDiacritical(isDiacritic);
	AddSearchLineEm(attpath, objpath, comp, ValueToCompare, options, checkfornull, numinstance);
}


void SearchTab::AddSearchLineEm(const VString& attpath, ObjectPath& objpath, sLONG comp, const VValueSingle* ValueToCompare, const VCompareOptions& inOptions, bool checkfornull, sLONG numinstance)
{
	if (fModel != nil)
	{
		AttributePath path(fModel, attpath);
		if (path.IsValid())
			AddSearchLineEm(path, objpath, comp, ValueToCompare, inOptions, checkfornull, numinstance);
	}
}


void SearchTab::AddSearchLineEm(const VString& attpath, ObjectPath& objpath, sLONG comp, const VValueSingle* ValueToCompare, Boolean isDiacritic, bool checkfornull, sLONG numinstance)
{
	if (fModel != nil)
	{
		AttributePath path(fModel, attpath);
		if (path.IsValid())
			AddSearchLineEm(path, objpath, comp, ValueToCompare, isDiacritic, checkfornull, numinstance);
	}
}
...
void SearchTab::AddSearchLineSimple(Field* cri, const VectorOfVString& objectPath, sLONG comp, const VValueSingle* ValueToCompare, Boolean isDiacritic, sLONG numinstance, bool checkfornull)
{
	CheckForNullOn(comp, checkfornull);
	if (testAssert(cri != nil))
	{
		...
		rt->numinstance = numinstance;
		rt->comparaison = comp;
		rt->fObjPath.FromVector(objectPath);
		SetCompOptionWithOperator(rt->fOptions, comp);
...
void SearchTab::AddSearchLineEmSimple(const EntityAttribute* att, ObjectPath& objpath, sLONG comp, const VValueSingle* ValueToCompare, const VCompareOptions& inOptions, sLONG numinstance, bool checkfornull)
{
	CheckForNullOn(comp, checkfornull);
...
void SearchTab::AddSearchLineEmSimple(const EntityAttribute* att, ObjectPath& objpath, sLONG comp, const VString& inParamToCompare, const VCompareOptions& inOptions, sLONG numinstance, bool checkfornull)
{
	CheckForNullOn(comp, checkfornull);
	if (testAssert(att != nil))
	{
...
void SearchTab::AddSearchLineEmArray(const EntityAttribute* att, ObjectPath& objpath, sLONG comp, DB4DArrayOfValues *Values, const VCompareOptions& inOptions, sLONG numinstance)
{
...
void SearchTab::AddSearchLineEmArray(const EntityAttribute* att, ObjectPath& objpath, sLONG comp, const VString& inParamToCompare, const VCompareOptions& inOptions, sLONG numinstance)
{
	RechTokenEmArrayComp *rt;
...
void SearchTab::AddSearchLineJoin(Field* cri, sLONG comp, Field* OtherCri, Boolean isDiacritic, sLONG numinstance, sLONG numinstanceOther)
{
	...
	xbox_assert(cri != nil && OtherCri != nil);
...
void SearchTab::AddSearchLineJoinEm(const EntityAttribute* att, sLONG comp, const EntityAttribute* OtherAtt, Boolean isDiacritic, sLONG numinstance, sLONG numinstanceOther)
{
	...
	xbox_assert(att != nil && OtherAtt != nil);
...
bool GetNextWord(const VString& input, sLONG& curpos, VString& result)
{
	...
	bool first = true;
	bool isregularword;
	bool insidequotes = false, insidedoublequotes = false,insidebrackets = false;
		...
		if (curpos<input.GetLength())
		{
			...
			else
			{
				...
				else
				{
					...
					{
						if (!insidebrackets && !insidequotes && !insidedoublequotes && isterminator(c))
						{
							if (first)
							{
								isregularword = false;
						...
						else
						{
							...
							else
							{
								if (isregularword)
								{
									if ((c == 32 || c == 9 || c == 10 || c == 13) && !result.IsEmpty() && !insidequotes && !insidedoublequotes && !insidebrackets)
										c = 0;
									else
										result.AppendUniChar(c);
									curpos++;
...
DB4DArrayOfValues* BuildArrayOfValuesFromString(VString s, sLONG expectedType, VCompareOptions& inOptions)
{
	...
	QuickDB4DArrayOfValues* result = CreateDB4DArrayOfDirectValues(expectedType, options, (VJSArray*)nil);
...
VError SearchTab::BuildFromString(const VString& input, VString& outOrderBy, BaseTaskInfo* context, EntityModel* model, bool keepmodel, QueryParamElementVector* qparams)
{
	...
	xbox_assert(model != nil);

	//bool remoteQuery = (model->QueriesAreProcessedRemotely());
	bool remoteQuery = false;
	VString finalQuery, attQueryPart, operQueryPart, valueQueryPart, cunjQueryPart, sProcessed;
			...
			if (waitforfield)
			{
				if (curkeyword == keyword_javascript)
				{
					if (fAllowJSCode)
					{
						...
						if (!wasInParenthesis)
						{
							err = ThrowError(VE_DB4D_PARENTHESIS_ERROR);
						}
						else
						{
					...
					{
						err = ThrowError(VE_DB4D_JS_NOT_ALLOWED_IN_THAT_QUERY);
					}
				}
				else
				{
					...
					else
					{
						...
						else
						{
							...
							if (!stop)
							{
								...
								VString remainPath;
								objpath.Clear();
								if (attpath.FromPath(model, s, true, &remainPath))
								{
									if (!remainPath.IsEmpty())
									{
										if (attpath.LastPart() != nil && attpath.LastPart()->fAtt->GetDataKind() == VK_BLOB_OBJ)
										{
											waitforfield = false;
											waitforoper = true;
											attQueryPart = s;
											objpath.FromString(remainPath);
										}
									}
									else
									{
			...
			else
			{
				...
				else
				{
					if (waitforvalue)
					{
						...
						VReal valueToComp;
						if (!wasInQuotes && !objpath.IsEmpty() && !s.IsEmpty() && s[0] != ':')
						{
							isnumValue = true;
							for (sLONG i = 0, nb = s.GetLength(); i < nb; ++i)
							{
								UniChar c = s[i];
							...
							if (isnumValue)
								valueToComp.FromString(s);
								
						}

						VValueSingle* pValue = &s;
						if (isnumValue)
							pValue = &valueToComp;

						if (keepmodel)
						{
							AddSearchLineEm(attpath, objpath, oper, pValue, false, checkfornull);
						}
						else
						{
							sLONG lastdestinstance = 0;
							bool simpleCheckForNull = false;
							...
							if (testAssert(attinst != nil))
							{
								...
								{
									if (next == nil && att->IsForeignKey() && checkfornull/* && (oper == DB4D_Like || oper == DB4D_NotLike)*/)
									{
										simpleCheckForNull = true;
										// if we query for null on a first level related attribute then we can optimize buy not doing a join
									}
									else
									{
								...
								kind = att->GetKind();
								uBOOL checkExists = 2;
								if (!simpleCheckForNull && (kind == eattr_relation_1toN || kind == eattr_relation_Nto1 || kind == eattr_composition))
								{
									if (checkfornull)
									{
								...
								if (att->GetKind() == eattr_computedField && !att->BehavesAsStorage())
								{
									if (att->GetScriptKind() == script_javascript && context != nil)
									{
										...
										if (objfunc != nil)
										{
													...
													if (qparams != nil)
													{
														...
														if (paramNum > 0 && paramNum <= qparams->size())
														{
															...
															else if (xqp->fType == DB4D_QPE_array)
															{
																...
																{
																	if (xqp->fJSONArray != nil)
																	{
																		VString s;
																		VJSONValue(xqp->fJSONArray).Stringify(s);
																		VJSJSON json(jscontext);
																		json.Parse(compValue,s);
																	}
																}
															}
														}
											...
											if (sresult.IsEmpty())
												err = ThrowError(VE_DB4D_WRONGFIELDREF); // en fait c'est le onQuery qui n'a pas retourne ce qu'il faut
											else
											{
										...
										{
											AttributePath finalpath;
											EntityAttributeInstance finalinst(att, 0);
											finalpath.Add(finalinst);
											...
											if (!s.IsEmpty() && s[0] == ':')
											{
												...
												AddSearchLineEm(finalpath, objpath, oper, s, options, lastdestinstance);
											}
											else
												AddSearchLineEm(finalpath, objpath, oper, pValue, false, checkfornull, lastdestinstance);
									...
									else
									{
										...
										if (sousQuery == nil || sousQuery->GetNbLine() == 0)
											err = ThrowError(VE_DB4D_WRONGFIELDREF);
										else
										{
								...
								{
									xbox_assert(att->GetKind() != eattr_alias);  // break here
								}

								if (checkExists != 2 || att->GetKind() == eattr_storage || att->BehavesAsStorage() || simpleCheckForNull)
								{
									okfield = true;
									...
									else
									{
										if (!s.IsEmpty() && s[0] == ':')
										{
											...
											options.SetDiacritical(false);
											if (oper == DB4D_IN)
												AddSearchLineEmArray(att, objpath, oper, s, options, lastdestinstance);
											else
												AddSearchLineEmSimple(att, objpath, oper, s, options, lastdestinstance);
										}
										else
										{
											if (oper == DB4D_IN)
											{
												...
												AddSearchLineEmArray(att, objpath, oper, values, options, lastdestinstance);
											...
											else
											{
												VCompareOptions options;
												options.SetDiacritical(false);
												AddSearchLineEmSimple(att, objpath, oper, pValue, options, lastdestinstance, checkfornull);
											}
										}
									}
								}
						...
						if (waitforcunj)
						{
							...
							else
							{
								...
								if (!stop)
								{
									...
									if (cunj == 0)
										err = ThrowError(VE_DB4D_WRONG_COMP_OPERATOR);
									else
									{
...
VError SearchTab::ThrowError(VError err, const VString* s1)
{
	if (fModel != nil)
		err = fModel->ThrowError(err, s1);
	else if (destFile != nil)
		err = destFile->ThrowError(err, DBaction_BuildingQueryFromString);
	else
	{
		if (s1 == nil)
			err = ThrowBaseError(err);
		else
			err = ThrowBaseError(err, *s1);
	}
	return err;
}
			...
			switch (tok->GetTyp())
			{
				case r_EMComp:
				{
					...
					if (! tokem->fParam.IsEmpty())
					{
						...
						if (!deja)
						{
							sLONG expectedType = tokem->fExpectedType;
							if (!tokem->fObjPath.IsEmpty())
								expectedType = VK_STRING;
							QueryParamElement* qpe = &fParamValues[tokem->fParam];
							if (qpe->fType != DB4D_QPE_scalar || qpe->fScalar == nil)
							{
								tokem->ch = (VValueSingle*) VValueSingle::NewValueFromValueKind(expectedType);
								if (qpe->fScalar == nil)
								{
									err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_NULL_OR_MISSING, VString(":")+tokem->fParam);
								}
								else
								{
									err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_WRONGTYPE, VString(":")+tokem->fParam);
								}
							}
							else
							{
								if (tokem->fSimpleDate && qpe->fSimpleDateScalar != nil)
									tokem->ch = qpe->fSimpleDateScalar->ConvertTo(expectedType);
								else
								{
									if (tokem->fSimpleDate && expectedType == VK_TIME && qpe->fScalar->GetValueKind() == VK_STRING)
									{
										VString* s = (VString*)qpe->fScalar;
										VectorOfVString arr;
										s->GetSubStrings(',', arr, false, false);
										if (arr.size() > 1)
											tokem->ch = arr[1].ConvertTo(expectedType);
										else
											tokem->ch = qpe->fScalar->ConvertTo(expectedType);
									}
									else
										tokem->ch = qpe->fScalar->ConvertTo(expectedType);
								}
					...
					if (! toksimple->fParam.IsEmpty())
					{
						...
						if (!deja)
						{
							...
							if (qpe->fType != DB4D_QPE_scalar || qpe->fScalar == nil)
							{
								...
								if (qpe->fScalar == nil)
								{
									err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_NULL_OR_MISSING, VString(":")+toksimple->fParam);
								}
								else
								{
									err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_WRONGTYPE, VString(":")+toksimple->fParam);
								}
							}
							else
							{
				...
				case r_LineEM:
				  {
					  ...
					  if (! toksimple->fParam.IsEmpty())
					  {
						  ...
						  {
							  QueryParamElement* qpe = &fParamValues[toksimple->fParam];
							  sLONG expectedType = toksimple->fExpectedType;
							  if (qpe->fType != DB4D_QPE_scalar || qpe->fScalar == nil)
							  {
								  if (expectedType == VK_BLOB_OBJ)
									  expectedType = VK_STRING;
								  toksimple->ch = (VValueSingle*) VValueSingle::NewValueFromValueKind(expectedType);
								  if (qpe->fScalar == nil)
								  {
									  err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_NULL_OR_MISSING, VString(":")+toksimple->fParam);
								  }
							  ...
							  else
							  {
								  ...
								  {
									  toksimple->ch = qpe->fSimpleDateScalar->ConvertTo(expectedType);
								  }
								  else
								  {
									  if (toksimple->fSimpleDate && expectedType == VK_TIME && qpe->fScalar->GetValueKind() == VK_STRING)
									  {
										  VString* s = (VString*)qpe->fScalar;
										  VectorOfVString arr;
										  s->GetSubStrings(',', arr, false, false);
										  if (arr.size() > 1)
											  toksimple->ch = arr[1].ConvertTo(expectedType);
										  else
											  toksimple->ch = qpe->fScalar->ConvertTo(expectedType);
									  }
									  else
									  {
										  if (expectedType == VK_BLOB_OBJ)
											  toksimple->ch = qpe->fScalar->Clone();
										  else
											toksimple->ch = qpe->fScalar->ConvertTo(expectedType);
									  }
								  }
				...
				case r_LineArray:
				{
					...
					if (! tokarray->fParam.IsEmpty())
					{
						...
						if (qpe->fType == DB4D_QPE_array && (qpe->fArray != nil || qpe->fJSONArray != nil))
						{
							...
							if (qpe->fArray != nil)
								...
								tokarray->values = GenerateConstArrayOfValues(qpe->fJSONArray, tokarray->fExpectedType, tokarray->fOptions);
						}
						else
						{
							if (qpe->fArray == nil && qpe->fJSONArray == nil)
							{
								err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_NULL_OR_MISSING, VString(":")+tokarray->fParam);
							}
							else
							{
								err = ThrowBaseError(VE_DB4D_QUERY_PLACEHOLDER_WRONGTYPE, VString(":")+tokarray->fParam);
							}
						}
					}
				}
...
VError SearchTab::From(const SearchTab& other)
{
	...
	fLines.resize(nb, nil);
	{
		...
		try
		{
			...
			fWithQueryPlan = other.fWithQueryPlan;
			fWithQueryPath = other.fWithQueryPath;
...
VJSONObject* SearchTab::ToJSON(BaseTaskInfo* context, UniChar wildcharReplace)
{
	VJSONObject* result = new VJSONObject();
	result->SetPropertyAsString("model", /*fModel->GetName()*/ fModel->GetBaseModelName());
	VJSONArray* arr = new VJSONArray();
	result->SetProperty("queryItems", VJSONValue(arr));

	for (RechTokenVector::iterator cur = fLines.begin(), end = fLines.end(); cur != end; ++cur)
	{
		VJSONObject* obj = (*cur)->ToJSON(context, wildcharReplace);
		if (obj != nil)
		{
...
VError SearchTab::BuildRefString(VString& outRef, bool canMeasure)
{
	VError err = VE_OK;
	Base4D	*bd;
	if (destFile != nil)
	{
		bd = destFile->GetOwner();
	}
	else
	{
		if (testAssert(fModel != nil))
		{
			bd = fModel->GetCatalog()->GetAssocBase();
			assert(bd != nil);
			if (bd == nil)
				return ThrowBaseError(VE_UNKNOWN_ERROR);
		}
		else
		{
			return ThrowBaseError(VE_UNKNOWN_ERROR);
	...
	outRef.Clear();
	for (RechTokenVector::iterator cur = fLines.begin(), end = fLines.end(); cur != end && err == VE_OK; ++cur)
	{
		err = (*cur)->BuildRefString(outRef, canMeasure, bd);
	}
	return err;
}
...
VError SearchTab::FromJSON(BaseTaskInfo* context, VJSONObject* jsonQuery)
{
	VError err = VE_OK;

	VString name;
	jsonQuery->GetPropertyAsString("model", name);
	xbox_assert(fModel->GetName() == name);
	VJSONValue val = jsonQuery->GetProperty("queryItems");
	if (val.IsArray())
	{
		VJSONArray* arr = val.GetArray();
		VSize nbitem = arr->GetCount();
		for (VIndex i = 0; i < nbitem && err == VE_OK; ++i)
		{
			VJSONValue val = (*arr)[i];
			if (val.IsObject())
			{
				VJSONObject* tokenobj = val.GetObject();
				sLONG toktyp;
				VString tok;
				if (tokenobj->GetPropertyAsString("tokenType", tok))
				{
					toktyp = ERechToken[tok];
					RechToken* rechtok = nil;
					switch (toktyp)
					{
						case r_ParentG:
						case r_ParentD:
						case r_Not:
							rechtok = new RechToken();
							break;

						case r_LineEM:
							rechtok = new RechTokenEMSimpleComp();
							break;

						case r_BoolOperator:
							rechtok = new RechTokenBoolOper();
							break;

						case r_LineEMArray:
							rechtok = new RechTokenEmArrayComp();
							break;

						case r_LineEMJoin:
							rechtok = new RechTokenEmJoin();
							break;

						case r_RecordEntityExist:
							rechtok = new RechTokenEntityRecordExist();
							break;

						case r_EmSel:
							rechtok = new RechTokenEmSel();
							break;
					...
					if (rechtok != nil)
					{
						err = rechtok->FromJSON(context, tokenobj);
						if (err == VE_OK)
						{
							fLines.push_back(rechtok);
						}
						else
							delete rechtok;
					}
						 ...
						 BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, LocalEntityModel* model)
{
	...
	if (!parseError)
	{
		if (cachefiche == nil)
		{
			...
			cachefiche->SetUsedAsCacheInQuery();
			if (tt != nil)
				tt->Release();
		}
						 ...
						 BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, LocalEntityModel* model)
{
	...
	if (curfic != nil && context != nil && model != nil && !parseError)
	{
		...
		if (erec != nil)
		{
			...
			if (!parseError)
			{
				VJSObject therecord(VJSEntitySelectionIterator::CreateInstance(jscontext, new EntitySelectionIterator(erec, context)));

				XBOX::VJSException excep;
				VJSValue outResult(jscontext);
				vector<VJSValue> Params;
				xbox_assert(funcobj.IsFunction());
...
void RechNodeScript::FullyDescribe(Base4D* bd, VString& result)
{
#if WITH_RTM_DETAILLED_ACTIVITY_INFO
	rt->expression->GetDescription(result);
#else
	Describe(bd, result);
#endif
}
...
static void BuildBeginIndexString(const IndexInfo* ind, sLONG newsel, VString& outString, const CDB4DQueryPathModifiers* inModifiers)
{
	...
	VString sind;
	ind->IdentifyIndex(sind, true, true, false);
	outString += sind;
							  ...
							  BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, LocalEntityModel* model)
{
	...
	cv=nil;
	xbox_assert((rt->numfile==0) || (rt->numfile==nf));
	
	cv=curfic->GetNthField(rt->numfield, err, false, true);
	
	if (!rt->fObjPath.IsEmpty())
	{
		VJSONValue val;
		cv->GetJSONValue(val, true);
		VJSONValue result;
		ObjectParser parser(val, rt->fObjPath, result);
		cv2 = result.CreateVValue(true);
		cv = cv2;
		if (!cv->IsNull() && rt->ch != nil)
		{
			if (cv->GetValueKind() != rt->ch->GetValueKind())
			{
				VValueSingle* cv3 = rt->ch->ConvertTo(cv->GetValueKind());
				delete rt->ch;
				rt->ch = cv3;
			}
		}
		...
		if (erec != nil)
		{
			...
			if (val != nil)
			{
				if (val->getVValue() != nil)
				{
					...
					{
						if (rt->ch != nil && !rt->ch->IsNull() && !cv->IsNull())
						{
							switch (rt->comparaison)
							{
							case DB4D_BeginsWith:
...
void RechNodeEmSeq::CalcTarget(sLONG nf, EntityModel* model)
{
	sLONG target = 0;
	if (rt->fAttPath.Count() > 0)
	{
		...
		model = attinst->fAtt->GetModel();
	}
	if (target != nf || rt->numinstance != 0 || model != nil)
	{
		SetTarget(target, rt->numinstance, model);
	}
}
...
void RechNodeWithArraySeq::FullyDescribe(Base4D* bd, VString& result)
{
	Describe(bd,result);
#if WITH_RTM_DETAILLED_ACTIVITY_INFO
	VIndex pos = result.Find(CVSTR("<Array Of Values>"));
	if (pos != -1)
	{
		XBOX::VJSONArray* textValues = values->ToJSON(nil, false);
		if (textValues)
		{
			VJSONWriter writer(JSON_Default);
			XBOX::VString s2;
			writer.StringifyArray(textValues,s2);
			
			result.SubString(1, pos - 1);
			result.AppendString(s2);
			QuickReleaseRefCountable(textValues);
		}
	}
...
VError RechNodeIndex::SubBuildCompFrom(sLONG comp, VValueSingle* val, VValueSingle* &val1, VValueSingle* &val2)
{
	val1 = nil;
	val2 = nil;
	switch (comp)
...
VError RechNodeIndex::BuildFrom(RechTokenSimpleComp *xrt, bool isObjInd)
{
	VError err = VE_OK;
	BTitemIndex *val;
...
VError RechNodeIndex::BuildFromMultiple(RechNode *IncludingFork, BaseTaskInfo* context)
{
	...
	xbox_assert(ind->MatchType(DB4D_Index_OnMultipleFields));
							  ...
							  BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, Bittab &Nulls, RechNode* &transformation)
{
...

	{
		...
		if (ind->GetTyp() == DB4D_Index_OnObj)
		{
			IndexInfoFromFieldObj* xind = dynamic_cast<IndexInfoFromFieldObj*>(ind);
			if (testAssert(xind != nil))
			{
				xind->Open(index_read);
				OccupableStack* curstack = gOccPool.GetStackForCurrentThread();
				VValueSingle* val1 = rt->ch;
				VValueSingle* val2 = rt2 == nil ? nil : rt2->ch;
				if (rt2 == nil)
				{
					SubBuildCompFrom(rt->comparaison, val1, val1, val2);
				}
				b = xind->FourcheOnPath(rt->fObjPath, checknulls, val1, xstrict1, val2, xstrict2, context, err, InProgress, fOptions, nil);
				xind->Close();
				if (b != nil && err == VE_OK)
				{
					if (mustinverse)
...
uBOOL RechNodeIndex::PeutFourche(RechNodeIndex* autre)
{
	...
	if (ind->MayBeSorted() && ind == autre->ind && rt->fObjPath.SamePath(autre->rt->fObjPath))
	{
...
VError RechNodeConst::Perform(OptimizedQuery *query, Bittab **super, Bittab* filtre, VDB4DProgressIndicator* InProgress,  VValueBag* curdesc,
							  BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, Bittab &Nulls, RechNode* &transformation)
{
	xbox_assert(false); // on ne devrait jamais arriver la
	Bittab *b = nil;
	VError err = VE_OK;
								 ...
								 BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, Bittab &Nulls, RechNode* &transformation)
{
	...
	xbox_assert(false); // on ne devrait plus jamais passer par la
...
CDB4DQueryPathNode* RechNodeOperator::BuildQueryPath(Boolean inBefore, Table* target, Base4D* bd, sLONG DejaSel, CDB4DQueryPathNode* parent, sLONG& curvar, sLONG &outvarnum, const CDB4DQueryPathModifiers* inModifiers)
{
	...
	else
	{
		...
		else
		{
			if (IsIndexe())
			{
				xbox_assert(left->IsIndexe() && !right->IsIndexe());
...

	{
		...
		else
		{
			...
			else
			{
				if (Nodes.GetCount() == 1)
				{
					err = Nodes[0]->Perform(query, super, filtre, InProgress, curdesc, context, HowToLock, exceptions, limit, Nulls, Nodes[0]);
				}
				else
				{
					uLONG start = query->GetStartingTime();
					...
					for (;cur != end && err == VE_OK && canperformindex; cur++)
					{
						if (testAssert(*cur != nil))
						{
							...
							else
							{
								if (b1 == nil)
								{
									err = (*cur)->Perform(query, &b1, filtre, InProgress, subline, context, HowToLock, exceptions, limit, Nulls, *cur);
								}
								else
								{
									b2 = nil;
										...
										else
										{
											if ((cur+1) != end)
											{
												...
												if ( ((sLONG8)recfiltre * ((sLONG8)CountBlobSources()*3+1)) < (tot/10))
												{
													...
													*cur = newnode;
													err = (*cur)->Perform(query, &b2, b1, InProgress, subline, context, HowToLock, exceptions, limit, Nulls, *cur);
													if (err == VE_OK)
													{
														err=b1->And(b2);
													}
									...
									{
										Bittab subNulls;
										err = (*cur)->Perform(query, &b2, filtre, InProgress, subline, context, HowToLock, exceptions, limit, subNulls, *cur);
										if (err == VE_OK)
										{
											err=b1->Or(b2);
											if (err == VE_OK)
...
VError JoinPath::AddJoin(const FieldArray& sources, const FieldArray& dests, sLONG xnuminstance1, sLONG xnuminstance2)
{
	VError err = VE_OK;
	JoinRefCouple couple;
	xbox_assert(sources.GetCount() > 0 && sources.GetCount() == dests.GetCount() );
	sLONG i = 0;
...
void RechNodeSubQuery::SolvePath(sLONG QueryTarget, sLONG QueryInstance, BaseTaskInfo* context)
{
	...
	if (!fUnSolved)
	{
		...
		xbox_assert(couple->fNumTable1 == QueryTarget && couple->fNumInstance1 == 0);
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						if (i == 0)
						{
							subqx.SetModel(fRoot->GetTargetModel());
							subq.SetTarget(subtarget, co->fNumInstance2);
							subq.SetModel(fRoot->GetTargetModel());
							subq.SetRoot(fRoot);
						}
						else
						{
							...
							xbox_assert(subtarget2 == subtarget);
					...
					QuickDB4DArrayOfValues* temp2 = CreateDB4DArrayOfDirectValues(subcri->GetTyp(), fOptions, (VJSArray*)nil );
					if (temp2 != nil)
					{
						if (sel != nil)
						{
		...
		if (fInd != nil)
		{
			...
			if (alternateway)
			{
				...
				if (err == VE_OK)
				{
					...
					subq.SetModel(GetTargetModel());
					subqx.SetModel(GetTargetModel());
					subq.SetRoot(fRoot);
								   ...
								   BaseTaskInfo* context, DB4D_Way_of_Locking HowToLock, Bittab *exceptions, sLONG limit, LocalEntityModel* model)
{
	...
	if (!fUnSolved)
	{
		...
		{
			cv=nil;
			xbox_assert(fFinalPath.GetCount()>0);
			sLONG numfield = fFinalPath.GetCouple(fFinalPath.GetCount()-1)->fNumField1;
			xbox_assert(fFinalPath.GetCouple(fFinalPath.GetCount()-1)->fNumTable1 == nf);
					...
					else
					{
						switch (tt)
						{
							case r_Sel:
								...
								{	
									rnsel->SetTarget(seltarget, ((RechTokenSel*)rt)->fNumInstance, nil);
								}
							...
							case r_EmSel:
								{
									...
									{
										rnsel->SetTarget(seltarget, ((RechTokenEmSel*)rt)->fNumInstance, EMsel->GetModel());
									}
							...
							case r_Line:
								...
								if (((RechTokenSimpleComp*)rt)->numfile != nf || ((RechTokenSimpleComp*)rt)->numinstance != fDefaultInstance)
									rnseq->SetTarget(((RechTokenSimpleComp*)rt)->numfile, ((RechTokenSimpleComp*)rt)->numinstance, nil);
								rn2 = rnseq;
							break;
							...
							case r_LineEM:
								...
								if (rnseq->rt->numfile != nf || rnseq->rt->numinstance != fDefaultInstance)
									rnseq->SetTarget(rnseq->rt->numfile, rnseq->rt->numinstance, ((RechTokenEMSimpleComp*)rt)->fAtt->GetModel() );
								rn2 = rnseq;
							break;
							...
							case r_LineArray:
								...
								if (((RechTokenArrayComp*)rt)->numfile != nf || ((RechTokenArrayComp*)rt)->numinstance != fDefaultInstance)
									rnarrayseq->SetTarget(((RechTokenArrayComp*)rt)->numfile, ((RechTokenArrayComp*)rt)->numinstance, nil);
								rn2 = rnarrayseq;
								break;
							...
							case r_LineEMArray:
								...
								if (rnarrayseq->rt->numfile != nf || rnarrayseq->rt->numinstance != fDefaultInstance)
									rnarrayseq->SetTarget(rnarrayseq->rt->numfile, rnarrayseq->rt->numinstance, ((RechTokenEmArrayComp*)rt)->fAtt->GetModel());
								rn2 = rnarrayseq;
								break;
							...
							case r_Script:
								...
								rnscript->SetTarget(((RechTokenScriptComp*)rt)->numtable, 0, nil);
								rn2 = rnscript;
								break;
							...
							case r_EMComp:
								...
								rnEmSeq->CalcTarget(nf, rnEmSeq->rt->fAttPath.LastPart()->fAtt->GetModel());
								rn2 = rnEmSeq;
								break;
							...
							case r_RecordExist:
								...
								if (((RechTokenRecordExist*)rt)->numfile != nf || ((RechTokenRecordExist*)rt)->numinstance != fDefaultInstance)
									rnRecExist->SetTarget(((RechTokenRecordExist*)rt)->numfile, ((RechTokenRecordExist*)rt)->numinstance, nil);
								rn2 = rnRecExist;
								break;
							...
							case r_RecordEntityExist:
								...
								if (rnRecExist->rt->numfile != nf || rnRecExist->rt->numinstance != fDefaultInstance)
									rnRecExist->SetTarget(rnRecExist->rt->numfile, rnRecExist->rt->numinstance, ((RechTokenEntityRecordExist*)rt)->fModel);
								rn2 = rnRecExist;
								break;
...
void OptimizedQuery::xAddToSubQuery(RechNodeArray *ListOfNodes, sLONG numtable, RechNode* rn, sLONG numinstance)
{
	...
	else
	{
		...
		if (found == nil)
		{
			...
			{
				xsub->SetTarget(numtable, numinstance, rn->GetTargetModel());
		...
		if (rn->GetTyp() == NoeudMultiOper)
		{
			...
			if (rnop->OperBool == DB4D_And)  // on va d'abord chercher tous les index composites sur une serie de AND
			{
				...
				for (; curm != endm; curm++)  // puis pour chque index composite on recherche si l'on trouve une combinasion des champs correspondant dans les AND
				{
					...
					xbox_assert(maxlevel<=kMaxCompositeParts);
					...
					if (found)
					{
						...
						xbox_assert(nbdeleted == maxlevel + (forkpos == 0 ? 0 : 1));
							...
							if (canbeindexed)
							{
										...
										if (xtab != nil)
										{
											...
											if (ind != nil)
											{
												...
												if (!ParseIndexSeq || (words.GetCount() == 1 && (*(words[0]) == *((VString*)rt->ch))))
												{
													...
													rnind->SetIndex(ind);
													rnind->BuildFrom(rt, false);
													rnind->SetTarget(rt->numfile, rt->numinstance, rn->GetTargetModel());
													rnind->SetParseAllIndex(ParseIndexSeq);
								...
								if (tryindexregular)
								{
									if ( ((rt->numfile==nf) || (rt->numfile==0)) && (rt->numinstance == 0) )
									{
										...
										if (ind == nil)
										{
											ind = fic->FindAndRetainIndexObj(rt->numfield, false, true, context);
											isObjInd = true;
										}
										if (ind != nil)
										{
									...
									else
									{
										...
										if (xtab != nil)
										{
											...
											if (ind == nil)
											{
												ind = xtab->FindAndRetainIndexObj(rt->numfield, false, true, context);
												isObjInd = true;
											}
											xtab->Release();
											if (ind != nil)
									...
									else
									{
										...
										if (xtab != nil)
										{
											...
											if (ind != nil)
											{
												...
												rninda->SetIndex(ind);
												rninda->BuildFrom(rta);
												rninda->SetTarget(rta->numfile, rta->numinstance, rn->GetTargetModel());
		...
		else
		{
			...
			else
			{
				if (rn->GetTyp() == NoeudSubQuery)
				{
					...
					if (((rnsub->GetTarget() == 0) || (rnsub->GetTarget() == nf)) && rnsub->GetInstance() == 0)
					{
						rn = rnsub->GetRoot();
						delete rnsub;
					}
...
RechNode* OptimizedQuery::xRegroupJoinPaths(RechNode* rn, bool checkForSubQueryAlone, BaseTaskInfo* context, sLONG subTarget, sLONG subInstance)
{
	if (VTask::GetCurrentFreeStackSize() < minStackSize)
	{
		ThrowBaseError(VE_DB4D_STACK_OVERFLOW);
	...
	if (rn != nil)
	{
		...
		if (rn->GetTyp() == NoeudMultiOper)
		{
			...
			if (rnop->OperBool == DB4D_And)
			{
							...
							if (rnsub->IsSolved())
							{
								...
								if (pathLen > 1) // on prend les niveaux superieurs a 1
								{
									...
									if (found == nil)
									{
										...
										if (jcouple.fNumInstance1 == subInstance && jcouple.fNumTable1 == subTarget)
										{
											found->SetTarget(jcouple.fNumTable2, jcouple.fNumInstance2, nil);
										}
										else
										{
											found->SetTarget(jcouple.fNumTable1, jcouple.fNumInstance1, nil);
										}
									...
									{
										rnsub->SetTarget(jcouple2.fNumTable2, jcouple2.fNumInstance2, nil);
									}
									else
									{
										rnsub->SetTarget(jcouple2.fNumTable1, jcouple2.fNumInstance1, nil);
									}
				...
				for (ArrayNode::Iterator cur = Nodes.First(), end = Nodes.End(); cur != end; cur++) // puis on fait une derniere passe pour la gestion recursive
				{
					RechNode* rn2 = *cur;
					if (rn2 != nil)
						*cur = xRegroupJoinPaths(rn2, false, context, subTarget, subInstance);
				}
			...
			else // pas d'optimisation possible sur les OU entre jointures
			{
				for (ArrayNode::Iterator cur = Nodes.First(), end = Nodes.End(); cur != end; cur++)
				{
					RechNode* rn2 = *cur;
					if (rn2 != nil)
						*cur = xRegroupJoinPaths(rn2, true, context, subTarget, subInstance);
				}
			}
		}
		else
		{
			if (rn->GetTyp() == NoeudNot)
			{
				...
				rnoper->SetLeft(xRegroupJoinPaths(rnoper->GetLeft(), true, context, subTarget, subInstance));
			...
			else
			{
				...
				{
					RechNodeSubQuery* rnsub = (RechNodeSubQuery*)rn;
					rnsub->SetRoot(xRegroupJoinPaths(rnsub->GetRoot(), true, context, rnsub->GetTarget(), rnsub->GetInstance()));
				}
			}
		}
	}
						...
						else
						{
							...
							if (rnind->isfourche)
							{
								...
								if (rn1->rt->numfile != nf || rn1->rt->numinstance != fDefaultInstance)
									rn1->SetTarget(rn1->rt->numfile, rn1->rt->numinstance, rn->GetTargetModel());
								
								rn2 = new RechNodeSeq;
								rn2->rt = rnind->rt2;
								if (rn2->rt->numfile != nf || rn2->rt->numinstance != fDefaultInstance)
									rn2->SetTarget(rn2->rt->numfile, rn2->rt->numinstance, rn->GetTargetModel());
					...
					else
					{
						if (seq && (rn->GetTyp() == NoeudWithArrayIndex))
						{
							...
							if (rna->rt->numfile != nf || rna->rt->numinstance != fDefaultInstance)
								rna->SetTarget(rna->rt->numfile, rna->rt->numinstance, rn->GetTargetModel());
...
VError OptimizedQuery::AnalyseSearch(SearchTab *model, BaseTaskInfo* context, bool forcompute)
{
	...
	if (fic != nil)
	{
		...
		model->BuildRefString(fRefStr, true);
		...
		if (!forcompute)
		{
			...
			{
				model->SetQueryPlan(true);
			}

			if (context != nil && context->ShouldBuildQueryPath())
			{
				model->SetQueryPath(true);
			}

			if (model->WithQueryPlan())
			{
		...
		err = model->ReplaceParams(context);
		...
		*/
			...
			root = xRegroupJoinPaths(root, true, context, nf, fDefaultInstance);
			...
			{
				Base4D *bd = fic->GetOwner();
				XBOX::VString* queryDescription = &fResult;
				if (model->WithQueryPlan())
				{
					if (root != nil)
						root->DisplayTree(bd, 0, queryDescription, fQueryPlan);
					if (gDebugDisplayRech)
					{
						DebugMsg(L"  \n\n");
						DebugMsg(fResult);
					...
					if (context != nil)
					{
						//if (context->ShouldBuildQueryPlan())
						context->SetQueryPlan(fQueryPlan);
						/*
						if (context->ShouldBuildQueryPath())
								   ...
								   DB4D_Way_of_Locking HowToLock, sLONG limit, Bittab* exceptions)
{
	...
	if (context != nil && context->ShouldBuildQueryPath() && fic->GetOwner()->GetStructure() != nil)
	{
		DescribeQuery(true);
	}
	...
	{
		if (fQueryPlan != nil)
			sel->SetQueryPlan(fQueryPlan);
		if (fExecutionDescription != nil)
			sel->SetQueryPath(fExecutionDescription);
	}
	...
	if (err == VE_OK && fic != nil && !fRefStr.IsEmpty())
	{
		sLONG8 now = VSystem::GetCurrentTime();
		fic->GetDF()->MeasureQuery(fRefStr, 1, now - fStartingTime);
	}
...
VError ColumnFormulas::ReadResultsFromStream(VStream* fromStream)
{
	...
	if (err == VE_OK)
	{
		xbox_assert(nb == fActions.GetCount());
