...
VError ComplexRech::AddBeginWhere()
{
	return AddGenericToken(rc_BeginWhere);
}


VError ComplexRech::AddBeginOn()
{
	return AddGenericToken(rc_BeginOn);
}


VError ComplexRech::AddEndOn()
{
	return AddGenericToken(rc_EndOn);
}
...
Bittab* SimpleQueryNodeJoin::PerformJoin(Bittab* sel, Bittab* selJoin, VDB4DProgressIndicator* InProgress, BaseTaskInfo* context, VError& err, VValueBag* curdesc)
{
	...
	if (err == VE_OK)
	{
		...
		if (!CanDoArrayIndexSearch)
		{
			...
			BitSel* TrueSelJoin = new BitSel(criJoin->GetOwner()->GetDF(), selJoin, NULL);
...
ComplexSelection* QueryNodeCompositeJoin::PerformComplexJoin(Bittab* filtre1, Bittab* filtre2, VDB4DProgressIndicator* InProgress, BaseTaskInfo* context, VError& err, DB4D_Way_of_Locking HowToLock, VValueBag* curdesc)
{
	...
	else
	{
		...
		if (filtre1 != nil)
		{
			TrueSel1 = new BitSel(df1, filtre1, NULL);
			if (TrueSel1 == nil)
				err = ThrowBaseError(memfull, DBaction_ExecutingComplexQuery);
		}
		else
		...
		if (err == VE_OK)
		{
			if (filtre2 != nil)
			{
				TrueSel2 = new BitSel(df2, filtre2, NULL);
				if (TrueSel2 == nil)
					err = ThrowBaseError(memfull, DBaction_ExecutingComplexQuery);
			}
			else
			...
			case rc_Line:
				{
					if (waitforstatement)
					{
						...
						{
							node->SetInWhere(fIsInWhere || (!fCurJoinLeft && !fCurJoinRight));
			...
			case rc_LineJoin:
				{
					if (waitforstatement)
					{
						...
						if (targets != nil)
						{
							...
							xbox_assert(!fIsInWhere);
							fCurJoinLeft = ((ComplexRechTokenJoin*)rt)->fLeftJoin;
							fCurJoinRight = ((ComplexRechTokenJoin*)rt)->fRightJoin;
						...
						if (err == VE_OK)
						{
							...
							if (err == VE_OK)
							{
								if (ref->fTargets.size() == 1 )
								{
									...
									{
										qnnot->SetInWhere(fIsInWhere || (!fCurJoinLeft && !fCurJoinRight));
	...
	*/
	{
		if (onemap->fNodes.size() == 1)
		{
			...
			if (targets->fTargets.size() == 1)
			{
				...
				{
					rn->SetInWhere(fIsInWhere || (!fCurJoinLeft && !fCurJoinRight));
		...
		else
		{
			...
			{
				rnroot->SetInWhere(fIsInWhere || (!fCurJoinLeft && !fCurJoinRight));
		...
		case QuerySimpleNode_Index:
			{
				if (seq)
				{
					...
					if (qnind->fNodeSeq == nil) // index composite
					{
						...
						{
							newand->SetInWhere(qnind->IsInWhere());
					...
					else
					{
						...
						if (canbeindexed)
						{
							...
							if (ind != nil)
							{
								...
								{
									qnind->SetInWhere(qnseq->IsInWhere());
					...
					else
					{
						if (rta->values->CanBeUsedWithIndex(GetContextIntl(context)))
						{
							...
							if (ind != nil)
							{
								...
								if (qnarrayindex != nil)
								{
									qnarrayindex->SetInWhere(qnarrayseq->IsInWhere());
