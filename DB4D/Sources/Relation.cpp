...
{
	xbox_assert(inSourceFields.GetCount() == inDestinationFields.GetCount() && inSourceFields.GetCount() > 1);
...
VError Relation::ActivateOneToMany(FicheInMem* rec, Selection* &result, BaseTaskInfo* context, Boolean OldOne, Boolean NoCache, DB4D_Way_of_Locking HowToLock,XBOX::VProgressIndicator* inProgress)
{
	ValPtr cv;
	VError err = VE_OK;
		...
		else
		{
			...
			if (result == nil)
			{
				...
				if (err == VE_OK)
				{
					...
					if (err == VE_OK)
					{
						result = query.Perform((Bittab*)nil, inProgress, context, err, HowToLock);
						if (result != nil && !NoCache)
						{
							DataTable* df = t->GetDF();
							if (df != nil)
...
VError Relation::GetFrom(VStream& buf, const Base4D* bd)
{
	...
	if (err == VE_OK)
	{
		...
		else
		{
			...
			xbox_assert(fSource == sourceField);
			xbox_assert(fSources.GetNth(1) == sourceField);
		}
		...
		if (fDestination == NULL && destField != NULL)	// sc 16/05/2008 ACI0057889
		{
			...
			xbox_assert(fDestinations.IsEmpty());
			fDestination->Retain();
		...
		{
			// sc 14/12/2010 should never occur
			xbox_assert(fDestination == destField);
			xbox_assert(fDestinations.GetNth(1) == destField);
		}
		...
		{
			xbox_assert(fSource != NULL && fDestination != NULL);
				...
				for (sLONG i = 1; i < nbfields; i++)  // on part de 1 car le premier element est deja dedans (fSource)
				{
					...
					if (err == VE_OK)
					{
						...
						if (ff != nil)
						{
							...
							{
								// sc 14/12/2010 should never occur
								xbox_assert(fSources.FindPos( ff) != -1);
								ff->Release();
							}
						}
					}
				...
				for (sLONG i = 1; i < nbfields; i++)  // on part de 1 car le premier element est deja dedans (fDestination)
				{
					...
					if (err == VE_OK)
					{
						...
						if (ff != nil)
						{
							...
							{
								// sc 14/12/2010 should never occur
								xbox_assert(fDestinations.FindPos( ff) != -1);
								ff->Release();
							}
						}
					}
...
{
	xbox_assert(fSources.GetCount() == fDestinations.GetCount());
...
Boolean RelationPath::BuildSubPath(BaseTaskInfo* context, Bittab* deja, Table* source_intermediaire, Table* finaldest, VError& err, Boolean Only_N_To_1, Boolean Only_Auto, sLONG curlevel, const vector<uBYTE>& inWayOfLocking)
{
	...
	if (curlevel < 8/* && OKTable(inWayOfLocking, source_intermediaire)*/)
	{
		...
		DepRelationArrayIncluded relsdep;
		source_intermediaire->CopyRelNto1Deps(relsdep);

		sLONG i,nb = relsdep.GetCount();
		...
		DepRelationArrayIncluded relsdep2;

		if (!res && !Only_N_To_1)
		{
			source_intermediaire->CopyRel1toNDeps(relsdep2);
			dejaget2 = true;
			nb = relsdep2.GetCount();

			for (i=1; i<=nb; i++)
			{
				Relation* rel2 = relsdep2[i];
