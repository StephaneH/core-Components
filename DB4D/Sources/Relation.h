...
class FieldNuplet : public Obj4D, public IObjCounter
{
	public:
		...
		inline void SetFieldNum(sLONG n, sLONG tablenum, sLONG fieldnum) 
		{ 
			fields[n].numfield = fieldnum; 
			fields[n].numfile = tablenum;
		};
...
class Relation : public Obj4D, public IObjCounter, public IDebugRefCountable, public IBaggable
{
	...
	public:
		...
		VError ActivateOneToMany(FicheInMem* rec, Selection* &result, BaseTaskInfo* context, 
			Boolean OldOne = false, Boolean NoCache = false, DB4D_Way_of_Locking HowToLock = DB4D_Do_Not_Lock,XBOX::VProgressIndicator* inProgressIndicator = nil);
