...
AdminRequest::AdminRequest(IHTTPResponse* inResponse, RIApplicationRef inApplicationRef, AdminRequestHandler* owner)
{
	fResponse = inResponse;
	fRequest = &(inResponse->GetRequest());
	fOwner = owner;
	fApplicationRef = inApplicationRef;
	fURL.ParseURL(fRequest->GetURL(), false);
	fInput = const_cast<VPtrStream*>(&(fRequest->GetRequestBody()));
	fOutput = &(fResponse->GetResponseBody());
	fOutput->SetCharSet(VTC_UTF_8 );
	fOutput->SetCarriageReturnMode(eCRM_NONE );
	fOutput->OpenWriting();
	fInput->OpenReading();
}
...
VError AdminRequest::ReturnAdminPage()
{
	VError err = VE_OK;
	fResponse->AddResponseHeader("Location", "/walib/WAF/admin/admin.html", true);
	err = fOutput->PutText("Admin Page");
	fResponse->SetResponseStatusCode(HTTP_MOVED_PERMANENTLY);
	return err;
}


VError AdminRequest::ExecuteRequest()
{
	VError err = VE_OK;
	const VString* s = fURL.NextPart();
	if (s != nil && *s == fOwner->GetPrefix() && fURL.IsValid())
	{
		s = fURL.NextPart();
		if (s == nil)
		{
...
VError AdminRequestHandler::SetPattern( const VString& inPattern)
{
...
VError AdminRequestHandler::GetPatterns( vector<VString>* outPatterns) const
{
	VTaskLock lock( &fMutex);
	outPatterns->push_back( fPattern);
	outPatterns->push_back( "(?i)/admin" );
	return VE_OK;
}


VError AdminRequestHandler::HandleRequest( IHTTPResponse* inResponse)
{
	AdminRequest req(inResponse, fApplicationRef, this);
	VError err = req.ExecuteRequest();
	return err;
}
...
void RestTools::GetServerAndBaseStats()
{

	bool ok = true;
	if (fUAGDirectory != nil)
	{
		VUUID adminid;
		if (fUAGDirectory->GetSpecialGroupID(CUAGDirectory::AdminGroup, adminid))
		{
			ok = okperm(fContext, adminid);
		}
	}
	if (ok)
	{
		...
		if (result == nil)
		{
			VJSONValue::sNull.Stringify(resultString, IsPrettyFormatting() ? JSON_PrettyFormatting : JSON_Default);
		}
		else
		{
			VJSONValue(result).Stringify(resultString, IsPrettyFormatting() ? JSON_PrettyFormatting : JSON_Default);
			result->Release();
		}
...
void RestTools::GetServerAndBaseInfo()
{
	...
	if (fUAGDirectory != nil)
	{
		...
		if (fUAGDirectory->GetSpecialGroupID(CUAGDirectory::AdminGroup, adminid))
		{
			if (fContext == nil)
			{
				CUAGSession* session = fResponse->GetRequest().GetAuthenticationInfos()->GetUAGSession();
				if (session != nil)
				{
					ok = session->BelongsTo(adminid, nil);
				}
			}
			else
			{
	...
	if (ok)
	{
		...
		if (s == nil || *s == "jsContextInfo")
		{
			GetJSContextInfos( bag);
		}
	...
	else
	{
		...
		err = ThrowBaseError(VE_DB4D_GETINFO_PERM);
	}
}
	...
	if (applicationIntf != nil)
	{
		...
		for (vector<VRefPtr<CUAGSession> >::iterator cur = sessions.begin(), end = sessions.end(); cur != end; ++cur)
		{
			...
			if (user != nil)
			{
				...
				ReleaseRefCountable(&user); // sc 05/12/2014 WAK0086560
			}
...
void RestTools::GetJSContextInfos( VValueBag& outBag) const
{
	IDB4D_ApplicationIntf *applicationIntf = VDBMgr::GetManager()->GetApplicationInterface();
	if (applicationIntf != nil)
	{
		applicationIntf->GetJSContextInformations( fApplicationRef, outBag);
	}
}
...
bool RestTools::ExtractKey(const VString* s, VString& outModelName, VectorOfVString& allIDs)
{
	VString valstr;
	bool withkeys = false;
	...
	{
		withkeys = true;
		s->GetSubString(1, p1 - 1, outModelName);
...
void RestTools::AnalyseGetModelAndEntities()
{
	if (labase != nil)
	{
		...
		bool withkeys = ExtractKey(s, tablename, allIDs);
		...
		if (em == nil)
		{
			if (!withkeys)
			{
				EntityModelCatalog* cat = labase->GetCatalog(tablename);
				if (cat != nil)
				{
					s = url.NextPart();
					if (s != nil)
					{
						withkeys = ExtractKey(s, tablename, allIDs);
						em = cat->FindEntity(tablename);
					}
				}
			}
			if (em == nil)
...
void RestTools::AnalyseGetExpandAttributes()
{
	...
	if (attributes.size() == 1 && !getDef && err == VE_OK)
	{
		...
		ValueKind datakind = att->GetDataKind();
		if (datakind == VK_IMAGE)
		{
			onepictfield = true;
		}
		else if (datakind == VK_BLOB || datakind == VK_BLOB_DB4D)
		{
			oneblobfield = true;
		}
	}
...
void RestTools::AnalyseGetOtherStuff()
{
	...
	if (url.GetValue(rest::binary, ss) && err == VE_OK)
	{
		if (ss == "true" || ss == "1")
		{
			waitforblobbinary = true;
		}
	}
...
void RestTools::CallMethod(EntityCollection* sel, EntityRecord* erec, DataSet* inDataSet)
{
		...
		if (err == VE_OK)
		{
			...
			if (applicationIntf != nil)
			{
				...
				if (localEm != nil)
				{
					...
					CDB4DSelection* ioselection = nil;
					...
					err = applicationIntf->Call4DMethod(meth->GetName(), params, table, &ioselection, record, result, fContext);

					if (err == VE_OK)
					{
						if (ioselection != nil)
						{
							LocalEntityModel* selectedModel = nil;

							sLONG tablenum = 0;
							if (result.IsObject())
								result.GetObject()->GetPropertyAsNumber("__$TABLE", &tablenum);
							if (tablenum != 0)
							{
								LocalEntityModelCatalog* cat = dynamic_cast<LocalEntityModelCatalog*>(tt->GetOwner()->GetGoodEntityCatalog(false));
								if (cat != nil)
								{
									selectedModel = dynamic_cast<LocalEntityModel*>(cat->RetainEntityModelByTableNum(tablenum));
								}

								//tt->GetOwner()->FindTable
							}
							if (selectedModel != nil)
							{
								VJSException	emptyException;
								VJSContext jscontext(fJSGlobalContext);
								VJSValue resultjs(jscontext);
								resultjs.SetNull();
								ReturnJSResult(err, resultjs, emptyException, inDataSet, ioselection, selectedModel);
							}
							else
								err = ThrowBaseError(VE_DB4D_NOT_A_LOCALENTITYMODEL);

							QuickReleaseRefCountable(selectedModel);
						}
						else
						{
							PutJsonBeginObject();
							PutJsonPropertyName(kPropResult);
							VString outJson;
					...
					else
					{
						...
						if (errors != nil)
						{
							...
							PutJsonPropertyName(kPropError);
							PutText(errjson);			
						}
						PutJsonEndObject();
					}

					QuickReleaseRefCountable(ioselection);
	...
	else
	{
		...
		VJSException	emptyException;
		ReturnJSResult(err, result, emptyException, inDataSet, nil, nil);
	}
}


void RestTools::ReturnJSResult(VError err, VJSValue& result, VJSException& excep, DataSet* inDataSet, CDB4DSelection* selFrom4D, LocalEntityModel* localEm)
{
	VJSContext jscontext( fJSGlobalContext);
	if (err == VE_OK && excep.IsEmpty() )
	{
		...
		else
		{
			...
			if (xsel != nil || selFrom4D != nil)
			{
				if (selFrom4D != nil)
				{
					Selection* sel4D = dynamic_cast<VDB4DSelection*>(selFrom4D)->GetSel();
					xsel = new LocalEntityCollection(localEm, sel4D);
				}
				...
				if (testAssert(resModel != nil))
				{
					...
					err = SelToJSON(true, resEM, xsel, attributes/*resAtts*/, resExpandAttributes, nil, true, skipfirst, countelem, newset, true);
				}
	...
	else
	{
		...
		else
		{
			...
			JSON.Stringify(excepval, outJson);
			if (outJson == L"undefined")
				outJson = L"null";
			PutJsonPropertyName(kPropError);
			PutText(outJson);
...
void RestTools::RetrieveOrDeleteEntitySel()
{
	...
	else
	{
		...
		*/
		...
		previousCollection = RetainRefCountable(sel);
		...
		if (url.GetValue(rest::filter, querystring) || url.GetValue("$query", fDeveloppedQueryString))
		{
			...
			{
				Boolean olddescribe = fContext->ShouldBuildQueryPath();
							...
							else
							{
								...
								else
								{
									if (fToArray)
									{
										...
										EntityCollection* prevsel = nil;
										VString retainpos;
										if (url.GetValue(rest::retainPositions, retainpos))
										{
											if (retainpos == "1" || retainpos == "true")
											{
												if (previousCollection == nil)
												{
													StErrorContextInstaller errs(false);
													VError err2;
													previousCollection = em->SelectAllEntities(fContext, &err2);
												}
												prevsel = previousCollection;
											}
										}
										arr = sel->ToJsArray(fContext, jscontext, attributes, expandattributes, SortingAtt, !nokey, false, skipfirst, countelem, err, prevsel);
									...
									else if (fToXML)
									{
										...
										err = SelToJSON(true, em, truesel, attributes, expandattributes, SortingAtt, true, skipfirst, countelem, newset, true);
								}
							}
						}
					}
...
void RestTools::GetDistinctValues(EntityCollection* sel)
{
	...
	if (att != nil)
	{
		if (!att->permissionMatch(DB4D_EM_Read_Perm, fContext))
		{
			err = att->ThrowError(VE_DB4D_NO_PERM_TO_READ_ATTRIBUTE_VALUE);
			fContext->SetPermError();
		}
		else
		{
	...
	if (em != nil)
	{
		...
		if (erec != nil)
		{
			...
			else
			{
								...
								if (err == VE_OK)
								{
									if (emsub != nil)
									{
										...
										if (err == VE_OK)
										{
											...
											VString reselect;
											if (url.GetValue(rest::reselect, reselect))
											{
												VectorOfVString vals;
												vals.push_back(reselect);
												RecIDType pos = selsub->FindKey(vals, fContext, err);
												if (pos >= 0)
												{
													newwafkeepselptr = new WafSelection();
													newwafkeepselptr->rows.push_back(pos);
												}
											}
											...
											if (fToXML)
											{
												...
												err = SelToJSON(true, emsub, selsub, atts, subexpand, nil, true, skipitem, countitem, newset, true);
										}
									}
								}
								QuickReleaseRefCountable(newset);
				...
				{
					if (oneblobfield && waitforblobbinary)
					{
						...
						if (val != nil && val->GetKind() == eav_vvalue)
						{
							...
							if (cv != nil && (cv->GetValueKind() == VK_BLOB || cv->GetValueKind() == VK_BLOB_DB4D))
							{
								if (restmethod == RestMethodeDelete)
								{
									cv->Clear();
								...
								else
								{
									...
									if (blob != nil)
									{
										VSize bloblen = blob->GetSize();
										VMemoryBuffer<> buff;
										if (buff.SetSize(bloblen))
										{
											err = blob->GetData(buff.GetDataPtr(), bloblen);
											if (err == VE_OK)
											{
												err = fOutput->PutData(buff.GetDataPtr(), bloblen);
												if (err == VE_OK)
												{
													toJSON = false;
													VString mime = "application/octet-stream";
													fResponse->AddResponseHeader(L"Content-Type", mime, true);
											...
											err = ThrowError(VE_DB4D_MEMFULL);
									}
								}
							}
						}
					}
					else if (onepictfield && waitforimagemime)
					{
						EntityAttributeValue* val = erec->getAttributeValue(attributes[0].fAttribute, err, fContext);
						if (val != nil && val->GetKind() == eav_vvalue)
						{
...
void RestTools::WorkOnData()
{
	switch (restmethod)
	{
		...
		case RestMethodeUpdate:
			{
				if (oneblobfield && em != nil)
				{
					if (!fEntityKey.IsEmpty())
					{
						fInput->ResetLastError();
						fInput->SetPos(0);
						sLONG8 nbbytes = fInput->GetSize();
						EntityRecord* erec = em->findEntityWithPrimKey(fEntityKey, fContext, err, DB4D_Do_Not_Lock);
						if (erec != nil)
						{
							const EntityAttribute* att = attributes.begin()->fAttribute;
							sLONG scalartype = att->ComputeScalarType();
							if (scalartype == VK_BLOB || scalartype == VK_BLOB_DB4D)
							{
								EntityAttributeValue* val = erec->getAttributeValue(att, err, fContext);
								if (val != nil)
								{
									...
									if (cv != nil)
									{
										...
										{
											VMemoryBuffer<> buff;
											buff.SetSize(nbbytes);
											VSize realNbBytes = nbbytes;
											err = fInput->GetData(buff.GetDataPtr(), &realNbBytes);
											if (err == VE_OK)
											{
												err = blob->PutData(buff.GetDataPtr(), realNbBytes);
												if (err == VE_OK)
												{
													val->Touch(fContext);
				...
				else if (onepictfield && em != nil)
				{
					if (!fEntityKey.IsEmpty())
					{
						fInput->ResetLastError();
						fInput->SetPos(0);
						VString rawPictType, streamedpict;
						if (url.GetValue("$streamedpict", streamedpict) && (streamedpict == "true"))
						{
							VPicture pict;
							err = pict.ReadRawFromStream(fInput);
							if (err == VE_OK)
							{
								EntityRecord* erec = em->findEntityWithPrimKey(fEntityKey, fContext, err, DB4D_Do_Not_Lock);
								if (erec != nil)
						...
						else if (url.GetValue(rest::rawPict, rawPictType))
						{
							...
							if (decoder != nil)
							{
								sLONG nbbytes = (sLONG)fInput->GetSize();
								sLONG curpos = (sLONG)fInput->GetPos();
...
void RestTools::ExecuteStaticRestMethod(const VString& JSNameSpace)
{
	...
	if (s != nil)
	{
		...
		else if (JSNameSpace == "RestDirectoryAccess" && lasuite == "logout")
		{
			const IAuthenticationInfos* auth =  fResponse->GetRequest().GetAuthenticationInfos();
			//if (auth != nil && (auth->GetAuthenticationMethod() == AUTH_BASIC || auth->GetAuthenticationMethod() == AUTH_DIGEST))
			{
				if ( fContext != nil )
					fContext->SetPermError();
		...
		jscontext.EvaluateScript(funcName, nil, &result,nil);
		...
		if (result.IsObject())
		{
			...
			if (funcobj.IsFunction())
			{
				...
				if (scope != "private")
				{
					...
					if (!JsonMethodParams.IsEmpty())
					{
						json.Parse(params,JsonMethodParams, &excep);
					}
					if (excep.IsEmpty())
					{
						...
						if (JSNameSpace == "RestDirectoryAccess" && (lasuite == "login" || lasuite == "loginByPassword" || lasuite == "loginByKey"))
						{
							if (err != VE_OK || !excep2.IsEmpty())
							{
								if (fContext != nil)
									fContext->SetPermError();
								SetHTTPError(rest::http_perm_error);
	...
	ReturnJSResult(err, result2, excep2, nil, nil, nil);
...
void RestTools::ExecuteRequest()
{
	...
	if (err != VE_OK)
	{
		...
		//if (GetHTTPError() != 401)
			GenereErrorReport(false);
		TransfertBufferToOutpout(); // 03-Jul-2014 - Put Error Report in Output Stream
		HTTPStatusCode status = (HTTPStatusCode)GetHTTPError();
		if (status == HTTP_OK || status == HTTP_UNDEFINED)
			status = HTTP_INTERNAL_SERVER_ERROR;
		fResponse->SetResponseStatusCode(status);
	}
	else
	{
		TransfertBufferToOutpout();
		GenerateInfoIfEmpty();
		if (GetHTTPError() == 0)
...
VError RestRequestHandler::HandleRequest( IHTTPResponse* inResponse)
{
...
#if debuglr == 10
	VString contexthexa;
	contexthexa.FromHexLong((uLONG8)baseContext);
	if (within4D)
	{
		VString s;
		restcontexts.debugDump(s);
		DebugMsg("URL = " + inResponse->GetRequestURL() + " : context = " + contexthexa + "\n");
		DebugMsg(s);
	}
	...
	if (within4D)
	{
		VString s;
		restcontexts.debugDump(s);
		DebugMsg("release : " + strrelease + " , URL = " + inResponse->GetRequestURL() + " : context = " + contexthexa + "\n");
		DebugMsg(s);
	}
#endif
