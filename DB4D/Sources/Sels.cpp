...
void Selection::SetSelAddr(DataAddr4D addr, IndexInfo* ind)
{		
	SelisCluster=true;
	fInd = ind;
	setaddr(addr);
...
#if 0
					// that whole part is disabled because primary keys cannot be updated in real time and may lead to inconsitencies
					...
					if (!DeleteDejaFait)
					{
						if (!willBeLogged && !target->HasSyncInfo() && (!parentfile->GetTable()->AtLeastOneBlob()) && GetCurrentTransaction(context) == nil && !db->IsThereATrigger(DB4D_DeleteRecord_Trigger, parentfile->GetTable()))
						{
							if (target->NonIndexed() || (parentfile->GetNbRecords(context, false)/3) < GetQTfic())
							{
								...
								VDB4DSelectionInfoCollector  infoCollector(context, parentfile, VDB4DBasicInfoCollector::kDeleteRecords);
								collector = &infoCollector;
	#endif //WITH_RTM_DETAILLED_ACTIVITY_INFO
								if (InProgress != nil)
								{
									InProgress->BeginSession(GetQTfic(), GetString(1005, 23), true, collector);
								}
...
#if WITH_RTM_DETAILLED_ACTIVITY_INFO
		//Automatically registers self as default info collector and unregisters in destructor
		VDB4DSelectionInfoCollector collector(context, cri, ind, InProgress, VDB4DBasicInfoCollector::kDistinctValues);
#endif // WITH_RTM_DETAILLED_ACTIVITY_INFO
		...
		*/
		{
			...
			if (fTabMem!=nil)
			{
				...
				fInd->GetHeader()->MeasureWrite(1, tot);
			}
			outSizeSaved = tot;
		}
	}
...
bool BitSel::SaveObj(VSize& outSizeSaved)
{
	if (SelisCluster)
	{
		...
		*/
			...
			if (outSizeSaved > 0)
				fInd->GetHeader()->MeasureWrite(1, outSizeSaved);
	}
	else
	{
		outSizeSaved = 0;
...
void BitSel::SetSelAddr(DataAddr4D addr, IndexInfo* ind)
{
	Selection::SetSelAddr(addr, ind);
	pTB->SetAddr(addr,db);
}
...
{
	sLONG found = -1;
	if (pTB->isOn(inRecordID))
	{
		found = pTB->CountHowManyBitAllumeAt(inRecordID);
	}
	return found;
}
...
VError BitSel::TransformIntoCluster(DataAddr4D addr, IndexInfo* ind)
{
