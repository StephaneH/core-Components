		...
		trackDebugMsg("before delete selection of records\n");
			...
			if (parentfile != nil)
			{
				...
				if (!target->HasSyncInfo() && !(target->GetOwner()->ReferentialIntegrityEnabled() && target->AtLeastOneReferentialIntegrity()) )
				{
					...
					if (!DeleteDejaFait)
					{
						if (!target->HasSyncInfo() && (!parentfile->GetTable()->AtLeastOneBlob()) && GetCurrentTransaction(context) == nil && !db->IsThereATrigger(DB4D_DeleteRecord_Trigger, parentfile->GetTable()))
						{
				...
				while (currec != -1)
				{
...

					{
						...
						if (rec != nil)
						{
							...
							err2 = parentfile->DelRecord(rec, context);
							if (RefIntRel != nil)
	...
	if (/*parentfile != nil && */GetQTfic() > 0)
	{
		...
		if (err == VE_OK)
		{
			...
			else
			{
				...
				{
					/*
					FicheOnDisk* ficD = parentfile->LoadNotFullRecord(currec, err, DB4D_Do_Not_Lock, context, false, buf.GetBuffer());
					if (ficD != nil)
					{
						err = job.WriteOneRecord(ficD, context);
						ficD->Release();
					}
					*/
...
PetiteSel::~PetiteSel()
{
	...
	{
		assert(!fIsRemote && !IsOwnerRemote());
...
VError PetiteSel::LibereEspaceDisk(VDB4DProgressIndicator* InProgress)
{
	sLONG len;
	DataAddr4D ou;
	VError err = VE_OK;
...
VError BitSel::LibereEspaceDisk(VDB4DProgressIndicator* InProgress)
{
	if (SelisCluster)
	{
		return pTB->LibereEspaceDisk(InProgress);
	}
