...
class _GCCAlign TypeSortElemOffsetArray
{
	public:
		...
		//inline void SetLastElemNull() { xbox_assert(fNbElem > 0); fOffsets[fNbElem-1]->SetDataNull(); };
...
class _GCCAlign TypeSortElemArray
{
	public:
		...
		// VError GenereObjectIndex(IndexInfo* ind, sLONG MaxElemsInPage, sLONG RequiredElemsInPage, BaseTaskInfo* context, VDB4DProgressIndicator* InProgress, sLONG typ, sLONG TypeSize);
...
class Selection : public ObjInCacheMem, public IObjToFlush, public IChainable<Selection>
{
	public:
		...
		virtual void SetSelAddr(DataAddr4D addr, IndexInfo* ind);
		...
		virtual VError TransformIntoCluster(DataAddr4D addr, IndexInfo* ind) = 0;
...
class PetiteSel : public Selection
{
	public:
		...
		virtual VError TransformIntoCluster(DataAddr4D addr, IndexInfo* ind)
		{
			SetSelAddr(addr, ind);
			return VE_OK;
		};
