...
VError ServerManager::Start(uWORD inPortNumber, bool inSSL)
{
	VError err = VE_OK;
	...
	if (err == VE_OK)
	{
		...
		else
		{
			...
			vtcpcListener->SetName( CVSTR( "DB4D Server"));
			vtcpcListener->SetKind( kDB4DTaskKind_Listener);
...
enum VConnectionHandler::E_WORK_STATUS DB4DConnectionHandler::Handle ( VError& outError )
{
	...
	if (err == VE_OK)
	{
		...
		if (err == VE_OK)
		{
			...
			{
				xbox_assert(tag == kTagReOpenConnection);
				sLONG len = 0;
				...
				if (err == VE_OK)
				{
					...
					if (err == VE_OK)
					{
						fContext = VDBMgr::GetManager()->StealPostPonedContext(sID, receivedClientID);
						if (fContext == nil)
							err = memfull;
					}
				}
	...
	if (err == VE_OK)
	{
		...
		{
			// ACI0079316 12-Jun-2013 DH
			// If we could not register the task it means the user doesn't exist on the server anymore and we must reject the connection
			bool registerOk = applicationIntf->RegisterVTaskForRemoteUID(currentTask, receivedClientID, fContext->GetID());
			if (!registerOk)
			{
				err = VE_ACCESS_DENIED;
			}
		}
	}
		...
		while (state != TS_DYING && state != TS_DEAD && err == VE_OK && !fShouldStop && !postpone) 
		{
			...
			if (err == VE_OK)
			{
				...
				if (reqid == req_PostPone)
				{
					...
					sID.Validate(len);
					if (err == VE_OK)
						VDBMgr::GetManager()->KeepPostPonedContext(sID, fContext, receivedClientID);
					if (err == VE_OK)
					{
						char c = 1;
...
VError TCPOutputStream::DoFlush()
{
	...
	xbox_assert(fBuffer.GetDataSize() >= 6);
...
VTCPEndPoint* DB4DClientSession::Restore ( VTCPEndPoint& inEndPoint, VError& outError )
{
	...
	if (err == VE_OK && newEndPoint != nil)
	{
		...
		if ( err != VE_OK && err != VE_SRVR_SESSION_PROTOCOL_FAILED )
			err = ThrowBaseError(VE_SRVR_SESSION_PROTOCOL_FAILED, DBaction_ServerConnecting);
	}
...
DB4DNetManager* DB4DNetManager::NewConnection(const VString& inServerName, uWORD inPortNum, VError& outerr, bool inSSL)
{
