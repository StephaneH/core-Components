...
class ServerManager : public VObject, public IRefCountable
{
	public:
		...
		VError Start(uWORD inPortNumber, bool inSSL = false);
		void Stop();
...
{
	public:
		inline DB4DNetManager(VTCPEndPoint* inEndPoint, const VString& inServerName, uWORD inPortNum, bool inSSL) 
		{ 
		...
		static DB4DNetManager* NewConnection(const VString& inServerName, uWORD inPortNum, VError& outerr, bool inSSL = false);
