											...
											VDB4DProgressIndicator* InProgress, Boolean TestUnicite, sLONG TypeSize, IndexInfo* ind)
{
	...
	if (OKLock)
	{
		...
		if (err == VE_OK)
		{
			...
			uLONG8 taillemax = (uLONG8)kMAX_VSize;
							...
							if (tempfolder != nil)
							{
								...
								else
								{
									...
									if (!stop)
									{
														...
														while (!stopsub)
														{
															...
															else
															{
																...
																else
																{
																	...
																	if (err == VE_OK)
																		assert(((char*)olddest) + len == (char*)dest);
																	else
																	{
																		stop = true;
																		break;
										...
										//assert(debugcount2 == count);
										//assert(debugcount1 == count);
										ReleaseRefCountable(&filtre);
	...
	{
		{
			...
			if (level > 1)
			{
				...
				else
				{
					...
					page->SetSousPage(0, sousBT);
					assert(sousBT->GetNKeys() >= HalfPage);
			...
			while (curelem < count && nbkey < nbkeytobuild && err == VE_OK)
			{
				...
				if (filedata != nil)
				{
					...
					if (err == VE_OK)
					{
						...
						sLONG len = 0;
						err = filedata->GetLong(len);
						assert(len > 0 && len < kAuxDataBufferSize);
						if (err == VE_OK)
						{
