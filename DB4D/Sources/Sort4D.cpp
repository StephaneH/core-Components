...
Boolean SortTab::AddAttribute(EntityAttribute* inAtt, Boolean ascendant)
{
	...
	LocalEntityModel* locModel = dynamic_cast<LocalEntityModel*>(inAtt->GetOwner());
	if (locModel != nil)
	{
		db4dEntityAttribute* localAtt = (db4dEntityAttribute*)inAtt;
		Field* cri = localAtt->RetainField(true);
		if (cri != nil)
			cri->MeasureSortCount();
	}
	xli.att = inAtt;
			...
			do 
			{
				...
				for (; cur != end; cur++)
				{
					switch (cur->GetDataType())
					{
						...
						case VK_BLOB_OBJ:
							result = (CompareResult)ObjAttKey::Compare(p1, p2, fHolder->fOptions);
							break;
					}
...
template<>
bool SortPredicate<ObjAttKey>::operator ()(const TypeSortElem<ObjAttKey>& val1, const TypeSortElem<ObjAttKey>& val2) const
{
	CompareResult compres = val1.value.CompareTo(val2.value, fHolder->fOptions);
...
CompareResult Compare2SortElems(void* xp1, void* xp2, const xMultiFieldDataOffsets& criterias, const VCompareOptions& inOptions)
{
	...
	for (; cur != end; cur++)
	{
		...
		case VK_BLOB_OBJ:
			result = (CompareResult)ObjAttKey::Compare(p1, p2, inOptions);
			break;
		}
...
typedef unordered_map_VString<sLONG> mapOfIDs;
											...
											VDB4DProgressIndicator* InProgress, Boolean TestUnicite, sLONG TypeSize, IndexInfo* ind)
{
	...
	if (criterias.WithExpression() || criterias.WithAttributes() || criterias.WithObjects())
	...
	if (pourindex && criterias.WithObjects())
	...
	if (OKLock)
	{
		...
		if (err == VE_OK)
		{
			...
			if (criterias.WithExpression() || PourKeyWords || criterias.WithObjects())
			{
				data = nil;
				tailledemandee = -1;
			}
			...
			// sorting is done in one pass in memory
			...
			if (err == VE_OK && !assez_de_memoire)  
			{
				...
				else
				{
					...
					// sorting is done in several passes by merging files
							...
							if (tempfolder != nil)
							{
								...
								else
								{
									...
									if (!stop)
									{
										...
										IndexInfoFromFieldObj* indexobj = nil;
										OccupableStack* curstack = gOccPool.GetStackForCurrentThread();

										if (criterias.WithObjects())
										{
											indexobj = dynamic_cast <IndexInfoFromFieldObj*>(ind);
											xbox_assert(indexobj != nil);
										}
										...
										while (i >= 0 && !stop)
										{
											...
											{	
												...
												if (fic != nil)
												{
													...
													ObjectNodeCollection objectnodes;
													if (PourKeyWords)
													{
													...
													{
														xMultiFieldDataOffset_constiterator cri = criterias.Begin();
														VValueSingle* objatt = fic->GetNthField(cri->GetNumCrit(), err, false, true);
														VBlobObj* cvobj = dynamic_cast<VBlobObj*>(objatt);
														if (testAssert(cvobj != nil))
														{
															VJSONValue jval;
															cvobj->GetJSONValue(jval);
															ObjectParser parser(jval, objectnodes);
														}
														stopsub = objectnodes.empty();
													}
													...
													if (err == VE_OK)
													{
														while (!stopsub)
														{
															...
															{
																VString path = *(objectnodes[curkeyword].GetPath());
																//curpathid = indexobj->FindPathID(curstack, path, context, fOptions);
																mapOfIDs::iterator found = pathIDs.find(path);
																if (found == pathIDs.end())
																{
																	++nextID;
																	curpathid = nextID;
																	pathIDs[path] = curpathid;
																}
																else
																	curpathid = found->second;
																
																len = ObjAttKey::CalculateLength(curpathid, objectnodes[curkeyword].GetValue());
																//len = indexobj->CalculatePureKeyLength();
															}
															else
															{
															...
															else
															{
																...
																{
																	dest = ObjAttKey::PutInto(dest, curpathid, objectnodes[curkeyword].GetValue());
																}
																else
																{
																	void* olddest = dest;
															...
															{
																curkeyword++;
																if (curkeyword >= objectnodes.size())
																	stopsub = true;
															}
															else
															{
										...
										if (!stop) // si tout c'est bien passe pendant la creation des differentes parties a merger
										{
											if (criterias.WithObjects())
											{
												VectorOfVString sortedPathIDs;
												xbox_assert(nextID == (sLONG)pathIDs.size());
												sortedPathIDs.resize(nextID);
												for (mapOfIDs::iterator cur = pathIDs.begin(), end = pathIDs.end(); cur != end; ++cur)
												{
													sLONG id = cur->second;
													sortedPathIDs[id-1] = cur->first;
												}
												sLONG verifID = 0;
												for (VectorOfVString::iterator cur = sortedPathIDs.begin(), end = sortedPathIDs.end(); cur != end; ++cur)
												{
													++verifID;
													sLONG id = indexobj->FindPathID(curstack, *cur, context, fOptions);
													xbox_assert(id == verifID);
												}
											}
												...
												else
												{
													...
													else
													{
														...
														if (err == VE_OK)
														{
															...
															{
#if debuglr
																if (!PourKeyWords && !criterias.WithObjects())
																	xbox_assert(count == qt);
#endif
																count = qt;
																result = true;
...
/*
template <class Type>
VError TypeSortElemArray<Type>::GenereObjectIndex(IndexInfo* ind, sLONG MaxElemsInPage, sLONG RequiredElemsInPage, BaseTaskInfo* context,
...
Selection* Selection::SortSel(VError& err, SortTab* tabs, BaseTaskInfo* context, VDB4DProgressIndicator* InProgress, Boolean TestUnicite, Table* remoteTable, WafSelection* inWafSel, WafSelection* outWafSel)
{
	...
	VDB4DSortInfoCollector collector(context, assoctable, tabs, InProgress);
#endif
...
Boolean IndexInfoFromFieldObj::GenereQuickIndex(VError &err, BaseTaskInfo* context, VDB4DProgressIndicator* InProgress, sLONG MaxElemsInPage, sLONG RequiredElemsInPage)
{
	...
	else
	{
		...
		TypeSortElemArray<ObjAttKey> tempsort;

		sLONG lenmax = crit->GetMaxLenForSort() + xString::NoDataSize();
		xMultiFieldDataOffsets off(crit, VK_BLOB_OBJ, true, lenmax);
		result = tempsort.TryToSort((Selection*)nil, into, err, off, context, InProgress, fUniqueKeys, lenmax, this);

		if (result)
		{
			err = tempsort.GenereClusterIndex(this, MaxElemsInPage, RequiredElemsInPage, context, InProgress, VK_BLOB_OBJ, lenmax, off);
		}
