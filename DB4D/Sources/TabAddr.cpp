...
void* TreeMem::GetFrom(sLONG nbelem, sLONG n, OccupableStack* curstack)
{
	...
	assert( n >= 0 && n <= nbelem); // L.E. 23/01/01 actually don't know if 0 is ok
...
void AddressTable::DebugKeepPage(AddressTable* addrtable)
{
	...
	{
		assert(found == debugKeepTables.end());
...
VError AddressTable::LibereEspaceDisk(sLONG nbelem, VDB4DProgressIndicator* InProgress, OccupableStack* curstack)
{
			...
			for (i=0;i<nbelem;i++)
			{
				...
				if (soustable!=nil)
				{
					if (i==(nbelem-1)) 
						...
						n=kNbElemTabAddr2-1;
					soustable->LibereEspaceDisk(n, InProgress, curstack);
					soustable->Release();
				}
			}
		}
			...
			for (i = 0; i <=nbelem; i++)
			{
					...
					|| (addrToCheck >= addrobj && addrToCheck < (addrobj+lenobj)) )
				{
					if (curelem != nomobjToCheck)
					{
						...
						assert(! ((addrobj >= addrToCheck && addrobj < (addrToCheck+lenToCheck)) || (addrToCheck >= addrobj && addrToCheck < (addrobj+lenobj))) );
					}
				}
				curelem++;
			}
...
sLONG AddressTableHeader::findtrou(BaseTaskInfo* context, VError& err, OccupableStack* curstack)
{
	...
	else
	{
		...
		{
			assert(result >= 0 && result <=*nbelem);
			result = result; // put a break here
		}
#endif
		...
		if (err == VE_OK)
		{
			*DebutTrou = ou;
			...
			if (ou != kFinDesTrou)
			{
				...
				if (xou < 0 || xou > *nbelem)
				{
					assert(xou >= 0 && xou <= *nbelem);
					xou = xou; // put a break here
				}
			}
#endif
			setmodif(true, db, context);
...
#if debuglr
	assert (xTempAddr.find(n) == xTempAddr.end());
	xTempAddr.insert(n);
#endif

	assert(*debuttroutrans < 0);
	VError err = VE_OK;
...
VError AddressTableHeader::LibereEspaceDisk(VDB4DProgressIndicator* InProgress, OccupableStack* curstack)
{
	//gOccPool.WaitForMemoryManager(curstack);
	Occupy(curstack, true);
	//gOccPool.EndWaitForMemoryManager(curstack);
	...
	{
		err = FirstPage->LibereEspaceDisk(*nbelem, InProgress, curstack);
...
VError AddressTableHeader::PutxAddr(sLONG n, DataAddr4D ValAddr, sLONG len, BaseTaskInfo* context, OccupableStack* curstack, uLONG inStamp, const char* debugmess )
{
	...
	assert(ValAddr <= 0 || db->IsAddrValid(ValAddr));		// mbucatariu, vu avec L.E., pour le compactage, on copie des trous, context = NULL
...
{
	VError err = VE_OK;
	assert(fKeepStamps != inKeepStamps);
	...
	if (*nbelem > 0)
	{
		...
		if (err == VE_OK && FirstPage != nil)
		{
			...
			{
				gOccPool.WaitForMemoryManager(curstack);
				FirstPage->LibereEspaceDisk(*nbelem, InProgress, curstack);
				gOccPool.WaitForMemoryManager(curstack);
			...
			else
			{
				...
				gOccPool.WaitForMemoryManager(curstack);
				newfirst->LibereEspaceDisk(*nbelem, InProgress, curstack);
				newfirst->Release();
...
VError AddrTable::LibereEspaceDisk(sLONG nbelem, VDB4DProgressIndicator* InProgress)
{
	sLONG i,n,xn2,oldnbelem;
	AddrTable *soustable;
	VError err = VE_OK;
		...
		if (nbelem>kNbElemTabAddr)
		{
			...
			for (i=0;i<nbelem;i++)
			{
				...
				if (soustable!=nil)
				{
					...
					soustable->LibereEspaceDisk(n, InProgress);
					soustable->libere();
					delete soustable;
				}
			}
		...
		else
		{
			if (tabmem!=nil)
			{
				for (i=0;i<nbelem;i++)
				{
					...
					{
						soustable = soustable; // put a break
					}
				#if debugplace
					shex.FromULong((uLONG)(tab[i].addr),kHexaDecimal);
					UnivStrDebug("LiberePlace Tabaddr = ",shex);
...
VError AddrTableHeader::LibereEspaceDisk(VDB4DProgressIndicator* InProgress)
{
	occupe();
	VError err = VE_OK;
	if (InProgress != nil)
	...
	if (FirstPage != nil && err == VE_OK)
	{
		err = FirstPage->LibereEspaceDisk(*nbelem, InProgress);
		FirstPage->libere();
		AddrTable* oldfirst = FirstPage;
		FirstPage->SetParent(nil, 0);
		FirstPage = nil;
