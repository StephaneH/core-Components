...
{
	xbox_assert(ficD != nil);
	fNumRec = ficD->getnumfic();
	xbox_assert(fNumRec>=0);
...
inline void ClearBlobPrimKeyInTrans(mapPrimKeysForBlobs::value_type& x)
{
	if (x.second.first != nil)
		x.second.first->Release();
}


inline void ClearPrimKeyInTrans(mapPrimKeys::value_type& x)
{
	if (x.second != nil)
		x.second->Release();
}
			...
			if (ficD != nil)
			{
				sLONG n = ficD->getnumfic();
				{
					...
					else
					{
						...
						{
							sLONG xdebug = 1;  // put a break here for debug
						}
					}
				}
...
Transaction::~Transaction()
{
	...
	if (fBaseTrans == this)
	{
		for (mapUniqIndexes::iterator cur = fUniqKeysToRelease.begin(), end = fUniqKeysToRelease.end(); cur != end; ++cur)
		{
			mapBTUniqKeys* mapkeys = &(cur->second);
			IndexInfo* ind = cur->first;
			ind->Open(index_write);
			for (mapBTUniqKeys::iterator curkey = mapkeys->begin(), endkey = mapkeys->end(); curkey != endkey; ++curkey)
			{
				ind->ReleaseGlobalUniqKey(curkey->fCle, this);
			}
			ind->Close();
		}
	}
	...
	fKeepLockIDs.clear();
	for_each(fDeletedBlobsPrimKeys.begin(), fDeletedBlobsPrimKeys.end(), ClearBlobPrimKeyInTrans);
	fDeletedBlobsPrimKeys.clear();
	...
	for_each(fDeletedPrimKeys.begin(), fDeletedPrimKeys.end(), ClearPrimKeyInTrans);
	fDeletedRecordIDs.clear();
	fDeletedPrimKeys.clear();
...
VError Transaction::Commit(Boolean &outTryAgain)
{
	...
	if (IsValid())
	{
		...
		if (mere == nil)  // c'est la transaction de base le commit va reellement se faire
		{
			...
			if (fOwner->GetBase()->OkToUpdate(err))
			{
				if (fOwner->GetBase()->TryToStartDataModif(fOwner))
				{
					...
					//fOwner->WriteLog(DB4D_Log_Commit, true);
					fOwner->WriteLog(DB4D_Log_StartCommit, true);
					...
					for (mapRecIDs::iterator cur = fDeletedBlobIDs.begin(), end = fDeletedBlobIDs.end(); cur != end && err == VE_OK; cur++)
					{
						...
						if (tt != nil)
						{
							bool withPrimKey = false;
							tt->occupe();
							DataTable* df = tt->GetDF();
							if (tt->IsLoggedWithPrimKey())
							{
								withPrimKey = true;
							}							
							tt->libere();
							if (df != nil)
							{
								...
								xbox_assert(i < kTransNewBlobNum);
								while (i != -1 && i < kTransNewBlobNum)
								{
									...
									if (withPrimKey)
									{
										insideblobref bref = fDeletedBlobsPrimKeys[make_pair(tableid, i)];
										primkey = bref.first;
										numfield = bref.second;
										xbox_assert(primkey != nil);
#if debuglr
										if (primkey == nil)
										{
											sLONG xdebug = 1; // break here
									...
									df->DelBlobForTrans(i, fOwner, true, primkey, numfield);
									i = b->FindNextBit(i+1);
									xbox_assert(i < kTransNewBlobNum);
								}
							}
					...
					{
						cur->second.fDF->MarkOutsideBlobToDelete(cur->first, cur->second.fPath, fOwner);
					}
					...
					for (mapBlobsInTrans::iterator cur = fBlobs.begin(), end = fBlobs.end(); cur != end && err == VE_OK; cur++)
					{
						...
						if (testAssert(blob != nil)) // ne devrait jamais se produire
						{
							...
							xbox_assert(numrec == blob->GetNum());
							xbox_assert(tableID == df->GetNum());
							df->SaveBlobForTrans(blob, fOwner);
							blob->Release();
						}
					}
						...
						if (tt != nil)
						{
							...
							if (df != nil)
							{
								...
								while (i != -1)
								{
									...
									if (withPrimKey)
									{
										primkey = fDeletedPrimKeys[make_pair(tableid, i)];
										xbox_assert(primkey != nil);
#if debuglr
										if (primkey == nil)
										{
									...
									df->DelRecordForTrans(i, fOwner, primkey);
									i = b->FindNextBit(i+1);
								}
					...
					for (mapRecsInTrans::iterator cur = fRecords.begin(), end = fRecords.end(); cur != end && err == VE_OK; cur++)
					{
						...
						if (testAssert(ficD != nil)) // ne devrait jamais se produire
						{
							...
							sLONG tableID = cur->first.fTableID;
							sLONG numrec = cur->first.fNumRec;
							xbox_assert(numrec == ficD->getnumfic());
							xbox_assert(tableID == df->GetNum());
					...
					fOwner->WriteLog(DB4D_Log_EndCommit, false);
		...
		else  // on merge la sous transaction avec sa mere
		{
			...
			// recopie des cles primaires des blobs detruits par la sous transaction dans la mere
			// si c'est un nouveau blob dans la mere, ne pas copier la cle de suppression

			for (mapPrimKeysForBlobs::iterator cur = fDeletedBlobsPrimKeys.begin(), end = fDeletedBlobsPrimKeys.end(); cur != end && err == VE_OK; cur++)
			{
				const recref* delid = &(cur->first);
				RecordIDInTrans id(delid->first, delid->second);
#if debuglr
				if (mere->fDeletedBlobsPrimKeys.find(*delid) != mere->fDeletedBlobsPrimKeys.end())
				{
					xbox_assert(false); // put a break here
				}
#endif
				if (mere->fNewBlobNums.find(id) == mere->fNewBlobNums.end())
				{
					// si le blob n'a pas ete cree dans la mere alors on peut ajouter la destruction dans la fille
					// sinon la creation sera retiree de la mere un peu plus bas
					insideblobref ib = cur->second;
					if (ib.first != nil)
						ib.first->Retain();
					mere->fDeletedBlobsPrimKeys[*delid] = ib;
				}
			...
			// recopie des cles primaires des fiches detruites par la sous transaction dans la mere
			// si c'est une nouvelle fiche dans la mere, ne pas copier la cle de suppression

			for (mapPrimKeys::iterator cur = fDeletedPrimKeys.begin(), end = fDeletedPrimKeys.end(); cur != end && err == VE_OK; cur++)
			{
				const recref* delid = &(cur->first);
				RecordIDInTrans id(delid->first, delid->second);
#if debuglr
				if (mere->fDeletedPrimKeys.find(*delid) != mere->fDeletedPrimKeys.end())
				{
					xbox_assert(false); // put a break here
				}
#endif
				if (mere->fNewRecNums.find(id) == mere->fNewRecNums.end())
				{
					// si la fiche n'a pas ete cree dans la mere alors on peut ajouter la destruction dans la fille
					// sinon la creation sera retiree de la mere un peu plus bas
					mere->fDeletedPrimKeys[*delid] = RetainRefCountable(cur->second);
				}
...
VError Transaction::DelRecord(sLONG TableID, sLONG RecID, PrimKey* primkey)
{
	VError err = VE_OK;
		...
		if (err == VE_OK)
		{
			...
			if (found == fNewRecNums.end())
			{
				...
				if (err == VE_OK)
				{
					...
					if (primkey != nil)
					{
						fDeletedPrimKeys[make_pair(TableID, RecID)] = RetainRefCountable(primkey);
					}
					//b->libere();
				}
				if (err == VE_OK)
...
VError Transaction::DelBlob(sLONG TableID, sLONG RecID, PrimKey* primkey, sLONG fieldnum)
{
	VError err = VE_OK;
	...
	if (IsValid())
	{
		...
		if (found == fNewBlobNums.end())
		{
			...
			if (err == VE_OK)
			{
				...
				{
					insideblobref ib;
					ib.first = RetainRefCountable(primkey);
					ib.second = fieldnum;
					fDeletedBlobsPrimKeys[make_pair(TableID, RecID)] = ib;
				}
			}
		}
		else 
...
DataAddr4D Transaction::FindTempSpace(sLONG len, VError& err)
{
	...
	{
#if debugtrans_temp
		xbox_assert(debug_Previous_EndOfTempFile <= fEndOfTempFile);
...
void Transaction::AddUniqKeyToRelease(BTitemIndex* val)
{
	IndexInfo* ind = val->GetInd();
	xbox_assert(ind != nil);
	mapBTUniqKeys* mapkeys = &fUniqKeysToRelease[ind];
	BTTransUniqCleIndex cle(val, this, false);
	mapkeys->insert(cle);
}
