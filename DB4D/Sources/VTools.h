...
#define CALLRANGECHECK(n) xbox_assert((n>=0)&&(n<fNbOccupe));
#else
...
{
public:
	Type& operator[]( VIndex inIndex) {xbox_assert( inIndex > 0 && inIndex <= this->fCount); return this->fFirst[inIndex-1];}
	const Type& operator[]( VIndex inIndex) const {xbox_assert( inIndex > 0 && inIndex <= this->fCount); return this->fFirst[inIndex-1];}
};
