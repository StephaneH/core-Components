...
namespace rest
{
	...
	const VError wrong_transaction_command = MAKE_VERROR(rest_signature, 1821); 
	const VError login_failed = MAKE_VERROR(rest_signature, 1822); 
	const VError max_number_of_sessions_reached = MAKE_VERROR(rest_signature, 1823); 
...
class RestTools
{
	...
	public:
		...
		RestTools(BaseTaskInfo* context, const VStream* inputStream, VStream* outputStream, const VString& hostName, const VString& inURLstring, IHTTPResponse* inResponse, 
				VJSGlobalContext* inGlobalContext, RIApplicationRef inApplicationRef, RestRequestHandler* reqhandler)
		{
			_staticInit();
			Init(context, inputStream, outputStream, hostName, inURLstring, inResponse, inGlobalContext, inApplicationRef, reqhandler);
		}
					...
					VJSGlobalContext* inGlobalContext, RIApplicationRef inApplicationRef, RestRequestHandler* reqhandler)
		{
			...
			if (context != nil)
			{
				...
				if (fJSGlobalContext == nil)
					...
					xbox_assert(fJSGlobalContext == context->GetJSContext());
			}
			else
			{
				labase = nil;
			...
			if (!VDBMgr::GetManager()->IsRunning4D())
				xbox_assert(inGlobalContext != NULL);
#endif
			...
			fOutput->OpenWriting();
			fInput->OpenReading();
			fOutputBuffer.EnsureSize(16384);
			fCurlevel = 0;
			...
			fReqHandler = reqhandler;
			fForceExpire4DSession = false;
			fRemoteClientRequestInfo = nil;
			previousCollection = nil;
		}

		bool GetForceExpire4DSession()
		{
			return fForceExpire4DSession;
		}

		void ComputeInputString()
		{
			if (fInputString.IsEmpty())
			{
				fInput->GetText(fInputString, fInput->GetSize(), true);
		...
		VError CalculateURI(VString& outURI, const EntityModel* em, const EntityAttribute* attribute, const VString& additionnalPart, bool WithUri, bool withquotes, bool withformat, bool forJSON, const VString& skey);
		...
		VError SelToJSON(bool firstLevel, EntityModel* em, EntityCollection* inSel, EntityAttributeSortedSelection& whichAttributes, EntityAttributeSelection* expandAttributes, 
		...
		{
			const_cast<RestTools*>(this)->ComputeInputString();
			return fInputString;
		}

		void BuildErrorStack(VValueBag& outBag);

		void GenereErrorReport(bool inAnotherObject);

		void GenerateInfoIfEmpty();
		void TransfertBufferToOutpout();
		...
		void GetServerAndBaseStats();

		void GetWAFFiles();
		void GetSessionsInfo(VValueBag& outbag);

		void GetJSContextInfos( VValueBag& outBag) const;

		bool ExtractKey(const VString* s, VString& outModelName, VectorOfVString& allIDs);

		void SetRequestFormat();
		void SetVerb();
		void WorkOnEntitySets();
		void WorkOnTransaction();
		...
		void ReturnJSResult(VError err, VJSValue& result, VJSException& excep, DataSet* inDataSet, CDB4DSelection* selFrom4D, LocalEntityModel* localEm);
		...
		inline VJSONObject* GetRemoteClientRequestInfo()
		{
			return fRemoteClientRequestInfo;
		}

	protected :

		static const VString kCR;
		static const VString kColon;
		static const VString kQuote;
		static const VString kBeginObject;
		static const VString kEndObject;
		static const VString kBeginArray;
		static const VString kEndArray;
		static const VString kSeparator;
		static const VString kPropNull;

		static const VString kPropDeferred;
		static const VString kPropKey;
		static const VString kPropImage;
		static const VString kPropQueryPath;
		static const VString kPropQueryPlan;
		static const VString kPropEntityModel;
		static const VString kPropError;
		static const VString kPropStamp;
		static const VString kPropTransformedSelection;
		static const VString kPropENTITYSET;
		static const VString kPropCount;
		static const VString kPropSent;
		static const VString kPropFirst;
		static const VString kPropEntities;
		static const VString kPropResult;
		...
		EntityCollection* previousCollection;
		...
		RestRequestHandler* fReqHandler;
		VJSONObject* fRemoteClientRequestInfo;

		CDB4DBaseContext* xUAGcontext;
		CDB4DBase* xUAGBase;
		Base4D* labase;

		VString fOutputBuffer;
