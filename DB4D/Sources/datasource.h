...
class DB4DArrayOfDirectValues : public QuickDB4DArrayOfValues
{
	...
	public:
		...
		{
			xbox_assert(false);  // must be specialized
			return nil;
		}
		...
		{
			xbox_assert(false); // must be specialized for QuickStrings
			return false;
		}
...
public:
	...
	void SetBlob4D(BlobWithPtr* blob)
	{
		fBlob = blob;
	}
...
class VBlobObjInfo : public VBlob::TypeInfo
{
	public:
		VBlobObjInfo():VBlob::TypeInfo( VK_BLOB_OBJ)	{_SetKind(VK_BLOB_OBJ);}
		VBlobObjInfo(ValueKind kind):VBlob::TypeInfo( kind)	{_SetKind(VK_BLOB_OBJ);}
		virtual void*	ByteSwapPtr( void *inBackStore, bool inFromNative) const;
};


class ENTITY_API VBlobObj : public VBlob4DWithPtr
{
	public:
		typedef VBlobObjInfo	InfoType;
		static const InfoType	sInfo;

		virtual const VValueInfo*	GetValueInfo() const;

		VBlobObj( sLONG *inNumBlob, BlobWithPtr *inBlob);
		VBlobObj( const VBlobObj& inBlob);
		virtual ~VBlobObj();
		...
		virtual VError GetJSONString(VString& outJSONString, JSONOption inModifier = JSON_Default) const
		{
			...
			if (fJsonObj != nil)
			{
				val.SetObject(fJsonObj);
			}
			else
				val.SetNull();
			return val.Stringify(outJSONString, inModifier);
		}
		...
		virtual VError FromJSONValue(const VJSONValue& inJSONValue)
		{
			...
			{
				CopyRefCountable(&fJsonObj, inJSONValue.GetObject());
				SetNull(false);
			}
			else
			{
		...
		virtual VError GetJSONValue(VJSONValue& outJSONValue, bool allowDates = false) const
		{
			if (fJsonObj == nil)
				...
				outJSONValue.SetObject(fJsonObj);
			return VE_OK;
		}


		VError ReloadObj();
		VError WriteObj() const;
	...
	protected:
		VJSONObject* fJsonObj;
		bool fJsonObjIsDirty;
};
...
class VSubTableDB4D : public VSubTable
{
	public:
		...
		virtual void Detach(Boolean inMayEmpty = false); // from VValue
...
const creVValue_ParamType creVValue_forquery = 2;
