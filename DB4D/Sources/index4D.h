...
class IndexHeader : public /*ObjAlmostInCacheInTree*/ ObjInCacheMem, public IObjToFlush
{
	...
	public:
		...
		*/
		...
		void MeasureInsertKey(sLONG nb = 1);
		void MeasureDeleteKey(sLONG nb = 1);
		void MeasureFindKey(sLONG nb = 1);
		void MeasureSort(sLONG nb = 1);
...
#if debuglog
		virtual void dumpToLog(BaseTaskInfo* context)
		{
		}
#endif
	...
	protected:
		...
		MeasureCollection fInsertKeyCount;
		MeasureCollection fDeleteKeyCount;
		MeasureCollection fFindKeyCount;
		MeasureCollection fSortCount;
...
#if debuglog
		virtual void dumpToLog(BaseTaskInfo* context, OccupableStack* curstack);
#endif
...
class BTTransUniqCleIndex
{
public:
	BTTransUniqCleIndex(const BTitemIndex* val, Transaction* owner, bool temporary)
	{
		fCle = val;
		if (fCle != nil)
			fCle->Use();
	...
	inline ~BTTransUniqCleIndex()
	{
		if (fCle != nil)
			fCle->Unuse();
	};

	inline BTTransUniqCleIndex(const BTTransUniqCleIndex& other)
	{
		fCle = other.fCle;
		if (fCle != nil)
			fCle->Use();
		fOwner = other.fOwner;
		fIsTemp = other.fIsTemp;
	};

	inline BTTransUniqCleIndex& operator =(const BTTransUniqCleIndex& other)
	{
		fCle = other.fCle;
		if (fCle != nil)
			fCle->Use();
	...
	bool operator < (const BTTransUniqCleIndex& other) const;

	const BTitemIndex* fCle;
	Transaction* fOwner;
	mutable bool fIsTemp;
};


typedef set<BTTransUniqCleIndex> mapBTUniqKeys;
...
//		VArrayStruct fSourceLang;
		...
		mapBTUniqKeys fBTUniqueKeysToBeProcessed;
...
class IndexInfoFromFieldObj;

class IndexInfoFromFieldPath : public IndexInfoFromFieldLexico
{
	public:
		IndexInfoFromFieldPath(IndexInfoFromFieldObj* parent, Field* inCrit, Table* inTable);
		virtual VError Save();

	protected:
		IndexInfoFromFieldObj* fParent;
};
...
{
	public:
		ObjAttKey(sLONG pathid, const VJSONValue& jval);
		ObjAttKey()
		{
			fData = nil;
		}

		void FromJSONVal(sLONG pathid, const VJSONValue& jval);

		CompareResult CompareTo(const ObjAttKey& other, const VCompareOptions& inOptions) const
		{
			return (CompareResult)Compare(fData, other.fData, inOptions);
		}

		/*
		const bool operator == (const ObjAttKey& other) const
		{
			return Compare(fData, other.fData,
		}

		const bool operator < (const ObjAttKey& other) const;
		*/

		static VSize CalculateLength(const void* p);
		static VSize CalculateLength(sLONG pathid, const VJSONValue& jval);
		static void* BuildFromJSONVal(sLONG pathid, const VJSONValue& jval, bool includeIndexKey, void *into, sLONG* lenOnly);
		static void* PutInto(void* into, sLONG pathid, const VJSONValue& jval);
		static sLONG Compare(const void* px1, const void* px2, const VCompareOptions& inOptions);
...
class IndexInfoFromFieldObj : public IndexInfoFromField
{
	public:
		inline IndexInfoFromFieldObj(void) { InfoTyp = DB4D_Index_OnObj; crit = nil; fic = nil; fPathHeader = nil; fPathEntete = nil; };
		IndexInfoFromFieldObj(Base4D* db, sLONG xnumfile, sLONG xnumfield, sLONG xtypindex);
		...
		BTitemIndex* NextPossibleLimit(BTitemIndex* key);
		BTitemIndex* PreviousPossibleLimit(BTitemIndex* key);

		BTitemIndex* BuildKeyWithJSONVal(sLONG pathid, const VJSONValue& jval);
		VSize CalculatePureKeyLength(const void* p);
		...
		Bittab* FourcheOnPath(const ObjectPath& path, bool checkfornull, VValueSingle* val1, uBOOL xstrict1, VValueSingle* val2, uBOOL xstrict2, BaseTaskInfo* context, VError& err,
			VDB4DProgressIndicator* InProgress, const VCompareOptions& inOptions, Bittab* dejasel);

		sLONG FindPathID(OccupableStack* curstack, const VString& path, BaseTaskInfo* context, const VCompareOptions& options);

	protected:
		virtual ~IndexInfoFromFieldObj();
		// indexheader contiendra les valeurs et l'id du path
		IndexHeader* fPathHeader; // fPathHeader contiendra les path pour connaitre leur id
		IndexInfoFromFieldPath* fPathEntete;
...
class IndexAction : public VObject
{
	public:
		...
		void SetAction(sLONG xindexcommande, IndexInfo* ind, VDB4DProgressIndicator* InProgress, const VJSONObject* inRequestOriginatorInfo = nil);
		//~IndexAction();
		...
		//info about who requested the index operation( task name & id, hostname, username...) for monitoring
		const VJSONObject*	fIndexRequestOriginatorInfo;
};
...
void AddRebuildIndex(IndexInfo* Ind, VDB4DProgressIndicator* InProgress, VSyncEvent* event, const VJSONObject* inRequestOriginatorInfo = nil);

void AddCreateIndex(IndexInfo* Ind, VDB4DProgressIndicator* InProgress, VSyncEvent* event, const VJSONObject* inRequestOriginatorInfo = nil);
...
class IndexBuilderTask : public VTask
{
	public:
		...
		void StopIndex()
		{
			VTaskLock lock(&fStopIndexMutex);
			++stopindex;
		}
	...
	protected:
		...
		mutable VCriticalSection fStopIndexMutex;
		sLONG stopindex;
};
