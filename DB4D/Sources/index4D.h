...
class IndexKeyArray
{
	public:
		...
		void AddKey(BTitemIndex* val) { assert(nbval < 30); vals[nbval] = val; nbval++; };
...
class BTreePageIndex : public ObjInCacheMem, public IObjToFlush
{
	public:
		...
		*/
		...
		{
			assert(oldlen == btp->CurrentSizeInMem);
			return true;
		}
...
class IndexInfo : public ObjInCacheMemory, public IObjToFlush, public IObjToFree, public IBaggable
{
	public:
		...
		virtual void setmodif(uBOOL xismodif, Base4D* bd, BaseTaskInfo* context)
		{
			assert(false); // ne devrait pas etre appelee
			Save();
		}

		virtual bool SaveObj(VSize& outSizeSaved)
		{
			assert(false); // ne devrait pas passer par la
			outSizeSaved = 0;
			return true;
		...
		virtual Boolean NeedToRebuildIndex(VIntlMgr* NewIntlMgr) = 0;
		...
		virtual void IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName) const = 0;
...
//		virtual VArrayStruct* GetSourceLang() = 0;
		...
		VError InitTablesAddr(OccupableStack* curstack) { assert(!fIsRemote); return header->InitTablesAddr(curstack); };
		VError NormalizeTablesAddr(OccupableStack* curstack) { assert(!fIsRemote); return header->NormalizeTablesAddr(curstack); };
...
typedef IndexInfo inherited;
	public:
		...
		virtual void IdentifyIndex(VString& outString, Boolean WithTableName, Boolean WithIndexName) const;
...
//		virtual VArrayStruct* GetSourceLang();
		...
		virtual Boolean NeedToRebuildIndex(VIntlMgr* NewIntlMgr);
