...
BaseTaskInfo *GetBaseTaskInfoFromJSContext( const XBOX::VJSContext& inContext, Base4D* inBase)
{
	CDB4DContext *db4dContext = GetDB4DContextFromJSContext( inContext);
	if (db4dContext != NULL)
		return VImpCreator<VDB4DContext>::GetImpObject(db4dContext)->RetainDataBaseContext( inBase, true, false);
	...
	if (structfile != NULL)
	{
		...
		if (datafile != NULL)
		{
			...
			if (err == XBOX::VE_OK)
			{
				...
				if (err == XBOX::VE_OK)
				{
					ioParms.ReturnValue(VJSDatabase::CreateInstance(ioParms.GetContextRef(), VImpCreator<VDB4DBase>::GetImpObject(newdb)->GetBase()));
				}
			}
