...
void VJSDatabase::_getJournalManager(XBOX::VJSParms_getProperty& ioParms, Base4D* inDatabase)
{
	JournalPool* journalManager = inDatabase->GetJournalManager();

	if (journalManager == nil)
		ioParms.ReturnNullValue();
	else
		ioParms.ReturnValue(VJSJournalPool::CreateInstance(ioParms.GetContext(), journalManager));
...
void VJSDatabase::GetDefinition( ClassDefinition& outDefinition)
{
	...
	static inherited::StaticValue values[] = 
	{
		...
		{ "journalManager", js_getProperty<_getJournalManager>, nil, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontEnum | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0, 0 }
	};
...
void VJSJournalPool::Initialize(const XBOX::VJSParms_initialize& inParms, JournalPool* inJournalPool)
{
	// nothing to do
}


void VJSJournalPool::Finalize(const XBOX::VJSParms_finalize& inParms, JournalPool* inJournalPool)
{
	// nothing to do
}


void VJSJournalPool::_setFolder(XBOX::VJSParms_callStaticFunction& ioParms, JournalPool* inJournalPool)
{
	VFolder* folder = ioParms.RetainFolderParam(1, true);
	if (folder != nil)
	{
		VError err = inJournalPool->SetBaseFolder(folder, false, true);
		folder->Release();
	}
}


void VJSJournalPool::_activate(XBOX::VJSParms_callStaticFunction& ioParms, JournalPool* inJournalPool)
{
	if (!inJournalPool->IsLogActivated())
	{
		VError err = inJournalPool->OpenJournal(false, true);
	}
}


void VJSJournalPool::_nextJournalFile(XBOX::VJSParms_callStaticFunction& ioParms, JournalPool* inJournalPool)
{
	VError err = inJournalPool->OpenJournal(true, true);
}



void VJSJournalPool::_getActivated(XBOX::VJSParms_getProperty& ioParms, JournalPool* inJournalPool)
{
	ioParms.ReturnBool(inJournalPool->IsLogActivated());
}


void VJSJournalPool::_getFolder(XBOX::VJSParms_getProperty& ioParms, JournalPool* inJournalPool)
{
	VFolder* folder = inJournalPool->RetainBaseFolder();
	if (folder != nil)
	{
		JS4DFolderIterator* result = new JS4DFolderIterator(folder);
...
void VJSJournalPool::GetDefinition(ClassDefinition& outDefinition)
{
	static inherited::StaticFunction functions[] =
	{
		{ "setFolder", js_callStaticFunction<_setFolder>, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ "activate", js_callStaticFunction<_activate>, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ "newJournal", js_callStaticFunction<_nextJournalFile>, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0 }
	};

	static inherited::StaticValue values[] =
	{
		{ "activated", js_getProperty<_getActivated>, nil, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ "folder", js_getProperty<_getFolder>, nil, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0, 0 }
	};

	outDefinition.className = "JournalManager";
	outDefinition.initialize = js_initialize<Initialize>;
	outDefinition.finalize = js_finalize<Finalize>;
	outDefinition.staticFunctions = functions;
	outDefinition.staticValues = values;
...
void VJSDatabaseForMaintenance::Initialize( const XBOX::VJSParms_initialize& inParms, Base4D* inDatabase)
{
	inDatabase->Retain();
}


void VJSDatabaseForMaintenance::Finalize( const XBOX::VJSParms_finalize& inParms, Base4D* inDatabase)
{
	inDatabase->Release();
}


void VJSDatabaseForMaintenance::GetDefinition( ClassDefinition& outDefinition)
{
	static inherited::StaticFunction functions[] = 
	{
		{ "backup", js_callStaticFunction<_backup>, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ "verify", js_callStaticFunction<_verify>, JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0}
	};

	outDefinition.className = "DatastoreForMaintenance";
	outDefinition.initialize = js_initialize<Initialize>;
	outDefinition.finalize = js_finalize<Finalize>;
	outDefinition.staticFunctions = functions;
}
...
void VJSDatabaseForMaintenance::_backup( XBOX::VJSParms_callStaticFunction& ioParms, Base4D* inDatabase)
{
	VJSDatabase::_Backup( ioParms, inDatabase);
}


void VJSDatabaseForMaintenance::_verify( XBOX::VJSParms_callStaticFunction& ioParms, Base4D* inDatabase)
{
	VJSDatabase::_verify( ioParms, inDatabase);
}
...

	{
		...
		*/
		...
		if (!pass) {
			...
			for (i = 0; i<36; i++) {
				adjwt[(long)sp[i][0]][(long)sp[i][1]] = 3;
				adjwt[(long)sp[i][1]][(long)sp[i][0]] = 3;
			}
		}
					...
					if (ying_flag[i] == ' ' && INRANGE(ying_hold[i])) {
						for (j = 0; j < yang_length; j++) {
							if (yang_flag[j] == ' ' && INRANGE(yang_hold[j])) {
								if (adjwt[(long)ying_hold[i]][(long)yang_hold[j]] > 0) {
									N_simi += adjwt[(long)ying_hold[i]][(long)yang_hold[j]];
									yang_flag[j] = '2';
									break;
								}
							}
...
{
	VJSDatabase::Class();
	VJSDatabaseForMaintenance::Class();
		...
		else
		{
			...
			else
			{
				if (propname.EqualToUSASCIICString("addEventListener"))
				{
					...
					ioParms.GetContext().EvaluateScript("DataClass.prototype.addEventListener", nil, &val,nil);
					val.Protect();
