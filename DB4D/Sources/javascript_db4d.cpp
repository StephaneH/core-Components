...
VJSObject VJSBackupSettings::CreateInstance( const VJSContext& inContext, IBackupSettings *inRetainedBackupSettings)
{
	return inherited::CreateInstance( inContext, inRetainedBackupSettings);
}
...
class JSTOOLSIntf : public IDB4D_DataToolsIntf
{
	public :
		...
		{
			VError err = VE_OK;
			if (!fOpenProgressionFunc.IsEmpty())
			{
				VJSException excep;
				VJSValue result(fJsContext);
				vector<VJSValue> params;
		...
		{
			VError err = VE_OK;
			if (!fCloseProgressionFunc.IsEmpty())
			{
				VJSException excep;
				VJSValue result(fJsContext);
				vector<VJSValue> params;
		...
		{
			VError err = VE_OK;
			if (!fSetProgressTitleFunc.IsEmpty())
			{
				VJSException excep;
				VJSValue result(fJsContext);
				vector<VJSValue> params;
...
void	VJSDatabase::_Backup(XBOX::VJSParms_callStaticFunction& ioParms, Base4D* base)
{
	...
	if (!ioParms.IsNullParam(1) && ioParms.IsObjectParam(1))
	{
		...
		//WAK0084613: O.R.: for ds.backup(), useUniqueNames defaults to true. Value will be overriden in FromJSObject() if specified
		workingBackupSettings->SetUseUniqueNames(true);
...
void VJSEntitySelection::_toArray(XBOX::VJSParms_callStaticFunction& ioParms, EntityCollection* inSelection)
{
	...
	{
		arr = inSelection->ToJsArray(context, ioParms.GetContext(), attlist, &expand, &sortingatts, withkey, false, startvalue, nbvalue, err);
		if (err != VE_OK)
			okresult = false;
	}
...
void VJSEntitySelection::_toJSON(XBOX::VJSParms_callStaticFunction& ioParms, EntityCollection* inSelection)
{
	...
	VJSArray obj(inSelection->ToJsArray(context, ioParms.GetContext(), atts, nil, nil, true, false, -1, -1, err));
...
{
	DynamicObj* obj = new DynamicObj();
	ioParms.ReturnConstructedObject(VJSDynamicObj::CreateInstance(ioParms.GetContext(), obj));
}
