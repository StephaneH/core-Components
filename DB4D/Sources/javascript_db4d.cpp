...
{
	ioParms.ReturnValue(VJSCatalogEMEnumerator::CreateInstance(ioParms.GetContextRef(), inCatalog));
}
...
VJSObject VJSEntityModelCatalog::CreateInstance( JS4D::ContextRef inContext, EntityModelCatalog* inCatalog)
{
	return VJSObject( inContext, inherited::CreateInstance( inContext, inCatalog));
}
...
VJSObject VJSCatalogEMEnumerator::CreateInstance( JS4D::ContextRef inContext, EntityModelCatalog* inCatalog)
{
	return VJSObject( inContext, inherited::CreateInstance( inContext, inCatalog));
}
...
void VJSEntityModel::_getDataStore(VJSParms_callStaticFunction& ioParms, EntityModel* inModel)
{
	...
	{
		ioParms.ReturnValue(VJSEntityModelCatalog::CreateInstance(ioParms.GetContextRef(), inModel->GetCatalog()));
	}
}
	...
	if (err == VE_OK)
	{
		...
		*/
		...
		else
		{
			...
			VJSValue retcat(VJSEntityModelCatalog::CreateInstance(ioParms.GetContextRef(), remotecat));
			if (ProjectRef != nil)
			{
...
void do_DynamicObj(VJSParms_callStaticFunction& ioParms, VJSGlobalObject*)
{
	...
	ioParms.ReturnValue(VJSDynamicObj::CreateInstance(ioParms.GetContextRef(), obj));
}
...
{
	// rien a faire pour l'instant
	obj->SetContextRef(inParms.GetContextRef());
}
...
void VJSDynamicObj::GetProperty( VJSParms_getProperty& ioParms, DynamicObj* obj)
{
	...
	if (! propname.EqualToUSASCIICString("toJSON"))
	{
		...
		if (found != values.end())
		{
			ioParms.ReturnValue(VJSValue(ioParms.GetContextRef(), found->second));
		}
		else
		{
			...
			else
			{
				...
				VJSValue val(VJSDynamicObj::CreateInstance(ioParms.GetContextRef(), subobj));
				JS4D::ProtectValue(val.GetContextRef(), val.GetValueRef());
				values.insert(make_pair(propname, val.GetValueRef()));
...
bool VJSDynamicObj::SetProperty( VJSParms_setProperty& ioParms, DynamicObj* obj)
{
	...
	if (found != values.end())
	{
		JS4D::UnprotectValue(ioParms.GetContextRef(), found->second);
		found->second = val.GetValueRef();
	}
	else
	{
		values.insert(make_pair(propname, val.GetValueRef()));
...
void VJSDynamicObj::CallAsConstructor(VJSParms_callAsConstructor& ioParms)
{
	DynamicObj* obj = new DynamicObj();
	ioParms.ReturnConstructedObject(VJSDynamicObj::CreateInstance(ioParms.GetContextRef(), obj));
}
