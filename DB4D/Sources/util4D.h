...
class IDebugRefCountable : public IRefCountable
{
public:
	IDebugRefCountable();
...
extern uBOOL debug_canRegisterLeaksAll;
...
class Obj4D : public VObject
{
	public:
		...
		inline void CheckUseCount() { assert(fOccupe.GetUseCount() < 20); };
		#else
...
public:
	...
	*/
	...
	inline void SetInTrans(uBOOL b = true) { assert(!b || (!modifie() && !IsDansCache())); if (b) SetFlag(DansTransFlag); else ClearFlag(DansTransFlag); };
	inline Boolean InTrans() const { return IsFlag(DansTransFlag); };

	inline void SetNewInTrans(uBOOL b = true) {/* assert(!modifie() && !IsDansCache()); */ if (b) SetFlag(NewInTransFlag); else ClearFlag(NewInTransFlag); };
...
class VErrorDB4D_OnEM : public VErrorDB4D_OnBase
{
	public:
		...
		VErrorDB4D_OnEM( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const EntityModel* em);
...
class VErrorDB4D_OnEMAttribute : public VErrorDB4D_OnEM
{
	public:
		...
		VErrorDB4D_OnEMAttribute( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const EntityModel* em, const EntityAttribute* att);
...
public:
	...
	VErrorDB4D_OnEMMethod( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const EntityModel* em, const EntityMethod* meth);
...
public:
	...
	VErrorDB4D_OnEMType( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const AttributeType* attType);
...
public:
	...
	VErrorDB4D_OnEMEnum( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const EmEnum* xenum);
...
public:
	...
	VErrorDB4D_OnEMRelation( VError inErrCode, ActionDB4D inAction, const Base4D *inBase, const EntityRelation* rel);
...
class xStringBase : public xTempObject
{
	public:
		...
		inline xStringBase(sLONG inMaxChar = kMaxXStringLen - 1) { fMaxLength = (sWORD)inMaxChar; assert((inMaxChar) >= (sizeof(void*)/2)); };  // xStringBase data must a least contain a pointer
...
class xMultiFieldDataOffsets
{
	...
	protected:
		...
		Boolean ismulti;
		Boolean fWithExpression, fWithAttributes;
};
...
class ExtraPropertiesHeader : public Obj4D
{
	public:
		...
		inline void ClearData() { assert(fExtraStream != nil); delete fExtraStream; fExtraStream = nil; };
