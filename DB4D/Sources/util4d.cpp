...
const VString kWaRemoteModelExt = L".waRemoteModel";
const VString kWaRemoteModel = L"waRemoteModel";
...
const VString kDataMatchExt = L".Match"; // used by 4D
const VString kDataTableDefExt = L".WakTDef"; // used by Wakanda
...
const VString kLocalJSPath("jsFile");
const VString khostname("hostname");
const VString kuser("user");
const VString kpassword("password");
const VString kSQL("SQL");
const VString kssl("ssl");
const VString kdatabase("database");
const VString kport("port");
const VString kremoteAccess("remoteAccess");
const VString krestPrefix("restPrefix");
const VString ktimeout("timeout");
...
const Feature feature_SendSubTableID = {1,(sLONG)0xF0021170, (sLONG)0xF0021200, (sLONG)0xA0021300}; // 11.7 HF1, 12.0 HF1, 13.0 alpha 2
...
{
	VString newName( inName), strCopyOf, number, token("#"), ellipse("...");
	sLONG n=1, tokenPos=0;

	VDBMgr* manager = VDBMgr::GetManager();
	VLocalizationManager* local = manager->GetDefaultLocalization();
	if (local != nil)
	{
		local->LocalizeStringWithKey( CVSTR("copy of"), strCopyOf);
	}
	if (strCopyOf.GetLength() == 0)
		strCopyOf.FromCString( "Copy#_of_");
	tokenPos = strCopyOf.Find( token);

	size_t count = inAlreadyUsedNames.size();
	for (size_t index = 0 ; index < count ; ++index)
	{
		if (inAlreadyUsedNames[index] == newName)
		{
			newName = strCopyOf;
			number.FromLong(n);
			if (n == 1)
				newName.Replace( CVSTR(""), tokenPos, token.GetLength());
			else
				newName.Replace( number, tokenPos, token.GetLength());
			newName.AppendString( inName);
			
			if (inWithEllipse && newName.GetLength() > inMaxLength)
				newName.Replace( ellipse, inMaxLength-ellipse.GetLength()+1, newName.GetLength()-inMaxLength+ellipse.GetLength());

			index = 0;
			++n;
		}
	}
	outUniqueName = newName;
	return;
}
...
void ObjInCacheMemory::operator delete (void *p )
{
	...
	free(p);
#else
	xbox_assert( VDBMgr::GetManager()->GetCacheManager() != nil);
...
{ 
	...
	if (xismodif)
	{
		if (!bd->IsWriteProtected())	// no objects to be written on disk if the db is in read-only (m.c)
		{
			...
			{
	#if 0 && VERSIONDEBUG
				xbox_assert(!VDBMgr::GetFlushManager()->FindObjInFlush(this));
	#endif
				SetFlag(isModifFlag);
				//xbox_assert(fPrisPar == 0 || fPrisPar == DefaultFlushTransac);
...
ObjCache::~ObjCache()
{
	...
	{
		xbox_assert(!IsCurTaskFaisPlace());
...
VError IObjToFlush::LibereEspaceDisk(OccupableStack* curstack, VDB4DProgressIndicator* InProgress)
{
	return VE_OK;
}
...
Boolean DebugOccupeTask::DoRun()
{
	...
	if( GetState() < TS_DYING) 
	{
		...
		for (;cur != end; cur++)
		{
			xbox_assert((curtime - cur->second.fmillisec) < timetocheck);
		}
...
VErrorDB4D_OnBase::VErrorDB4D_OnBase( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog)
:VErrorDB4DBase(inErrCode, inAction)
{
	if (inCatalog != nil)
		inCatalog->GetName(fBaseName);
	GetBag()->SetString(Db4DError::BaseName, fBaseName);
}
...
VErrorDB4D_OnEM::VErrorDB4D_OnEM( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog, const EntityModel* em)
:VErrorDB4D_OnBase(inErrCode, inAction, inCatalog)
{
	fModel = em;
	if (em != nil)
	{
...
VErrorDB4D_OnEMRec::VErrorDB4D_OnEMRec( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog, const EntityModel* em, const VString& key)
:VErrorDB4D_OnEM(inErrCode, inAction, inCatalog, em)
{
	fEntityKey = key;
...
VErrorDB4D_OnEMAttribute::VErrorDB4D_OnEMAttribute( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog, const EntityModel* em, const EntityAttribute* att)
:VErrorDB4D_OnEM(inErrCode, inAction, inCatalog, em)
{
	fAtt = att;
	if (att != nil)
...
VErrorDB4D_OnEMMethod::VErrorDB4D_OnEMMethod( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog, const EntityModel* em, const EntityMethod* meth)
:VErrorDB4D_OnEM(inErrCode, inAction, inCatalog, em)
{
	fMethod = meth;
	if (meth != nil)
...
VErrorDB4D_OnEMType::VErrorDB4D_OnEMType( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog* inCatalog, const AttributeType* attType)
:VErrorDB4D_OnBase(inErrCode, inAction, inCatalog)
{
	fAttType = attType;
	if (attType != nil)
...
VErrorDB4D_OnEMEnum::VErrorDB4D_OnEMEnum( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog* inCatalog, const EmEnum* xenum)
:VErrorDB4D_OnBase(inErrCode, inAction, inCatalog)
{
	fEnum = xenum;
	if (xenum != nil)
...
VErrorDB4D_OnEMRelation::VErrorDB4D_OnEMRelation( VError inErrCode, ActionDB4D inAction, const EntityModelCatalog *inCatalog, const EntityRelation* rel)
:VErrorDB4D_OnBase(inErrCode, inAction, inCatalog)
{
	fRel = rel;
	if (rel != nil)
...
VValueBag* ExtraPropertiesHeader::RetainExtraProperties(VError &err)
{
	...
	else
	{
		...
		if (fExtra == nil)
		{
			if (*fExtraAddr != 0 && *fExtraLen != 0 && (!db->StoredAsXML() || db->GetStructure() != nil))
			{
				...
				if (err==VE_OK)
				{
					if (tag.ValidateTag(fTagInfo, fPos1, fPos2) == VE_OK)
					{
						...
						xbox_assert(lenx == (*fExtraLen - kSizeDataBaseObjectHeader));
			...
			else if (db->StoredAsXML() && db->GetStructure() == nil)
			{
				VFilePath path; 
...
VError ExtraPropertiesHeader::SetExtraProperties(VValueBag* inExtraProperties, bool loadonly)
{
	...
	else
	{
		if (db->OkToUpdate(err))
		{
			...
			if (!db->StoredAsXML() || db->GetStructure() != nil)
			{
				if (fExtraStream != nil)
				{
					delete fExtraStream;
...
void _PutThingsToForget( VStream* into, VDBMgr *inMgr, BaseTaskInfo* context)
{
	...
	if (context != nil)
	{
		// pushrecordsIDs
		{
			...
			context->StealMarkedRecordsAsPushed(pushedRecs);
			if (!pushedRecs.empty())
			{
				into->Put( 'p');
...
{
	xbox_assert(inQueryOptions != nil);
	VStream* reqstream = GetOutputStream();
				...
				if (testAssert(context != nil))
				{
					...
					for (sLONG i = 0; i< count && err == VE_OK; i++)
					{
						...
						if (tt != nil)
						{
							if (markforpush)
							{
								tt->MarkRecordAsPushed(numrec, context);
								context->MarkRecordAsPushed(tt->GetDF(), numrec);
							}
							else
							{
								tt->UnMarkRecordAsPushed(numrec, context);
...
ReadWriteSemaphore::~ReadWriteSemaphore()
{
	xbox_assert(fPendingActions.empty());
	xbox_assert(fWriteOwner == NULL_TASK_ID);
	xbox_assert(fReadCount == 0);
	xbox_assert(fWriteCount == 0);
}
#endif
...
bool ReadWriteSemaphore::TryToLock(ReadWriteSemaphore_Access inAccessRights)
{
...

	{
		...
		if (inAccessRights == RWS_ReadOnly)
		{
			...
			xbox_assert((*pReadOwnerCount) >= 0 && (*pReadOwnerCount) <= 32000);
...
void ReadWriteSemaphore::Unlock(bool WasFreeMem)
{
	...
	else
	{
		...
		if (fReadCount > 0)
			fReadCount--;
		// xbox_assert(fWriteCount == 0); may happen because of RWS_FreeMemAccess
		xbox_assert(fReadCount >= 0);
		{
			if (fReadCount == 0)
			{
...
void OccupableStack::Add(const IOccupable* inObject)
{
	...
	do
	{
		...
		if (fLocker == 0 || fLocker == fTaskID)
		{
			...
			xbox_assert(fStack.size() < kMaxElemInStack);
			fStamp++;
			fLocker = 0;
		}
		else
					...
					for (OccupableStackCollection::iterator cur = fAllStacks.begin(), end = fAllStacks.end(); cur != end; cur++)
					{
						...
						if (stack != nil)
						{
							...
							if (stack->fLocker != currentTaskID)
							{
								...
								xbox_assert(stack->fLocker == currentTaskID);
							}
#endif
...
void debug_CheckWriting(DataAddr4D ou, sLONG len)
{
	...
	else
	{
		...
		if (closest != debug_IndexPageRefs.end())
		{
			...
			{
				xbox_assert(false); // nous sommes en train d'ecrire sur une autre page que la bonne
			}
		}
	}
...
bool okperm(BaseTaskInfo* context, const VUUID& inGroupID)
{
	...
	else
	{
		...
		{
			return context->GetCurrentUserSession()->BelongsTo(inGroupID, context->GetThreadPrivileges());
		}
	}
}
...
DB4DLogger::DB4DLogger(VFile* logfile, bool apppendAtTheEnd, VJSONObject* options)
{
	fLogFile = RetainRefCountable(logfile);
	fLog = nil;
	if (!apppendAtTheEnd)
	{
		if (logfile->Exists())
	...
	fOptions = RetainRefCountable(options);
}


VError DB4DLogger::StartLogging()
{
	VTaskLock lock(&fMutex);
	VError err = VE_OK;
	if (fLog == nil)
	{
		VTime::Now(fLastTimeFlushed);
		if (!fLogFile->Exists())
			err = fLogFile->Create();
		if (err == VE_OK)
		{
			fLog = new VFileStream(fLogFile, FO_RandomAccess);
			err = fLog->OpenWriting();
			if (err == VE_OK)
				err = fLog->SetPos(fLog->GetSize());
		}
	}
...
VError DB4DLogger::StopLogging()
{
	VTaskLock lock(&fMutex);
	VError err = VE_OK;
	if (fLog != nil)
	{
		err = fLog->CloseWriting();
		delete fLog;
	}
	return err;
}
...
DB4DLogger::~DB4DLogger()
{
	StopLogging();
	QuickReleaseRefCountable(fLogFile);
	QuickReleaseRefCountable(fOptions);
}


VError DB4DLogger::Write(VJSONObject* obj)
{
	VTaskLock lock(&fMutex);
	VError err = VE_OK;
	if (fLog != nil)
	{
		VTime now;
		VTime lastflush = fLastTimeFlushed;
		lastflush.AddSeconds(10);
		VTime::Now(now);

		VString s;
		VJSONValue(obj).Stringify(s);
		s += "\n";
		err = fLog->PutData(s.GetCPointer(), s.GetLength() * 2);
...
void AttributeStringPath::FromString(const VString& inPath)
{
...
{
	VJSONObject* info = nil;
#if WITH_RTM_DETAILLED_ACTIVITY_INFO
	{
		BaseTaskInfo* b = ConvertContext(inContext);
		if (b != nil)
		{
			info = b->BuildLockInfo();
		}
	}
