...
public:
									VHTTPRequest (VHTTPServer *inHTTPServer = NULL, XBOX::VTCPEndPoint *inEndPoint = NULL);
	virtual							~VHTTPRequest();

	void							Reset();

	XBOX::VError					ReadFromEndPoint (uLONG inTimeout = 0);
	...
	XBOX::VError					GetParsingError() const { return fParsingError; }
	...
	const XBOX::VString&			GetPeerIP() const { return fPeerIP; }
	PortNumber						GetPeerPort() const { return fPeerPort; }

	IVirtualHost *					GetVirtualHost();
	XBOX::VJSONObject*				BuildRequestInfo() const;
...
private:
	...
	XBOX::VString					fPeerIP;
	PortNumber						fPeerPort;
	bool							fIsSSL;
