...
bool _EqualMediaType (const XBOX::VString& inMediaType1, const XBOX::VString& inMediaType2)
{
	...
	if ((typeValues1.size() > 0) && (typeValues2.size() > 0))
	{
		if ((HTTPServerTools::EqualASCIIVString (typeValues1[0], typeValues2[0]) || HTTPServerTools::EqualASCIIVString (typeValues1[0], STRING_STAR)) &&
			( ((typeValues1.size() > 1) && (typeValues2.size() > 1) && HTTPServerTools::EqualASCIIVString (typeValues1[1], typeValues2[1])) || 
			  HTTPServerTools::EqualASCIIVString ((typeValues1.size() > 1) ? typeValues1[1] : typeValues1[0], STRING_STAR) ) )
			return true;
	}
...
/* static */
bool HTTPProtocol::ParseRangeHeaderValue (const XBOX::VString& inRangeHeader, std::vector< std::pair<XBOX::VIndex, xbox::VIndex> >& outIndexes)
{
	if (inRangeHeader.IsEmpty())
		return false;

	XBOX::VString	rangeHeaderValue (inRangeHeader);
	bool			isOK = false;
	XBOX::VIndex	startByte = -1;
	XBOX::VIndex	endByte = -1;

	rangeHeaderValue.TrimeSpaces();
	if (HTTPServerTools::BeginsWithASCIIVString (rangeHeaderValue, STRING_HEADER_VALUE_BYTES))
	{
		/*
			...
			Or:
				bytes=9500-
		*/
		XBOX::VectorOfVString	multipleValues;
		XBOX::VIndex			posEqualSign = rangeHeaderValue.FindUniChar (CHAR_EQUALS_SIGN);

		if (posEqualSign > 0)
			rangeHeaderValue.SubString (posEqualSign + 1, rangeHeaderValue.GetLength() - posEqualSign);
		rangeHeaderValue.GetSubStrings (CHAR_COMMA, multipleValues, false, true);

		for (XBOX::VectorOfVString::const_iterator it = multipleValues.begin(); it != multipleValues.end(); ++it)
		{
			XBOX::VString	stringValue (*it);
			XBOX::VIndex	pos = stringValue.FindUniChar (CHAR_HYPHEN_MINUS);
			if (pos > 0)
			{
				XBOX::VString startByteString;
				XBOX::VString endByteString;

				if (pos > 1)
					stringValue.GetSubString (1, pos - 1, startByteString);

				if (pos < stringValue.GetLength())
					stringValue.GetSubString (pos + 1, stringValue.GetLength() - pos, endByteString);

				startByte = (!startByteString.IsEmpty()) ? startByteString.GetLong() : -1;
				endByte = (!endByteString.IsEmpty()) ? endByteString.GetLong() : -1;

// 				if (((startByte >= 0) && (startByte <= endByte)) ||
// 					(((startByte == - 1) && (endByte > 0)) || ((endByte == -1) && (startByte >= 0))))
				{
					if (!isOK)
						isOK = true;

					outIndexes.push_back (std::make_pair (startByte, endByte));
				}
			}
		}
	}
...
/* static */
void HTTPProtocol::MakeRangeHeaderValue (XBOX::VIndex inStartIndex, XBOX::VIndex inEndIndex, XBOX::VSize inMaxSize, XBOX::VString& outRangeHeader)
{
	if ((inStartIndex >= 0) && (inEndIndex >= inStartIndex)) // YT 22-May-2013 - WAK0082144
		outRangeHeader.Printf ("%S %d-%d/%d", &STRING_HEADER_VALUE_BYTES, inStartIndex, inEndIndex, inMaxSize);
	else
		outRangeHeader.Printf ("%S */%d", &STRING_HEADER_VALUE_BYTES, inMaxSize);
}


/* static */
bool HTTPProtocol::AcceptUpdateRangeIndexes (XBOX::VIndex& ioStartIndex, XBOX::VIndex& ioEndIndex, const XBOX::VSize inMaxSize)
{
	/*
	 *	The client requested the last N bytes using one of the following notation:
	 *	For example (assuming a representation of length 10000): bytes=-500 or bytes=9500-
	 */
	if ((ioStartIndex == -1) && (ioEndIndex > 0))
	{
		ioStartIndex = (ioEndIndex < inMaxSize) ? (inMaxSize - ioEndIndex) : 0;
		ioEndIndex = inMaxSize - 1;
	}
	else if ((ioEndIndex == -1) && (ioStartIndex >= 0))
	{
		if (ioStartIndex < inMaxSize)
			ioEndIndex = inMaxSize - 1;
		else
			return false;
	}

	if ((ioStartIndex < 0) || (ioEndIndex < ioStartIndex))
		return false;
