									...
									VCachedObject **outCachedObject)
{
	...
	fCacheMapLock.Lock();
	if (inData && ((inDataSize + fCurrentSize) > fCacheMaxSize))
		isOK = _MakeRoom();

	if (isOK && !inURL.IsEmpty() && (NULL != inData) && !inFullPath.IsEmpty())	// if we cache both data and URLs...
	{
		try
		{	
...
bool VCacheManager::IsFileInCache(const XBOX::VString& inURL, const XBOX::VString& inHostUUIDString)
{
	XBOX::VString keyValueString;
	XBOX::VTaskLock	lock(&fCacheMapLock);

	_MakeCachedObjectKeyValueString(inHostUUIDString, inURL, keyValueString);
	MapOfVCachedObject::iterator it = fCacheMap->find(keyValueString);

	return (it != fCacheMap->end());
}


bool VCacheManager::AddFileInCache(const XBOX::VString& inURL, const XBOX::VString& inHostUUIDString, const XBOX::VFilePath& inFilePath)
{
	bool			isOK = false;
		...
		if (NULL != file && file->Exists())
		{
			...
			if (testAssert(fileStream.OpenReading() == VE_OK))
			{
				...
				if (NULL != buffer)
				{
					...
					XBOX::VTime		lastChecked, lastModified;
					XBOX::VString	contentType;
					XBOX::VString	fileExtension;

					file->GetTimeAttributes(&lastModified);

					XBOX::VTime::Now(lastChecked);

					inFilePath.GetExtension(fileExtension);
					VMimeTypeManager::FindContentType(fileExtension, contentType);

					isOK = AddPageToCache(inURL, inHostUUIDString, contentType, buffer, bufferSize, inFilePath, lastChecked, lastModified, true, COMPRESSION_NONE, NULL);
