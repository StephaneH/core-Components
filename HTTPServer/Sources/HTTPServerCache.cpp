...
{
	if ((inSize < 0) || (inSize < (fCachedObjectMaxSize * 2)))
		return;
...
{
	if ((inSize < 0) || (inSize > (fCacheMaxSize / 2)))
		return;
...
XBOX::VError VCacheManager::AddRetainResource (const XBOX::VString& inPattern, VHTTPResource *inResource)
{
...
XBOX::VError VCacheManager::RemoveReleaseResource (const XBOX::VString& inPattern)
{
	XBOX::VError error = XBOX::VE_OK;
	...
	if (!inPattern.IsEmpty())
	{
		...
		if (cacheRule != fCacheRulesMap.end())
		{
			cacheRule->second->Release();

			if (0 == cacheRule->second->GetRefCount())
