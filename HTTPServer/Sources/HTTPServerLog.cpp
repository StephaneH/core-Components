...
const sLONG			MAX_REQUEST_IN_BUFFER = 10;
const sLONG			LOG_FLUSHER_SLEEP_TIME = 100;
...
sLONG VHTTPServerLog::_BackgroundFlushLog (XBOX::VTask *inTask)
{
	...
	if (NULL != log)
	{
		...
		{
			if ((log->fRequestsInBuffer >= MAX_REQUEST_IN_BUFFER) || log->_NeedFileRotation())
			{
				if (log->_NeedFileRotation())
					log->_RotateFile();

				if (log->fRequestsInBuffer >= MAX_REQUEST_IN_BUFFER)
					log->_Flush();
...
XBOX::VError VHTTPServerLog::LogRequest (const IHTTPResponse& inHTTPResponse)
{
	...
	if (fSettings.GetLogFormat() != LOG_FORMAT_NO_LOG)
	{
		...
		if ((++fRequestsInBuffer >= MAX_REQUEST_IN_BUFFER) || fNeedSplit)
			_WakeUpBackgroundLogFlusher();
	}
