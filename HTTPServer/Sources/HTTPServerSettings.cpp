...
const bool			DEFAULT_ALLOW_HTTP_ON_LOCAL = false;
...
namespace RIASettingsKeys
{
	...
	namespace HTTP
	{
		...
		CREATE_BAGKEY_WITH_DEFAULT_SCALAR (allowHttpOnLocal, XBOX::VBoolean, bool, DEFAULT_ALLOW_HTTP_ON_LOCAL);
	...
	*/
	namespace Resources
	{
		...
		CREATE_BAGKEY_NO_DEFAULT (urlPath, XBOX::VString);
		CREATE_BAGKEY_NO_DEFAULT (urlRegex, XBOX::VString);
	...
	namespace VirtualFolders
	{
		...
		CREATE_BAGKEY_NO_DEFAULT (localPath, XBOX::VString);
		CREATE_BAGKEY_NO_DEFAULT (location, XBOX::VString);
...
void VVirtualFolderSetting::_VerifyAndUpdateLocation (XBOX::VFileSystemNamespace *inFileSystemNameSpace)
{
	...
	{
		XBOX::VFolder *folder = NULL;

		if (inFileSystemNameSpace->ParsePathWithFileSystem (fLocation, NULL, &folder))
		{
			XBOX::ReleaseRefCountable (&folder);
		}
		else if (!fLocation.IsEmpty())
		{
			XBOX::VFileSystem *	projectFileSystem = inFileSystemNameSpace->RetainFileSystem (CVSTR ("PROJECT"));
			XBOX::VFilePath		path;
			BuildFolderPath (projectFileSystem->GetRoot(), fLocation, path);
			if (path.IsFolder())
			{
			...
			XBOX::ReleaseRefCountable (&projectFileSystem);
		}
	}
}


void VVirtualFolderSetting::LoadFromBag (const XBOX::VValueBag& inValueBag, XBOX::VFileSystemNamespace *inFileSystemNameSpace)
{
	inValueBag.GetString (RIASettingsKeys::VirtualFolders::name, fURLPath);
	...
	_VerifyAndUpdateLocation (inFileSystemNameSpace);
}


void VVirtualFolderSetting::LoadFromJSONValue (const XBOX::VJSONValue& inJSONValue, XBOX::VFileSystemNamespace *inFileSystemNameSpace)
{
	inJSONValue.GetProperty ("urlPath").GetString (fURLPath);
	if (fURLPath.IsEmpty())
		inJSONValue.GetProperty ("urlRegex").GetString (fURLPath);
    inJSONValue.GetProperty ("localPath").GetString (fLocation);
	if (fLocation.IsEmpty())
        inJSONValue.GetProperty ("location").GetString (fLocation);
	inJSONValue.GetProperty ("index").GetString (fIndex);
...
{
	if (!fURLPath.IsEmpty())
		ioValueBag.SetString (RIASettingsKeys::VirtualFolders::name, fURLPath);
	...
	if (!fLocation.IsEmpty())
    {
		if (fUseRedirect)
            ...
            ioValueBag.SetString (RIASettingsKeys::VirtualFolders::localPath, fLocation);
    }
...
VHTTPServerProjectSettings::VHTTPServerProjectSettings (XBOX::VFileSystemNamespace *inFileSystemNameSpace)
...
, fAllowHTTPOnLocal (DEFAULT_ALLOW_HTTP_ON_LOCAL)
...
, fSignal_SettingsChanged (XBOX::VSignal::ESM_Asynchonous)
{
	...
	fProjectFileSystem = fFileSystemNameSpace->RetainFileSystem (CVSTR ("PROJECT"));
}
...
VHTTPServerProjectSettings::~VHTTPServerProjectSettings()
{
	...
	XBOX::ReleaseRefCountable (&fProjectFileSystem);
}
...
void VHTTPServerProjectSettings::ResetToFactorySettings()
{
	...
	SetAllowHTTPOnLocal (DEFAULT_ALLOW_HTTP_ON_LOCAL);
...
void VHTTPServerProjectSettings::LoadResourcesSettingsFromJSONFile (const XBOX::VFilePath& inFilePath)
{
	...
	if (file.Exists())
	{
		...
		err = VJSONImporter::ParseFile (&file, jsonValue, VJSONImporter::EJSI_Strict);

		if ((XBOX::VE_OK == err) && jsonValue.IsArray())
		{
			...
			for (sLONG i = 0; i < jsonArray->GetCount(); ++i)
			{
				...
				if (elem.IsObject())
				{
					...
					if (NULL != resource)
					{
						...
						if ((resource->GetLifeTime() > 0) || IsVTimeValid (resource->GetExpires()))
							fResourcesVector.push_back (resource);
						resource->Release();
					}
				}
		...
		if (count > 0)
		{
			...
			for (XBOX::VIndex i = 1; i <= count; ++i)
			{
				...
				if (NULL != bag)
				{
					...
					if (NULL != settings)
					{
						settings->LoadFromBag (*bag, fFileSystemNameSpace);
...
void VHTTPServerProjectSettings::LoadVirtualFoldersSettingsFromJSONFile (const XBOX::VFilePath& inFilePath)
{
	XBOX::VError	err = XBOX::VE_OK;
	XBOX::VFile		file (inFilePath);
	...
	if (file.Exists())
	{
		XBOX::VJSONValue	jsonValue;

		err = XBOX::VJSONImporter::ParseFile (&file, jsonValue, XBOX::VJSONImporter::EJSI_Strict);

		if ((XBOX::VE_OK == err) && jsonValue.IsArray())
		{
			XBOX::VJSONArray *jsonArray = jsonValue.GetArray();
			for (sLONG i = 0; i < jsonArray->GetCount(); ++i)
			{
				XBOX::VJSONValue elem = (*jsonArray)[i];
				if (elem.IsObject())
				{
					VVirtualFolderSetting *settings = new VVirtualFolderSetting();
					if (NULL != settings)
					{
						settings->LoadFromJSONValue (elem, fFileSystemNameSpace);
						fVirtualFoldersVector.push_back (settings);
						settings->Release();
					}
				}
...
XBOX::VError VHTTPServerProjectSettings::LoadFromBag (const XBOX::VValueBag *inBag)
{
	...
	if (httpSettings)
	{
        ...
        fAllowHTTPOnLocal = RIASettingsKeys::HTTP::allowHttpOnLocal.Get (httpSettings);
		...
		if (fAllowSSL)
		{
			...
			BuildFolderPath (fProjectFileSystem->GetRoot(), certificatePath, fSSLCertificatesFolderPath);
		}
...
*/
		...
		BuildFolderPath (fProjectFileSystem->GetRoot(), logFolderPathString, fLogFolderPath);
	...
	if (webAppSettings)
	{
		...
		BuildFolderPath (fProjectFileSystem->GetRoot(), webFolderPath, fWebFolderPath);
...
XBOX::VError VHTTPServerProjectSettings::SaveToBag (XBOX::VValueBag *outBag)
{
	...
	if (httpSettings)
	{
		...
		httpSettings->SetBool (RIASettingsKeys::HTTP::allowHttpOnLocal, fAllowHTTPOnLocal);
...
{
	BuildFolderPath (fProjectFileSystem->GetRoot(), inValue.GetPath(), fSSLCertificatesFolderPath);
...
{
	BuildFolderPath (fProjectFileSystem->GetRoot(), inValue, fSSLCertificatesFolderPath);
...
{
	BuildFolderPath (fProjectFileSystem->GetRoot(), inValue.GetPath(), fWebFolderPath);
...
{
	BuildFolderPath (fProjectFileSystem->GetRoot(), inValue, fWebFolderPath);
