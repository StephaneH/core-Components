...
const XBOX::VString STRING_HEADER_UPGRADE				= CVSTR	("Upgrade");
...
const XBOX::VString STRING_HEADER_VALUE_BYTES			= CVSTR ("bytes");
...
const XBOX::VString STRING_HEADER_VALUE_WEBSOCKET		= CVSTR ("websocket");
	/**
...
const XBOX::VString& GetHTTPHeaderName (const HTTPCommonHeaderCode inHeaderCode)
{
	switch (inHeaderCode)
	{
		...
		case HEADER_UPGRADE:					return STRING_HEADER_UPGRADE;
...
void ExtractContentTypeAndCharset (const XBOX::VString& inString, XBOX::VString& outContentType, XBOX::CharSet& outCharSet)
{
	...
	if (!outContentType.IsEmpty())
	{
		...
		if (pos > 0)
		{
			...
			{
				HTTPServerTools::TrimUniChar (charSetName, CHAR_QUOTATION_MARK);
...
void VDebugTimer::DebugMsg (const XBOX::VString& inMsg)
{
	...
	debugMsg.AppendPrintf (" (%d microsecs)\n", microsec);
