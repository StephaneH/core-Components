			...
			if (authenticationInfos->IsValid())
			{
				...
				if ((directory != NULL) && directory->HasLoginListener())
				{
					if (testAssert(fAuthenticationDelegate != NULL))
					{
						...
						error = fSecurityManager->ValidateBasicAuthentication (userName, password, &isOK, uagSession, globalContext, reqinfo);
						fAuthenticationDelegate->ReleaseJSContext( globalContext);
					}
				}
				else
				{
					error = fSecurityManager->ValidateBasicAuthentication (userName, password, &isOK, uagSession, NULL, reqinfo);
				}
			}
			break;
