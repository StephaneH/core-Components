...
{
	fAuthenticationReferee = new VAuthenticationReferee();
}
...
bool VAuthenticationManager::_ValidateAuthentication (IAuthenticationInfos *ioAuthenticationInfos, const IHTTPRequest *inRequest)
{
	...
	XBOX::VJSONObject* reqinfo = (inRequest == nil) ? nil : inRequest->BuildRequestInfo();
	...
	if (XBOX::VE_OK == error)
	{
		...
		case AUTH_BASIC:
			if (authenticationInfos->IsValid())
			{
				...
				XBOX::VString headerValue;
				const XBOX::VHTTPHeader headers = inRequest->GetHTTPHeaders();
				headers.GetHeaderValue( "HashedPassword", headerValue );
				bool hashedPassword = headerValue == "true";
				...
				if ((directory != NULL) && directory->HasLoginListener())
				{
					if (testAssert(fAuthenticationDelegate != NULL))
					{
						...
						error = fSecurityManager->ValidateBasicAuthentication (userName, password, hashedPassword, &isOK, uagSession, globalContext, reqinfo);
						fAuthenticationDelegate->ReleaseJSContext( globalContext);
					}
				}
				else
				{
					error = fSecurityManager->ValidateBasicAuthentication (userName, password, hashedPassword, &isOK, uagSession, NULL, reqinfo);
				}
			}
			break;
		...
		case AUTH_KERBEROS:
			if (authenticationInfos->IsValidForKerberos())
			{
				...
				error = fSecurityManager->ValidateKerberosAuthentication (krbTicket, &isOK, &outAuthData, uagSession, reqinfo);
			}
			break;
				...
				if (authenticationInfos->IsValid())
				{
					...
					if ((directory != NULL) && directory->HasLoginListener())
					{
						if (testAssert(fAuthenticationDelegate != NULL))
						{
							...
							XBOX::VJSGlobalContext *globalContext = fAuthenticationDelegate->RetainJSContext( NULL, true, inRequest);
							assert(globalContext != NULL);
							error = fSecurityManager->ValidateDigestAuthentication (user, algorithm, realm, nonce, cnonce, qop, nonceCount, uri, method, response, &isOK, uagSession, globalContext, reqinfo);
							fAuthenticationDelegate->ReleaseJSContext( globalContext);
						}
					}
					else
					{
						error = fSecurityManager->ValidateDigestAuthentication (user, algorithm, realm, nonce, cnonce, qop, nonceCount, uri, method, response, &isOK, uagSession, NULL, reqinfo);
					}
				}
				else
				{
		...
		{ 
			...
			if (!userGroup.IsEmpty())
			{
				...
				else
				{
					...
					if (group != NULL)
					{
						isOK = uagSession->BelongsTo(group, nil);
						group->Release();
					}
				}
			}
...
XBOX::VError VAuthenticationManager::CheckAdminAccessGranted (IHTTPResponse *ioResponse)
{
	...
	if (NULL != fSecurityManager)
	{
		...
		if ((NULL != uagDirectory) && (NULL != adminGroup) && ((NULL == uagSession) || !uagSession->BelongsTo (adminGroup, nil)))
		{
			VVirtualHost *	virtualHost = dynamic_cast<VVirtualHost *>(ioResponse->GetVirtualHost());
...
XBOX::VError VAuthenticationManager::SetAuthorizationHeader (IHTTPResponse *ioResponse)
{
	...
	/*
	 *	We have an empty "WWW-Authenticate" header, remove it to prevent authentication dialog to be displayed !!!
	 */
	else if (authHeaderValue.IsEmpty())
	{
		response->GetHeaders().RemoveHeader (STRING_HEADER_WWW_AUTHENTICATE);
	}
