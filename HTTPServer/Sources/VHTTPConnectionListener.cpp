...
namespace NSConnectionListener
{
	CREATE_BAGKEY(listeningIP);
	CREATE_BAGKEY(listeningPort);
	CREATE_BAGKEY(listeningSSLPort);
	CREATE_BAGKEY(sslEnabled);
	CREATE_BAGKEY(sslMandatory);
	CREATE_BAGKEY(acceptHTTPOnLocal);
	CREATE_BAGKEY(exclusiveWorkerCount);
	CREATE_BAGKEY(exclusiveWorkerMaxCount);
	CREATE_BAGKEY(certificatesFolderPath);
}
...
XBOX::VError VHTTPConnectionListener::StartListening()
{
	...
	if (NULL == fServerListener)
	{
		...
		if (XBOX::VE_OK == error)
		{
			...
			if (!fSSLMandatory || fAcceptHTTPOnLocal)
			...
			if (XBOX::VE_OK == error)
			{
				...
				bool ok=fServerListener->StartListening();

				//jmo - WAK0087465 (and cie) : We have to call Run() so the child task may exit
				//      cleanly. If we fail to do that, the child task will be blocked on some
				//      Linux task implementation semaphore, and pthread_join called later from
				//      some destructor in parent task will wait forever, blocking the whole app.
...
Boolean VHTTPConnectionListener::DoRun()
{
	...
	while (IsListening())
	{
		...
		XTCPSock *	newSock = fServerListener->GetNewConnectedSocket (1000 /*MsTimeout*/);

		if (NULL != newSock)
		{
			if (_AcceptSocket (newSock))
			{
				XBOX::VTCPEndPoint *endPoint = new XBOX::VTCPEndPoint (newSock, fSelectIOPool);
...
bool VHTTPConnectionListener::_AcceptSocket (XBOX::XTCPSock *inSocket)
{
	if (NULL == inSocket)
		return false;
	
	if ((fSSLMandatory && fAcceptHTTPOnLocal) && !inSocket->IsSSL())
	{
		XBOX::VNetAddress	addr;

		addr.FromPeerAddr (inSocket->GetRawSocket());

        return ServerNetTools::IsLocalInterface(addr.GetIP());
    }
...
void VHTTPConnectionListener::SaveToBag(XBOX::VValueBag &ioBag)
{
	ioBag.SetString(NSConnectionListener::listeningIP, fListeningIP);
	ioBag.SetLong(NSConnectionListener::listeningPort, fPort);
	ioBag.SetLong(NSConnectionListener::listeningSSLPort, fSSLPort);
	ioBag.SetBool(NSConnectionListener::sslEnabled, fSSLEnabled);
	ioBag.SetBool(NSConnectionListener::sslMandatory, fSSLMandatory);
	ioBag.SetBool(NSConnectionListener::acceptHTTPOnLocal, fAcceptHTTPOnLocal);
	ioBag.SetLong(NSConnectionListener::exclusiveWorkerCount, fExclusiveWorkerCount);
	ioBag.SetLong(NSConnectionListener::exclusiveWorkerMaxCount, fExclusiveWorkerMaxCount);
	ioBag.SetString(NSConnectionListener::certificatesFolderPath, fCertificatesFolderPath.GetPath());
}
