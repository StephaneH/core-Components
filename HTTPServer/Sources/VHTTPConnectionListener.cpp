...
, fAbortTask (false)
{
	...
	const short	EXCLUSIVE_WORKER_MAX_COUNT = 50 * VSystem::GetNumberOfProcessors ( );
...
XBOX::VError VHTTPConnectionListener::StartListening()
{
	...
	if (NULL == fServerListener)
	{
		...
		if (XBOX::VE_OK == error)
		{
			...
			/*
			 *	4 scenarios here:
			 *	1. fSSLEnable=FALSE && fSSLMandatory=FALSE	-> Publish on HTTP Port
			 *	2. fSSLEnable=TRUE && fSSLMandatory=FALSE	-> Publish both on HTTP & HTTPS Ports
			 *	3. fSSLEnable=TRUE && fSSLMandatory=TRUE	-> Publish on HTTPS Port
			 *	4. fSSLEnable=FALSE && fSSLMandatory=TRUE	-> Publish both on HTTP & HTTPS Ports
			 */
			...
			if (isOK && (fSSLEnabled || fSSLMandatory))
...
bool VHTTPConnectionListener::_AcceptSocket (XBOX::XTCPSock *inSocket)
{
	/*
	 *	4 scenarios here:
	 *	1. fSSLEnable=FALSE && fSSLMandatory=FALSE	-> Accept Only HTTP connections
	 *	2. fSSLEnable=TRUE && fSSLMandatory=FALSE	-> Accept both HTTP & HTTPS connections
	 *	3. fSSLEnable=TRUE && fSSLMandatory=TRUE	-> Accept HTTPS connections
	 *	4. fSSLEnable=FALSE && fSSLMandatory=TRUE	-> Accept Only HTTPS connections & allow HTTP for local addresses
	 */

	if (NULL == inSocket)
		return false;
	
	if ((!fSSLEnabled && fSSLMandatory) && !inSocket->IsSSL())
	{
		XBOX::VNetAddress	addr;
		IP4					ip4Addr = 0;
		
		addr.FromPeerAddr (inSocket->GetRawSocket());
//		addrString.FromString (addr.GetIP());
		addr.FillIpV4 (&ip4Addr);

		XBOX::VNetAddress	addr2 (ip4Addr);

		//return (addr.IsLoopBack() || addr.IsLocal());
		return (addr2.IsLoopBack() || addr2.IsLocal());
	}
