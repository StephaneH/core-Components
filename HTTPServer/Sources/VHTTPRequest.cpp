...
XBOX::VError _ReadExactlyFromEndPoint(XBOX::VTCPEndPoint *inEndPoint, void *ioBuffer, uLONG inBufferSize, uLONG inTimeoutMS)
{
	if ((NULL == inEndPoint) || (NULL == ioBuffer))
	...
	/*
	 *	Loop until:
	 *	-	ReadExactly returns a value different from VE_SRVR_READ_TIMED_OUT (VE_OK when there are incoming data),
	 *	-	Keep-Alive Timeout expired 
	 *	-	Current Task died (because Server is not running anymore)
	 */
	while ((XBOX::VSystem::GetCurrentTime() - startTime) <= inTimeoutMS)
	{
		endPointError = inEndPoint->ReadExactly(ioBuffer, (uLONG)inBufferSize, READ_TIMEOUT_MS);

		if (endPointError != VE_SRVR_READ_TIMED_OUT)
			return endPointError;

		if (XBOX::VTask::GetCurrent()->IsDying())
			return XBOX::VE_SRVR_READ_FAILED; // Cancel Read on socket
	}
...
VHTTPRequest::VHTTPRequest (VHTTPServer *inHTTPServer, XBOX::VTCPEndPoint *inEndPoint)
...
{
	if (NULL != inEndPoint)
		fEndPoint = XBOX::RetainRefCountable (inEndPoint);
}
...
void VHTTPRequest::_ReleaseEndPoint()
{
	XBOX::ReleaseRefCountable (&fEndPoint);
}


XBOX::VError VHTTPRequest::ReadFromEndPoint (uLONG inTimeout)
{
	if (NULL == fEndPoint)
		return XBOX::VE_OK; // The EndPoint has probably been detached from Response and Request objects (See VHTTPRequest::DetachEndPoint())
...
#define	MAX_BUFFER_LENGTH					16384
	...
	const sLONG		READ_TIMEOUT_MS = (inTimeout != 0) ? inTimeout : 3000;
	...
	if (fParsingState != PS_WaitingForBody)
	{
        //FromPeerAddr may return different values of native errors, which cannot be filtered properly in previous errorContext
        XBOX::StErrorContextInstaller silentContext(false /*inKeepingErrors*/, true /*inSilentContext*/);
        
		fParsingState = PS_Undefined;
		fRequestLine.Clear();

		fLocalAddress.FromLocalAddr (fEndPoint->GetRawSocket());
		fIsSSL = fEndPoint->IsSSL();

		XBOX::XNetAddr peerAddr;
		endPointError = peerAddr.FromPeerAddr (fEndPoint->GetRawSocket());

        if ((XBOX::VE_OK == endPointError))
        {
            fPeerIP.FromString (peerAddr.GetIP());
            fPeerPort = peerAddr.GetPort();
        }
	}
	/*
	...
	if ((XBOX::VE_OK == endPointError) && !skipRequestLineAndHeader)
	{
		...
		endPointError = _ReadExactlyFromEndPoint (fEndPoint, buffer, unreadBytes, READ_TIMEOUT_MS); // YT 22-May-2014 - ACI0088058
	}
	...
	while ((XBOX::VE_OK == endPointError) && !stopReadingSocket)
	{
		...
		endPointError = fEndPoint->ReadWithTimeout (buffer + bufferOffset, (uLONG *)&bufferSize, READ_TIMEOUT_MS);
		...
		/*
		 *	YT 26-Aug-2013 - ACI0081509
		 *	Do not return an error when endPoint returns VE_SRVR_READ_TIMED_OUT anymore,
		 *	this will prevent the request to be responded. The VE_SRVR_READ_TIMED_OUT is
		 *	typically returned when Content-Length specify a data length and the data is
		 *	actually smaller...
		 */
		if (XBOX::VE_SRVR_READ_TIMED_OUT == endPointError)
		{
			stopReadingSocket = true;
			...
			if (fParsingState <= PS_ReadingHeaders)
			{
				...
				*/
				...
				if ((PS_ReadingHeaders == fParsingState) && (NULL != endLinePtr))
				{
					...
					/*
					 *	YT 26-Nov-2013 - WAK0072868 - Do we have one or more LWS?
					 */
					while ((NULL != crlfPtr) && ((*crlfPtr == HTTP_SP) || (*crlfPtr == HTTP_HT)))
					{
						while ((NULL != crlfPtr) && isspace (*crlfPtr))
							++crlfPtr;

						// Relocate the true end of line
						endLinePtr = strstr (crlfPtr, HTTP_CRLF);
						if (NULL != endLinePtr)
							crlfPtr = endLinePtr + 2; // Skip CRLF
					}
				}
				...
				case PS_ReadingRequestLine:
				{
					if (NULL != startLinePtr)
					{
						...
						else
						{
							...
							/*
							 *	YT 16-Sep-2013 - WAK0072869 & WAK0077243 
							 *	Retrieve and Set Host header when Request-URI is an absoluteURI
							 */
							if (HTTPServerTools::BeginsWithASCIICString (fRawURL, "http://") ||
								HTTPServerTools::BeginsWithASCIICString (fRawURL, "https://"))
							{
								XBOX::VString	hostString;
								XBOX::VString	rawString (fRawURL);
								sLONG			endSchemePos = HTTPServerTools::FindASCIICString (rawString, "://");
								sLONG			startPathPos = rawString.FindUniChar (CHAR_SOLIDUS, endSchemePos + 4);

								if (startPathPos > endSchemePos)
									rawString.GetSubString (endSchemePos + 3, startPathPos - (endSchemePos + 3), fHost);
								rawString.GetSubString (startPathPos, rawString.GetLength() - (startPathPos - 1), fRawURL);

								if (!fHost.IsEmpty())
									hostHeaderFound = GetHeaders().SetHeaderValue (STRING_HEADER_HOST, fHost, false);
							}
						...
						if (startLinePtr != endHeaderPtr)
						{
							...
							if ((NULL != colonPtr) && (colonPtr < endLinePtr))
							{
								...
								if (!header.IsEmpty())
								{
									...
									bool bIsHostHeader = HTTPServerTools::EqualASCIIVString (header, STRING_HEADER_HOST);

									/*
									 *	Do not override Host when Request-URI is an absoluteURI
									 */ 
									if (!bIsHostHeader || (!hostHeaderFound && bIsHostHeader))
										GetHeaders().SetHeaderValue (header, value, false);

									if (!hostHeaderFound && bIsHostHeader)
									{
...
// 								fParsingError = VE_HTTP_PROTOCOL_REQUEST_ENTITY_TOO_LARGE;
						...
						if (NULL != bodyContentBuffer)
						{
							...
							/*
							 *	YT 08-Nov-2013 - ACI0083402	Some proxy servers like Vulture appends chars to body (BOM, CRLF, etc...)
							 *								so don't check that bodyContentSize is strictly equal to Content-Length
							 */
...
IVirtualHost *VHTTPRequest::GetVirtualHost()
{
	...
	if ((fHTTPVersion == VERSION_1_0) && fHost.IsEmpty())
	{
		fEndPoint->GetIP (fHost);
		fHost.AppendUniChar (CHAR_COLON);
		fHost.AppendLong (fEndPoint->GetPort());
	}

	return fHTTPServer->GetVirtualHostManager()->GetMatchingVirtualHost (*this);
}


XBOX::VJSONObject* VHTTPRequest::BuildRequestInfo() const
{
	XBOX::VJSONObject* result = new XBOX::VJSONObject();

	result->SetPropertyAsString("url", fRawURL);
	const VHTTPHeader& headers = GetHeaders();
	
	const VNameValueCollection& headerlist = headers.GetHeaderList();

	for (XBOX::VNameValueCollection::ConstIterator it = headerlist.begin(); it != headerlist.end(); ++it)
	{
		result->SetPropertyAsString((*it).first, (*it).second);
	}

	result->SetPropertyAsString("Peer-IP", fPeerIP);
	result->SetPropertyAsNumber("Peer-Port", fPeerPort);
