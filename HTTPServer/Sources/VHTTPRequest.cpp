...
#define	MAX_BUFFER_LENGTH					16384
	...
	if (fParsingState != PS_WaitingForBody)
	{
		...
		peerAddr.FromPeerAddr (fEndPoint->GetRawSocket());
	...
	if (!skipRequestLineAndHeader)
	{
		unreadBytes = bufferOffset = 5;
		endPointError = fEndPoint->ReadExactly (buffer, unreadBytes, READ_TIMEOUT);
	}
	...
	while ((XBOX::VE_OK == endPointError) && !stopReadingSocket)
	{
		...
		endPointError = fEndPoint->ReadWithTimeout (buffer + bufferOffset, (uLONG *)&bufferSize, 5000 /*timeout ms*/);
