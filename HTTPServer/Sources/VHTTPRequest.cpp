...
#define	MAX_BUFFER_LENGTH					16384
	...
	if (fParsingState != PS_WaitingForBody)
	{
		...
		fPeerAddress.FromPeerAddr (fEndPoint->GetRawSocket());
...
XBOX::VJSONObject* VHTTPRequest::BuildRequestInfo() const
{
	...
	VString ipaddr = fPeerAddress.GetIP();
	sLONG portnum = fPeerAddress.GetPort();
	result->SetPropertyAsString("Peer-IP", ipaddr);
	result->SetPropertyAsNumber("Peer-Port", portnum);
