...
void VHTTPResource::LoadFromBag (const XBOX::VValueBag& inValueBag)
{
	...
	*/
	if (!fURL.IsEmpty())
	{
		if (fURL.GetUniChar (1) == CHAR_SOLIDUS)
			fURL.Insert (CHAR_CIRCUMFLEX_ACCENT, 1);

		if (!HTTPServerTools::FindASCIICString (fURL, "(?i)"))
			fURL.Insert (CVSTR ("(?i)"), 1);

		if (fURL.GetUniChar (fURL.GetLength()) != CHAR_SOLIDUS)
			fURL.AppendUniChar (CHAR_DOLLAR_SIGN);
	}
}
