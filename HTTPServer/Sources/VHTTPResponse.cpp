...
, fCanCompressBody (false)	// YT 15-May-2014 - ACI0087781 - Activate compression by default may cause memory corruption in ZLib component... Need more investigation...
...
static bool GetCrossSiteRequestForgeryFound( VHTTPRequest* inRequest)
{
	...
	for (VIndex iter = 0 ; !crossSiteRequestForgeryFound && (iter < 2) ; ++iter)
	{
		...
		if (!headerValue.IsEmpty())
		{
			...
			bool secure = HTTPServerTools::EqualASCIICString(scheme, "https");
#if WITH_CSRF_EXCEPTION_FOR_FILE_SCHEME
			/*
			*	YT 19-May-2014 -	Check 'referer' and/or 'origin' header to prevent a CSRF attack.
			*						Consider request as a CSRF attack when 'referer' and/or 'origin' header is different from host
			*						except when these headers contain a 'file://' scheme in URL
			*/
			bool bUseFileScheme = false;
			if (!secure)
				bUseFileScheme = HTTPServerTools::EqualASCIICString(scheme, "file");
#endif
...
#if WITH_CSRF_EXCEPTION_FOR_FILE_SCHEME
			if (bUseFileScheme && crossSiteRequestForgeryFound)
				crossSiteRequestForgeryFound = false;
#endif
		}
	}
...
XBOX::VError VHTTPResponse::_WriteToSocket (void *inBuffer, uLONG *ioBytes)
{
	...
	error = fEndPoint->WriteExactly (inBuffer, *ioBytes); // YT 05-May-2014 - ACI0086830 - Don't need any timeout here
...
XBOX::VError VHTTPResponse::ReplyWithStatusCode (HTTPStatusCode inValue, XBOX::VString *inExplanationString)
{
	...
	{
		if ((HTTP_UNAUTHORIZED == inValue) && (NULL != GetVirtualHost())) // YT 30-May-2014 - ACI0087844 - VirtualHost may be NULL when it cannot be determined...
		{
