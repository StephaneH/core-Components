...
bool VHTTPResponse::_NormalizeResponseHeader()
{
	...
	*/
	...
	if (!fIsChunked && !GetHeaders().IsHeaderSet (STRING_HEADER_CONTENT_LENGTH) && (bodySize >= 0)) // YT 30-Nov-2012 - ACI0079288 - Set Content-Length even if body is empty (to prevent browser waiting a nonexistent body)
	{
...
XBOX::VError VHTTPResponse::SendResponse()
{
	...
	else
	{
		...
		if (GetHeaders().GetContentType (contentType, &charset))
		{
			...
			{
				charset = (XBOX::VTC_UNKNOWN == GetBody().GetCharSet()) ? GetBody().GetCharSet() : fDefaultCharset;
