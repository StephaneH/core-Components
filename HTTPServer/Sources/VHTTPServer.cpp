...
namespace NSHTTPServer
{
	...
	CREATE_BAGKEY (connectionListeners);
}
...
, fSleepHandler (NULL)
{
	...
	XBOX::VSleepNotifier::Instance()->RegisterSleepHandler (fSleepHandler);
}
...
void VHTTPServer::SaveToBag (XBOX::VValueBag &outBag)
{
	...
	/* Display connectionListeners */
	XBOX::VBagArray *listenerBagArray = new XBOX::VBagArray();
	if (NULL != listenerBagArray)
	{
		XBOX::VTaskLock lock(&fConnectionListenersLock);
		for (VectorOfHTTPConnectionListener::iterator it = fConnectionListeners.begin(); it != fConnectionListeners.end(); ++it)
		{
			XBOX::VValueBag *listenerBag = new XBOX::VValueBag();
			if (NULL != listenerBag)
			{
				(*it)->SaveToBag(*listenerBag);
				listenerBagArray->AddTail(listenerBag);
				XBOX::QuickReleaseRefCountable(listenerBag);
			}
		}

		outBag.SetElements(NSHTTPServer::connectionListeners, listenerBagArray);
		XBOX::ReleaseRefCountable(&listenerBagArray);
	}
...
sLONG VHTTPServer::GetMaxRequestInLogBuffer() const
{
	return VHTTPServerLog::GetMaxRequestsInBuffer();
}


void VHTTPServer::SetMaxRequestInLogBuffer(sLONG inValue)
{
	VHTTPServerLog::SetMaxRequestsInBuffer(inValue);
}
