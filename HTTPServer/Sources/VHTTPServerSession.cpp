...
const uLONG DEFAULT_KEEP_ALIVE_TIMEOUT = 15000;		// 15 secs
...
XBOX::VError VHTTPServerSession::HandleRequest()
{
	...
	if (XBOX::VE_OK == error)
	{
		...
		if (authDelegate != nil)
		{
			if (uagSessionCookie.IsEmpty() /*|| uagSessionCookie.EqualToUSASCIICString("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF")*/)
				VTask::SetCurrentData(VTask::GetHTTPServerSessionDataKey(), nil);
			else
				VTask::SetCurrentData(VTask::GetHTTPServerSessionDataKey(), TaskNeedSession);
		}
	...
	{
		/*
		 *	YT 15-May-2014 - ACI0087903
		 *	Log request after having send response, otherwise  informations  (based on response header) such response data size are missing.
		 *	This bug is a side effect of my previous correction for ACI0087781
		 */
		/* Log Request */
