...
//** All code now goes through IWebSocketHandler instead of IHTTPRequestHandler.
//** Remove this after test confirms everything is ok.

				/*

//					&& !HTTPServerTools::BeginsWithASCIIVString(fHTTPResponse->GetRequest().GetURL(), "/wka_dbg")
//					&& !HTTPServerTools::BeginsWithASCIIVString(fHTTPResponse->GetRequest().GetURL(), "/devtools");

				if (bIsWebSocket) {

					DebugMsg("WebSocket => ");
					DebugMsg(fHTTPResponse->GetRequest().GetURL());
					DebugMsg("\n");
