...
XBOX::VError VStaticPagesHTTPRequestHandler::HandleRequest (IHTTPResponse *ioResponse)
{
	...
	if ((method == HTTP_GET) || (method == HTTP_HEAD))
	{
					...
					if (cacheBuffer)
					{
						...
						*/
						...
						if (method == HTTP_GET)
						{
							...
							httpResponse->SetResponseBody (cacheBuffer, cacheBufferSize); // YT 13-Feb-2014 - WAK0086417
						}
		...
		if (loadPageFromDisk && (XBOX::VE_OK == error))
		{
			...
			else if ((error == VE_HTTP_PROTOCOL_FOUND) || (error == VE_HTTP_PROTOCOL_MOVED_PERMANENTLY))
			{
