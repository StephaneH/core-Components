...
void MakeVirtualFolderPattern (const XBOX::VString& inKeyword, XBOX::VString& outPatternString)
{
	...
	if (!HTTPServerTools::EndsWithASCIICString (keyword, "/.*"))
		outPatternString.AppendCString ("/.*"); // YT 18-Dec-2013 - WAK0084576 - Fix regex adding "/.*" instead of "/*"
}
...
VVirtualHost::VVirtualHost (VHTTPServerProject *inProject)
: fVirtualFolderMap()
...
{
	return (fProject != NULL) ? dynamic_cast<VHTTPServerProjectSettings *>(fProject->GetSettings()) : NULL;
}
...
void VVirtualHost::_Init()
{
	...
	MakeHostPattern (GetSettings()->GetHostName(), ipString, GetSettings()->GetListeningPort(), (GetSettings()->GetAllowSSL() || GetSettings()->GetSSLMandatory()), GetSettings()->GetListeningSSLPort(), fHostPattern);
...
void VVirtualHost::_Deinit()
{
	...
	for (VHTTPResourcesVector::const_iterator it = resourcesVector.begin(); it != resourcesVector.end(); ++it)
	{
		...
		{
			error = cacheManager->RemoveResource (patternString);
			if (XBOX::VE_OK != error)
				break;
		}
	}
...
VVirtualFolder * VVirtualHost::RetainVirtualFolder (const XBOX::VString& inLocationPath, const XBOX::VString& inIndexPage, const XBOX::VString& inKeyword, XBOX::VFileSystemNamespace *inFileSystemNameSpace)
{
	XBOX::VTaskLock			lock (&fVirtualFolderMapLock);
	XBOX::VString			regexString;
	VVirtualFolder *		resultVirtualFolder = NULL;
	XBOX::VString			locationPath (inLocationPath);

	if (NULL != inFileSystemNameSpace)
	{
		XBOX::VFolder *		fileSystemFolder = NULL;

		bool isOK = inFileSystemNameSpace->ParsePathWithFileSystem (inLocationPath, NULL, &fileSystemFolder);
		if (!isOK && (NULL != fileSystemFolder))
			XBOX::ReleaseRefCountable (&fileSystemFolder);

		if (NULL != fileSystemFolder)
		{
			fileSystemFolder->GetPath().GetPosixPath (locationPath);
			XBOX::ReleaseRefCountable (&fileSystemFolder);
		}
		else if (!HTTPServerTools::BeginsWithASCIICString (inLocationPath, "http://") &&
				 !HTTPServerTools::BeginsWithASCIICString (inLocationPath, "https://"))
		{
			XBOX::VFilePath filePath (inLocationPath);
			filePath.GetPosixPath (locationPath);
		}
	}
	...
	if (found != fVirtualFolderMap.end())
	{
		...
		if ((!HTTPServerTools::EqualASCIIVString (resultVirtualFolder->GetLocationPath(), locationPath)) ||
...
bool VVirtualHost::GetMatchingVirtualFolderInfos (const XBOX::VString& inURL, XBOX::VFilePath& outVirtualFolderPath, XBOX::VString& outDefaultIndexName, XBOX::VString& outVirtualFolderName)
{
	...
	if ((NULL != virtualFolder) && (NULL != virtualFolder->GetFolder()))
	{
...
{
	return ((GetSettings()->GetListeningPort() == inPort) || ((GetSettings()->GetAllowSSL() || GetSettings()->GetSSLMandatory()) && (GetSettings()->GetListeningSSLPort() == inPort)));
}
