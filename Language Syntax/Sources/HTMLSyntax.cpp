...
void VHTMLSyntax::Init( ICodeEditorDocument* inDocument )
{
	...
	inDocument->SetStyle( eString, false, false, false, 136, 0, 0 );
	inDocument->SetStyle( eTag, false, false, false,   0,   0,   0 );
}
...
{
	return inStyle == eKeyword || inStyle == eNumber || inStyle == eString || inStyle == eSelector || inStyle == eValue|| inStyle == eIdentifier ;
}
...
void VHTMLSyntax::SetLine( ICodeEditorDocument *inDocument, sLONG inLineNumber, bool inLoading )
{
	...
	inDocument->SetLineStyle(inLineNumber,0,xstr.GetLength(),eNormal);		//initiate the line
	...
	while (cur) {
		...
		if (currentLineParams->IsOpenComment()) {
			if (kCommentClose == cur->fStyle) {
				...
				inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eComment );
		...
		if (kCompleteComment == cur->fStyle) {
			...
			inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eString );
		} else if ( kSymbolicChar == cur->fStyle ) {
			inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eSeparator );
			...
			if (currentLineParams->IsProcessingTag()) {
				inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eKeyword );
			...
			inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eNumber );
			...
			inDocument->SetLineStyle( inLineNumber, cur->fOffset, cur->fOffset + cur->fLength, eTag );
		}
...
void VHTMLSyntax::GetSuggestions( ICodeEditorDocument* inDocument, sLONG inLineNumber, sLONG inPos, ITipInfoArray *outSuggestions, sLONG& outStartOffset, bool inAll )
{
	...
	if (lastTokenProcessed == 1) {
		...
		outSuggestions->AddTip( new VCodeEditorTipInfo( inDocument, suggestion, eKeyword ) );
	}
...
void VHTMLSyntax::OpenTagOpened( const BasicInformation *inInfo, const VString &inName )
{
	...
	cookie->fDocument->SetLineStyle( cookie->fLineNumber, inInfo->fStartPosition, inInfo->fStartPosition + inInfo->fLength, eTag );
}
...
{
	ParsingCookie *cookie = (ParsingCookie *)inInfo->fCookie;
	cookie->fDocument->SetLineStyle( cookie->fLineNumber, inInfo->fStartPosition, inInfo->fStartPosition + inInfo->fLength, eSelector );
}

void VHTMLSyntax::AttributeValueParsed( const BasicInformation *inInfo, const VString &inName, const VString &inValue )
{
	ParsingCookie *cookie = (ParsingCookie *)inInfo->fCookie;
	cookie->fDocument->SetLineStyle( cookie->fLineNumber, inInfo->fStartPosition, inInfo->fStartPosition + inInfo->fLength, eValue );
}
...
{
	ParsingCookie *cookie = (ParsingCookie *)inInfo->fCookie;
	cookie->fDocument->SetLineStyle( cookie->fLineNumber, inInfo->fStartPosition, inInfo->fStartPosition + inInfo->fLength, eIdentifier );
}

void VHTMLSyntax::CommentParsed( const BasicInformation *inInfo, bool inIsUnfinished )
{
	ParsingCookie *cookie = (ParsingCookie *)inInfo->fCookie;
	cookie->fDocument->SetLineStyle( cookie->fLineNumber, inInfo->fStartPosition, inInfo->fStartPosition + inInfo->fLength, eComment );
}
