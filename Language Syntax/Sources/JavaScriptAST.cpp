...
ESymbolDeclarationType TypesDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeTypes;
}
...
ESymbolDeclarationType VariableDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeVariable;
}
...
ISymbolDeclaration* VariableDeclaration::Clone() const
{
	...
	cloned->SetInstanceFlag(fInstanceFlag);
...
void VariableDeclaration::ToDatabase(ISymbolTable* inSymbolTable, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext, SymbolDefinitionContext& inDefinitionContext, sLONG& outSymbolId) const
{
	...
	xbox_assert( inSymbolTable != NULL );
	if( inSymbolTable != NULL )
	{
		if( inSymbolTable->GetName(fName, inParentScopeId, inExecutionContext, outSymbolId) == false )
		{
			inSymbolTable->SetName(fName, inParentScopeId, inExecutionContext, outSymbolId);
			if( fRhs != NULL )
			{
				ESymbolDeclarationType rhsType = fRhs->GetSymbolDeclarationType();
				if( rhsType == eSymbolDeclarationTypeNew )
				{
					SymbolDefinitionContext context;
					context.SetLocalVariableFlag(true);
					
					fRhs->ToDatabase(inSymbolTable, inParentScopeId, inSourceId, inExecutionContext, context, outSymbolId);
				}
				else
				{
					ESymbolDefinitionType definitionType = eSymbolDefinitionTypeUndefined;
					
					if(	rhsType == eSymbolDeclarationTypeStringLiteral )
						definitionType = eSymbolDefinitionTypeString;
					else if( rhsType == eSymbolDeclarationTypeNumberLiteral )
						definitionType = eSymbolDefinitionTypeNumber;
					else if( rhsType == eSymbolDeclarationTypeBooleanLiteral )
						definitionType = eSymbolDefinitionTypeBoolean;
					else if( rhsType == eSymbolDeclarationTypeRegExpLiteral )
						definitionType = eSymbolDefinitionTypeRegExp;
					else if( rhsType == eSymbolDeclarationTypeNullLiteral )
						definitionType = eSymbolDefinitionTypeNull;
					else if( rhsType == eSymbolDeclarationTypeArrayLiteral )
						definitionType = eSymbolDefinitionTypeArray;
					
					bool constructorFlag = false;
					bool localVariableFlag = true;
					
					inSymbolTable->SetDefinition(inParentScopeId, outSymbolId, inExecutionContext, eSymbolDefinitionKindProperty, eSymbolDefinitionTypeArray, eSymbolDefinitionVisibilityPublic, constructorFlag, fInstanceFlag, localVariableFlag);
				}
...
void VariableDeclaration::SetInstanceFlag(const bool& inValue)
{
	fInstanceFlag = inValue;
}
...
ESymbolDeclarationType ObjectFieldDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeObjectField;
}
...
ESymbolDeclarationType ObjectDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeObject;
}
...
ESymbolDeclarationType FunctionCallDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeFunctionCall;
}
...
ESymbolDeclarationType NewDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeNew;
}
...
{
#if ACTIVATE_NEXT_JS_AST_WALKER
	xbox_assert( fIdentifiers != NULL );
	if( fIdentifiers != NULL )
	{
		// Add some extras information to the definition context
		inDefinitionContext.SetFromDeclarationType(eSymbolDeclarationTypeNew);
...
ESymbolDeclarationType ReturnsDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeReturns;
}
...
ESymbolDeclarationType FunctionArgumentsDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeFunctionArguments;
}
...
ESymbolDeclarationType FunctionExpressionDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeFunctionExpression;
}
...
ESymbolDeclarationType StringLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeStringLiteral;
}
...
ESymbolDeclarationType NumberLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeNumberLiteral;
}
...
ESymbolDeclarationType BooleanLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeBooleanLiteral;
}
...
ESymbolDeclarationType RegExpLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeRegExpLiteral;
}
...
ESymbolDeclarationType ArrayLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeArrayLiteral;
}
...
ESymbolDeclarationType NullLiteralDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeNullLiteral;
}
...
ESymbolDeclarationType IdentifiersChainSymbolDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeIdentifiers;
}
...
void IdentifiersChainSymbolDeclaration::ToDatabase(ISymbolTable* inSymbolTable, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext, SymbolDefinitionContext& inDefinitionContext, sLONG& outSymbolId) const
{
	...
	if( inSymbolTable != NULL )
	{
		ESymbolDeclarationType fromType = inDefinitionContext.GetFromDeclarationType();
		if( fromType == eSymbolDeclarationTypeNew )
		{
			...
			if( count > 0 )
			{
				...
				bool localVariableFlag = inDefinitionContext.GetLocalVariableFlag();
				bool instanceFlag = true;
				bool constructorFlag = false;
				
				ESymbolDefinitionType symbolType = eSymbolDefinitionTypeUndefined;
				if(	typeName == "Object" )
					symbolType = eSymbolDefinitionTypeObject;
				else if( typeName == "Function" )
					symbolType = eSymbolDefinitionTypeFunction;
				else if( typeName == "Boolean" )
					symbolType = eSymbolDefinitionTypeBoolean;
				else if( typeName == "Number" )
					symbolType = eSymbolDefinitionTypeNumber;
				else if( typeName == "String" )
					symbolType = eSymbolDefinitionTypeString;
				else if( typeName == "Array" )
					symbolType = eSymbolDefinitionTypeArray;
				else if( typeName == "RegExp" )
					symbolType = eSymbolDefinitionTypeRegExp;
				else if( typeName == "Date" )
					symbolType = eSymbolDefinitionTypeDate;
				else if( typeName == "Error" )
					symbolType = eSymbolDefinitionTypeError;
				else if( typeName == "EvalError" )
					symbolType = eSymbolDefinitionTypeEvalError;
				else if( typeName == "RangeError" )
					eSymbolDefinitionTypeRangeError;
				else if( typeName == "ReferenceError" )
					symbolType = eSymbolDefinitionTypeReferenceError;
				else if( typeName == "SyntaxError" )
					symbolType = eSymbolDefinitionTypeSyntaxError;
				else if( typeName == "TypeError" )
					symbolType = eSymbolDefinitionTypeTypeError;
				else if( typeName == "URIError" )
					symbolType = eSymbolDefinitionTypeURIError;
				
				inSymbolTable->SetDefinition(inParentScopeId, outSymbolId, inExecutionContext, eSymbolDefinitionKindProperty, symbolType, eSymbolDefinitionVisibilityPublic, constructorFlag, instanceFlag, localVariableFlag);
			}
		}
	}
#endif
...
ESymbolDeclarationType AssignmentDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeAssignment;
}
...
ISymbolDeclaration* AssignmentDeclaration::GetLhs() const
{
	return fLhs;
}
...
ESymbolDeclarationType PropertyDeclaration::GetSymbolDeclarationType() const
{
	return eSymbolDeclarationTypeProperty;
}
...
void PropertyDeclaration::SetInstanceFlag(const bool& inValue)
{
	fInstanceFlag = inValue;
}
 ...
 *
 */
SymbolScopeDeclaration::SymbolScopeDeclaration(SymbolScopeDeclaration* inParent, const XBOX::VString& inName, const ESymbolScopeType& inType, const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset, const bool& inConstructorFlag):
fParent(inParent),
fName(inName),
fType(inType),
fFromLine(inFromLine),
...
void SymbolScopeDeclaration::AddFunctionDeclaration(ISymbolDeclaration* inDeclaration)
{
	fFunctions.push_back(inDeclaration);
}

const std::vector<ISymbolDeclaration*>& SymbolScopeDeclaration::GetFunctionDeclarations() const
{
	return fFunctions;
}

bool SymbolScopeDeclaration::Compare(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;

	if( inFromScope != NULL && inToScope != NULL )
	{
		if( SymbolScopeDeclaration::_CompareName(inFromScope, inToScope) && SymbolScopeDeclaration::_CompareType(inFromScope, inToScope) )
		{
			if( SymbolScopeDeclaration::_ComparePropertyDeclarations(inFromScope, inToScope) &&
				SymbolScopeDeclaration::_CompareAssignmentDeclarations(inFromScope, inToScope) &&
				SymbolScopeDeclaration::_CompareVariableDeclarations(inFromScope, inToScope) &&
				SymbolScopeDeclaration::_CompareFunctionDeclarations(inFromScope, inToScope) )
			{
				if( SymbolScopeDeclaration::_CompareChildren(inFromScope, inToScope) )
				{
					areEquals = true;
				}
			}
...
bool SymbolScopeDeclaration::_CompareName(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;

	if( inFromScope != NULL && inToScope != NULL )
	{
		if( inFromScope->GetScopeName() == inToScope->GetScopeName() )
			areEquals = true;
	}
...
bool SymbolScopeDeclaration::_CompareType(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;
	
	if( inFromScope != NULL && inToScope != NULL )
	{
		if( inFromScope->GetScopeType() == inToScope->GetScopeType() )
			areEquals = true;
	}
...
bool SymbolScopeDeclaration::_CompareDeclarations(const std::vector<ISymbolDeclaration*>& inFrom, const std::vector<ISymbolDeclaration*> inTo)
{
	bool areEquals = true;
	
	VIndex fromCount	= inFrom.size();
	VIndex toCount		= inTo.size();
	if( fromCount == toCount )
	{
		for(VIndex curIndex=0; curIndex<fromCount; ++curIndex)
		{
			ISymbolDeclaration* fromDeclaration =	NULL;
			ISymbolDeclaration* toDeclaration	=	NULL;
			
			fromDeclaration = inFrom[curIndex];
			toDeclaration	= inTo[curIndex];
			
			if( fromDeclaration != NULL && toDeclaration != NULL )
			{
				VString fromString;
				fromDeclaration->ToString(fromString);
				
				VString toString;
				toDeclaration->ToString(toString);
...
bool SymbolScopeDeclaration::_ComparePropertyDeclarations(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;
	
	if( inFromScope && inToScope )
		areEquals = SymbolScopeDeclaration::_CompareDeclarations(inFromScope->GetPropertyDeclarations(), inToScope->GetPropertyDeclarations());
	
	return areEquals;
}

bool SymbolScopeDeclaration::_CompareAssignmentDeclarations(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;

	if( inFromScope && inToScope )
		areEquals = SymbolScopeDeclaration::_CompareDeclarations(inFromScope->GetAssignmentDeclarations(), inToScope->GetAssignmentDeclarations());

	return areEquals;
}

bool SymbolScopeDeclaration::_CompareVariableDeclarations(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;
	
	if( inFromScope && inToScope )
		areEquals = SymbolScopeDeclaration::_CompareDeclarations(inFromScope->GetVariableDeclarations(), inToScope->GetVariableDeclarations());
	
	return areEquals;
}

bool SymbolScopeDeclaration::_CompareFunctionDeclarations(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = false;
	
	if( inFromScope && inToScope )
		areEquals = SymbolScopeDeclaration::_CompareDeclarations(inFromScope->GetFunctionDeclarations(), inToScope->GetFunctionDeclarations());

	return areEquals;
}

bool SymbolScopeDeclaration::_CompareChildren(const SymbolScopeDeclaration* inFromScope, const SymbolScopeDeclaration* inToScope)
{
	bool areEquals = true;

	if( inFromScope != NULL && inToScope != NULL )
	{
		VIndex fromCount	= inFromScope->GetChildScopeCount();
		VIndex toCount		= inToScope->GetChildScopeCount();
		if( fromCount == toCount )
		{
			for(VIndex curInd=0; curInd<fromCount; ++curInd)
			{
				areEquals = SymbolScopeDeclaration::Compare(inFromScope->GetChildScopeAt(curInd), inToScope->GetChildScopeAt(curInd));
				if( areEquals == false )
					break;
			}
		}
...
void SymbolScopeDeclaration::_PrintScope(const XBOX::VString& inTabSpace, SymbolScopeDeclaration* inScope)
{
	// Print Scope Infos
	...
	{
		case eSymbolScopeTypeGlobal:				msg.AppendString("GLOBAL");					break;
		case eSymbolScopeTypeFunctionDeclaration:	msg.AppendString("FUNCTION_DECLARATION");	break;
		case eSymbolScopeTypeFunctionExpression:	msg.AppendString("FUNCTION_EXPRESSION");	break;
		case eSymbolScopeTypeCatch:					msg.AppendString("CATCH");					break;
		case eSymbolScopeTypeWith:					msg.AppendString("WITH");					break;
...
void SymbolScopeDeclaration::_PrintDeclarations(const XBOX::VString& inTabSpace, const XBOX::VString& inCountMsg, const std::vector<ISymbolDeclaration*>& inDeclarations)
{
	XBOX::VString msg = inTabSpace;
...
void SymbolScopeDeclaration::Print(SymbolScopeDeclaration* inScope, const XBOX::VString& inTabSpace)
{
	...
	_PrintDeclarations(curTab, "VARIABLE DECLARATIONS COUNT : ", inScope->GetVariableDeclarations());
	
	// Print function declarations
	_PrintDeclarations(curTab, "FUNCTION DECLARATIONS COUNT : ", inScope->GetFunctionDeclarations());

	// Print assignment declarations
	_PrintDeclarations(curTab, "ASSIGNMENT DECLARATIONS COUNT : ", inScope->GetAssignmentDeclarations());

	// Move to child scope
	XBOX::VIndex total = inScope->GetChildScopeCount();
	for(XBOX::VIndex index=0; index<total ; index++)	{	SymbolScopeDeclaration::Print(inScope->GetChildScopeAt(index), curTab);	}
...
private:
	...
	void PushScope(const XBOX::VString& inName, const ESymbolScopeType& inType, const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset, const bool& inConstructorFlag)
	{
		SymbolScopeDeclaration* newScope = new SymbolScopeDeclaration(fCurrentScope, inName, inType, inFromLine, inFromOffset, inToLine, inToOffset, inConstructorFlag);
...
{
#if ACTIVATE_NEXT_JS_AST_WALKER_TRACES
	SymbolScopeDeclaration::Print(fCurrentScope, "");
#endif
}
...
bool NextGenJsAstWalker::VisitFunctionDeclarationStatementNodeEnter(class FunctionDeclarationStatementNode *inNode )
{
	...
	PushScope(inNode->GetIdentifier(), eSymbolScopeTypeFunctionDeclaration, inNode->GetFromLine(), inNode->GetFromOffset(), inNode->GetToLine(), inNode->GetToOffset(), inNode->HasConstructorTag());
...
bool NextGenJsAstWalker::VisitCatchStatementNodeEnter(class CatchStatementNode *inNode )
{
	...
	VString name("CATCH_SCOPE#");
	name.AppendLong(fCatchScopeCounter);
	
	bool constructorFlag = false;
	PushScope(name, eSymbolScopeTypeCatch, inNode->GetFromLine(), inNode->GetFromOffset(), inNode->GetToLine(), inNode->GetToOffset(), constructorFlag);
...
bool NextGenJsAstWalker::VisitWithStatementNodeEnter(class WithStatementNode *inNode )
{
	...
	VString name("WITH_SCOPE#");
	name.AppendLong(fWithScopeCounter);
	
	bool constructorFlag = false;
	PushScope(name, eSymbolScopeTypeWith, inNode->GetFromLine(), inNode->GetFromOffset(), inNode->GetToLine(), inNode->GetToOffset(), constructorFlag);
...
bool NextGenJsAstWalker::VisitVariableDeclarationStatementNodeEnter(class VariableDeclarationStatementNode *inNode)
{
	...
	// Get the identifiers chain (a for example)
	IdentifiersChainSymbolDeclaration identifiers;
	inNode->FillIdentifiersChainDeclaration(identifiers);
	
	// Each token represents an undefined scope, except for the last ('c')
	// For this specific case, the scope type depends on the right hand side declaration
	VIndex count = identifiers.GetIdentifierCount();
	if( count == 1 )
	{
		const XBOX::VString& scopeName = identifiers.GetIdentifierAt(0);
		if( scopeName != CVSTR("this") )
		{
			AssignmentDeclaration assignment;
			inNode->FillAssignmentDeclaration(assignment);
			...
			ISymbolDeclaration*	rhsDeclaration = assignment.GetRhs();
			if( rhsDeclaration != NULL )
			{
				rhsDeclaration->GetBondaries(fromLine, fromOffset, toLine, toOffset);
				
				bool constructorFlag = false;

				ESymbolDeclarationType rhsType = rhsDeclaration->GetSymbolDeclarationType();
				if( rhsType == eSymbolDeclarationTypeFunctionExpression )
				{
					PushScope(scopeName, eSymbolScopeTypeFunctionExpression, fromLine, fromOffset, toLine, toOffset, constructorFlag);
				}
				else if( rhsType == eSymbolDeclarationTypeObject )
				{
					PushScope(scopeName, eSymbolScopeTypeObject, fromLine, fromOffset, toLine, toOffset, constructorFlag);
				}
				else
				{
					VariableDeclaration declaration;
					
					declaration.SetName(scopeName);
					declaration.SetRhs(rhsDeclaration->Clone());
					( rhsType == eSymbolDeclarationTypeNew ) ? declaration.SetInstanceFlag(true) : declaration.SetInstanceFlag(false);
					
					AddVariableDeclarationToScope(declaration.Clone());
				}
			}
		}
	}
	...
	if( count == 1 )
	{
		...
		if( rhsDeclaration != NULL )
		{
			...
			else
			{
				...
				if(	rhsType == eSymbolDeclarationTypeStringLiteral ||
					rhsType == eSymbolDeclarationTypeNumberLiteral ||
					rhsType == eSymbolDeclarationTypeBooleanLiteral ||
					rhsType == eSymbolDeclarationTypeRegExpLiteral ||
					rhsType == eSymbolDeclarationTypeArrayLiteral ||
					rhsType == eSymbolDeclarationTypeNullLiteral )
				{
					declaration.SetRhs(assignment.GetRhs()->Clone());
				}
				
				AddPropertyDeclarationToScope(declaration.Clone());
			}
		}
	}
...
bool NextGenJsAstWalker::VisitAssignmentExpressionNodeEnter(class AssignmentExpressionNode *inNode)
{
	...
	// Get the identifiers chain (a.b.c for example)
	IdentifiersChainSymbolDeclaration identifiers;
	inNode->FillIdentifiersChainDeclaration(identifiers);
	...
	{
		const XBOX::VString& scopeName = identifiers.GetIdentifierAt(index);
		if( scopeName != CVSTR("this") )
		{
			bool constructorFlag = false;
			...
			if( index < (count-1) )
			{
				PushScope(scopeName, eSymbolScopeTypeUndefined, 0, 0, 0, 0, constructorFlag);
			}
			else
			{
				AssignmentDeclaration assignment;
 ...
 *
 */
Visitor* Visitor::GetDeclarationParser( ISymbolTable* inSymTable, Symbols::IFile* inOwningFile, JavaScriptParserDelegate* inDelegate, SymbolScopeDeclaration* inDeclarationScopes )
{
#if ACTIVATE_NEXT_JS_AST_WALKER
