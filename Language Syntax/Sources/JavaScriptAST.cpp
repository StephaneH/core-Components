                ...
                if( functionCall->GetName() == "require" )
                {
                    ...
                    if( args->ArgumentCount() == 1 )
                    {
                        ...
                        if( module )
                        {
                            ...
                            std::vector<Symbols::IFile*> files = this->fSymTable->RetainFilesByName(moduleName);
                            if( files.size() >= 1 )
                            {
                                ...
                                for(file=files.begin(); file!=files.end(); file++)
                                {
											...
											((*file)->GetBaseFolder() == eSymbolFileBaseFolderProject && (*file)->GetPath().BeginsWith("Modules/")) )
                                    {
                                        ...
                                        std::vector<Symbols::ISymbol*> exports = this->fSymTable->RetainSymbolsByName(NULL, "exports", (*file));
                                        if( exports.size() == 1 )
                                        {
