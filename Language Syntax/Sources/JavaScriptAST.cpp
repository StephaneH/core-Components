...
ISymbolDeclaration* TypesDeclaration::Clone() const
{
	...
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void TypesDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void VariableDeclaration::ToDatabase(ISymbolTable* inSymbolTable, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext, SymbolDefinitionContext& inDefinitionContext, sLONG& outSymbolId) const
{
	if( inSymbolTable != NULL )
	{
		...
		updateParameters->SetBoundaries(fAtLine, -1, -1, -1);
		
		sLONG lhsDefinitionId;
		if( inSymbolTable->GetDefinition(inParentScopeId, outSymbolId, inSourceId, definitionKind, -1, updateParameters, lhsDefinitionId) == false )
			inSymbolTable->SetDefinition(inSourceId, inParentScopeId, outSymbolId, inExecutionContext, definitionKind, fAtLine, lhsDefinitionId);
		...
		if( fRhs != NULL )
		{
			...
			sLONG globalScopeId;
			inSymbolTable->GetGlobalScope(inExecutionContext, NULL, globalScopeId);
			...
			switch (declarationTypeValue)
			{
				...
				{
					sLONG rhsScopeId;
					if( inSymbolTable->GetScope(declarationTypeName, inExecutionContext, inParentScopeId, globalScopeId, rhsScopeId) == true )
					{
						sLONG referenceId;
						if( inSymbolTable->GetReference(eSymbolReferenceKindVariableDeclaration, lhsDefinitionId, rhsScopeId, updateParameters, referenceId) == false )
							inSymbolTable->SetReference(eSymbolReferenceKindVariableDeclaration, lhsDefinitionId, rhsScopeId, inSourceId);
					}

					if( declarationTypeValue == eSymbolDeclarationTypeObject )
						definitionType = CVSTR("Object");
					else if( declarationTypeValue == eSymbolDeclarationTypeFunctionExpression )
						definitionType = CVSTR("Function");
				}
					break;
				...
				case eSymbolDeclarationTypeFunctionCall:
				{
					...
					definitionType.GetSubStrings('.', identifiers);
					if( identifiers.size() == 3 )
					{
						if( identifiers[0] == CVSTR("ds") )
						{
							if( identifiers[2] == CVSTR("createEntity") || identifiers[2] == CVSTR("createEntityCollection") )
							{
								processed = true;
								
								if( identifiers[2] == CVSTR("createEntity") )
								{
									definitionType = identifiers[1];
									definitionType.AppendString(CVSTR("+Entity"));
								}
								else
								{
									definitionType = identifiers[1];
									definitionType.AppendString(CVSTR("+EntityCollection"));
								}
							}
						}
					}
					...
					{
						if( identifiers[1] == CVSTR("createEntity") || identifiers[1] == CVSTR("createEntityCollection") )
						{
							processed = true;
							
							if( identifiers[1] == CVSTR("createEntity") )
								definitionType = CVSTR("Entity");
							else
								definitionType = CVSTR("EntityCollection");
						}
					}
					
					if( processed == false )
					{
						// declarationTypeName is the name of the function call
						// 1. get the scope id associated to this function
						// 2. get the return values of that function
						sLONG callScopeId;
						if( inSymbolTable->GetScope(declarationTypeName, eSymbolScopeTypeAll, inParentScopeId, -1, inExecutionContext, updateParameters, callScopeId) == true )
						{
							std::vector<VString> returnNames;
							if( inSymbolTable->GetReturnTypeNames(callScopeId, returnNames) == true )
								definitionType = returnNames[0];
						}
					}
				}
					break;
				...
				case eSymbolDeclarationTypeIdentifiers:
				{
					...
					if (identifiersCount != 0 )
					{
						...
						// At least, search from inParentScopeId
						std::vector<sLONG> fromScopeIds;
						fromScopeIds.push_back(inParentScopeId);

						for(VIndex identifierIndex=0; identifierIndex<identifiersCount; ++identifierIndex)
						{
							VString currentIdentifier = identifiers[identifierIndex];
						
							std::vector<sLONG> foundScopeIds;
							if( inSymbolTable->GetScopes(currentIdentifier, fromScopeIds, foundScopeIds) == true )
							{
								fromScopeIds = foundScopeIds;
								
								std::vector<sLONG> foundScopeIdsInCurrentSource;
								for(std::vector<sLONG>::const_iterator scopeIt=foundScopeIds.begin(); scopeIt!=foundScopeIds.end(); ++scopeIt)
								{
									sLONG currentScopeId = (*scopeIt);
									
									sLONG sourceId;
									inSymbolTable->GetScopeSourceId(currentScopeId, sourceId);
									
									if( inSourceId == sourceId )
										foundScopeIdsInCurrentSource.push_back(currentScopeId);
								}
								...
								for(std::vector<sLONG>::const_iterator scopeIt=foundScopeIds.begin(); scopeIt!=foundScopeIds.end(); ++scopeIt)
								{
									...
									ESymbolScopeType currentScopeType;
									inSymbolTable->GetScopeType(currentScopeId, currentScopeType);
									
									if( currentScopeType == eSymbolScopeTypeObject )
										declarationTypeName = CVSTR("Object");
									else if( currentScopeType == eSymbolScopeTypeFunctionExpression )
										declarationTypeName = CVSTR("Function");
									
									if( identifierIndex == (identifiersCount-1) )
									{
										if( foundScopeIdsInCurrentSource.size() > 0 )
											currentScopeId = foundScopeIdsInCurrentSource[0];

										std::vector<sLONG> referenceIds;
										if( inSymbolTable->GetReference(eSymbolReferenceKindVariableDeclaration, currentScopeId, updateParameters, referenceIds) == true )
										{
											for(std::vector<sLONG>::const_iterator referenceIt=referenceIds.begin(); referenceIt!=referenceIds.end(); ++referenceIt)
											{
												sLONG currentReferenceId = (*referenceIt);
												
												sLONG referencedSymbolId;
												inSymbolTable->GetReferenceToId(currentReferenceId, referencedSymbolId);
												
												ESymbolReferenceKind referenceKind;
												inSymbolTable->GetReferenceKind(currentReferenceId, referenceKind);
												
												if( inSymbolTable->GetReference(referenceKind, lhsDefinitionId, referencedSymbolId, updateParameters, currentReferenceId) == false )
													inSymbolTable->SetReference(referenceKind, lhsDefinitionId, referencedSymbolId, inSourceId);
											}
										}
										else
										{
											ESymbolReferenceKind referenceKind;
											if( currentScopeType == eSymbolScopeTypeObject || currentScopeType == eSymbolScopeTypeFunctionExpression )
												referenceKind = eSymbolReferenceKindVariableDeclaration;
											
											sLONG referenceId;
											if( inSymbolTable->GetReference(referenceKind, lhsDefinitionId, currentScopeId, updateParameters, referenceId) == false )
												inSymbolTable->SetReference(referenceKind, lhsDefinitionId, currentScopeId, inSourceId);
										}
									}
								}
							}
							...
							{
								sLONG nameId;
								for(std::vector<sLONG>::const_iterator scopeIt=fromScopeIds.begin(); scopeIt!=fromScopeIds.end(); ++scopeIt)
								{
									sLONG currentScopeId = (*scopeIt);
									
									// First search ONLY one INSTANCE of name in the current file
									bool nameFound = inSymbolTable->GetName(currentIdentifier, currentScopeId, inExecutionContext, inSourceId, NULL, nameId);

									// Then also search in other files
									if( nameFound == false )
										nameFound = inSymbolTable->GetName(currentIdentifier, currentScopeId, inExecutionContext, -1, NULL, nameId);

									if( nameFound == false && declarationTypeName == CVSTR("Datastore") )
									{
										// We may also want to look in model for any Data Classes
										std::vector<sLONG> dataclasseIds;
										if( inSymbolTable->GetScopes(CVSTR("model"), inExecutionContext, globalScopeId, globalScopeId, dataclasseIds) == true )
										{
											for(std::vector<sLONG>::const_iterator it=dataclasseIds.begin(); it!=dataclasseIds.end(); ++it)
											{
												std::vector<VString> names;
												std::vector<sLONG> nameIds;

												inSymbolTable->GetNames(inExecutionContext, currentIdentifier, *it, *it, names, nameIds);

												if( names.size() != 0 )
												{
													declarationTypeName = names[0];
													break;
												}
											}
										}
									...
									{
										sLONG definitionId;
										if( inSymbolTable->GetDefinition(currentScopeId, nameId, -1, eSymbolDefinitionKindAll, -1, NULL, definitionId) == true )
										{
											...
											if( types.size() == 1 )
											{
												...
												if( declarationTypeName == CVSTR("Object") || declarationTypeName == CVSTR("Function") )
												{
													std::vector<sLONG> referenceIds;
													if( inSymbolTable->GetReference(eSymbolReferenceKindVariableDeclaration, definitionId, updateParameters, referenceIds) == true )
													{
														for(std::vector<sLONG>::const_iterator it=referenceIds.begin(); it!=referenceIds.end(); ++it)
														{
															sLONG currentReferenceId = (*it);
...
//				case eSymbolDeclarationTypeProperty:
		...
		if( inSymbolTable->GetType(lhsDefinitionId, definitionType, updateParameters, typeId) == false )
			inSymbolTable->SetType(lhsDefinitionId, inSourceId, definitionType);
...
void VariableDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void ObjectFieldDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void FunctionCallDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void NewDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void ReturnsDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void FunctionArgumentsDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new StringLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void StringLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new NumberLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void NumberLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new BooleanLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void BooleanLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new RegExpLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void RegExpLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new ArrayLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void ArrayLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new NullLiteralDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void NullLiteralDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
{
	ISymbolDeclaration* cloned = new UndefinedDeclaration();
	cloned->SetBoundaries(fAtLine, 0, 0, 0);
...
void UndefinedDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void IdentifiersChainSymbolDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void IdentifiersChainSymbolDeclaration::ProcessAsAssignmentDeclaration(ISymbolTable* inSymbolTable, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext, SymbolDefinitionContext& inDefinitionContext, sLONG& outSymbolId) const
{
	...
	for(VIndex index=0; index<count; ++index)
	{
		...
		bool isInstanceAttachedTo = false;
		if( index > 0 )
		{
			if( GetIdentifierAt(0) == CVSTR("this") || GetIdentifierAt(index-1) == CVSTR("prototype") )
				isInstanceAttachedTo = true;
		}
		...
		if( createScopeForAllIdentifiersChainElement == true || index < (count-1) )
		{
			...
			{
				updateParameters->MustSetBoundaries(true);
				updateParameters->SetBoundaries(fAtLine, 0, fAtLine, 0);
			}
			
			bool constructorFlag = ( modelKludge == true && index == 1 ) ? true : false;

			sLONG savedScopeId = currentScopeId;
			
			// Look for a scope first
			bool symbolFound = inSymbolTable->GetScope(identifier, eSymbolScopeTypeAll, currentScopeId, inSourceId, inExecutionContext, updateParameters, currentScopeId);
			if( symbolFound == true && isInstanceAttachedTo == true )
			{
				sLONG tempId;
				inSymbolTable->GetDefinition(savedScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, tempId);
			}
			else if( symbolFound == true && identifier == CVSTR("prototype") && index == 1 )
			{
				sLONG tempId;
				if( inSymbolTable->GetDefinition(savedScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, tempId) == false )
					inSymbolTable->SetDefinition(inSourceId, savedScopeId, nameId, inExecutionContext, eSymbolDefinitionKindProperty, fAtLine, tempId);
			}
			...
			if( symbolFound == false )
			{
				...
				if( inSymbolTable->GetDefinition(currentScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, definitionId) == true )
				{
					std::vector<sLONG> referenceIds;
					
					bool referenceFound = inSymbolTable->GetReference(eSymbolReferenceKindVariableDeclaration, definitionId, updateParameters, referenceIds);
					if( referenceFound == false )
						referenceFound = inSymbolTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, definitionId, updateParameters, referenceIds);
					
					if( referenceFound == true )
					{
						symbolFound = true;

						inSymbolTable->GetReferenceToId(referenceIds[0], currentScopeId);
					}
				}
			}
			...
			if( symbolFound == false )
			{
				inSymbolTable->SetScope(identifier, eSymbolScopeTypeUndefined, currentScopeId, inSourceId, inExecutionContext, fAtLine, 0, fAtLine, 0, constructorFlag, currentScopeId);
				
				if( isInstanceAttachedTo == true )
				{
					sLONG tempId;
					if( inSymbolTable->GetDefinition(savedScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, tempId) == false )
						inSymbolTable->SetDefinition(inSourceId, savedScopeId, nameId, inExecutionContext, eSymbolDefinitionKindInstanceProperty, fAtLine, tempId);
				}
				else if( identifier == CVSTR("prototype") && index == 1 )
				{
					sLONG tempId;
					if( inSymbolTable->GetDefinition(savedScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, tempId) == false )
						inSymbolTable->SetDefinition(inSourceId, savedScopeId, nameId, inExecutionContext, eSymbolDefinitionKindProperty, fAtLine, tempId);
		...
		else
		{
			...
			if( inSymbolTable->GetDefinition(currentScopeId, nameId, inSourceId, eSymbolDefinitionKindAll, -1, updateParameters, outSymbolId) == false )
				inSymbolTable->SetDefinition(inSourceId, currentScopeId, nameId, inExecutionContext, definitionKind, fAtLine, outSymbolId);
		}
	}
...
{
	// We set a new return type only if its identifier match something existing from current scope to global scope
	sLONG globalScopeId;
	if( inSymbolTable->GetGlobalScope(inExecutionContext, NULL, globalScopeId) == true )
	{
		...
		if( count > 0 )
		{
			...
			sLONG returnTypeScopeId;
			if( inSymbolTable->GetScope(typeName, inExecutionContext, inParentScopeId, globalScopeId, returnTypeScopeId) == true )
			{
				ISymbolUpdateParameters* updateParameters = inSymbolTable->RetainSymbolUpdateParameters();
				updateParameters->MustSetStatus(true);
				updateParameters->SetStatus(eSymbolStatusUpToDate);
			...
			{
				sLONG returnTypeNameId;
				if( inSymbolTable->GetNameFromScope(typeName, inParentScopeId, inExecutionContext, inSourceId, returnTypeNameId) == true )
				{
					sLONG scopeId;
					inSymbolTable->GetNameScopeId(returnTypeNameId, scopeId);
					
					sLONG returnTypeDefinitionId;
					if( inSymbolTable->GetDefinition(scopeId, returnTypeNameId, inSourceId, eSymbolDefinitionKindAll, -1, NULL, returnTypeDefinitionId) == true )
					{
						std::vector<XBOX::VString> typeNames;
						if( inSymbolTable->GetTypeNames(returnTypeDefinitionId, typeNames) == true )
						{
							ISymbolUpdateParameters* updateParameters = inSymbolTable->RetainSymbolUpdateParameters();
							updateParameters->MustSetStatus(true);
							updateParameters->SetStatus(eSymbolStatusUpToDate);
							
							VIndex typesCount = typeNames.size();
							for(VIndex typeIndex=0; typeIndex<typesCount; ++typeIndex)
							{
	...
	if( inSymbolTable != NULL )
	{
		if( fRhs != NULL )
		{
			if( fLhs != NULL )
			{
				...
				else if( type == eSymbolDeclarationTypeIdentifiers || type == eSymbolDeclarationTypeNew )
				{
					VectorOfVString identifiers;
					...
					if (identifiersCount != 0 )
					{
						...
						if( inSymbolTable->GetGlobalScope(inExecutionContext, NULL, globalScopeId) == true )
						{
							...
							for(VIndex index=0; index<identifiersCount; ++index)
							{
								...
								std::vector<sLONG> foundScopeId;
								if( inSymbolTable->GetScopes(name, inExecutionContext, fromScopeId, globalScopeId, foundScopeId) == true )
								{
									fromScopeId = foundScopeId[0];
									if( index == (identifiersCount-1) )
									{
										rhsFound = true;
										rhsId = fromScopeId;
									}
								...
								else
								{
									sLONG nameId;
									// First search ONLY one INSTANCE of name in the current file
									bool nameFound = inSymbolTable->GetName(name, fromScopeId, inExecutionContext, inSourceId, NULL, nameId);
									
									// Then also search in other files
									if( nameFound == false )
										nameFound = inSymbolTable->GetName(name, fromScopeId, inExecutionContext, -1, NULL, nameId);
									
									if( nameFound == false && typeName == CVSTR("Datastore") )
									{
										// We may also want to look in model for any Data Classes
										std::vector<sLONG> dataclasseIds;
										if( inSymbolTable->GetScopes(CVSTR("model"), inExecutionContext, globalScopeId, globalScopeId, dataclasseIds) == true )
										{
											for(std::vector<sLONG>::const_iterator it=dataclasseIds.begin(); it!=dataclasseIds.end(); ++it)
											{
												...
												inSymbolTable->GetNames(inExecutionContext, name, *it, *it, names, nameIds);
									...
									else if( nameFound == true )
									{
										...
										if( inSymbolTable->GetDefinition(fromScopeId, nameId, -1, eSymbolDefinitionKindAll, -1, NULL, definitionId) == true )
										{
											...
											if( types.size() != 0 )
											{
												...
												if( typeName == CVSTR("Object") || typeName == CVSTR("Function") )
												{
													rhsFound = true;
													rhsId = definitionId;
												}
				...
				{
					if( createScopeForLastIdentifierChainElement == true && (scopeType == eSymbolScopeTypeFunctionExpression || scopeType == eSymbolScopeTypeObject) )
					...
					if( type == eSymbolDeclarationTypeFunctionExpression || type == eSymbolDeclarationTypeObject )
					{
						sLONG globalScopeId;
						inSymbolTable->GetGlobalScope(inExecutionContext, NULL, globalScopeId);

						std::vector<sLONG> rhsScopeIds;
						if( inSymbolTable->GetScopes(typeName, inExecutionContext, inParentScopeId, globalScopeId, rhsScopeIds) == true )
						{
							sLONG rhsScopeId = -1;
							for(std::vector<sLONG>::const_iterator it=rhsScopeIds.begin(); it!=rhsScopeIds.end(); ++it)
							{
								sLONG scopeSourceId;
								if( inSymbolTable->GetScopeSourceId(*it, scopeSourceId) == true )
								{
									if( scopeSourceId == inSourceId )
									{
										rhsScopeId = *it;
										break;
									}
							...
							{
								std::vector<sLONG> referenceIds;
								if( inSymbolTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, rhsScopeId, updateParameters, referenceIds) == true )
								{
									for(std::vector<sLONG>::const_iterator it=referenceIds.begin(); it!=referenceIds.end(); ++it)
									{
										sLONG currentReferenceId = (*it);
										...
										if( inSymbolTable->GetReference(referenceKind, rightMostSymbolId, referencedSymbolId, updateParameters, currentReferenceId) == false )
											inSymbolTable->SetReference(referenceKind, rightMostSymbolId, referencedSymbolId, inSourceId);
									}
								}
								else
								{
									sLONG referenceId;
									if( inSymbolTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, rightMostSymbolId, rhsScopeId, updateParameters, referenceId) == false )
										inSymbolTable->SetReference(eSymbolReferenceKindAssignmentDeclaration, rightMostSymbolId, rhsScopeId, inSourceId);
								}
							}
						}
					}
					else if( type == eSymbolDeclarationTypeIdentifiers && rhsFound == true )
					{
						bool foundReferences = false;

						std::vector<sLONG> referenceIds;
						foundReferences = inSymbolTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, rhsId, updateParameters, referenceIds);
						if( foundReferences == false )
							foundReferences = inSymbolTable->GetReference(eSymbolReferenceKindVariableDeclaration, rhsId, updateParameters, referenceIds);
						
						if( foundReferences == true )
						{
							for(std::vector<sLONG>::const_iterator it=referenceIds.begin(); it!=referenceIds.end(); ++it)
							{
								sLONG currentReferenceId = (*it);
						...
						{
							sLONG referenceId;
							if( inSymbolTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, rightMostSymbolId, rhsId, updateParameters, referenceId) == false )
								inSymbolTable->SetReference(eSymbolReferenceKindAssignmentDeclaration, rightMostSymbolId, rhsId, inSourceId);
						}
					}
...
void AssignmentDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
void PropertyDeclaration::SetBoundaries(const sLONG& inFromLine, const sLONG& inFromOffset, const sLONG& inToLine, const sLONG& inToOffset)
{
	fAtLine = inFromLine;
}
...
SymbolScopeDeclaration* SymbolScopeDeclaration::GetChildScope(const XBOX::VString& inScopeName) const
{
	...
	for(VIndex index=0; index<total; ++index)
	{
		...
		if( child != NULL )
		{
			if( inScopeName.EqualToString(child->GetScopeName(), true) )
			{
				scope = child;
				break;
			}
...
bool Node::HasFunctionParameterTag(std::map<VString, VString>& outParamsNameAndValues) const
{
	bool doHasParamTag = false;
	
	if( fScriptDoc != NULL )
	{
		std::vector<ScriptDocLexer::Element*> params;
		
		fScriptDoc->GetElements( IScriptDocCommentField::kParam, params );
		
		VIndex paramCount = params.size();
		for(VIndex paramIndex=0; paramIndex<paramCount; ++paramIndex)
		{
			ScriptDocLexer::ParamElement* currentParam = dynamic_cast<ScriptDocLexer::ParamElement*>(params[paramIndex]);
			if( currentParam != NULL )
			{
				outParamsNameAndValues[currentParam->fName] = currentParam->fTypes;
...
ISymbolDeclaration* VoidExpressionNode::GetDeclaration() const
{
	ISymbolDeclaration* declaration = NULL;

	if ( fLHS != NULL )
		declaration = fLHS->GetDeclaration();
...
ISymbolDeclaration* CommaExpressionNode::GetDeclaration() const
{
	ISymbolDeclaration* declaration = NULL;
	...
	if( fLHS != NULL )
		...
		declaration = fRHS->GetDeclaration();
...
{
	IdentifiersChainSymbolDeclaration identifiersChain;
	if( fLHS != NULL )
	{
		fLHS->FillIdentifiersChainDeclaration(identifiersChain);
		VIndex identifiersCount = identifiersChain.GetIdentifierCount();
		if( identifiersCount > 1 )
		{
			VString completeIdentifier;
			for(VIndex index=0; index<identifiersCount; ++index)
			{
				if( index > 0 )
					completeIdentifier.AppendString(".");
				
				completeIdentifier.AppendString(identifiersChain.GetIdentifierAt(index));
			}
			inDeclaration.SetName(completeIdentifier);
		}
		else
			inDeclaration.SetName(fIdentifier);
	}
...
void VariableDeclarationStatementNode::AttachScriptDocComment( ScriptDocComment *inComment )
{
	if( inComment != NULL )
	{
		Node::AttachScriptDocComment(inComment);
		if( mAssignment != NULL )
		{
			if( mAssignment->GetNodeTypeNG() == Node::kTypeFunctionExpressionNode )
				mAssignment->AttachScriptDocComment( ScriptDocComment::Create(inComment->GetText()) );
		}
	}
}
...
bool NextGenJsAstWalker::VisitFunctionDeclarationStatementNodeEnter(class FunctionDeclarationStatementNode *inNode )
{
	...
	// By defaults, function declaration must inherits from Function if nothing else specified
	if( extendFlag == false )
	{
		extendedName = CVSTR("Function");
		extendFlag = true;
	}
	...
	std::map<VString, VString> paramsNameAndValues;
	if( inNode->HasFunctionParameterTag(paramsNameAndValues) == true )
	{
		std::map<VString, VString>::const_iterator it;
		for(it=paramsNameAndValues.begin(); it!=paramsNameAndValues.end(); ++it)
		{
			VString name = it->first;
			
			VString typeString = it->second;
			VectorOfVString types;
			typeString.GetSubStrings('|', types);
			
			VIndex typesCount = types.size();
			for(VIndex typeIndex=0; typeIndex<typesCount; ++typeIndex)
			{
				VString currentType = types[typeIndex];
				
				VariableDeclaration variable;
				variable.SetName(name);
				variable.SetRhs(NULL);
				variable.SetRhsTypeName(currentType);
				variable.SetBoundaries(inNode->GetFromLine(), 0, 0, 0);
...
bool NextGenJsAstWalker::VisitVariableDeclarationStatementNodeEnter(class VariableDeclarationStatementNode *inNode)
{
	...
	if( count == 1 )
	{
		...
		variable.SetBoundaries(atLine, 0, 0, 0);
...
bool NextGenJsAstWalker::VisitObjectLiteralFieldNodeEnter(class ObjectLiteralFieldNode *inNode)
{
    ...
    if( identifierNode != NULL )
    {
        ...
        if( count == 1 )
        {
            ...
            property.SetBoundaries(atLine, 0, 0, 0);
...
bool NextGenJsAstWalker::VisitAssignmentExpressionNodeEnter(class AssignmentExpressionNode *inNode)
{
	...
	bool variableDeclarationHack = false;
	
	Node* rhsNode = inNode->GetRHS();
	if( rhsNode != NULL )
	{
		EASTNodeType rhsType = rhsNode->GetNodeTypeNG();
		if( rhsType == Node::kTypeObjectLiteralNode || rhsType == Node::kTypeFunctionExpressionNode )
		{
			IdentifiersChainSymbolDeclaration identifiers;
			inNode->FillIdentifiersChainDeclaration(identifiers);
			...
			if( count == 1 )
			{
				...
				variable.SetName(name);
				variable.SetLocalFlag(false);
				
				sLONG atLine = inNode->GetLineNumber();
				variable.SetBoundaries(atLine, 0, 0, 0);
				
				XBOX::VString rhsTypeName = "Undefined";
				if( rhsType == Node::kTypeObjectLiteralNode )
					rhsTypeName = CVSTR("Object");
				else if( rhsType == Node::kTypeFunctionExpressionNode )
					rhsTypeName = CVSTR("Function");
				
				ISymbolDeclaration* rhs = rhsNode->GetDeclaration();
				variable.SetRhsTypeName(rhsTypeName);
				variable.SetRhs(rhs);
				
				AddVariableDeclarationToScope(variable.Clone());
				
				variableDeclarationHack = true;
			}
		}
	}
	
	if( variableDeclarationHack == false )
	{
		ISymbolDeclaration* lhsDeclaration = inNode->GetLHS()->GetDeclaration();
		if( lhsDeclaration != NULL )
		{
			sLONG fromLine = inNode->GetLineNumber();
			
			AssignmentDeclaration assignDeclaration;
			assignDeclaration.SetBoundaries(fromLine, 0, 0, 0);
			
			lhsDeclaration->SetBoundaries(fromLine, 0, 0, 0);
			assignDeclaration.SetLhs(lhsDeclaration);
			
			ISymbolDeclaration* rhsDeclaration = inNode->GetRhsDeclaration();
			if( rhsDeclaration != NULL )
			{
				rhsDeclaration->SetBoundaries(fromLine, 0, 0, 0);
...
bool NextGenJsAstWalker::VisitForExpressionInClauseNodeEnter(class ForExpressionInClauseNode *inNode)
{
	...
	Node* declaration = inNode->GetForDeclaration();
	if( declaration != NULL )
	{
		EASTNodeType declarationType = declaration->GetNodeTypeNG();
		if( declarationType == Node::kTypeIdentifierNode )
		{
			IdentifiersChainSymbolDeclaration identifiers;
			declaration->FillIdentifiersChainDeclaration(identifiers);
			...
			if( count == 1 )
			{
				...
				sLONG atLine = declaration->GetLineNumber();
				variable.SetBoundaries(atLine, 0, 0, 0);
