...
bool JavaScriptLexer::AdvanceOneToken( int &outToken, TokenList *outTokens, bool inIgnoreNewLines )
{
	...
	if ( IsRegularExpressionStart(outTokens, uChar) && ConsumePossibleRegularExpression(body, flags) ) {
		...
		if (outTokens)	outTokens->push_back( new JavaScriptLexerToken( ILexerToken::TT_INVALID, fLexerInput->GetCurrentPosition(), 1, VString( fLexerInput->MoveToNextChar() ) ) );
		outToken = -1;
		return true;
	}
...
#if _DEBUG
static int SetSourceAndLexToken( JavaScriptLexer *lexer, const char *source )
{
	VString sourceString = source;
	lexer->SetLexerInput( &sourceString );
	return lexer->GetNextTokenForParser();
}

void JavaScriptLexer::Test( void )
{
	// Unit testing!
	JavaScriptLexer *lexer = new JavaScriptLexer();

	// The lexer requires that the ownership of the source string
	// reside with the user, which means we cannot use a temporary
	// string, since that will be destroyed.
	VString sourceString = "function foo( i, j )";
	lexer->SetLexerInput( &sourceString );
	TokenList list;
	ILexerToken::TYPE expectedResults[] = { ILexerToken::TT_JAVASCRIPT_KEYWORD, 
		ILexerToken::TT_WHITESPACE, ILexerToken::TT_NAME, 
		ILexerToken::TT_PUNCTUATION, ILexerToken::TT_WHITESPACE, ILexerToken::TT_NAME,
		ILexerToken::TT_PUNCTUATION, ILexerToken::TT_WHITESPACE, ILexerToken::TT_NAME,
		ILexerToken::TT_WHITESPACE, ILexerToken::TT_PUNCTUATION };

	lexer->Tokenize( list, false );
	xbox_assert( list.size() == 11 );
	for (int i = 0; i < 11; i++) {
		xbox_assert( list[ i ]->GetType() == expectedResults[ i ] );
	}

	// Reset the expected input and test that the values we get individually are correct
	sourceString = "if (1.0e-12 + 14 === 'some string' && functionCall()) { var j++; }";
	lexer->SetLexerInput( &sourceString );
	int count = 0;
	int expectedValues[] = { JavaScriptTokenValues::IF, '(', JavaScriptTokenValues::NUMBER,
		'+', JavaScriptTokenValues::NUMBER, JavaScriptTokenValues::STRICTEQUALITY,
		JavaScriptTokenValues::STRING_QUOTATION_MARK, JavaScriptTokenValues::LOGICALAND, JavaScriptTokenValues::IDENTIFIER,
		'(', ')', ')', '{', JavaScriptTokenValues::VAR, JavaScriptTokenValues::IDENTIFIER, JavaScriptTokenValues::INCREMENTOR,
		';', '}' };
	for (int tk = lexer->GetNextTokenForParser(); tk != -1; tk = lexer->GetNextTokenForParser()) {
		xbox_assert( tk == expectedValues[ count++ ] );
	}

	// Test out some regular expressions
	sourceString = "var objRegex = /ab+c/i;";
	lexer->SetLexerInput( &sourceString );
	
	xbox_assert( JavaScriptTokenValues::VAR == lexer->GetNextTokenForParser() );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == lexer->GetNextTokenForParser() );
	xbox_assert( '=' == lexer->GetNextTokenForParser() );
	xbox_assert( '/' == lexer->GetNextTokenForParser() );
	// Now we expect a regular expression
	VString body, flags;
	xbox_assert( lexer->GetNextRegularExpressionLiteral( &body, &flags ) );
	xbox_assert( body == "ab+c" );
	xbox_assert( flags == "i" );
	xbox_assert( ';' == lexer->GetNextTokenForParser() );

	sourceString = "if (mystring.match(/c\\\\ap.*?/)) {";
	lexer->SetLexerInput( &sourceString );
	xbox_assert( JavaScriptTokenValues::IF == lexer->GetNextTokenForParser() );
	xbox_assert( '(' == lexer->GetNextTokenForParser() );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == lexer->GetNextTokenForParser() );
	xbox_assert( '.' == lexer->GetNextTokenForParser() );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == lexer->GetNextTokenForParser() );
	...
	list.clear();
	lexer->Tokenize( list, false );
	xbox_assert( list.size() == 1 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_STRING );
	xbox_assert( list[ 0 ]->GetText() == "\"This\nis a test.\tHow 'well' does it work?\"" );

	// Test lexing the various legal numerical formats
	sourceString = "123. 123.0005 1123.0e12 121.012e-35 1121212.0e+11 .213231 .1e155 .1333e+13 .1e-1 112323 "
					"112e13 1231e+11 31e-13 0x123ABC 0X456DEF";
	const char *expectedText[] = { "123.", "123.0005", "1123.0e12", "121.012e-35", "1121212.0e+11", ".213231", 
									".1e155", ".1333e+13", ".1e-1", "112323", "112e13", "1231e+11", "31e-13",
									"0x123ABC", "0X456DEF" };
	count = 0;
	lexer->SetLexerInput( &sourceString );
	list.clear();
	lexer->Tokenize( list, false );
	xbox_assert( list.size() == 29 );	// The whitespace counts too!
	for (TokenList::iterator iter = list.begin(); iter != list.end(); ++iter) {
		if ((count % 2) == 0) {
			xbox_assert( (*iter)->GetType() == ILexerToken::TT_NUMBER );
			xbox_assert( (*iter)->GetText() == expectedText[ count / 2 ] );
		} else {
			xbox_assert( (*iter)->GetType() == ILexerToken::TT_WHITESPACE );
		}
		count++;
	}

	// Test to make sure the illegal numerical formats come back as being errors
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "000" ) );
	xbox_assert( '.' == SetSourceAndLexToken( lexer, "." ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "3in" ) );
	xbox_assert( JavaScriptTokenValues::NUMBER == SetSourceAndLexToken( lexer, "0" ) );

	// Test to make sure legal and illegal string forms are lexed properly
	xbox_assert( JavaScriptTokenValues::STRING_APOSTROPHE == SetSourceAndLexToken( lexer, "\"test\"" ) );
	xbox_assert( JavaScriptTokenValues::STRING_QUOTATION_MARK == SetSourceAndLexToken( lexer, "'test'" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "\"test that should not work" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "'test that should not work" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "'test that should not work\"" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "\"test that should not work'" ) );
	
	
	// Test that all of the punctuation comes back properly
	xbox_assert( '{' == SetSourceAndLexToken( lexer, "{" ) );
	xbox_assert( '}' == SetSourceAndLexToken( lexer, "}" ) );
	xbox_assert( '(' == SetSourceAndLexToken( lexer, "(" ) );
	xbox_assert( ')' == SetSourceAndLexToken( lexer, ")" ) );
	...
	xbox_assert( '<' == SetSourceAndLexToken( lexer, "<" ) );
	xbox_assert( '>' == SetSourceAndLexToken( lexer, ">" ) );
	xbox_assert( JavaScriptTokenValues::LESSTHANOREQUALTO == SetSourceAndLexToken( lexer, "<=" ) );
	xbox_assert( JavaScriptTokenValues::GREATERTHANOREQUALTO == SetSourceAndLexToken( lexer, ">=" ) );
	xbox_assert( JavaScriptTokenValues::EQUALITY == SetSourceAndLexToken( lexer, "==" ) );
	xbox_assert( JavaScriptTokenValues::NONEQUALITY == SetSourceAndLexToken( lexer, "!=" ) );
	xbox_assert( JavaScriptTokenValues::STRICTEQUALITY == SetSourceAndLexToken( lexer, "===" ) );
	xbox_assert( JavaScriptTokenValues::STRICTNONEQUALITY == SetSourceAndLexToken( lexer, "!==" ) );
	xbox_assert( '+' == SetSourceAndLexToken( lexer, "+" ) );
	xbox_assert( '-' == SetSourceAndLexToken( lexer, "-" ) );
	xbox_assert( '*' == SetSourceAndLexToken( lexer, "*" ) );
	xbox_assert( '/' == SetSourceAndLexToken( lexer, "/" ) );
	xbox_assert( '%' == SetSourceAndLexToken( lexer, "%" ) );
	xbox_assert( JavaScriptTokenValues::INCREMENTOR == SetSourceAndLexToken( lexer, "++" ) );
	xbox_assert( JavaScriptTokenValues::DECREMENTOR == SetSourceAndLexToken( lexer, "--" ) );
	xbox_assert( JavaScriptTokenValues::LEFTSHIFT == SetSourceAndLexToken( lexer, "<<" ) );
	xbox_assert( JavaScriptTokenValues::SIGNEDRIGHTSHIFT == SetSourceAndLexToken( lexer, ">>" ) );
	xbox_assert( JavaScriptTokenValues::UNSIGNEDRIGHTSHIFT == SetSourceAndLexToken( lexer, ">>>" ) );
	xbox_assert( '&' == SetSourceAndLexToken( lexer, "&" ) );
	xbox_assert( '|' == SetSourceAndLexToken( lexer, "|" ) );
	xbox_assert( '^' == SetSourceAndLexToken( lexer, "^" ) );
	xbox_assert( '!' == SetSourceAndLexToken( lexer, "!" ) );
	xbox_assert( '~' == SetSourceAndLexToken( lexer, "~" ) );
	xbox_assert( JavaScriptTokenValues::LOGICALAND == SetSourceAndLexToken( lexer, "&&" ) );
	xbox_assert( JavaScriptTokenValues::LOGICALOR == SetSourceAndLexToken( lexer, "||" ) );
	xbox_assert( '?' == SetSourceAndLexToken( lexer, "?" ) );
	xbox_assert( ':' == SetSourceAndLexToken( lexer, ":" ) );
	xbox_assert( '=' == SetSourceAndLexToken( lexer, "=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNADD == SetSourceAndLexToken( lexer, "+=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNSUBTRACT == SetSourceAndLexToken( lexer, "-=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNMULTIPLY == SetSourceAndLexToken( lexer, "*=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNDIVIDE == SetSourceAndLexToken( lexer, "/=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNMODULUS == SetSourceAndLexToken( lexer, "%=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNLEFTSHIFT == SetSourceAndLexToken( lexer, "<<=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNSIGNEDRIGHTSHIFT == SetSourceAndLexToken( lexer, ">>=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNUNSIGNEDRIGHTSHIFT == SetSourceAndLexToken( lexer, ">>>=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNBITAND == SetSourceAndLexToken( lexer, "&=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNBITOR == SetSourceAndLexToken( lexer, "|=" ) );
	xbox_assert( JavaScriptTokenValues::ASSIGNBITXOR == SetSourceAndLexToken( lexer, "^=" ) );

	// Test legal and illegal identifiers
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == SetSourceAndLexToken( lexer, "legalIdent" ) );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == SetSourceAndLexToken( lexer, "_legalIdent" ) );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == SetSourceAndLexToken( lexer, "$legalIdent" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "3illegalIdent" ) );
	xbox_assert( JavaScriptTokenValues::IDENTIFIER == SetSourceAndLexToken( lexer, "legalIdent123" ) );
	
	// Make sure this comes back as IDENTIFIER - IDENTIFIER instead of a single identifier
	sourceString = "illegal-identifier";
	lexer->SetLexerInput( &sourceString );
	list.clear();
	lexer->Tokenize( list, false );
	if (testAssert( list.size() == 3 )) {
		xbox_assert( JavaScriptTokenValues::IDENTIFIER == list[ 0 ]->GetValue() );
		xbox_assert( '-' == list[ 1 ]->GetValue() );
		xbox_assert( JavaScriptTokenValues::IDENTIFIER == list[ 2 ]->GetValue() );
	}

	// Now it's time for more comprehensive tests where we string a bunch of these together
	sourceString = "var i = 12.5e-12; function foo( i, j, k ) { alert( \"this is a test\" ); \"true\\false\".match( /[0-9]+\".\"[0-9]*[Ee][+-]?[0-9]+/gi ); }";
	int newExpectedValues[] = { JavaScriptTokenValues::VAR, JavaScriptTokenValues::IDENTIFIER, '=', JavaScriptTokenValues::NUMBER,
							';', JavaScriptTokenValues::FUNCTION, JavaScriptTokenValues::IDENTIFIER, '(', JavaScriptTokenValues::IDENTIFIER,
							',', JavaScriptTokenValues::IDENTIFIER, ',', JavaScriptTokenValues::IDENTIFIER, ')', '{', 
							JavaScriptTokenValues::IDENTIFIER, '(', JavaScriptTokenValues::STRING_APOSTROPHE, ')', ';', JavaScriptTokenValues::STRING_APOSTROPHE,
							'.', JavaScriptTokenValues::IDENTIFIER, '(', '/' }; // After the / comes a RegEx we have to manually handle, then ')', ';' and '}'
	lexer->SetLexerInput( &sourceString );
	count = 0;
	while (true) {
		int value = lexer->GetNextTokenForParser();
		if (!testAssert( -1 != value ))	break;
		if ('/' == value) {
			VString body, flags;
			xbox_assert( lexer->GetNextRegularExpressionLiteral( &body, &flags ) );
			xbox_assert( body == "[0-9]+\".\"[0-9]*[Ee][+-]?[0-9]+" );
			xbox_assert( flags == "gi" );
			break;
		}
		xbox_assert( value == newExpectedValues[ count++ ] );
	}
	// Once we get here, there are still three tokens expected
	xbox_assert( ')' == lexer->GetNextTokenForParser() );
	xbox_assert( ';' == lexer->GetNextTokenForParser() );
	xbox_assert( '}' == lexer->GetNextTokenForParser() );
	xbox_assert( -1 == lexer->GetNextTokenForParser() );

	// Test comment consumption, both in regular and continuation modes
	sourceString = "/* This is a comment and it should work in theory*/";
	lexer->SetLexerInput( &sourceString );
	list.clear();
	lexer->Tokenize( list );
	xbox_assert( list.size() == 1 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_COMMENT );

	sourceString = "/* This is an open comment that's not terminated";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 1 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_OPEN_COMMENT );

	sourceString = "this is the termination of the previous comment.*/";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list, true );
	xbox_assert( list.size() == 1 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_COMMENT );

	sourceString = "// This is a single line comment and should work";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 1 );
	...
	sourceString = "// This is a single line comment also, but\n"
					"// it is followed by a second single-line comment.";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 2 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_COMMENT );
	xbox_assert( list[ 1 ]->GetType() == ILexerToken::TT_COMMENT );
	xbox_assert( lexer->GetCurrentLineNumber() == 1 );

	sourceString = "var i = 12; // This ends a line";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 10 );
	xbox_assert( list[ 9 ]->GetType() == ILexerToken::TT_COMMENT );

	// Make sure multi-line lexing works as expected also
	sourceString = "var i = 1,\n"
					"var j;";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 12 );
	xbox_assert( list[ 0 ]->GetValue() == JavaScriptTokenValues::VAR );
	xbox_assert( list[ 2 ]->GetValue() == JavaScriptTokenValues::IDENTIFIER );
	xbox_assert( list[ 4 ]->GetValue() == '=' );
	xbox_assert( list[ 6 ]->GetValue() == JavaScriptTokenValues::NUMBER );
	xbox_assert( list[ 7 ]->GetValue() == ',' );
	xbox_assert( list[ 8 ]->GetValue() == JavaScriptTokenValues::VAR );
	xbox_assert( list[ 10 ]->GetValue() == JavaScriptTokenValues::IDENTIFIER );
	xbox_assert( list[ 11 ]->GetValue() == ';' );

	// Make sure we handle whitespace and newlines properly
	sourceString = "test\n\ttest";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 3 );
	xbox_assert( list[ 0 ]->GetValue() == JavaScriptTokenValues::IDENTIFIER );
	xbox_assert( list[ 1 ]->GetType() == ILexerToken::TT_WHITESPACE );
	xbox_assert( list[ 2 ]->GetValue() == JavaScriptTokenValues::IDENTIFIER );
	xbox_assert( lexer->GetCurrentLineNumber() == 1 );

	// Make sure that 0 is parsed as a number!
	sourceString = "var i = 0;";
	list.clear();
	lexer->SetLexerInput( &sourceString );
	lexer->Tokenize( list );
	xbox_assert( list.size() == 8 );
	xbox_assert( list[ 6 ]->GetValue() == JavaScriptTokenValues::NUMBER );

	// Make sure that illegal values of 0 aren't parsed as a number
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "00" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "0i" ) );

	// Make sure octal numbers lex properly
	xbox_assert( JavaScriptTokenValues::NUMBER == SetSourceAndLexToken( lexer, "013" ) );
	xbox_assert( JavaScriptTokenValues::NUMBER == SetSourceAndLexToken( lexer, "0777" ) );
	xbox_assert( -1 == SetSourceAndLexToken( lexer, "080" ) );

	// Test octal escape sequences too
	sourceString = "\"This is \\0141 test.\"";
	lexer->SetLexerInput( &sourceString );
	list.clear();
	lexer->Tokenize( list, false );
	xbox_assert( list.size() == 1 );
	xbox_assert( list[ 0 ]->GetType() == ILexerToken::TT_STRING );
	xbox_assert( list[ 0 ]->GetText() == "\"This is a test.\"" );

	// Make sure that newlines and whitespaces are handled properly
	sourceString = "\t\t\n\t\t\n\n// test comment\n/* Another test comment */\n\t\nvar foo\t\n";
	lexer->SetLexerInput( &sourceString );
	xbox_assert( lexer->GetNextTokenForParser() == JavaScriptTokenValues::VAR );
	xbox_assert( lexer->GetNextTokenForParser() == JavaScriptTokenValues::IDENTIFIER );

	// Test to make sure ScriptDoc comments are caught properly.
	sourceString = "/**\n"
					"* This is a test of the ScriptDoc capabilities\n"
					"*\n"
					"* @author	Aaron Ballman aballman@4d.fr\n"
					"* @version 1.0\n"
					"*/";
	lexer->SetLexerInput( &sourceString );
	xbox_assert( lexer->GetNextTokenForParser() == -1 );
	xbox_assert( lexer->ProcessedScriptDocToken() );

	sourceString = "/* This is not a ScriptDoc\n"
					"* comment./*";
	lexer->SetLexerInput( &sourceString );
	xbox_assert( lexer->GetNextTokenForParser() == -1 );
	xbox_assert( !lexer->ProcessedScriptDocToken() );
