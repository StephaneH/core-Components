...
static int IsKeyword( VString* vstrIn )
{
	...
	for (int i = 0; i < JS_KEYWORD_COUNT; i++) {
		if (nFirstChar > JS_KEYWORDS[ i ].vstrName->GetUniChar( 1 ) )
			continue;
		else if (nFirstChar < JS_KEYWORDS [ i ].vstrName->GetUniChar( 1 ))
			break;
		
		if (vstrIn->EqualToString( *(JS_KEYWORDS [ i ].vstrName), true ))
			return JS_KEYWORDS[ i ].nCode;
	}
...
bool JavaScriptLexer::IsNumericStart( UniChar inChar, ENumericType &outType )
{
	...
	if (inChar == CHAR_DIGIT_ZERO) {
		...
		fLexerInput->MoveToNextChar();	// Move beyond the current char, which was the inChar
		bool ret = false;
		if (fLexerInput->HasMoreChars()) {
...
int JavaScriptLexer::ConsumePossiblePunctuation( UniChar inChar, TokenList *outTokens )
{
	...
	switch (inChar)
	{
		case CHAR_PLUS_SIGN:
		{
			...
			if (fLexerInput->HasMoreChars() && CHAR_PLUS_SIGN == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
				tk = JavaScriptTokenValues::INCREMENTOR;
			}
			else if (fLexerInput->HasMoreChars() && CHAR_EQUALS_SIGN == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
		...
		case CHAR_HYPHEN_MINUS:
		{
			...
			// is an assignment
			tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
			if (fLexerInput->HasMoreChars() && CHAR_HYPHEN_MINUS == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
		...
		case CHAR_ASTERISK:
		{
			...
			tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
			if (fLexerInput->HasMoreChars() && CHAR_EQUALS_SIGN == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
		...
		case CHAR_LESS_THAN_SIGN:
		{
			...
			if (fLexerInput->HasMoreChars() && CHAR_LESS_THAN_SIGN == fLexerInput->PeekAtNextChar())
			{
		...
		case CHAR_GREATER_THAN_SIGN:
		{
			...
			tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
			if (fLexerInput->HasMoreChars() && CHAR_GREATER_THAN_SIGN == fLexerInput->PeekAtNextChar())
			{
		...
		case CHAR_AMPERSAND:
		{
			...
			tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
			if (fLexerInput->HasMoreChars() && CHAR_AMPERSAND == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
		...
		case CHAR_VERTICAL_LINE:
		{
			...
			tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
			if (fLexerInput->HasMoreChars() && CHAR_VERTICAL_LINE == fLexerInput->PeekAtNextChar())
			{
				tokenStr.AppendUniChar( fLexerInput->MoveToNextChar() );
		...
		case CHAR_FULL_STOP:
		{
			...
			if (fLexerInput->HasMoreChars() && fLexerInput->PeekAtNextChar() >= CHAR_DIGIT_ZERO && fLexerInput->PeekAtNextChar() <= CHAR_DIGIT_NINE)
			{
	...
	if( tk && outTokens )
	{
		outTokens->push_back( new JavaScriptLexerToken( bIsComparison ? (ILexerToken::TT_COMPARISON) : (ILexerToken::TT_PUNCTUATION), nStartingPosition, tokenStr.GetLength(), tokenStr, tk ) );
	}
...
bool JavaScriptLexer::AdvanceOneToken( int &outToken, TokenList *outTokens, bool inIgnoreNewLines, bool inIgnoreRegExp )
{
	outToken = -1;
	fConsumedNewLine = false;
	fConsumedWhiteSpace = false;
	...
	std::vector<bool> whiteSpaceConsumedFlags;
	std::vector<bool> lineEndingConsumedFlags;
	...
	while (fLexerInput->HasMoreChars())
	{
		...
		{
			fConsumedWhiteSpace = false;
			if (!SkipWhitespaces( fConsumedWhiteSpace, outTokens ))
				return false;
			...
			while (fLexerInput->HasMoreChars() && IsLineEnding( fLexerInput->PeekAtNextChar() ))
			{
				...
				if (!inIgnoreNewLines)
				{
					...
					if (outTokens)
						outTokens->push_back( new JavaScriptLexerToken( ILexerToken::TT_WHITESPACE,	fLexerInput->GetCurrentPosition() - 1, 1, VString( uChar ), outToken ) );
			...
			whiteSpaceConsumedFlags.push_back(fConsumedWhiteSpace);
			lineEndingConsumedFlags.push_back(fConsumedNewLine);

			// If we're done consuming newlines and whitespaces, then we're done with this loop
			if (!fConsumedWhiteSpace && !fConsumedNewLine)
				break;
		}
	}
	...
	// Update the new line consumed flag.
	( std::find(lineEndingConsumedFlags.begin(), lineEndingConsumedFlags.end(), true) == lineEndingConsumedFlags.end() ) ? fConsumedNewLine = false : fConsumedNewLine = true;
	...
	if ( !inIgnoreRegExp && IsRegularExpressionStart(outTokens, uChar) && ConsumePossibleRegularExpression(body, flags) )
	{
		...
		if (outTokens)
			outTokens->push_back( new JavaScriptLexerToken( tokenType, fLexerInput->GetCurrentPosition() - regExp.GetLength(), regExp.GetLength(), regExp, outToken ) );

	}
	else if( (outToken = ConsumePossiblePunctuation(uChar, outTokens)) != 0 )
	{
	...
	else if (IsIdentifierStart( uChar ))
	{
		...
		ILexerToken::TYPE tokenType;
		if( (outToken = IsKeyword(vstrNAME)) != 0 )
		{
			tokenType = ILexerToken::TT_JAVASCRIPT_KEYWORD;
		}
		else if( (outToken = IsFutureReservedWord(vstrNAME)) != 0 )
		{
		...
		if (outTokens)
			outTokens->push_back( new JavaScriptLexerToken( tokenType, fLexerInput->GetCurrentPosition() - vstrNAME->GetLength(), vstrNAME->GetLength(), *vstrNAME, outToken ) );
	...
	else
	{
		...
		if (outTokens)
			outTokens->push_back( new JavaScriptLexerToken( ILexerToken::TT_INVALID, fLexerInput->GetCurrentPosition()-1, 1, VString( fLexerInput->MoveToNextChar() ) ) );
		outToken = -1;
		return true;
	}
...
int JavaScriptLexer::PeekAtNextTokenForParser()
{
	...
	int ret = GetNextTokenForParser( true, false );
	fPeeked = true;
	return ret;
}

int JavaScriptLexer::GetNextTokenForParser( bool inIgnoreNewLines, bool inIgnoreRegExp )
{
	...
	int token;
	if (AdvanceOneToken( token, NULL, inIgnoreNewLines, inIgnoreRegExp ))
		return token;
	return -1;
}
...
VError JavaScriptLexer::Tokenize( TokenList &outTokens, bool inContinuationOfComment, sLONG inOpenStringType )
{
	...
	int	nToken = 0;
	while (AdvanceOneToken( nToken, &outTokens, true, false ))
		;	// Infinite loop!
