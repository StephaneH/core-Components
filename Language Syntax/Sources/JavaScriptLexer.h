...
private:
	...
	bool fPeeked;
	bool fConsumedNewLine, fConsumedWhiteSpace;
...
public:
	...
	bool ConsumedWhiteSpaceBeforeToken() { return fConsumedWhiteSpace; }
	...
	sLONG GetCurrentLineNumber() {
		return fLineNumber;
	}
	
	sLONG GetCurrentCharacterOffset() {
		sLONG currentPosition = fLexerInput->GetCurrentPosition(false);
	...
	virtual void SetLexerInput(ILexerInput* inInput)
	{
		...
		fConsumedNewLine = false;
		fConsumedWhiteSpace = false;
		fMultilineCommentStart = -1;
		fMultilineCommentEnd = -1;
		fLexedRegionComment = false;
		fRegionLineNumber = 0;
