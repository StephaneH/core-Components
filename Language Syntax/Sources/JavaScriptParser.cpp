...
JavaScriptAST::Node *JavaScriptParser::ParsePrimaryExpression( int &tk )
{
	...
	if (JavaScriptTokenValues::IDENTIFIER == tk) {
		...
		if (fSymbolTable) {
			...
			if(			fContextFile->GetPath().Find(".waPage")
					&&	fContextFile->GetPath().Find("script")
					&&	fContextFile->GetPath().EndsWith(".js")
					&&	ident == "WAF" )
			{
				...
				if( _GetHtmlRelativeFilePathString(htmlRelativeFilePathString) )
				{
					...
					if( files.size() > 0 )
					{
						std::vector<Symbols::ISymbol*> waf = fSymbolTable->RetainSymbolsByName(NULL, ident, files[0]);
						if( waf.size() > 0 )
						{
							contextSym = waf[0];
							contextSym->Retain();
		...
		if (refLookupSym)
		{
			if ( mapOfSymsID[ refLookupSym->GetID() ] != true )
			{
				...
				for (itRefLookupSymProto = refLookupSym->GetPrototypes().begin(); itRefLookupSymProto != refLookupSym->GetPrototypes().end(); ++itRefLookupSymProto)
				{
					...
					{
						std::vector< Symbols::ISymbol * > protoOwners = fSymbolTable->RetainSymbolsByName(NULL, owner->GetName());
	...
	for (std::vector<Symbols::ISymbol *>::iterator it = fCompletionSymbolList.begin(); it != fCompletionSymbolList.end(); ++it)
	{
		...
		else
		{
            ...
            if( sym->GetName() == "require" )
            {
                ...
                files = this->fSymbolTable->RetainFilesByName(moduleName);
                if( files.size() >= 1 )
                {
                    ...
                    for(file=files.begin(); file!=files.end(); file++)
                    {
								...
								((*file)->GetBaseFolder() == eSymbolFileBaseFolderProject && (*file)->GetPath().BeginsWith("Modules/")) )
                        {
                            ...
                            modules = this->fSymbolTable->RetainSymbolsByName(NULL, "exports", (*file));
                            if( modules.size() == 1 )
                            {
                                sym = modules.at(0);
                                break;
                ...
                outReturnTypes.insert(outReturnTypes.begin(), sym);
                return;
            }
...
public:
	...
	size_t GetSymbolCount() {
		...
		syms = mTable->RetainNamedSubSymbols( NULL );

		size_t ret = syms.size();
		mTable->ReleaseSymbols(syms);
		return ret;
	}
	...
	Symbols::ISymbol *GetSymbolByName( Symbols::ISymbol *inSym, VString inName ) {
		std::vector< Symbols::ISymbol * > syms;
		syms = mTable->RetainNamedSubSymbols( inSym );
...
void TestSubsymbolNames( SymbolTableAdapter *table, Symbols::ISymbol *owner, int count, ... )
{
	...
	std::vector< Symbols::ISymbol * > nodes = table->GetBase()->RetainNamedSubSymbols( owner );
