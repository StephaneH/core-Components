...
JavaScriptParser::JavaScriptParser(IJsSuggestionStrategy& inSuggestionStrategy)
#else
...
, kParsingError( -50 )
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
, fSuggestionStrategy(inSuggestionStrategy)
#else
, fSuggestions( NULL )
...
JavaScriptAST::Node *JavaScriptParser::ParseLiteral( int &tk )
{
	...
	switch (tk) {
		...
		case JavaScriptTokenValues::KWORD_TRUE: {
			...
			tk = fLexer->GetNextTokenForParser();
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("Boolean"), true);
#else
		...
		case JavaScriptTokenValues::STRING_QUOTATION_MARK: {
			...
			tk = fLexer->GetNextTokenForParser();
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("String"), true);
#else
		...
		case JavaScriptTokenValues::NUMBER:	{
			...
			tk = fLexer->GetNextTokenForParser();
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("Number"), true);
#else
		...
		case JavaScriptTokenValues::REGEXP: {
			...
			tk = fLexer->GetNextTokenForParser();
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("RegExp"), true);
#else
		...
		case -1: {
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordNull);
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordTrue);
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordFalse);
#else
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("Array"), true);
#else
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.SuggestIdentifiers(true);
#else
			if (fSuggestions)
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	fSuggestionStrategy.AddSuggestionContextSymbol(CVSTR("Object"), true);
#else
	else
	{
...
JavaScriptAST::Node *JavaScriptParser::ParsePrimaryExpression( int &tk )
{
	...
	if (JavaScriptTokenValues::IDENTIFIER == tk) {
		...
		if (fSymbolTable) {
			...
			{
				_SuggestSourceIdentifiers(ident);
			}
			else
			{
	...
	if( -1 == tk )
	{
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordThis);
		fSuggestionStrategy.SuggestIdentifiers(true);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordArguments);
	}
#else
...
JavaScriptAST::Node *JavaScriptParser::ParseMemberExpression( int &tk, bool argsRequired )
{
	...
	if( -1 == tk )
	{
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordNew);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordFunction);
	}
#else
	...
	do {
		...
		switch (tk)
		{
			...
			case '.': {
				...
				{
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
					fSuggestionStrategy.SuggestIdentifiers(false);
#else
...
JavaScriptAST::Node *JavaScriptParser::ParseUnaryExpression( int &tk )
{
	...
	switch (tk) {
		...
		case -1: {
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordDelete);
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordVoid);
			fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordTypeof);
#else
...
#define HANDLE_CASE( nodeType )												\
	...
	do {
		switch (tk) {
			...
			case -1: {
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
				fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordInstanceof);
				if( !noIn )
					fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordIn);
#else
				// We're at the end of input, so suggest possible keywords
				if (fSuggestions) {
...
JavaScriptAST::Node *JavaScriptParser::ParseStatement( int &tk )
{
	...
	{
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordFunction);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordVar);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordIf);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordDo);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordWhile);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordFor);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordBreak);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordContinue);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordReturn);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordWith);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordSwitch);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordThrow);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordTry);
		fSuggestionStrategy.SuggestIdentifiers(true);
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordArguments);
	}
#else
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	if( -1 == tk )
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordCase);
#else
	if (fSuggestions && -1 == tk) {
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	if( -1 == tk )
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordDefault);
#else
	if (fSuggestions && -1 == tk) {
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	if( -1 == tk )
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordCatch);
#else
	if (fSuggestions && -1 == tk) {
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	if( -1 == tk )
		fSuggestionStrategy.SuggestKeyword(IJsSuggestionStrategy::kKeywordFinally);
#else
	if (fSuggestions && -1 == tk) {
...
Symbols::ISymbol* JavaScriptParser::_SuggestSourceIdentifiers(const VString& sourceId)
{
	Symbols::ISymbol* contextSym = NULL;
