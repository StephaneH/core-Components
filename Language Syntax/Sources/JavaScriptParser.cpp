...
JavaScriptAST::Node *JavaScriptParser::GetAST( VFilePath inFilePath, sLONG inFileId, VString *inInput, bool& outError, std::vector<XBOX::VString>* outSyntaxErrors/*=NULL*/ )
{
...
JavaScriptAST::Node* JavaScriptParser::ParseMemberExpression( int &tk, bool argsRequired, bool inFromNewStatement )
{
	...
	do
	{
		...
		switch (tk)
		{
			...
			{
				tk = fLexer->GetNextTokenForParser();
				JavaScriptAST::Node* rhs = ParseMemberExpression( tk, true, true );
				if( rhs == NULL )
				{
					if( fUseNextGen == true && fSuggestions != NULL && fLexer->ConsumedWhiteSpaceBeforeToken() )
					{
						_ResetSuggestions();
						_SuggestConstructors(CVSTR(""));
					}
				...
				else
				{
					...
					bool suggestConstructor = true;
					
					if ('(' == tk)
					{
						suggestConstructor = false;
						args = ParseArguments( tk, argsCount );
					}
					
					done = false;
					
					if( fUseNextGen == true && fSuggestions != NULL && suggestConstructor == true )
					{
						VString constructorName = "";
						
						if( rhs->GetNodeTypeNG() == JavaScriptAST::Node::kTypeDotExpressionNode )
							constructorName = fContextIdentifier;
						else
							constructorName = rhs->GetIdentifierNG();
						
						_ResetSuggestions();
						if( fLexer->ConsumedWhiteSpaceBeforeToken() == false )
							_SuggestConstructors(constructorName);
					}
			...
			case '.':
			{
				...
				if (JavaScriptTokenValues::IDENTIFIER != tk)
				{
					...
					else if( fUseNextGen == true && fSuggestions != NULL )
					{
						if( fLexer->ConsumedWhiteSpaceBeforeToken() == false )
						{
							if( inFromNewStatement == false )
								_SuggestIdentifiers(fContextIdentifier, true, false);
							else
								_SuggestConstructors(fContextIdentifier);
						}
						else
							_ResetSuggestions();
					}
			...
			default:
			{
				if( fUseNextGen == true && fSuggestions != NULL )
				{
					if( (tk == -1 || tk == JavaScriptTokenValues::IDENTIFIER) && ret->GetNodeTypeNG() == JavaScriptAST::Node::kTypeIdentifierNode )
					{
						if( fLexer->ConsumedWhiteSpaceBeforeToken() == false )
						{
							...
							_SuggestIdentifiers(identifier, false, false);
						}
						else
						{
							_ResetSuggestions();
			...
			case '.':
				{
					...
					if (JavaScriptTokenValues::IDENTIFIER != tk)
					{
						...
						else if( fUseNextGen == true && fSuggestions != NULL )
						{
							...
							{
								if( fContextIdentifier.BeginsWith(CVSTR("$$")) == true && fContextIdentifierFunctionCallStatus.size() > 0 && fContextIdentifierFunctionCallStatus[0] == true )
								{
									fContextIdentifier = "WAF.widgets.";
									VString widgetName = fLexer->GetTokenText();
									widgetName.SubString(2, widgetName.GetLength()-2);
									
									fContextIdentifier.AppendString(widgetName);
									fContextIdentifier.AppendString(".");
									
									fContextIdentifierFunctionCallStatus.clear();
									fContextIdentifierFunctionCallStatus.push_back(false);
									fContextIdentifierFunctionCallStatus.push_back(false);
									fContextIdentifierFunctionCallStatus.push_back(false);
...
JavaScriptAST::Node *JavaScriptParser::ParseLeftHandSideExpression( int &tk )
{
	...
	JavaScriptAST::Node* ret = NULL;
	ret = ParseMemberExpression( tk, false, false );
	if( ret == NULL )
		return NULL;
...
void JavaScriptParser::_SuggestConstructors(const XBOX::VString& inStartsWith)
{
	...
	{
		if( inStartsWith.Find(CVSTR(".")) != 0 )
		{
			// Suggest identifiers
			_SuggestIdentifiers(inStartsWith, true, true);
		}
		
		std::vector<VString> names;
		std::vector<sLONG> nameScopeIds;
		
		fSymbolTable->GetConstructorNames(fContextFileExecutionContext, inStartsWith, fContextScopeId, fContextGlobalScopeId, names, nameScopeIds);
		
		_FillIdentifiers(inStartsWith, names, nameScopeIds, false);
	}
}

void JavaScriptParser::_SuggestWidgets(const XBOX::VString& inStartsWith)
{
	VString widgetName = inStartsWith;
	if( widgetName.GetLength() == 0 )
		widgetName.AppendUniChar('\'');
	
	UniChar stringStarter = widgetName.GetUniChar(1);
	if( stringStarter == '\'' || stringStarter == '\"' )
	{
		if( fUseNextGen == true && fSuggestions != NULL && fSymbolTable != NULL )
		{
			...
			{
				sLONG htmlFileId;
				if( fSymbolTable->GetHtmlSourceIdLinkedToJsSourceId(file->GetID(), htmlFileId) == true )
				{
					VIndex inStartsWithLength = widgetName.GetLength();
						
					VString widgetNameStart = widgetName;
					...
					for(VIndex index=0; index<total; ++index)
					{
						...
						text.AppendString(displayText);
						text.AppendUniChar(stringStarter);
						text.AppendUniChar(')');
							
						VIndex starterLength = widgetName.GetLength();
...
sLONG JavaScriptParser::_ResolveIdentifier(const XBOX::VString& inStartsWith, const ESymbolFileExecContext& inExecutionContext, const sLONG& inFromScopeId, const sLONG& inToScopeId, std::vector<sLONG>& outScopeIds, VString& outDataClassName, VString& outDataClassTypeName)
{
	sLONG result = 0;
	outDataClassName = "";
	outDataClassTypeName = "";
	
	if( fSymbolTable->GetScopes(inStartsWith, inExecutionContext, inFromScopeId, inToScopeId, outScopeIds) == true )
	{
		result = 1;
	}
	else
	{
		sLONG definitionNameId;
		if( fSymbolTable->GetName(inStartsWith, inFromScopeId, inExecutionContext, -1, NULL, definitionNameId) == true )
		{
			sLONG definitionId;
			if( fSymbolTable->GetDefinition(inFromScopeId, definitionNameId, -1, eSymbolDefinitionKindAll, -1, NULL, definitionId) == true )
			{
				std::vector<VString> typeNames;
				if( fSymbolTable->GetTypeNames(definitionId, typeNames) == true )
				{
					VIndex typeNamesCount = typeNames.size();
					for(VIndex currentTypeNameIndex=0; currentTypeNameIndex<typeNamesCount; ++ currentTypeNameIndex)
					{
						outDataClassTypeName = typeNames[currentTypeNameIndex];
						
						// DS KLUDGE
						VIndex kludgeIndex = outDataClassTypeName.Find(CVSTR("+"));
						if( kludgeIndex != 0 )
						{
							outDataClassTypeName.GetSubString(1, kludgeIndex-1, outDataClassName);
							outDataClassTypeName.SubString(kludgeIndex+1, outDataClassTypeName.GetLength()-kludgeIndex);
							
							if( fSymbolTable->GetScopes(outDataClassTypeName, inExecutionContext, inFromScopeId, inToScopeId, outScopeIds) == true )
							{
								result = 4;
							}
						}
...
void JavaScriptParser::_SuggestIdentifiers(const XBOX::VString& inStartsWith, const bool& inProcessDotExpression, const bool& inConstructorsOnly)
{
	if( fUseNextGen == true && fSuggestions != NULL && fSymbolTable != NULL )
	{
		sLONG suggestionStrategy = 0;
		
		// inStartWith contains the identifier to resolve
		// it may also contains an identifiers chain where each identifier is separated using a dot
		//
		// First of all, extract each identifiers
		VectorOfVString identifiers;
		inStartsWith.GetSubStrings('.', identifiers, true);
		
		std::vector<sLONG> fromScopeIds;
		fromScopeIds.push_back(fContextScopeId);
		
		std::vector<VString> prototypeChainNames;

		std::vector<sLONG> modelScopeIds;
		
		VString dataClassName;
		VString dataClassTypeName;

		// Resolve each identifiers, except the last one on which we have to compute suggestions
		VIndex identifiersCount = identifiers.size();
		for(VIndex currentIdentifierIndex=0; currentIdentifierIndex<identifiersCount; ++currentIdentifierIndex)
		{
			VString currentIdentifier = identifiers[currentIdentifierIndex];

			if( currentIdentifierIndex < (identifiersCount-1) )
			{
				suggestionStrategy = 0;
				
				std::vector<sLONG> childrenScopeIds;
			
				VIndex fromScopeIdsCount = fromScopeIds.size();
				for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
				{
					sLONG currentFromScopeId = fromScopeIds[currentFromScopeIndex];

					// Resolve identifier in the current scopes
					sLONG temp = _ResolveIdentifier(currentIdentifier, fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, childrenScopeIds, dataClassName, dataClassTypeName);
					if( currentFromScopeIndex == 0 || (suggestionStrategy == 0 && temp != 0) )
						suggestionStrategy = temp;

					// DS KLUDGE
					if( (currentIdentifier == CVSTR("ds") && currentIdentifierIndex == 0) || (suggestionStrategy == 4 && dataClassName.GetLength() > 0) )
						fSymbolTable->GetScopes(CVSTR("model"), fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, modelScopeIds);

					// DS KLUDGE
					if( suggestionStrategy == 0 && currentIdentifierIndex == 1 && modelScopeIds.size() >= 1 )
					{
						if( fSymbolTable->GetScopes(currentIdentifier, modelScopeIds, childrenScopeIds) == true )
						{
							if( this->fContextIdentifierFunctionCallStatus[currentIdentifierIndex] == true )
								suggestionStrategy = 4;
							else
								suggestionStrategy = 3;
						}
					}

					if( suggestionStrategy == 0 )
					{
						// Resolve the identifier in its prototype chain if any available
						for(std::vector<VString>::const_iterator itProtoNames=prototypeChainNames.begin(); itProtoNames!=prototypeChainNames.end(); ++itProtoNames)
						{
							sLONG resolveState = 0;
							VString protoName = (*itProtoNames);
							
							std::vector<sLONG> resolvedScopeIds;
							
							std::vector<sLONG> protoScopeIds;
							if( fSymbolTable->GetScopes(protoName, fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, protoScopeIds) == true )
							{
								for(std::vector<sLONG>::const_iterator itProtoScopeIds=protoScopeIds.begin(); itProtoScopeIds!=protoScopeIds.end(); ++itProtoScopeIds)
								{
									resolveState = _ResolveIdentifier(currentIdentifier, fContextFileExecutionContext, (*itProtoScopeIds), fContextGlobalScopeId, resolvedScopeIds, dataClassName, dataClassTypeName);
									if( resolveState == 0 )
									{
										std::vector<sLONG> protoPropertyScopeIds;
										if( fSymbolTable->GetScopes(CVSTR("prototype"), fContextFileExecutionContext, (*itProtoScopeIds), fContextGlobalScopeId, protoPropertyScopeIds) == true )
										{
											for(std::vector<sLONG>::const_iterator itProtoPrototypePropertyScopeIds=protoPropertyScopeIds.begin(); itProtoPrototypePropertyScopeIds!=protoPropertyScopeIds.end(); ++itProtoPrototypePropertyScopeIds)
											{
												resolveState = _ResolveIdentifier(currentIdentifier, fContextFileExecutionContext, (*itProtoPrototypePropertyScopeIds), fContextGlobalScopeId, resolvedScopeIds, dataClassName, dataClassTypeName);
												if( resolveState != 0 )
													break;
											}
										}
							...
							if( resolveState != 0 )
							{
								// Check if scope id a reference and replace it with the originale scope id
								for(std::vector<sLONG>::const_iterator itResolveScopeIds=resolvedScopeIds.begin(); itResolveScopeIds!=resolvedScopeIds.end(); ++itResolveScopeIds)
								{
									std::vector<sLONG> referenceIds;
									if( fSymbolTable->GetReference(eSymbolReferenceKindAll, (*itResolveScopeIds), NULL, referenceIds) == true )
									{
										for(std::vector<sLONG>::const_iterator itReferenceId=referenceIds.begin(); itReferenceId!=referenceIds.end(); ++itReferenceId)
										{
											sLONG referencedScopeId;
											if( fSymbolTable->GetReferenceToId((*itReferenceId), referencedScopeId) == true )
												childrenScopeIds.push_back(referencedScopeId);
										}
									}
									else
									{
										childrenScopeIds.push_back((*itResolveScopeIds));
									}
								}
								
								suggestionStrategy = resolveState;
								break;
							}
						}
					}
				...
				fromScopeIds = childrenScopeIds;
				fromScopeIdsCount = fromScopeIds.size();
				
				// Compute the current prototype chain
				prototypeChainNames.clear();
				for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
				{
					sLONG currentFromScopeId = fromScopeIds[currentFromScopeIndex];
					
					ESymbolScopeType scopeType;
					if( fSymbolTable->GetScopeType(currentFromScopeId, scopeType) == true )
					{
						if( scopeType == eSymbolScopeTypeFunctionExpression || scopeType == eSymbolScopeTypeFunctionDeclaration || scopeType == eSymbolScopeTypeObject || (suggestionStrategy == 3 && scopeType == eSymbolScopeTypeUndefined) )
						{
							VString scopeName;
							
							if( fContextIdentifierFunctionCallStatus[currentIdentifierIndex] == true )
							{
								if( suggestionStrategy == 4 )
								{
									if( fSymbolTable->GetPrototypeChain(fContextFileExecutionContext, currentIdentifier, prototypeChainNames) == true )
									{
										if( prototypeChainNames[0] == CVSTR("dataClass") )
											prototypeChainNames[0] = CVSTR("Entity");
								...
								{
									std::vector<VString> returnTypeNames;
									if( fSymbolTable->GetReturnTypeNames(currentFromScopeId, returnTypeNames) == true )
									{
										for(std::vector<VString>::const_iterator itTypeNames=returnTypeNames.begin(); itTypeNames!=returnTypeNames.end(); ++itTypeNames)
										{
											scopeName = (*itTypeNames);
											
											// DS KLUDGE
											if( scopeName == CVSTR("Entity") || scopeName == CVSTR("EntityCollection") )
											{
												VString entityContainerType = scopeName;
												
												if( identifiers.size() > 1 )
												{
													if( identifiers[0] == CVSTR("ds") )
													{
														scopeName = identifiers[1];
														
														if( fSymbolTable->GetPrototypeChain(fContextFileExecutionContext, scopeName, prototypeChainNames) == true )
														{
															if( prototypeChainNames[0] == CVSTR("dataClass") )
																prototypeChainNames[0] = entityContainerType;
							...
							{
								if( suggestionStrategy == 4 && dataClassName.GetLength() > 0 )
								{
									if( fSymbolTable->GetPrototypeChain(fContextFileExecutionContext, dataClassName, prototypeChainNames) == true )
									{
										if( prototypeChainNames[0] == CVSTR("dataClass") )
											prototypeChainNames[0] = dataClassTypeName;
								...
								{
									if( fSymbolTable->GetScopeName(currentFromScopeId, scopeName) == true )
									{
										if( fSymbolTable->GetPrototypeChain(fContextFileExecutionContext, scopeName, prototypeChainNames) == true )
											break;
									}
			...
			else
			{
				switch(suggestionStrategy)
				{
					case 1:
					{
						std::vector<VString> nameValues;
						std::vector<sLONG> nameScopeIds;
						
						// In each FromScope, get all registered name id starting with CurrentIdentifier
						VIndex fromScopeIdsCount = fromScopeIds.size();
						for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
						{
							sLONG currentFromScopeId = fromScopeIds[currentFromScopeIndex];
							...
							if( fSymbolTable->GetNames(currentIdentifier, currentFromScopeId, nameIds) == true )
							{
								// Get each definitions matching ScopeId and NameId
								VIndex nameIdsCount = nameIds.size();
								for(VIndex currentNameIndex=0; currentNameIndex<nameIdsCount; ++ currentNameIndex)
								{
									sLONG currentNameId = nameIds[currentNameIndex];
									
									sLONG definitionId;
									if( fSymbolTable->GetDefinition(currentFromScopeId, currentNameId, -1, eSymbolDefinitionKindProperty, -1, NULL, definitionId) == true )
									{
										VString nameValue;
										fSymbolTable->GetName(currentNameId, nameValue);
										
										nameValues.push_back(nameValue);
										nameScopeIds.push_back(currentFromScopeId);
									}
								}
							}
						}
						
						_RemoveDuplicateIdentifiers(nameValues, nameScopeIds);
						_FillIdentifiers(currentIdentifier, nameValues, nameScopeIds, false);
						...
						for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
						{
							...
							for(std::vector<VString>::const_iterator it=prototypeChainNames.begin(); it!=prototypeChainNames.end(); ++it)
							{
								sLONG prototypeScopeId;
								if( fSymbolTable->GetScopePrototypeId((*it), fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, prototypeScopeId) == true )
									_FillIdentifiers(CVSTR(""), prototypeScopeId, prototypeScopeId, true);
							}
						}
					}
						break;
					...
					case 2:
					{
						...
						// DataClasses Kludge
						if( inStartsWith == CVSTR("ds.") )
						{
							// Must suggest data classes names
							fSymbolTable->GetNames(CVSTR(""), modelScopeIds, nameValues, nameScopeIds);
						}
						else if( inStartsWith.BeginsWith(CVSTR("ds.")) == true && currentIdentifierIndex == 1 )
						{
							// Must suggest data classes names
							fSymbolTable->GetNames(currentIdentifier, modelScopeIds, nameValues, nameScopeIds);
						}
						...
						for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
						{
							...
							VString fromScopeName;
							if( fSymbolTable->GetScopeName(currentFromScopeId, fromScopeName) == true )
							{
								// Get instance properties attached to constructor through this
								std::vector<sLONG> nameIds;
								if( fSymbolTable->GetNames(currentIdentifier, currentFromScopeId, nameIds) == true )
								{
									// Get each definitions matching ScopeId and NameId
									...
									for(VIndex currentNameIndex=0; currentNameIndex<nameIdsCount; ++ currentNameIndex)
									{
										...
										sLONG definitionId;
										if( fSymbolTable->GetDefinition(currentFromScopeId, currentNameId, -1, eSymbolDefinitionKindInstanceProperty, -1, NULL, definitionId) == true )
										{
											VString nameValue;
											fSymbolTable->GetName(currentNameId, nameValue);
								...
								// Get instance properties attached to prototype property
								sLONG prototypeScopeId;
								if( fSymbolTable->GetScopePrototypeId(fromScopeName, fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, prototypeScopeId) == true )
								{
									std::vector<sLONG> nameIds;
									if( fSymbolTable->GetNames(currentIdentifier, prototypeScopeId, nameIds) == true )
									{
										// Get each definitions matching ScopeId and NameId
										VIndex nameIdsCount = nameIds.size();
										for(VIndex currentNameIndex=0; currentNameIndex<nameIdsCount; ++ currentNameIndex)
										{
											...
											sLONG definitionId;
											if( fSymbolTable->GetDefinition(prototypeScopeId, currentNameId, -1, eSymbolDefinitionKindInstanceProperty, -1, NULL, definitionId) == true )
											{
												VString nameValue;
												fSymbolTable->GetName(currentNameId, nameValue);
						...
						for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
						{
							...
							for(std::vector<VString>::const_iterator it=prototypeChainNames.begin(); it!=prototypeChainNames.end(); ++it)
							{
								VString prototypeTypeName = (*it);
								
								sLONG prototypeTypeNameScopeId;
								if( fSymbolTable->GetScope(prototypeTypeName, fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, prototypeTypeNameScopeId) == true )
								{
									// Get instance properties attached to this
									std::vector<sLONG> nameIds;
									if( fSymbolTable->GetNames(currentIdentifier, prototypeTypeNameScopeId, nameIds) == true )
									{
										nameValues.clear();
										nameScopeIds.clear();

										for(std::vector<sLONG>::const_iterator itNames=nameIds.begin(); itNames!=nameIds.end(); ++itNames)
										{
											sLONG currentNameId = (*itNames);
											
											sLONG definitionId;
											if( fSymbolTable->GetDefinition(prototypeTypeNameScopeId, currentNameId, -1, eSymbolDefinitionKindInstanceProperty, -1, NULL, definitionId) == true )
											{
												VString nameValue;
												fSymbolTable->GetName(currentNameId, nameValue);
												
												nameValues.push_back(nameValue);
												nameScopeIds.push_back(prototypeTypeNameScopeId);
											}
										}
									...
									sLONG prototypeTypeNamePropertyScopeId;
									if( fSymbolTable->GetScopeChild(prototypeTypeNameScopeId, fContextFileExecutionContext, CVSTR("prototype"), prototypeTypeNamePropertyScopeId) == true )
									{
										_FillIdentifiers(currentIdentifier, prototypeTypeNamePropertyScopeId, prototypeTypeNamePropertyScopeId, true);
									}
								}
							}
						}
					...
					case 3:
					{
						...
						// In each FromScope, get all registered name id starting with CurrentIdentifier
						fSymbolTable->GetNames(currentIdentifier, fromScopeIds, nameValues, nameScopeIds);
						
						_FillIdentifiers(currentIdentifier, nameValues, nameScopeIds, false);
						
						// Walk through the prototype chain
						if( fromScopeIds.size() >= 1 )
						{
							sLONG currentFromScopeId = fromScopeIds[0];
					...
					case 4:
					{
						...
						if( dataClassName.GetLength() == 0 )
							dataClassName = identifiers[1];
						
						fromScopeIds.clear();
						fSymbolTable->GetScopes(dataClassName, modelScopeIds, fromScopeIds);
						
						std::vector<VString> nameValuesTemp;
						std::vector<sLONG> nameScopeIdsTemp;

						// In each FromScope, get all registered name id starting with CurrentIdentifier
						fSymbolTable->GetNames(currentIdentifier, fromScopeIds, nameValuesTemp, nameScopeIdsTemp);
						VIndex count = nameValuesTemp.size();
						if( prototypeChainNames.size() > 0 )
						{
							for(VIndex currentIndex=0; currentIndex<count; ++currentIndex)
							{
								if( prototypeChainNames[0] == CVSTR("EntityCollection") )
								{
									if( nameValuesTemp[currentIndex] == CVSTR("collectionMethods") )
									{
										nameValues.push_back(nameValuesTemp[currentIndex]);
										nameScopeIds.push_back(nameScopeIdsTemp[currentIndex]);
									}
								}
								else if( prototypeChainNames[0] == CVSTR("Entity") )
								{
									if( nameValuesTemp[currentIndex] != CVSTR("collectionMethods") && nameValuesTemp[currentIndex] != CVSTR("methods") )
									{
										nameValues.push_back(nameValuesTemp[currentIndex]);
										nameScopeIds.push_back(nameScopeIdsTemp[currentIndex]);
									}
						...
						for(VIndex currentFromScopeIndex=0; currentFromScopeIndex<fromScopeIdsCount; ++currentFromScopeIndex)
						{
							...
							for(std::vector<VString>::const_iterator it=prototypeChainNames.begin(); it!=prototypeChainNames.end(); ++it)
							{
								...
								if( fSymbolTable->GetScope(prototypeTypeName, fContextFileExecutionContext, currentFromScopeId, fContextGlobalScopeId, prototypeTypeNameScopeId) == true )
								{
									// Get instance properties attached to this
									std::vector<sLONG> nameIds;
									if( fSymbolTable->GetNames(CVSTR(""), prototypeTypeNameScopeId, nameIds) == true )
									{
										nameValues.clear();
										nameScopeIds.clear();
									...
									if( fSymbolTable->GetScopeChild(prototypeTypeNameScopeId, fContextFileExecutionContext, CVSTR("prototype"), prototypeTypeNamePropertyScopeId) == true )
									{
										_FillIdentifiers(CVSTR(""), prototypeTypeNamePropertyScopeId, prototypeTypeNamePropertyScopeId, true);
									}
								}
							}
							break;
					...
					default:
						_FillIdentifiers(currentIdentifier, fContextScopeId, fContextGlobalScopeId, false);
						break;
				}
			}
		}
...
void JavaScriptParser::_FillIdentifiers(const XBOX::VString& inStartsWith, const std::vector<XBOX::VString>& inNames, const std::vector<sLONG>& inScopeIds, const bool& inSuggestPrototypeProperties)
{
	...
	for(VIndex index=0; index<total; ++index)
	{
		...
		if( displayText.BeginsWith(CVSTR("anonymous_function")) == true || displayText.BeginsWith(CVSTR("anonymous_object")) == true )
			continue;
...
void JavaScriptParser::_RemoveDuplicateIdentifiers(std::vector<XBOX::VString>& ioNames, std::vector<sLONG>& ioScopeIds) const
{
	std::vector<VString> tempNames;
	std::vector<sLONG> tempNameScopeIds;

	VIndex total = ioNames.size();
	for(VIndex index=0; index<total; ++index)
	{
		if( find(tempNames.begin(), tempNames.end(), ioNames[index]) == tempNames.end() )
		{
			tempNames.push_back(ioNames[index]);
			tempNameScopeIds.push_back(ioScopeIds[index]);
		}
	}
	
	if( ioNames.size() != tempNames.size() )
	{
		ioNames = tempNames;
		ioScopeIds = tempNameScopeIds;
	}
}
