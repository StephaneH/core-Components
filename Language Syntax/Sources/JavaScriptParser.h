...
typedef sLONG EJavaScriptKeyword;
// The base class to all JS suggestion strategies
class IJsSuggestionStrategy
{
public:
	enum {
		kKeywordNull = 0,
	...
	virtual ~IJsSuggestionStrategy() {}
	
	virtual void SuggestKeyword(const EJavaScriptKeyword& inKeyWord) = 0;
	virtual void SuggestIdentifiers(const bool& inJavaScriptIdentifiersOnly) = 0;
	virtual void SuggestCallParameters() = 0;
	virtual void SuggestReturnTypes() = 0;
	
	virtual void ResetSuggestions() = 0;
	
	virtual void AddSuggestionContextSymbol(const XBOX::VString& inSymbolName, const bool& inIsCore) = 0;
	virtual void ResetSuggestionContextSymbols() = 0;
};


// This strategy does nothing, it should be used while parsing only
class NoJsSuggestionStrategy : public IJsSuggestionStrategy
{
public:
	NoJsSuggestionStrategy() {}
	virtual ~NoJsSuggestionStrategy() {}
	
	virtual void SuggestKeyword(const EJavaScriptKeyword& inKeyWord) {}
	virtual void SuggestIdentifiers(const bool& inJavaScriptIdentifiersOnly) {}
	virtual void SuggestCallParameters() {}
	virtual void SuggestReturnTypes() {}
	
	virtual void ResetSuggestions() {}
	
	virtual void AddSuggestionContextSymbol(const XBOX::VString& inSymbolName, const bool& inIsCore) {}
	virtual void ResetSuggestionContextSymbols() {}
	
private:
	// Non copiable
	NoJsSuggestionStrategy(const NoJsSuggestionStrategy&);
	NoJsSuggestionStrategy& operator=(const NoJsSuggestionStrategy&);
};



// This strategy is the default one, it should be used while suggesting only
class DefaultJsSuggestionStrategy : public IJsSuggestionStrategy
{
public:
	DefaultJsSuggestionStrategy(ISymbolTable& inSymbolTable, Symbols::IFile& inContextFile, SuggestionList& outSuggestions):
	fSymbolTable(inSymbolTable),
	fContextFile(inContextFile),
	fSuggestions(outSuggestions)
	{
	}
	
	virtual ~DefaultJsSuggestionStrategy()
	{
		// !!! DO NOT CALL ANY RELEASE ON THE REFERENCES WE USE !!!
		
		// Clean the context symbols
		while( !fContextSymbols.empty() )
		{
			// Release the symbol
			Symbols::ISymbol* sym = fContextSymbols.back();
			ReleaseRefCountable(&sym);
			
			// Remove it from the vector
			fContextSymbols.pop_back();
		}
	}
...
private:
	// Non copiable
	DefaultJsSuggestionStrategy(const DefaultJsSuggestionStrategy&);
	DefaultJsSuggestionStrategy& operator=(const DefaultJsSuggestionStrategy&);
	
	// A RETAIN MUST BE DONE BEFORE PASSING THE REFERENCES
	// A RELEASE MUST BE DONE AFTER
	ISymbolTable&	fSymbolTable;
	Symbols::IFile& fContextFile;
	SuggestionList& fSuggestions;
	
	std::vector<Symbols::ISymbol*> fContextSymbols;
};
...
public:
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	JavaScriptParser(IJsSuggestionStrategy& inSuggestionStrategy);
#else
	JavaScriptParser();
#endif
...
private:
...
#if ACTIVATE_SUGGESTION_STRATEGY_CLASS
	IJsSuggestionStrategy& fSuggestionStrategy;
#else
	...
	Symbols::ISymbol*	_SuggestSourceIdentifiers(const VString& sourceId);
