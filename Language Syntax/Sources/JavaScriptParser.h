...
public:
	JavaScriptParser(const bool& inUseNextGen);
	...
	JavaScriptAST::Node*	GetAST(	VFilePath inFilePath,
									...
									VString* inInput,
									bool& outError,
								   std::vector<XBOX::VString>* outSyntaxErrors=NULL);
	...
	void	GetSuggestions(	VString* inInput,
							SuggestionList* inSuggestions,
						    const sLONG& inScopeGlobalId,
						    const sLONG& inScopeContextId,
							const sLONG& inFileId,
							const ESymbolFileExecContext& inFileExecutionContext,
							ISymbolTable* inTable);
...
private:
	sLONG					fContextGlobalScopeId;
	sLONG					fContextScopeId;
	sLONG					fContextFileId;
	ESymbolFileExecContext	fContextFileExecutionContext;
	XBOX::VString			fContextIdentifier;
	std::vector<bool>		fContextIdentifierFunctionCallStatus;

	sLONG fAnonymousFunctionExpressionCounter;
	sLONG fAnonymousObjectCounter;
	...
	std::vector<XBOX::VString>* fSyntaxErrors;
	...
	JavaScriptAST::Node*						ParseMemberExpression			(int& inoutToken, bool inArgsRequired, bool inFromNewStatement);
	...
	{
		sLONG line = fLexer->GetCurrentLineNumber();
		sLONG column = fLexer->GetCurrentCharacterOffset();
		
		if( fSyntaxErrors != NULL )
		{
			XBOX::VString message;
			message.AppendString("Line(");
			message.AppendLong(line);
			message.AppendString(") ErrorMessage(");
			message.AppendString(inMessage);
			message.AppendString(")");

			fSyntaxErrors->push_back(message);
		}
	
		fWasError = true;
		if (fDelegate)
			fDelegate->Error( fFilePath, line, column, inMessage );
	...
	void _AppendToSuggestionIdentifier(const XBOX::VString& inToAppend, const bool& inIsFunctionCall);
	void _SuggestConstructors(const XBOX::VString& inStartsWith);
	void _SuggestWidgets(const XBOX::VString& inStartsWith);
	void _SuggestIdentifiers(const XBOX::VString& inStartsWith, const bool& inProcessDotExpression, const bool& inConstructorsOnly);
	void _FillIdentifiers(const XBOX::VString& inStartsWith, const sLONG& inFromScopeId, const sLONG& inToScopeId, const bool& inSuggestPrototypeProperties);
	void _FillIdentifiers(const XBOX::VString& inStartsWith, const std::vector<XBOX::VString>& inNames, const std::vector<sLONG>& inScopeIds, const bool& inSuggestPrototypeProperties);
	void _RemoveDuplicateIdentifiers(std::vector<XBOX::VString>& ioNames, std::vector<sLONG>& ioScopeIds) const;
	
	sLONG _ResolveIdentifier(const XBOX::VString& inStartsWith, const ESymbolFileExecContext& inExecutionContext, const sLONG& inFromScopeId, const sLONG& inToScopeId, std::vector<sLONG>& outScopeIds, XBOX::VString& outDataClassName, XBOX::VString& outDataClassTypeName);
	
	bool _IsJsScriptFromWaPage(Symbols::IFile* inFile) const;
