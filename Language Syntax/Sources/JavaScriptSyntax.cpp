...
void VJavaScriptSyntax::GetASTForTesting(const XBOX::VString& inSourceLine, const XBOX::VString& inTypeOfASTComputationType, std::vector<XBOX::VString>& outDeclarations)
{
	...
	JavaScriptAST::SymbolScopeDeclaration* declarationsScope = new JavaScriptAST::SymbolScopeDeclaration(NULL, CVSTR("global"), eSymbolScopeTypeGlobal, 0, 0, 0, 0, false);

	JavaScriptAST::Visitor* declParser = JavaScriptAST::Visitor::GetDeclarationParser( NULL, NULL, NULL, declarationsScope );
	if( declParser )
	{
			...
			if( program )
			{
				...
				if( declarationsScope )
				{
					...
					if( inTypeOfASTComputationType == "ASSIGNMENTS" )
						results = declarationsScope->GetAssignmentDeclarations();
						...
						results = declarationsScope->GetFunctionDeclarations();
...
void VJavaScriptSyntax::GetSuggestions( ICodeEditorDocument* inDocument, sLONG inLineIndex, sLONG inPos, ITipInfoArray *outSuggestions, sLONG& outStartOffset, bool inAll )
{
	...
	if( inDocument != NULL )
	{
		...
		VString constructorStarter;
		if( _MustSuggestConstructor(tokens, constructorStarter) == true )
		{
#if ACTIVATE_JSLOG
			JSLog log(true);
			log.SetTitle(CVSTR("Suggesting constructors"));
#endif

			std::vector<XBOX::VString> names;
			if( constructorStarter.IsEmpty() )
				fSymTable->GetConstructorNames(eSymbolFileExecContextServer, names);
			else
				fSymTable->GetConstructorNames(eSymbolFileExecContextServer, constructorStarter, names);
			
			VIndex total = names.size();
			for(VIndex index=0; index<total; ++index)
				outSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, names[index], names[index], eName) );
			
			outStartOffset = inPos - constructorStarter.GetLength();
		}
		else
		{
			Symbols::IFile* file = FileFromDocument(inDocument);
			if( file != NULL )
			{
				sLONG sourceId = file->GetID();
				
				// Get the current scope
				sLONG currentScopeId;
				fSymTable->GetScope(sourceId, eSymbolFileExecContextServer, inLineIndex, inPos, currentScopeId);

#if ACTIVATE_JSLOG
				{
					VString scopeName;
					fSymTable->GetScopeName(currentScopeId, scopeName);
					
					JSLog log;
					log.SetTitle(CVSTR("Suggesting from scope"));
					log.Append(CVSTR("ID"), currentScopeId);
					log.Append(CVSTR("Name"), scopeName);
					log.Append(CVSTR("FromLine"), inLineIndex);
					log.Append(CVSTR("FromOffset"), inPos);
				}
#endif

				// Try to build an identifier chain
				std::vector<XBOX::VString> identifiers;
				bool endsWithDot = false;
				sLONG chainLength = 0;
				if( _BuildIdentifiersChain(tokens, identifiers, endsWithDot, chainLength) == true )
				{
#if ACTIVATE_JSLOG
					JSLog log(true);
					log.SetTitle(CVSTR("Suggesting identifiers"));
#endif
					
					// We have built an identifier chain such as
					// Obj OR Object. OR Object.pro
					VIndex chainCount = identifiers.size();
					VString identifierStart;
					
					if( chainCount == 1 )
					{
						identifierStart = identifiers[0];
						...
						if( endsWithDot == true )
						{
							// Get the scope
							sLONG newScopeId;
							if( fSymTable->GetScope(identifierStart, eSymbolScopeTypeAll, currentScopeId, 0, eSymbolFileExecContextServer, newScopeId) == true )
							{
								fSymTable->GetIdentifierNames(eSymbolFileExecContextServer, newScopeId, names);
							}
							else
							{
								// We got no scope matching, so, get the identifier definition first ...
								sLONG nameId;
								if( fSymTable->GetName(identifierStart, currentScopeId, eSymbolFileExecContextServer, nameId) == true )
								{
									// ... then get the identifier type definition
									sLONG definitionId;
									if( fSymTable->GetDefinition(currentScopeId, nameId, eSymbolFileExecContextServer, definitionId) == true )
									{
										fSymTable->GetIdentifierNames(definitionId, names);
									}
								}
							}
						}
						...
						{
							// Suggest every identifiers attached to the current scope
							fSymTable->GetIdentifierNames(eSymbolFileExecContextServer, currentScopeId, identifierStart, names);
							outStartOffset = inPos - chainLength;
						}
					...
					{
						sLONG identifierScopeId = currentScopeId;
						
						if( endsWithDot == true )
						{
							for(VIndex index=chainCount-1; index>=0; --index)
							{
								identifierStart = identifiers[index];
								
								// Get the scope
								sLONG nextIdentifierScopeId;
								if( fSymTable->GetScope(identifierStart, eSymbolScopeTypeAll, identifierScopeId, 0, eSymbolFileExecContextServer, nextIdentifierScopeId) == true )
									identifierScopeId = nextIdentifierScopeId;
							}
							
							std::vector<XBOX::VString> names;
							fSymTable->GetIdentifierNames(eSymbolFileExecContextServer, identifierScopeId, names);
						...
						else
						{
							...
							identifierStart = identifiers[0];
							std::vector<XBOX::VString> names;
							fSymTable->GetIdentifierNames(eSymbolFileExecContextServer, identifierScopeId, identifierStart, names);
							
							VIndex total = names.size();
							for(VIndex index=0; index<total; ++index)
								outSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, names[index], names[index], eName) );
							
							outStartOffset = inPos - identifierStart.GetLength();
						}
					}
				}
			}
