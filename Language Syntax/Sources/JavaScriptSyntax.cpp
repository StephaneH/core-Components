...
void VJavaScriptSyntax::GetASTForTesting(const XBOX::VString& inSourceLine, const XBOX::VString& inTypeOfASTComputationType, std::vector<XBOX::VString>& outDeclarations)
{
	if( fUseNextGen == true )
	{
		JavaScriptAST::SymbolScopeDeclaration* declarationsScope = new JavaScriptAST::SymbolScopeDeclaration(NULL, CVSTR("global"), CVSTR("global"), eSymbolScopeTypeGlobal, 0, 0, 0, 0, false, false, CVSTR(""));
		
		JavaScriptAST::Visitor* declParser = JavaScriptAST::Visitor::GetDeclarationParser( fUseNextGen, NULL, NULL, NULL, declarationsScope );
		if( declParser )
		{
			...
			if( parser )
			{
				...
				JavaScriptAST::Node* program = parser->GetAST( filePath, -1, &input, errors );
				if( program )
				{
					program->Accept( declParser );
					if( declarationsScope )
					{
						std::vector<JavaScriptAST::ISymbolDeclaration*> results;
						
						if( inTypeOfASTComputationType == "VARIABLES" )
							results = declarationsScope->GetVariableDeclarations();
						...
						delete declarationsScope;
						declarationsScope = NULL;
					}
	...
	if( fUseNextGen == true )
	{
		if( fSymTable != NULL )
		{
			...
			for(std::vector<ESymbolFileExecContext>::const_iterator it=executionContexts.begin(); it!=executionContexts.end(); ++it)
			{
				...
				sLONG globalScopeId;
				fSymTable->GetGlobalScope(executionContext, NULL, globalScopeId);
				
				std::vector<sLONG> parentScopeIds;
				parentScopeIds.push_back(globalScopeId);
				...
				for(VIndex index=0; index<count; ++index)
				{
					...
					if( childrenScopeCount == 0 )
					{
						// Instead of looking for scopes, search for names defined in thoose scopes
						parentScopeCount = parentScopeIds.size();
						for(VIndex parentScopeIndex=0; parentScopeIndex<parentScopeCount; ++parentScopeIndex)
						{
							sLONG parentScopeId = parentScopeIds[parentScopeIndex];
							
							sLONG nameId;
							if( fSymTable->GetName(identifier, parentScopeId, executionContext, -1/*inFile->GetID()*/, NULL, nameId) == true )
								childrenScopeIds.push_back(parentScopeId);
						}
						
						childrenScopeCount = childrenScopeIds.size();
					}
						...
						else
						{
							for(VIndex childrenScopeIndex=0; childrenScopeIndex<childrenScopeCount; ++childrenScopeIndex)
							{
								...
								if( fSymTable->GetScopeSourceId(currentScopeId, sourceId) == true )
								{
									...
									outDefinitions.push_back(IDefinition(path, baseFolder, fromLine, identifier, currentScopeId));
								}
							}
						}
					}
		...
		if( inDocument != NULL )
		{
			...
			if( file != NULL )
			{
				...
				if( parser != NULL )
				{
					...
					sLONG globalScopeId;
					fSymTable->GetGlobalScope(fileExecutionContext, NULL, globalScopeId);
					
					// Now try to get the scope based on the line number
					sLONG contextScopeId;
					if( fSymTable->GetScope(fileExecutionContext, fileId, inLineIndex, contextScopeId) == false )
					...
					parser->GetSuggestions( &line, &suggestions, globalScopeId, contextScopeId, fileId, fileExecutionContext, fSymTable );
					...
					for (SuggestionList::iterator iter = suggestions.begin(); iter != suggestions.end(); ++iter)
					{
						...
						switch( suggestion->fType )
						{
							...
							case SuggestionInfo::eName:
							{
								...
								outSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, displayText, suggestion->fText, eName) );
							}
								break;
							...
							case SuggestionInfo::eWidget:
							{
								...
								if( fSymTable->GetPrototypeChainParentName(fileExecutionContext, suggestion->fDisplayText, prototypeName) == true )
									outSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, suggestion->fDisplayText + CVSTR("||") + prototypeName, suggestion->fText, eWidget) );
							}
								break;
