...
void VJavaScriptSyntax::Init( ICodeEditorDocument* inDocument )
{
	...
	inDocument->SetStyle( eKeyword, true,  false, false,  0,  77,   3 );
	inDocument->SetStyle( eComment, false, false, false, 83,  83,  83 );
	inDocument->SetStyle( eNumber,  false, false, false,  0,  35,  83 );
	inDocument->SetStyle( eString,  true,  true,  false, 136,  0,   0 );
	inDocument->SetStyle( eName,    false, false, false, 255, 49,   0 );
	inDocument->SetStyle( eRegExp,  false, false, false, 128,  0, 255 );
...
bool VJavaScriptSyntax::CanHiliteSameWord( sBYTE inStyle )
{
	return inStyle==eName;
}
...
void VJavaScriptSyntax::GetColoringForTesting( const VString &inSourceLine, std::vector< size_t > &outOffsets, std::vector< size_t > &outLengths, std::vector< sLONG > &outStyles )
{
	...
	for (vector< ILexerToken * >::iterator iter = tokens.begin(); 
		...
		switch (current->GetType())
		{
			...
			case ILexerToken::TT_STRING:
				style = eString;
				break;
				
			case ILexerToken::TT_JAVASCRIPT_KEYWORD:
				style = eKeyword;
				break;
				
			case ILexerToken::TT_JAVASCRIPT_FUTURE_RESERVED_WORD:
				style = eKeyword;
				break;
				
			case ILexerToken::TT_COMPARISON:
				style = eComparison;
				break;
				
			case ILexerToken::TT_OPEN_NAME:
				style = eName;
				break;
	...
	if (NULL != inSymTable)
	{
		...
		if ( ! files.empty() )
		{
			...
			if (NULL != file)
			{
				...
				GetDefinitions(parts, owner, file, inLineNumber, outDefinitions);
				if (NULL != owner)
					owner->Release();
				file->Release();	
			}
...
void VJavaScriptSyntax::GetSyntaxErrorsForTesting(const XBOX::VFilePath& inFileToParse, std::vector<XBOX::VString>* outSyntaxErrors)
{
	if( fUseNextGen == true )
	{
		...
		if( parser != NULL )
		{
			...
			if( file.Exists() )
			{
				...
				if (VE_OK == stream.OpenReading())
				{
					...
					stream.GetText( content );
					stream.CloseReading();
					content.ConvertCarriageReturns( eCRM_CR );
				}
			}
		
			bool errorStatus;

			JavaScriptAST::Node* program = parser->GetAST( inFileToParse, -1, &content, errorStatus, outSyntaxErrors );
			if( program != NULL )
			{
				delete program;
				program = NULL;
...
void VJavaScriptSyntax::SetLine( ICodeEditorDocument* inDocument, sLONG inLineNumber, bool inLoading )
{
	...
	memset(&previousLineKind, 0x0, sizeof(JavaScriptSyntaxLineKind));
	...
	memset(&kind, 0x0, sizeof(JavaScriptSyntaxLineKind));
	...
	for (vector< ILexerToken * >::iterator iter = tokens.begin(); iter != tokens.end(); iter++)
	{
		...
		switch (current->GetType()) {
			...
			case ILexerToken::TT_JAVASCRIPT_REGEXP:
				style = eRegExp;
				break;
...
void VJavaScriptSyntax::GetTip( ICodeEditorDocument* inDocument, sLONG inLine, sLONG inPos, VString& outText, Boolean& outRtfText )
{
	...
	if( inDocument != NULL )
	{
		...
		if( path.GetPath().EndsWith("_STLR.js") == true )
			return;
	}
...
void VJavaScriptSyntax::GetDefinitions( ICodeEditorDocument* inDocument, sLONG inLineIndex, std::vector<VString>& inParts, std::vector<IDefinition>& outDefinitions ) 
{
	if ( inParts.size() )
	{
		...
		if( file != NULL )
		{
			ISymbolTable* symTable = inDocument->GetSymbolTable();
			if( symTable != NULL )
			{
				Symbols::ISymbol* contextSymbol = symTable->GetSymbolOwnerByLine( file, inLineIndex );
				...
				if (inParts.front() == CVSTR("ds") && inParts.size() > 2)
				{
					...
					for (VectorOfVString::iterator itModels = models.begin(); itModels != models.end() && !outDefinitions.size(); ++itModels)
					{
						for (VectorOfVString::iterator itMethodKinds = methodKinds.begin(); itMethodKinds != methodKinds.end() && !outDefinitions.size(); ++itMethodKinds)
						{
							...
							GetDefinitions(hackedParts, contextSymbol, file, inLineIndex, outDefinitions);
						}
					}
				}
				// Standard case
				...
				{
					GetDefinitions(inParts, contextSymbol, file, inLineIndex, outDefinitions);
				}
				file->Release();
...
{
	GetDefinitions(inParts, 0, 0, -1, outDefinitions);
}

void VJavaScriptSyntax::GetDefinitions(VectorOfVString& inParts, Symbols::ISymbol *inContextSymbol, Symbols::IFile *inFile, const sLONG& inLineNumber, std::vector<IDefinition> & outDefinitions)
{
	if( fUseNextGen == true )
	{
		if( fSymTable != NULL )
		{
			std::vector<ESymbolFileExecContext> executionContexts;
			
			if( inFile != NULL )
			{
				executionContexts.push_back(inFile->GetExecutionContext());
			}
			else
			{
				executionContexts.push_back(eSymbolFileExecContextClient);
				executionContexts.push_back(eSymbolFileExecContextServer);
			}
			
			for(std::vector<ESymbolFileExecContext>::const_iterator it=executionContexts.begin(); it!=executionContexts.end(); ++it)
			{
				ESymbolFileExecContext executionContext = (*it);
				
				sLONG currentParentScopeId;
				fSymTable->GetGlobalScope(executionContext, NULL, currentParentScopeId);

				std::vector<sLONG> parentScopeIds;
				
				sLONG currentSourceId = -1;
				if( inFile != NULL )
				{
					currentSourceId = inFile->GetID();

					sLONG scopeId;
					if( fSymTable->GetScope(executionContext, currentSourceId, inLineNumber, scopeId) == true )
						currentParentScopeId = scopeId;
				}
				
				parentScopeIds.push_back(currentParentScopeId);
				
				VIndex count = inParts.size();
				for(VIndex index=0; index<count; ++index)
				{
					VString identifier = inParts[index];
					
					std::vector<sLONG> childrenScopeIds;
					std::vector<sLONG> definitionIds;

					VIndex parentScopeCount = parentScopeIds.size();
					for(VIndex parentScopeIndex=0; parentScopeIndex<parentScopeCount; ++parentScopeIndex)
					{
						sLONG parentScopeId = parentScopeIds[parentScopeIndex];

						// Get all named chidren scopes
						fSymTable->GetScopeChildren(parentScopeId, executionContext, identifier, childrenScopeIds);
						
						std::vector<sLONG> nameIds;
						fSymTable->GetNames(identifier, parentScopeId, executionContext, -1, NULL, nameIds);
						
						VIndex namesCount = nameIds.size();
						for(VIndex nameIndex=0; nameIndex<namesCount; ++nameIndex)
						{
							sLONG nameId = nameIds[nameIndex];
							sLONG definitionId;
							if( fSymTable->GetDefinitionId(parentScopeId, nameId, -1, definitionId) == true )
					...
					if( index != (count-1) )
					{
						// Go on with the next identifier in the chain looking in the children scopes
						if( childrenScopeIds.size() > 0 )
						{
							parentScopeIds = childrenScopeIds;

							// ... or try to get any references from definitions
							VIndex definitionsCount = definitionIds.size();
							for(VIndex currentDefinitionIndex=0; currentDefinitionIndex<definitionsCount; ++currentDefinitionIndex)
							{
								sLONG definitionId = definitionIds[currentDefinitionIndex];
								
								sLONG referenceId;
								
								bool foundAnyReference = false;
								std::vector<sLONG> referenceIds;
								foundAnyReference = fSymTable->GetReference(eSymbolReferenceKindVariableDeclaration, definitionId, NULL, referenceIds);
								if( foundAnyReference == false )
									foundAnyReference = fSymTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, definitionId, NULL, referenceIds);
								
								if( foundAnyReference == false )
									break;
								
								VIndex referencesCount = referenceIds.size();
								for(VIndex referenceIndex=0;referenceIndex<referencesCount; ++referenceIndex)
								{
									sLONG referencedScopeId;
									fSymTable->GetReferenceToId(referenceIds[referenceIndex], referencedScopeId);
									
									parentScopeIds.push_back(referencedScopeId);
								}
							}
						}
						else
					...
					{
						std::vector<IDefinition> scopeDefinitionsInsideSource;
						std::vector<IDefinition> scopeDefinitionsOutsideSource;
						
						// We reached the last identifier in the chain
						// We should have filled the children scopes ids and definitions ids, regardless the source id
						VIndex childrenScopeCount = childrenScopeIds.size();
						for(VIndex childrenScopeIndex=0; childrenScopeIndex<childrenScopeCount; ++childrenScopeIndex)
						{
							sLONG currentScopeId = childrenScopeIds[childrenScopeIndex];
							
							sLONG sourceId;
							if( fSymTable->GetScopeSourceId(currentScopeId, sourceId) == true )
							{
								VString path;
								fSymTable->GetSourcePath(sourceId, path);
								
								ESymbolFileBaseFolder baseFolder;
								fSymTable->GetSourceBaseFolder(sourceId, baseFolder);
								
								sLONG fromLine, fromOffset, toLine, toOffset;
								fSymTable->GetScopeBoundaries(currentScopeId, fromLine, fromOffset, toLine, toOffset);
								
								if( currentSourceId != -1 && sourceId == currentSourceId )
									scopeDefinitionsInsideSource.push_back(IDefinition(path, baseFolder, fromLine, identifier, currentScopeId));
								else
									scopeDefinitionsOutsideSource.push_back(IDefinition(path, baseFolder, fromLine, identifier, currentScopeId));
							}
						}
						
						std::vector<IDefinition> variableDefinitionsInsideSource;
						std::vector<IDefinition> variableDefinitionsOutsideSource;

						VIndex definitionsCount = definitionIds.size();
						for(VIndex definitionIndex=0; definitionIndex<definitionsCount; ++definitionIndex)
						{
							sLONG definitionId = definitionIds[definitionIndex];
							
							sLONG definitionSourceId;
							fSymTable->GetDefinitionSourceId(definitionId, definitionSourceId);
							
							VString definitionSourcePath;
							fSymTable->GetSourcePath(definitionSourceId, definitionSourcePath);
							
							ESymbolFileBaseFolder definitionSourceBaseFolder;
							fSymTable->GetSourceBaseFolder(definitionSourceId, definitionSourceBaseFolder);
							
							sLONG definitionLineNumber;
							fSymTable->GetDefinitionLine(definitionId, definitionLineNumber);
							
							sLONG parentScopeId;
							fSymTable->GetDefinitionParentScopeId(definitionId, parentScopeId);
							
							if( currentSourceId != -1 && definitionSourceId == currentSourceId )
								variableDefinitionsInsideSource.push_back(IDefinition(definitionSourcePath, definitionSourceBaseFolder, definitionLineNumber, identifier, parentScopeId));
							else
								variableDefinitionsOutsideSource.push_back(IDefinition(definitionSourcePath, definitionSourceBaseFolder, definitionLineNumber, identifier, parentScopeId));
						}

						if( variableDefinitionsInsideSource.size() > 0 )
						{
							outDefinitions = variableDefinitionsInsideSource;
						}
						else if( scopeDefinitionsInsideSource.size() > 0 && variableDefinitionsOutsideSource.size() > 0 )
						{
							outDefinitions = variableDefinitionsOutsideSource;
						}
						else if( scopeDefinitionsInsideSource.size() > 0 && scopeDefinitionsOutsideSource.size() > 0 )
						{
							bool scopeDefined = false;
							VIndex scopeCount = scopeDefinitionsInsideSource.size();
							for(VIndex definitionIndex=0; definitionIndex<scopeCount; ++definitionIndex)
							{
								ESymbolScopeType scopeType;
								if( fSymTable->GetScopeType(scopeDefinitionsInsideSource[definitionIndex].GetSymbolID(), scopeType) == true )
								{
							...
							if( scopeDefined == true )
							{
								outDefinitions.insert(outDefinitions.end(), scopeDefinitionsInsideSource.begin(), scopeDefinitionsInsideSource.end());
							}
							else
							{
								scopeCount = scopeDefinitionsOutsideSource.size();
								for(VIndex definitionIndex=0; definitionIndex<scopeCount; ++definitionIndex)
								{
									ESymbolScopeType scopeType;
									if( fSymTable->GetScopeType(scopeDefinitionsOutsideSource[definitionIndex].GetSymbolID(), scopeType) == true )
									{
										if( scopeType != eSymbolScopeTypeUndefined )
											scopeDefined = true;
									}
								...
								if( scopeDefined == true )
									outDefinitions.insert(outDefinitions.end(), scopeDefinitionsOutsideSource.begin(), scopeDefinitionsOutsideSource.end());
							}
						}
						else if( scopeDefinitionsInsideSource.size() > 0 )
						{
							outDefinitions = scopeDefinitionsInsideSource;
						}
						else
						{
							if( variableDefinitionsOutsideSource.size() > 0 )
								outDefinitions.insert(outDefinitions.end(), variableDefinitionsOutsideSource.begin(), variableDefinitionsOutsideSource.end());
							
							if( scopeDefinitionsOutsideSource.size() )
								outDefinitions.insert(outDefinitions.end(), scopeDefinitionsOutsideSource.begin(), scopeDefinitionsOutsideSource.end());
						}
					}
				}
			...
			/* WAK00088041
			 We should allow waModel's file definitions only for attributes because they are only defined in waModel
			 Thus, if outDefinition contains more than one entry and and one referencing a waModel symbols, it must be removed from the vector
			*/
			if( outDefinitions.size() > 1 )
			{
				for(VIndex index=outDefinitions.size()-1; index>=0; --index)
				{
					IDefinition def = outDefinitions[index];
					if( def.GetFilePath().EndsWith(CVSTR(".waModel")) == true )
						outDefinitions.erase(outDefinitions.begin()+index);
				}
			}
		}
	}
...
void VJavaScriptSyntax::_GetNextGenSuggestions(ISymbolTable* inSymbolTable, const EJSCompletionTestingMode& inSuggestionMode, const sLONG& inFileID, const ESymbolFileExecContext& inFileExecutionContext, const XBOX::VString& inLine, const sLONG& inLineIndex, ICodeEditorDocument* inDocument, ITipInfoArray* outTipsSuggestions, std::vector<XBOX::VString>* outTextsSuggestions)
{
	JavaScriptParser* parser = new JavaScriptParser(fUseNextGen);
	if( parser != NULL )
	{
		// Get the global scope id which will be our default scope context
		sLONG globalScopeId;
		inSymbolTable->GetGlobalScope(inFileExecutionContext, NULL, globalScopeId);
		
		// Now try to get the scope based on the line number
		sLONG contextScopeId;
		if( inSymbolTable->GetScope(inFileExecutionContext, inFileID, inLineIndex, contextScopeId) == false )
			contextScopeId = globalScopeId;
		
		VString line = inLine;
		
		SuggestionList suggestions;
		parser->GetSuggestions(&line, &suggestions, globalScopeId, contextScopeId, inFileID, inFileExecutionContext, inSymbolTable);
		...
		for(SuggestionList::iterator iter=suggestions.begin(); iter!=suggestions.end(); ++iter)
		{
			...
			switch( suggestion->fType )
			{
				...
				{
					VString displayText = suggestion->fDisplayText;
					
					if( displayText.Find(CVSTR("||")) == false )
					{
						displayText.AppendString(CVSTR("||"));
						displayText.AppendString(suggestion->fDisplayText);
					}

					// Suggestion as VString vector for test purpose only
					if( outTextsSuggestions != NULL && inSuggestionMode != eBoth )
						outTextsSuggestions->push_back( inSuggestionMode == eText ? suggestion->fText : displayText );

					// Suggestions as tips info
					if( inDocument != NULL && outTipsSuggestions != NULL )
						outTipsSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, displayText, suggestion->fText, eName) );
				}
					break;
					
				case SuggestionInfo::eWidget:
				{
					VString prototypeName;
					if( inSymbolTable->GetPrototypeChainParentName(inFileExecutionContext, suggestion->fDisplayText, prototypeName) == true )
					{
						VString displayText = suggestion->fDisplayText;
						...
						{
							displayText.AppendString(CVSTR("||"));
							displayText.AppendString(prototypeName);
						}
						...
						// Suggestions as tips info
						if( inDocument != NULL && outTipsSuggestions != NULL )
							outTipsSuggestions->AddTip( new VCodeEditorTipInfo(inDocument, displayText, suggestion->fText, eWidget) );
					}
				}
					break;
					
				case SuggestionInfo::eKeyword:
				case SuggestionInfo::eTable:
				case SuggestionInfo::eView:
				case SuggestionInfo::eColumn:
				case SuggestionInfo::eSchema:
				case SuggestionInfo::e4DMethod:
				case SuggestionInfo::eScalarMethod:
				case SuggestionInfo::eIndex:
				case SuggestionInfo::eGroup:
				case SuggestionInfo::e4DLocalVariable:
				case SuggestionInfo::e4DProcessVariable:
				case SuggestionInfo::e4DInterProcessVariable:
...
void VJavaScriptSyntax::GetSuggestionsForTesting(const VString& inSourceLine, ISymbolTable* inSymTable, const XBOX::VString& inFilePathStr, sLONG inLineIndex, EJSCompletionTestingMode inSuggestionMode, std::vector<XBOX::VString>& outSuggestions)
{
	if( fUseNextGen == true )
	{
		...
		if( files.size() != 0 )
		{
			const Symbols::IFile* file = files[0];
			if( file != NULL )
			{
				sLONG fileID = file->GetID();
				ESymbolFileExecContext fileExecutionContext = file->GetExecutionContext();
				
				_GetNextGenSuggestions(inSymTable, inSuggestionMode, fileID, fileExecutionContext, inSourceLine, inLineIndex, NULL, NULL, &outSuggestions);
			}
			inSymTable->ReleaseFiles(files);
		}
		return;
...
void VJavaScriptSyntax::GetSuggestions( ICodeEditorDocument* inDocument, sLONG inLineIndex, sLONG inPos, ITipInfoArray *outTipsSuggestions, sLONG& outStartOffset, bool inAll )
{
#if ACTIVATE_SYMBOL_TABLE_LIVE_REQUEST
	if( inDocument != NULL )
	{
	...
	if( fUseNextGen == true )
	{
		if( inDocument != NULL )
		{
			...
			if( file != NULL )
			{
				...
				_GetNextGenSuggestions(fSymTable, eBoth, fileID, fileExecutionContext, line, inLineIndex, inDocument, outTipsSuggestions, NULL);
			}
		}
	...
	else
	{
		...
		{
#if ACTIVATE_SUGGESTING_TRACES
#if ACTIVATE_JSLOG
			JSLog log(true);
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_SUGGESTING_TRACES
			...
			for (vector< Completion >::iterator iter = completions.begin(); iter != completions.end(); iter++)
			{
				...
				if (collator->EqualString_Like( comp.fText.GetCPointer(), comp.fText.GetLength(), userInput.GetCPointer(), userInput.GetLength(), suggestUpperCaseOnly ))
				{
					...
					// caller are empty.
					assert( !comp.fText.IsEmpty() );
					if (!outTipsSuggestions->Contains( comp.fDisplayText, true ) && comp.fText != original)
					{
						outTipsSuggestions->AddTip( new VCodeEditorTipInfo( inDocument, comp.fDisplayText, comp.fText, (comp.fStyle == eWidget) ? eName : comp.fStyle ) );
			...
			if( outTipsSuggestions->GetCount() == 1 && widgetKludge )
				outTipsSuggestions->Clear();
			...
			if ( outTipsSuggestions->GetCount() == 2 )
			{
				VString text1, text2;
				ITipInfo* tip1 = (*outTipsSuggestions)[0];
				ITipInfo* tip2 = (*outTipsSuggestions)[1];
				...
				if ( text1 == text2 )
				{
					if ( text1.BeginsWith( original, true ) )
						outTipsSuggestions->RemoveTip( 1 );
					else
						outTipsSuggestions->RemoveTip( 0 );
				}
			}
	...
	if (NULL != inFile)
	{
		...
		if (bMakeSuggestions)
		{
			...
			{ 
				...
				if( fUseNextGen == true )
				{
					parser->GetSuggestions( &completionLine, &suggestions, contextSymbol, inFile, inSymTable );
				}
				else
				{
...
bool VJavaScriptSyntax::_BuildIdentifiersChain(const std::vector<ILexerToken*>& inTokens, std::vector<XBOX::VString>& outNames, bool& outEndsWithDot, sLONG& outChainLength) const
{
	bool chainBuilt = false;
	
	VIndex tokenCount = inTokens.size();
	if( tokenCount >= 1 )
	{
		ILexerToken* lastToken = inTokens[tokenCount-1];
		if( lastToken != NULL )
		{
			ILexerToken::TYPE lastType = lastToken->GetType();
			VString lastName = lastToken->GetText();
			
			bool wasAName = (lastType == ILexerToken::TT_NAME);
			bool wasADot = (lastType == ILexerToken::TT_PUNCTUATION && lastName == CVSTR("."));
			
			if( wasAName == true || wasADot == true )
			{
				if( wasAName == true )
				{
					outNames.push_back(lastName);
					chainBuilt = true;
					outChainLength += lastName.GetLength();
				}
				else
				{
					outEndsWithDot = true;
				...
				for(VIndex index=tokenCount-2; index>=0; --index)
				{
					ILexerToken* currentToken = inTokens[index];
					if( currentToken != NULL )
					{
						ILexerToken::TYPE currentType = currentToken->GetType();
						VString currentName = currentToken->GetText();
							
						bool isAName = (currentType == ILexerToken::TT_NAME);
						bool isADot = (currentType == ILexerToken::TT_PUNCTUATION && currentName == CVSTR("."));

						if( isAName == false && isADot == false )
							break;
						
						if( isADot == true && wasAName == true )
						{
							wasAName = isAName;
							wasADot = isADot;
						...
						else if( isAName == true && wasADot == true )
						{
							wasAName = isAName;
							wasADot = isADot;
							...
							outChainLength += currentName.GetLength();
						}
					}
				}
			}
...
bool VJavaScriptSyntax::_MustSuggestConstructor(const std::vector<ILexerToken*>& inTokens, XBOX::VString& outConstructorStarter) const
{
	bool isConstructor = false;
	
	VIndex tokenCount = inTokens.size();
	for(VIndex index=tokenCount-1; index>=0; --index)
	{
		ILexerToken* currentToken = inTokens[index];
		if( currentToken != NULL )
		{
			ILexerToken::TYPE tokenType = currentToken->GetType();
			if( tokenType == ILexerToken::TT_WHITESPACE || tokenType == ILexerToken::TT_COMMENT )
				continue;
			
			VString tokenName = currentToken->GetText();
			
			if( tokenType == ILexerToken::TT_NAME )
			{
				outConstructorStarter = tokenName;
			}
			else if( tokenType == ILexerToken::TT_JAVASCRIPT_KEYWORD && tokenName == CVSTR("new") )
			{
				isConstructor = true;
				break;
			}
...
bool VJavaScriptSyntax::_MustSuggestIdentifier(const std::vector<ILexerToken*>& inTokens, XBOX::VString& outIdentifierStarter, sLONG& outFromScopeId) const
{
	bool isIdentifier = false;
	
	ILexerToken* currentToken = inTokens[inTokens.size()-1];
	if( currentToken != NULL )
	{
		if( currentToken->GetType() == ILexerToken::TT_NAME )
		{
			outIdentifierStarter = currentToken->GetText();
			isIdentifier = true;
		}
	}
...
void VJavaScriptSyntax::UseNextGen( bool inUseNextGen )
{
