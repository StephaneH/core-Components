...
private:
	...
	DisplayDocumentationSignal& fRequestDocumentationSignal;
	...
	bool _BuildIdentifiersChain(const std::vector<ILexerToken*>& inTokens, std::vector<XBOX::VString>& outNames, bool& outEndsWithDot, sLONG& outChainLength) const;
	bool _MustSuggestConstructor(const std::vector<ILexerToken*>& inTokens, XBOX::VString& outConstructorStarter) const;
	bool _MustSuggestIdentifier(const std::vector<ILexerToken*>& inTokens, XBOX::VString& outIdentifierStarter, sLONG& outFromScopeId) const;
	
	void _GetNextGenSuggestions(ISymbolTable* inSymbolTable, const EJSCompletionTestingMode& inSuggestionMode, const sLONG& inFileID, const ESymbolFileExecContext& inFileExecutionContext, const XBOX::VString& inLine, const sLONG& inLineIndex, ICodeEditorDocument* inDocument, ITipInfoArray* outTipsSuggestions, std::vector<XBOX::VString>* outTextsSuggestions);
	
public:
	VJavaScriptSyntax(DisplayDocumentationSignal& inSignal);
	...
	virtual void GetSyntaxErrorsForTesting(const XBOX::VFilePath& inFileToParse, std::vector<XBOX::VString>* outSyntaxErrors);

	virtual bool TokenizeOnKeyEnter() { return false; }

	virtual bool CanGotoDefinition( ICodeEditorDocument* inDocument, sLONG inLineIndex, sLONG inOffset, sLONG& outStartOffset, sLONG& outEndOffset );
	...
	void GetDefinitions( ICodeEditorDocument* inDocument, sLONG inLineIndex, std::vector<XBOX::VString>& inParts, std::vector<IDefinition>& outDefinitions );
	
	void GetDefinitions(XBOX::VectorOfVString& inParts, Symbols::ISymbol *contextSymbol, Symbols::IFile *inFile, const sLONG& inLineNumber, std::vector<IDefinition> & outDefinitions);
	void GetDefinitionsSelection(ICodeEditorDocument* inDocument, XBOX::VString& outSymbol, std::vector<XBOX::VString>& outParts);
	void GetDefinitionsSelection(ICodeEditorDocument* inDocument, XBOX::VString& inLine, const XBOX::VString& inSelection, sLONG& ioPos, XBOX::VString& outSymbolName, std::vector<XBOX::VString>& outParts);
...
public:
	void UseNextGen( bool inUseNextGen );
};
