...
public:
	...
	virtual VString GetResult( size_t inIndex ) { 
		VString ret;
		if(inIndex < GetResultCount())
			ret = mResults[ inIndex ];
		return ret;
	}
...
void VJSLanguageSyntaxTester::_GetCompletions( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	...
	VString symTablePathStr;
	VString testFilePathStr;
	VString completionString;
	VString completionModeStr;
	
	sLONG completionLocation;
	
	if( ioParams.CountParams() == 5 )
	{
		if( !ioParams.GetStringParam(1, symTablePathStr) )
			return;

		if( !ioParams.GetStringParam(2, completionString) )
			return;
		
		if( !ioParams.GetStringParam(3, testFilePathStr) )
			return;
		
		if( !ioParams.GetLongParam(4, &completionLocation) )
			return;
		
		if( !ioParams.GetStringParam(5, completionModeStr) )
			return;
	}
	else if( ioParams.CountParams() == 3 )
	{
		bool curlyBraces;
		if( !ioParams.GetStringParam(1, completionString) )
			return;
		
		if( !ioParams.GetBoolParam(2, &curlyBraces) )
			return;
			...
			if( symTable )
			{
				...
				if(symTable->OpenSymbolDatabase(file) )
				{
					...
					if( syntaxEngine )
					{
						...
						ioParams.ReturnValue( VJSLanguageSyntaxTesterResults::CreateInstance( ioParams.GetContext(), results ) );
						results->Release();
					}
				}
				symTable->Release();
...
{
	// We are expecting 5 JavaScript parameters:
	// - path to the symbol table
	// - symbol name
	// - symbol definition file
	// - symbol definition line number
	// - symbol kind (to know if function signature computation is required)
	
	VString	symTablePathStr;
	VString symbolName;
	VString symbolDefPath;
	...
	if( ioParams.CountParams() == 5 )
	{
		...
		if( !ioParams.GetStringParam(2, symbolName) )
			return;
		
		if( !ioParams.GetStringParam(3, symbolDefPath) )
			return;
		
		if( !ioParams.GetLongParam(4, &symbolDefLineNumber) )
			return;
		
		if( !ioParams.GetStringParam(5, symbolKind) )
			return;
	}
	else
		return;
	...
	if( languageSyntax != NULL )
	{
		...
		if( !symTablePathStr.IsEmpty() )
		{
			...
			if( symTable != NULL )
			{
				...
				if( symTable->OpenSymbolDatabase(file) )
				{
					if( symTable->UseNextGen() == true )
					{
						VString relativePath;
						symTable->GetRelativePath(symbolDefFilePath, eSymbolFileBaseFolderProject, relativePath);
						
						sLONG sourceId;
						if( symTable->GetSourceId(relativePath, eSymbolFileExecContextServer, eSymbolFileBaseFolderProject, sourceId) == true )
						{
							sLONG globalScopeId;
							symTable->GetGlobalScope(eSymbolFileExecContextServer, NULL, globalScopeId);
							
							sLONG contextScopeId;
							if( symTable->GetScope(eSymbolFileExecContextServer, sourceId, symbolDefLineNumber, contextScopeId) == false )
								contextScopeId = globalScopeId;
						
							if( symbolKind == CVSTR("Function") )
							{
								sLONG functionScopeId;
								if( symTable->GetScope(symbolName, eSymbolFileExecContextServer, contextScopeId, globalScopeId, functionScopeId) == true )
								{
									VJSObject	result(ioParams.GetContext());
									VJSValue	jsval(ioParams.GetContext());
									...
									XBOX::VString name;
									symTable->GetScopeName(functionScopeId, name);
									jsval.SetString( name );
									...
									sLONG toLine;
									sLONG toOffset;
									symTable->GetScopeBoundaries(functionScopeId, fromLine, fromOffset, toLine, toOffset);

									jsval.SetNumber( fromLine + 1 );
									...
									VString fullName;
									symTable->GetScopeFullName(functionScopeId, fullName);
									
									std::vector<XBOX::VString> returnTypes;
									symTable->GetReturnTypeNames(functionScopeId, returnTypes);
									
									VString returnSignature;
									if( returnTypes.size() == 0 )
										returnSignature = CVSTR("void");
									else
									{
										sort(returnTypes.begin(), returnTypes.end());
										unique(returnTypes.begin(), returnTypes.end());
										
										returnSignature.Join(returnTypes, '|');
									}
									
									VString fullSignature(fullName);
									fullSignature.AppendString(CVSTR(" : "));
									fullSignature.AppendString(returnSignature);
									
									jsval.SetString( fullSignature );
									...
									jsval.SetString( CVSTR("Function") );
								...
								{
									// We didn't find any function scope
									// Try to find a definition referencing our function
									sLONG nameId;
									if( symTable->GetName(symbolName, contextScopeId, eSymbolFileExecContextServer, sourceId, NULL, nameId) == true )
									{
										sLONG definitionId;
										if( symTable->GetDefinition(contextScopeId, nameId, sourceId, eSymbolDefinitionKindAll, -1, NULL, definitionId) == true )
										{
											sLONG typeId;
											if( symTable->GetType(definitionId, CVSTR("Function"), NULL, typeId) == true )
											{
												bool referenceFound = false;
												
												std::vector<sLONG> referenceIds;
												referenceFound = symTable->GetReference(eSymbolReferenceKindVariableDeclaration, definitionId, NULL, referenceIds);
												if( referenceFound == false)
													referenceFound = symTable->GetReference(eSymbolReferenceKindAssignmentDeclaration, definitionId, NULL, referenceIds);
												
												if( referenceFound == true )
												{
													if( referenceIds.size() == 1 )
													{
														symTable->GetReferenceToId(referenceIds[0], functionScopeId);
														...
														// Replace the original name of the function by its reference
														XBOX::VString name;
														symTable->GetScopeName(functionScopeId, name);
														fullSignature.Replace(symbolName, 1, name.GetLength());
							...
							{
								sLONG symbolNameId;
								if( symTable->GetName(symbolName, contextScopeId, eSymbolFileExecContextServer, sourceId, NULL, symbolNameId) == true )
								{
									sLONG symbolDefinitionId;
									if( symTable->GetDefinition(contextScopeId, symbolNameId, sourceId, eSymbolDefinitionKindAll, -1, NULL, symbolDefinitionId) == true )
									{
										VJSObject	result(ioParams.GetContext());
										VJSValue	jsval(ioParams.GetContext());
										...
										XBOX::VString name;
										symTable->GetDefinitionName(symbolDefinitionId, name);
										jsval.SetString( name );
										result.SetProperty(L"name", jsval, JS4D::PropertyAttributeNone);
										
										
										sLONG line;
										symTable->GetDefinitionLine(symbolDefinitionId, line);
										jsval.SetNumber( line + 1 );
										result.SetProperty(L"line", jsval, JS4D::PropertyAttributeNone);
										
										
										ESymbolDefinitionKind symbolKind;
										symTable->GetDefinitionKind(symbolDefinitionId, symbolKind);
										XBOX::VString kind("UNKWOWN_SYMBOL_KIND");
										if( symbolKind == eSymbolDefinitionKindLocalVariable || symbolKind == eSymbolDefinitionKindVariable )
											kind = CVSTR("Local Variable");
										jsval.SetString( kind );
										result.SetProperty(L"kind", jsval, JS4D::PropertyAttributeNone);
										
										
										std::vector<XBOX::VString> types;
										symTable->GetTypeNames(symbolDefinitionId, types);
										if( types.size() > 0 )
										{
											// DS KLUDGE
											VString typeName = types[0];
											VIndex kIndex = typeName.Find(CVSTR("+"));
											if( kIndex != 0 )
												typeName.SubString(kIndex+1, typeName.GetLength()-kIndex);
											
											jsval.SetString( typeName );
					...
					{
						// Get symbol file
						std::vector< Symbols::IFile * > files = symTable->RetainFilesByPathAndBaseFolder( symbolDefFilePath, eSymbolFileBaseFolderProject );
						if ( ! files.empty() )
						{
							Symbols::IFile *file = files.front();
							file->Retain();
							
							symTable->ReleaseFiles(files);
							...
							if (NULL == sym)
							{
								std::vector< Symbols::ISymbol * >	syms = symTable->RetainSymbolsByName(owner, symbolName,file);
...
void VJSLanguageSyntaxTester::_Tokenize( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	...
	if (languageSyntax)
	{
		...
		if (syntaxEngine)
		{
			...
			VJSArray result( ioParams.GetContext() );
			VJSValue jsval(ioParams.GetContext());
			...
			if (symTable)
			{
				...
				if (symTable->OpenSymbolDatabase( file ))
				{
					...
					if (syntaxEngine)
					{
						...
						ioParams.ReturnValue( VJSLanguageSyntaxTesterDefinitionResults::CreateInstance( ioParams.GetContext(), results ) );
						results->Release();
					}
				}
				symTable->Release();
...
void VJSLanguageSyntaxTester::_GetCSSCompletions( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	XBOX::VString stringToComplete;
	if( ioParams.CountParams() == 1 )
	{
		if( !ioParams.GetStringParam(1, stringToComplete) )
			return;
	}
	else 
	{
	...
	if( languageSyntax )
	{
		ISyntaxInterface* syntaxEngine = languageSyntax->GetSyntaxByExtension(CSS_EXTENSION);
		if( syntaxEngine )
		{
			// Now that we've got the syntax engine, we can ask it for the completions we need
			std::vector<VString> suggestions;
			syntaxEngine->GetSuggestionsForTesting( stringToComplete, NULL, "" , 0, eText, suggestions );
	...
	ReleaseRefCountable(&languageSyntax);
}

void VJSLanguageSyntaxTester::_GetSyntaxErrorResults( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	XBOX::VString filePathStr;
	if( ioParams.CountParams() == 1 )
	{
		if( !ioParams.GetStringParam(1, filePathStr) )
			return;
	}
	else
	{
	...
	if( languageSyntax )
	{
		ISyntaxInterface* syntaxEngine = languageSyntax->GetSyntaxByExtension(JAVASCRIPT_EXTENSION);
		if( syntaxEngine )
		{
			XBOX::VFilePath fileToParse(filePathStr, FPS_POSIX);
			
			std::vector<XBOX::VString> syntaxErrors;
			syntaxEngine->GetSyntaxErrorsForTesting(fileToParse, &syntaxErrors);

			JSResult* results = new JSResult( syntaxErrors );
			ioParams.ReturnValue(VJSLanguageSyntaxTesterResults::CreateInstance(ioParams.GetContext(), results));
			ReleaseRefCountable(&results);
		}
	}
...
#if ACTIVATE_JSLOG
	JSLog log;
	log.SetTitle(CVSTR("UNITARY_TESTS"));
	log.Append(CVSTR("PARSING"), testFilePathStr);
...
void VJSLanguageSyntaxTester::GetDefinition( ClassDefinition &outDefinition )
{
	...
	{
		{ "getCSSCompletions",			js_callStaticFunction< _GetCSSCompletions >,		JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontEnum | JS4D::PropertyAttributeDontDelete },
		...
		{ "getSyntaxErrorResults",		js_callStaticFunction< _GetSyntaxErrorResults >,	JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontEnum | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0 }
	};
