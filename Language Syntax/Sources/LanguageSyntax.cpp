...
void VJSLanguageSyntaxTester::_GetSyntaxColoring( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	...
	if (syntaxEngine) {
		...
		for (size_t i = 0; i < offsets.size(); i++) {
			...
			results.push_back( VJSLanguageSyntaxTesterColorResults::CreateInstance( ioParams.GetContext(), result ) );
		}

		// Return an array of our results
		VJSArray arr( ioParams.GetContext() );
...
void VJSLanguageSyntaxTester::_GetASTComputationResults( XBOX::VJSParms_callStaticFunction &ioParams, void * )
{
	XBOX::VString stringToComputeASTFrom;
	XBOX::VString typeOfASTComputationType;
	if( ioParams.CountParams() == 2 )
	{
		if( !ioParams.GetStringParam(1, stringToComputeASTFrom) )
			return;
		if( !ioParams.GetStringParam(2, typeOfASTComputationType) )
			return;
	}
	else
	{
	...
	if( languageSyntax )
	{
		...
		if( syntaxEngine )
		{
			std::vector<XBOX::VString> declarations;
			syntaxEngine->GetASTForTesting(stringToComputeASTFrom, typeOfASTComputationType, declarations);

			JSResult* results = new JSResult( declarations );
			ioParams.ReturnValue(VJSLanguageSyntaxTesterResults::CreateInstance(ioParams.GetContext(), results));
			ReleaseRefCountable(&results);
		}
	}
...
void VJSLanguageSyntaxTester::GetDefinition( ClassDefinition &outDefinition )
{
	static inherited::StaticFunction szFunctions [ ] =
	{
		...
		{ "getASTComputationResults",	js_callStaticFunction< _GetASTComputationResults >,	JS4D::PropertyAttributeReadOnly | JS4D::PropertyAttributeDontEnum | JS4D::PropertyAttributeDontDelete },
		{ 0, 0, 0 }
	};
