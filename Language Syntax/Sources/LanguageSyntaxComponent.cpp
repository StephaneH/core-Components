...
#if ACTIVATE_JSLOG
								{	JSLog logEdit(true);	logEdit.SetTitle("Parsing");	logEdit.Append("EditionFlag", "TRUE");	logEdit.Append("FileId", ownerFile->GetID()); }
#endif // ACTIVATE_JSLOG
								// Flag the file's symbols to EDITED
								bool editionState = true;
								symTable->SetSymbolsEditionState(ownerFile, editionState);
								
								// GET ALL EDITED SYMBOLS IDS (i.e. SYMBOLS TO BE DELETED)
								symTable->GetSymbolIds(ownerFile, editionState, deletedSymbolIds);
								...
								if( parsed )
								{
									...
									if( fileIds.size() > 0 )
									{
										for(VIndex index=0; index<fileIds.size(); index++)
										{
											...
											symTable->DeleteSymbolsForFile( fileTemp );
