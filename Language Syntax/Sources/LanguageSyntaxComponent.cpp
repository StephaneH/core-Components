...
public:
	VDocumentParserManager(const bool& inUseNextGen);
...
private:
	...
	void PrepareSymbolScopeDeclarationUpdate(const sLONG& inSourceId)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
			fCurrentWorkItem->fSymTable->PrepareUpdates(inSourceId, false);
	}
	
	void FinalizeSymbolScopeDeclarationUpdate(const sLONG& inSourceId)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
			fCurrentWorkItem->fSymTable->FinalizeUpdates(inSourceId, false);
	}
	
	void UpdateSymbolScopeDeclarations(const JavaScriptAST::SymbolScopeDeclaration* inDeclarations, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
		{
			VString scopeName			= inDeclarations->GetScopeName();
			ESymbolScopeType scopeType	= inDeclarations->GetScopeType();
			
			sLONG fromLine				= inDeclarations->GetFromLine();
			sLONG fromOffset			= inDeclarations->GetFromOffset();
			sLONG toLine				= inDeclarations->GetToLine();
			sLONG toOffset				= inDeclarations->GetToOffset();
			bool scopeConstructorFlag	= inDeclarations->GetConstructorFlag();
			
			// The special execution context case ClientServer will be processed as follow :
			// * symbols will be created with the Client execution context value
			// * symbols will be created with the Server execution context value
			//
			// This represent only a few symbols (those defined in JSCore)
			// It is not optimized regarding memory but it is better regarding algorithm because
			// it avoid a special case to deal with
			if( inExecutionContext == eSymbolFileExecContextClientServer )
			{
				UpdateSymbolScopeDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextClient);
				UpdateSymbolScopeDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextServer);
			}
			else
			{
				// Get the scope symbol (or create it if necessary)
				sLONG scopeId = 0;
				if( scopeType == eSymbolScopeTypeGlobal )
				{
					// We can have only one global symbol per execution context, without regarding the source id
					ISymbolUpdateParameters* updateParameters = fCurrentWorkItem->fSymTable->RetainSymbolUpdateParameters();
					updateParameters->MustSetStatus(true);
					updateParameters->SetStatus(eSymbolStatusUpToDate);
					
					if( fCurrentWorkItem->fSymTable->GetGlobalScope(inExecutionContext, updateParameters, scopeId) == false )
						fCurrentWorkItem->fSymTable->SetGlobalScope(inExecutionContext, scopeId);

					fCurrentWorkItem->fSymTable->ReleaseSymbolUpdateParameters(updateParameters);
				}
				else
				{
					// We can have only one global symbol per execution context, without regarding the source id
					...
					updateParameters->MustSetBoundaries(true);
					updateParameters->SetBoundaries(fromLine, fromOffset, toLine, toOffset);
					
					updateParameters->MustSetConstructorFlag(true);
					updateParameters->SetConstructorFlag(scopeConstructorFlag);
					
					if( fCurrentWorkItem->fSymTable->GetScope(scopeName, eSymbolScopeTypeAll, inParentScopeId, inSourceId, inExecutionContext, updateParameters, scopeId) == false )
						fCurrentWorkItem->fSymTable->SetScope(scopeName, scopeType, inParentScopeId, inSourceId, inExecutionContext, fromLine, fromOffset, toLine, toOffset, scopeConstructorFlag, scopeId);
					
					// Update function signatures
					if( scopeType == eSymbolScopeTypeFunctionDeclaration || scopeType == eSymbolScopeTypeFunctionExpression )
					{
						VString functionSignature = inDeclarations->GetScopeFullName();
						fCurrentWorkItem->fSymTable->SetScopeFullName(scopeId, functionSignature);
					}
					
					updateParameters->MustSetBoundaries(false);
					sLONG nameId;
					if( fCurrentWorkItem->fSymTable->GetName(scopeName, inParentScopeId, inExecutionContext, inSourceId, updateParameters, nameId) == false )
						fCurrentWorkItem->fSymTable->SetName(scopeName,	inParentScopeId, inExecutionContext, inSourceId, nameId);
					
					// Create the prototype chain
					if( inDeclarations->GetExtendFlag() == true )
					{
						VString extendedName;
						inDeclarations->GetExtendedName(extendedName);
						
						VectorOfVString names;
						extendedName.GetSubStrings(CVSTR("|"), names);
						
						VIndex namesCount = names.size();
						for(VIndex nameIndex=0; nameIndex<namesCount; ++nameIndex)
						{
							VString currentParent = names[nameIndex];
							
							sLONG prototypeChainId;
							if( fCurrentWorkItem->fSymTable->GetPrototypeChain(inExecutionContext, scopeName, currentParent, updateParameters, prototypeChainId) == false )
								fCurrentWorkItem->fSymTable->SetPrototypeChain(inExecutionContext, inSourceId, scopeName, currentParent);
						}
					}
					
					// Process symbols from returns declarations
					const std::vector<JavaScriptAST::ISymbolDeclaration*> returns = inDeclarations->GetReturnDeclarations();
					for(std::vector<JavaScriptAST::ISymbolDeclaration*>::const_iterator it=returns.begin(); it!=returns.end(); ++it)
					{
						JavaScriptAST::ISymbolDeclaration* current = (*it);
						if( current != NULL )
						{
							JavaScriptAST::SymbolDefinitionContext context;
							
							sLONG symbolId;
							current->ToDatabase(fCurrentWorkItem->fSymTable, scopeId, inSourceId, inExecutionContext, context, symbolId);
						}
					}
				...
				// Go on with next scopes
				VIndex total = inDeclarations->GetChildScopeCount();
				for(VIndex index=0; index<total; ++index)
					UpdateSymbolScopeDeclarations(inDeclarations->GetChildScopeAt(index), scopeId, inSourceId, inExecutionContext);
			}
		}
	}
	
	void UpdateSymbolScopeVariableDeclarations(const JavaScriptAST::SymbolScopeDeclaration* inDeclarations, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
		{
			VString scopeName			= inDeclarations->GetScopeName();
			...
			if( inExecutionContext == eSymbolFileExecContextClientServer )
			{
				UpdateSymbolScopeVariableDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextClient);
				UpdateSymbolScopeVariableDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextServer);
			}
			else
			{
				// Get the scope symbol (or create it if necessary)
				sLONG scopeId = 0;
				if( scopeType == eSymbolScopeTypeGlobal )
					fCurrentWorkItem->fSymTable->GetGlobalScope(inExecutionContext, NULL, scopeId);
				else
					fCurrentWorkItem->fSymTable->GetScope(scopeName, eSymbolScopeTypeAll, inParentScopeId, inSourceId, inExecutionContext, NULL, scopeId);
				
				// Process symbols from local variable declarations
				const std::vector<JavaScriptAST::ISymbolDeclaration*> variables = inDeclarations->GetVariableDeclarations();
				for(std::vector<JavaScriptAST::ISymbolDeclaration*>::const_iterator it=variables.begin(); it!=variables.end(); ++it)
				{
					JavaScriptAST::ISymbolDeclaration* current = (*it);
					if( current != NULL )
					{
				...
				VIndex total = inDeclarations->GetChildScopeCount();
				for(VIndex index=0; index<total; ++index)
					UpdateSymbolScopeVariableDeclarations(inDeclarations->GetChildScopeAt(index), scopeId, inSourceId, inExecutionContext);
			}
		}
	}
	
	void UpdateSymbolScopeAssignmentDeclarations(const JavaScriptAST::SymbolScopeDeclaration* inDeclarations, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
		{
			VString scopeName			= inDeclarations->GetScopeName();
			...
			if( inExecutionContext == eSymbolFileExecContextClientServer )
			{
				UpdateSymbolScopeAssignmentDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextClient);
				UpdateSymbolScopeAssignmentDeclarations(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextServer);
			}
			else
			{
				// Get the scope symbol (or create it if necessary)
				...
				// Process symbols from property declarations
				const std::vector<JavaScriptAST::ISymbolDeclaration*> properties = inDeclarations->GetPropertyDeclarations();
				for(std::vector<JavaScriptAST::ISymbolDeclaration*>::const_iterator it=properties.begin(); it!=properties.end(); ++it)
				{
					JavaScriptAST::ISymbolDeclaration* current = (*it);
					if( current != NULL )
					{
				...
				// Process symbols from assignment declarations
				const std::vector<JavaScriptAST::ISymbolDeclaration*> assignments = inDeclarations->GetAssignmentDeclarations();
				for(std::vector<JavaScriptAST::ISymbolDeclaration*>::const_iterator it=assignments.begin(); it!=assignments.end(); ++it)
				{
					JavaScriptAST::ISymbolDeclaration* current = (*it);
					if( current != NULL )
					{
				...
				VIndex total = inDeclarations->GetChildScopeCount();
				for(VIndex index=0; index<total; ++index)
					UpdateSymbolScopeAssignmentDeclarations(inDeclarations->GetChildScopeAt(index), scopeId, inSourceId, inExecutionContext);
			}
		}
	}
	...
	bool _IsDataModelContent(const VDocumentParserManager::WorkItem* inWorkItem) const
	{
		...
		if( inWorkItem->fCatalog != NULL && inWorkItem->fFilePath.GetPath().EndsWith(CVSTR("waModel")) == true )
			ret = true;
	...
	void _GetWidgetType(const VJSONValue& inValue, VString& outType) const
	{
		...
		if( outType == CVSTR("accordion") )				outType = CVSTR("Accordion");
		else if( outType == CVSTR("autoForm") )			outType = CVSTR("AutoForm");
		else if( outType == CVSTR("button") )			outType = CVSTR("Button");
		else if( outType == CVSTR("buttonImage") )		outType = CVSTR("ButtonImage");
		else if( outType == CVSTR("chart") )			outType = CVSTR("Chart");
		else if( outType == CVSTR("calendar") )			outType = CVSTR("Calendar");
		else if( outType == CVSTR("canvas") )			outType = CVSTR("Canvas");
		...
		else if( outType == CVSTR("dataGrid") )			outType = CVSTR("Grid");
		...
		else if( outType == CVSTR("googleMap") )		outType = CVSTR("GoogleMap");
		else if( outType == CVSTR("googleMaps") )		outType = CVSTR("GoogleMaps");
		...
		else if( outType == CVSTR("navigationView") )	outType = CVSTR("NavigationView");
		else if( outType == CVSTR("label") )			outType = CVSTR("Label");
		else if( outType == CVSTR("listView") )			outType = CVSTR("ListView");
		else if( outType == CVSTR("login") )			outType = CVSTR("Login");
		else if( outType == CVSTR("popover") )			outType = CVSTR("Popover");
		else if( outType == CVSTR("progressBar") )		outType = CVSTR("ProgressBar");
		else if( outType == CVSTR("queryForm") )		outType = CVSTR("QueryForm");
		else if( outType == CVSTR("radioGroup") )		outType = CVSTR("RadioGroup");
		else if( outType == CVSTR("richText") )			outType = CVSTR("RichText");
		else if( outType == CVSTR("section") )			outType = CVSTR("Section");
		else if( outType == CVSTR("select") )			outType = CVSTR("Select");
		else if( outType == CVSTR("slider") )			outType = CVSTR("Slider");
		else if( outType == CVSTR("splitView") )		outType = CVSTR("SplitView");
		else if( outType == CVSTR("switchbox") )		outType = CVSTR("SwitchBox");
		else if( outType == CVSTR("tabView") )			outType = CVSTR("TabView");
		else if( outType == CVSTR("textField") )		outType = CVSTR("TextField");
		else if( outType == CVSTR("video") )			outType = CVSTR("Video");
		else if( outType == CVSTR("wysiwyg") )			outType = CVSTR("TinyMCE");
		else if( outType == CVSTR("yahooWeather") )		outType = CVSTR("YahooWeather");
	}
	...
	void _GetName(const VJSONValue& inValue, const VString& inKeyName, VString& outName) const
	{
		VJSONValue name = inValue.GetProperty(inKeyName);
		name.GetString(outName);
	}

	void _GetFrom(const VJSONValue& inValue, VString& outName) const
	{
		VJSONValue name = inValue.GetProperty("from");
		name.GetString(outName);
	}

	void _GenerateSymbolDeclarationsForAttribute(const VString& inFullName, const VString& inTypeName, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VectorOfVString attributeTokens;
		inFullName.GetSubStrings(CVSTR("."), attributeTokens);
		
		JavaScriptAST::SymbolScopeDeclaration* currentScope = ioGlobalScope;
		
		VIndex count = attributeTokens.size();
		for(VIndex index=0; index<count; ++index)
		{
			VString token = attributeTokens[index];
			
			if( index != count-1 )
			{
				// Each token is a scope, except the last one
				bool constructorFlag = ( (index == 1) ? true : false);
				JavaScriptAST::SymbolScopeDeclaration* scope = currentScope->GetChildScope(token);
				if( scope == NULL )
				{
					VString extendClassName;
					bool extendClassNameFlag = false;
					
					if( index == 1 )
					{
						extendClassName = CVSTR("dataClass");
						extendClassNameFlag = true;
					}
					
					scope = new JavaScriptAST::SymbolScopeDeclaration(currentScope, token, token, eSymbolScopeTypeObject, 0, 0, 0, 0, constructorFlag, extendClassNameFlag, extendClassName);
			...
			{
				JavaScriptAST::PropertyDeclaration declaration;
				
				declaration.SetName(token);

				XBOX::VString jsTypeName = inTypeName;
				for(int count=0; count<ENTITY_MODEL_ATTR_TYPE_COUNT; ++count)
				{
					if( ENTITY_MODEL_ATTR_TYPES[count].type == inTypeName )
					{
						jsTypeName = ENTITY_MODEL_ATTR_TYPES[count].jsType;
						break;
					}
				}
				declaration.SetRhsTypeName(jsTypeName);
				
				JavaScriptAST::IdentifiersChainSymbolDeclaration typeIdentifier;
				typeIdentifier.AppendIdentifier(inTypeName);
				
				JavaScriptAST::NewDeclaration newDeclaration;
				newDeclaration.SetIdentifiersDeclaration(typeIdentifier.Clone());
				
				declaration.SetRhs(newDeclaration.Clone());
				
				currentScope->AddPropertyDeclaration(declaration.Clone());
			}
		}
	}

	void _GenerateSymbolDeclarationsForMethod(const VString& inFullName, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VectorOfVString attributeTokens;
		inFullName.GetSubStrings(CVSTR("."), attributeTokens);
		...
		for(VIndex index=0; index<count; ++index)
		{
			...
			// Each token is an object scope, except the last one
			// the last one is a function
			ESymbolScopeType scopeType = ( (index != count-1) ? eSymbolScopeTypeObject : eSymbolScopeTypeFunctionExpression);
			
			JavaScriptAST::SymbolScopeDeclaration* scope = currentScope->GetChildScope(token);
			if( scope == NULL )
			{
				scope = new JavaScriptAST::SymbolScopeDeclaration(currentScope, token, token, scopeType, 0, 0, 0, 0, constructorFlag, false, CVSTR(""));
				currentScope->AddChildScope(scope);
			}
...
public:
	WorkerThread( VDocumentParserManager *inCreator, const bool& inUseNextGen ) : VTask( inCreator, 0, eTaskStylePreemptive, WorkerRunner ), fCurrentWorkItem( NULL ), fManager( inCreator ), fWaitingForWork( true ), fUseNextGen(inUseNextGen) {
...
VDocumentParserManager::VDocumentParserManager(const bool& inUseNextGen):fUseNextGen(inUseNextGen)
{
...
void VDocumentParserManager::CreateWorkerThreads( sLONG inNumThreads )
{
	...
	for (sLONG i = 0; i < inNumThreads; i++) {
		WorkerThread *worker = new WorkerThread( this, fUseNextGen );
...
bool VDocumentParserManager::ScheduleTaskHelper( const void *inCaller, const VSymbolFileInfos &inFileInfos, IEntityModelCatalog *inCatalog, const VString &inContents, const VJSONValue& inValue, const bool& inFileContentIsSet, ISymbolTable *inSymTable, Priority inPriority, IJob *inJob, const bool& inParsingMandatory )
{
	...
	else if( inFileInfos.GetFilePath().GetPath().EndsWith(CVSTR("JSCore.js")) && fUseNextGen == true )
	{
		// BUGFIX WAK0089133
		bAddToQueue = false;
	}
...
#if ACTIVATE_PARSING_ERROR_TRACES
#if ACTIVATE_JSLOG
	JSLog log;	
...
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_PARSING_ERROR_TRACESs
}
...
bool WorkerThread::ParseDocument( VDocumentParserManager::WorkItem *inWorkItem, bool& outHasParsingError, bool& outContentOverwritenFromDisk, bool& outTimeStampUpdated )
{
	if( fUseNextGen == true )
	{
		...
		if( !IsDying() )
		{
			...
			if( inWorkItem->fFilePath.GetPath().EndsWith(CVSTR("_STLR.js")) )
			{
				if( inWorkItem->fFileContents.BeginsWith(CVSTR("STLR_START")) && inWorkItem->fFileContents.EndsWith(CVSTR("STLR_STOP")) )
				{
					// Remove !!
					XBOX::VString temp = inWorkItem->fFileContents;
					temp.SubString(11, temp.GetLength()-19);
					
					// Each request must be writen on a single line
					VectorOfVString requests;
					temp.GetSubStrings("\r", requests);
					...
					for(VectorOfVString::const_iterator it=requests.begin(); it!=requests.end(); ++it)
					{
						...
						if( !curReq.BeginsWith("//") && curReq.EndsWith("!") )
						{
							...
							// Execute request
							inWorkItem->fSymTable->Dump(curReq);
						}
					}
					...
					if( inWorkItem->fSymTable && !IsDying() )
					{
						...
						if (ownerFile)
						{
								...
								(inWorkItem->fParsingIsMandatory) )								// The file has been modified, not saved yet and we discard modifications
							{
								...
								if( _IsJavaScriptContent(inWorkItem) || _IsDataModelContent(inWorkItem) || (_IsHTMLContent(inWorkItem) && _IsWithinWaPage(inWorkItem)) )
								{
									// Notify parsing start
									fManager->GetParsingStartSignal().Trigger(inWorkItem->fFilePath);
									
#if ACTIVATE_GLOBAL_PARSING_TRACES
#if ACTIVATE_JSLOG
									JSLog logGlobalParsing(true);
									logGlobalParsing.SetTitle("GlobalParsing");
									logGlobalParsing.Append("FileName", inWorkItem->fFilePath.GetPath());
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_GLOBAL_PARSING_TRACES
									
									JavaScriptAST::SymbolScopeDeclaration* globalScopeDeclarations = new JavaScriptAST::SymbolScopeDeclaration(NULL, CVSTR("global"), CVSTR("global"), eSymbolScopeTypeGlobal, 0, 0, 0, 0, false, false, CVSTR(""));
									
									if( _IsHTMLContent(inWorkItem) && _IsWithinWaPage(inWorkItem) )
									{
										...
										if( widgetIdsAndTypes.size() > 0 || dataSourceIdsAndTypes.size() > 0 )
										{
											// Add WAF object scope to global scope
											JavaScriptAST::SymbolScopeDeclaration* wafScope = new JavaScriptAST::SymbolScopeDeclaration(globalScopeDeclarations, CVSTR("WAF"), CVSTR("WAF"), eSymbolScopeTypeObject, 0, 0, 0, 0, false, false, CVSTR(""));
											globalScopeDeclarations->AddChildScope(wafScope);
											
											if( widgetIdsAndTypes.size() > 0 )
											{
												JavaScriptAST::PropertyDeclaration propertyDeclaration;
												propertyDeclaration.SetName(CVSTR("widgets"));
												propertyDeclaration.SetInstancePropertyFlag(false);
												wafScope->AddPropertyDeclaration(propertyDeclaration.Clone());

												JavaScriptAST::SymbolScopeDeclaration* widgetsScope = new JavaScriptAST::SymbolScopeDeclaration(wafScope, CVSTR("widgets"), CVSTR("widgets"), eSymbolScopeTypeObject, 0, 0, 0, 0, false, false, CVSTR(""));
												wafScope->AddChildScope(widgetsScope);
												
												for(std::map<XBOX::VString, XBOX::VString>::const_iterator it=widgetIdsAndTypes.begin(); it!=widgetIdsAndTypes.end(); ++it)
												{
													const VString& widgetName = it->first;
													const VString& widgetTypeName = it->second;
													
													JavaScriptAST::PropertyDeclaration propertyDeclaration;
													propertyDeclaration.SetName(widgetName);
													propertyDeclaration.SetInstancePropertyFlag(false);
													widgetsScope->AddPropertyDeclaration(propertyDeclaration.Clone());
													
													JavaScriptAST::SymbolScopeDeclaration* widget = new JavaScriptAST::SymbolScopeDeclaration(widgetsScope, widgetName, widgetName, eSymbolScopeTypeObject, 0, 0, 0, 0, false, true, widgetTypeName);
													widgetsScope->AddChildScope(widget);
											...
											{
												// We can access data sources using 'WAF.sources.'
												JavaScriptAST::PropertyDeclaration propertyDeclaration;
												propertyDeclaration.SetName(CVSTR("sources"));
												propertyDeclaration.SetInstancePropertyFlag(false);
												wafScope->AddPropertyDeclaration(propertyDeclaration.Clone());

												JavaScriptAST::SymbolScopeDeclaration* wafSourcesScope = new JavaScriptAST::SymbolScopeDeclaration(wafScope, CVSTR("sources"), CVSTR("sources"), eSymbolScopeTypeObject, 0, 0, 0, 0, false, false, CVSTR(""));
												wafScope->AddChildScope(wafSourcesScope);
												
												for(std::map<XBOX::VString, XBOX::VString>::const_iterator it=dataSourceIdsAndTypes.begin(); it!=dataSourceIdsAndTypes.end(); ++it)
												{
													const VString& sourceName = it->first;
													const VString& sourceTypeName = it->second;
													
													JavaScriptAST::PropertyDeclaration propertyDeclaration;
													propertyDeclaration.SetName(sourceName);
													propertyDeclaration.SetInstancePropertyFlag(false);
													wafSourcesScope->AddPropertyDeclaration(propertyDeclaration.Clone());

													JavaScriptAST::SymbolScopeDeclaration* wafSource = new JavaScriptAST::SymbolScopeDeclaration(wafSourcesScope, sourceName, sourceName, eSymbolScopeTypeObject, 0, 0, 0, 0, false, true, CVSTR(""));
													wafSourcesScope->AddChildScope(wafSource);
									...
									else if( _IsJavaScriptContent(inWorkItem) )
									{
										JavaScriptAST::Visitor* declParser = JavaScriptAST::Visitor::GetDeclarationParser( fUseNextGen, NULL, NULL, NULL, globalScopeDeclarations );
										
										if( !IsDying() && declParser )
										{
											JavaScriptParser* parser = new JavaScriptParser(fUseNextGen);
											if( parser )
											{
...
#if ACTIVATE_CORE_PARSING_TRACES
#if ACTIVATE_JSLOG
												JSLog logCoreParsing(true);
												logCoreParsing.SetTitle("CoreParsing");
												logCoreParsing.Append("FileName", inWorkItem->fFilePath.GetPath());
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_CORE_PARSING_TRACES
												
												JavaScriptAST::Node *program = parser->GetAST( inWorkItem->fFilePath, ownerFile->GetID(), &inWorkItem->fFileContents, outHasParsingError );
												...
												if( program )
												{
#if ACTIVATE_CORE_AST_BROWING_TRACES
#if ACTIVATE_JSLOG
...
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_CORE_AST_BROWING_TRACES
									...
									{
										VString modelName = "model";

										VectorOfVString entityNames;
										inWorkItem->fCatalog->GetEntityModelNames(entityNames);
										
										for(VectorOfVString::iterator iterEntity=entityNames.begin(); iterEntity!=entityNames.end(); ++iterEntity)
										{
											VString entityName = (*iterEntity);
											
											std::vector<IEntityModelCatalogAttribute*>	attributes;
											std::vector<IEntityModelCatalogMethod*>		methods;
											
											inWorkItem->fCatalog->GetEntityModelAttributes( entityName, attributes );
											inWorkItem->fCatalog->GetEntityModelMethods( entityName, methods );

											for (std::vector<IEntityModelCatalogAttribute* >::iterator iterAttribute=attributes.begin(); iterAttribute!=attributes.end(); ++iterAttribute)
											{
												VString attributeName, attributeType;
												
												(*iterAttribute)->GetName(attributeName);
												(*iterAttribute)->GetType(attributeType);

												VString fullName = modelName + "." + entityName + "." + attributeName;

												_GenerateSymbolDeclarationsForAttribute(fullName, attributeType, globalScopeDeclarations);
											}
											
											for(std::vector<IEntityModelCatalogMethod*>::iterator iterMethod=methods.begin(); iterMethod!=methods.end(); ++iterMethod)
											{
												VString methodName, methodType;

												(*iterMethod)->GetName(methodName);
												(*iterMethod)->GetApplyTo(methodType);
												
												VString fullName = modelName + "." + entityName + ".";
												if( methodType == "entity" )					fullName.AppendString("entityMethods.");
												else if( methodType == "entityCollection" )		fullName.AppendString("collectionMethods.");
												else if( methodType == "dataClass" )			fullName.AppendString("methods.");
												fullName.AppendString(methodName);
												
												_GenerateSymbolDeclarationsForMethod(fullName, globalScopeDeclarations);
											}
									...
									if( globalScopeDeclarations != NULL )
									{
										{
#if ACTIVATE_NEXT_GEN_AST_PREPARING_TRACES
#if ACTIVATE_JSLOG
											JSLog logPrepare(true);
											logPrepare.SetTitle(CVSTR("NextGenASTPreparing"));
											logPrepare.Append("FileName", inWorkItem->fFilePath.GetPath());
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_NEXT_GEN_AST_PREPARING_TRACES
											PrepareSymbolScopeDeclarationUpdate(ownerFile->GetID());
#if ACTIVATE_NEXT_GEN_AST_PREPARING_TRACES
#if ACTIVATE_JSLOG
											logPrepare.Print();
#endif //ACTIVATE_JSLOG
										...
										{
#if ACTIVATE_NEXT_GEN_AST_UPDATING_TRACES
#if ACTIVATE_JSLOG
											JSLog logUpdate(true);
											logUpdate.SetTitle(CVSTR("NextGenASTUpdating"));
											logUpdate.Append("FileName", inWorkItem->fFilePath.GetPath());
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_NEXT_GEN_AST_UPDATING_TRACES
											sLONG sourceId = ownerFile->GetID();
											ESymbolFileExecContext executionContext = ownerFile->GetExecutionContext();
											
											UpdateSymbolScopeDeclarations(globalScopeDeclarations, 0, sourceId, executionContext);
											UpdateSymbolScopeVariableDeclarations(globalScopeDeclarations, 0, sourceId, executionContext);
											UpdateSymbolScopeAssignmentDeclarations(globalScopeDeclarations, 0, sourceId, executionContext);
#if ACTIVATE_NEXT_GEN_AST_UPDATING_TRACES
#if ACTIVATE_JSLOG
											logUpdate.Print();
#endif //ACTIVATE_JSLOG
										...
										{
#if ACTIVATE_NEXT_GEN_AST_FINALIZING_TRACES
#if ACTIVATE_JSLOG
											JSLog logFinalize(true);
											logFinalize.SetTitle(CVSTR("NextGenASTFinalizing"));
											logFinalize.Append("FileName", inWorkItem->fFilePath.GetPath());
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_NEXT_GEN_AST_FINALIZING_TRACES
											FinalizeSymbolScopeDeclarationUpdate(ownerFile->GetID());
#if ACTIVATE_NEXT_GEN_AST_FINALIZING_TRACES
#if ACTIVATE_JSLOG
											logFinalize.Print();
#endif //ACTIVATE_JSLOG
										...
										// RELEASE MEMORY
										delete globalScopeDeclarations;
										globalScopeDeclarations = NULL;
									}
#if ACTIVATE_GLOBAL_PARSING_TRACES
#if ACTIVATE_JSLOG
									logGlobalParsing.Print();
#endif //ACTIVATE_JSLOG
#endif //ACTIVATE_GLOBAL_PARSING_TRACES
						...
						if (ownerFile)
						{
								...
								(inWorkItem->fParsingIsMandatory) )								// The file has been modified, not saved yet and we discard modifications
							{
								...
								if( _IsJavaScriptContent(inWorkItem) )
								{
									...
									// Notify parsing start
									fManager->GetParsingStartSignal().Trigger(inWorkItem->fFilePath);
									JavaScriptAST::Visitor* declParser = JavaScriptAST::Visitor::GetDeclarationParser( fUseNextGen, symTable, ownerFile, this );
									if( !IsDying() && declParser )
									{
										JavaScriptParser* parser = new JavaScriptParser(fUseNextGen);
										if( parser )
									...
									if( parsed )
									{
										...
										if( fileIds.size() > 0 )
										{
											...
											for(VIndex index=0; index<fileIds.size(); index++)
											{
												...
												JavaScriptAST::Visitor* tmpDeclParser = JavaScriptAST::Visitor::GetDeclarationParser( fUseNextGen, symTable, fileTemp, this );
												
												if( !IsDying() && tmpDeclParser )
												{
													JavaScriptParser* tmpParser = new JavaScriptParser(fUseNextGen);
													if( tmpParser )
													{
														// We will parse by gathering an AST.  If we manage to get a valid AST back,
														// then we can see if the user wants it declaration parsed.
														...
														bool dummyErrorflag;
														JavaScriptAST::Node* tmpProgram = tmpParser->GetAST( tmpFilePath, ownerFile->GetID(), &tmpFileContent, dummyErrorflag );
														if( tmpProgram )
														{
	...
	for (int i = 0; i <= 1; i++)
	{
		...
		for (std::vector<IEntityModelCatalogAttribute* >::iterator iter = inAttributes.begin(); iter != inAttributes.end(); ++iter)
		{
			...
			if ( type.GetLength() )
			{
				for (int count = 0; count < ENTITY_MODEL_ATTR_TYPE_COUNT; count++)
				{
					if( ENTITY_MODEL_ATTR_TYPES[count].type == type && ENTITY_MODEL_ATTR_TYPES[count].jsFile.GetLength() > 0 )
					{
...
VLanguageSyntaxComponent::VLanguageSyntaxComponent() : fJavaScriptSyntax( NULL ),
	fXMLSyntax( NULL ), fHTMLSyntax( NULL ), fCSSSyntax( NULL ), fSQLSyntax( NULL ), fPHPSyntax( NULL ), fUseNextGen( false )
{
	gLanguageSyntax = this;
	InitLocalizationManager();

	fJavaScriptSyntax = new VJavaScriptSyntax(fDisplayDocumentationSignal);
	fJavaScriptSyntax->UseNextGen(fUseNextGen);
...
{
	return new VDocumentParserManager(fUseNextGen);
}

ISymbolTable *VLanguageSyntaxComponent::CreateSymbolTable()
{
	return NewSymbolTable(fUseNextGen);
}
...
void VLanguageSyntaxComponent::UseNextGen( bool inUseNextGen )
{
	fUseNextGen = inUseNextGen;
	fJavaScriptSyntax->UseNextGen(inUseNextGen);
}