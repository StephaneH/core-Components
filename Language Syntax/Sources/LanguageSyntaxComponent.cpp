...
private:
	...
	void UpdateSymbolScopeDeclaration(const JavaScriptAST::SymbolScopeDeclaration* inDeclarations, const sLONG& inParentScopeId, const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
		{
			...
			{
				UpdateSymbolScopeDeclaration(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextClient);
				UpdateSymbolScopeDeclaration(inDeclarations, inParentScopeId, inSourceId, eSymbolFileExecContextServer);
			}
			else
			{
				...
				else
				{
					...
					if( inDeclarations->GetExtendFlag() == true )
					{
						...
						if( fCurrentWorkItem->fSymTable->GetPrototypeChain(inExecutionContext, scopeName, extendedName, updateParameters, prototypeChainId) == false )
							fCurrentWorkItem->fSymTable->SetPrototypeChain(inExecutionContext, inSourceId, scopeName, extendedName);
					}
				...
				{
					UpdateSymbolScopeDeclaration(inDeclarations->GetChildScopeAt(index), scopeId, inSourceId, inExecutionContext);
				}
	...
	void _GenerateSymbolDeclarationsForAttribute(const VString& inFullName, const VString& inTypeName, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		...
		for(VIndex index=0; index<count; ++index)
		{
			...
			if( index != count-1 )
			{
				...
				JavaScriptAST::SymbolScopeDeclaration* scope = new JavaScriptAST::SymbolScopeDeclaration(currentScope, token, token, eSymbolScopeTypeObject, 0, 0, 0, 0, constructorFlag, false, CVSTR(""));
	...
	void _GenerateSymbolDeclarationsForMethod(const VString& inFullName, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		...
		for(VIndex index=0; index<count; ++index)
		{
			...
			else
			{
				...
				bool constructorFlag = false;
				JavaScriptAST::SymbolScopeDeclaration* scope = new JavaScriptAST::SymbolScopeDeclaration(currentScope, token, token, eSymbolScopeTypeFunctionExpression, 0, 0, 0, 0, constructorFlag, false, CVSTR(""));
				currentScope->AddChildScope(scope);
				currentScope = scope;
			}
		}
								...
								if( _IsJavaScriptContent(inWorkItem) || _IsDataModelContent(inWorkItem) || (_IsHTMLContent(inWorkItem) && _IsWithinWaPage(inWorkItem)) )
								{
									...
									if( _IsHTMLContent(inWorkItem) && _IsWithinWaPage(inWorkItem) )
									{
										...
										if( widgetIdsAndTypes.size() > 0 || dataSourceIdsAndTypes.size() > 0 )
										{
											...
											{
												// We can access data sources using 'sources.' or 'WAF.sources.'
												JavaScriptAST::SymbolScopeDeclaration* sourcesScope = new JavaScriptAST::SymbolScopeDeclaration(wafScope, CVSTR("sources"), CVSTR("sources"), eSymbolScopeTypeObject, 0, 0, 0, 0, false, false, CVSTR(""));
												globalScopeDeclarations->AddChildScope(sourcesScope);
												...
												for(std::map<XBOX::VString, XBOX::VString>::const_iterator it=dataSourceIdsAndTypes.begin(); it!=dataSourceIdsAndTypes.end(); ++it)
												{
													...
													JavaScriptAST::SymbolScopeDeclaration* source = new JavaScriptAST::SymbolScopeDeclaration(sourcesScope, sourceName, sourceName, eSymbolScopeTypeObject, 0, 0, 0, 0, false, true, CVSTR(""));
													sourcesScope->AddChildScope(source);
									...
									if( globalScopeDeclarations != NULL )
									{
										...
										
										{
											...
											UpdateSymbolScopeDeclaration(globalScopeDeclarations, 0, ownerFile->GetID(), ownerFile->GetExecutionContext());
#if ACTIVATE_NEXT_GEN_AST_UPDATING_TRACES
#if ACTIVATE_JSLOG
											logUpdate.Print();
#endif //ACTIVATE_JSLOG
