...
private:
	...
	bool _IsWithinWaPage(const VDocumentParserManager::WorkItem* inWorkItem) const
	{
		...
		if( inWorkItem->fFilePath.GetPath().Find(".waPage") )
			ret = true;
	...
	{
		return inValue.GetProperty("document");
	}
	...
	VJSONValue _GetHtml(const VJSONValue& inValue) const
	{
		...
		if( array )
		{
			...
			for(int index=0; index<count; index++)
			{
				...
				VJSONValue nodeName = current.GetProperty("nodeName");
	...
	{
		VJSONValue htmlChildren = inValue.GetProperty("childNodes");
	...
	{
		return inValue.GetProperty("childNodes");
	}
	...
	bool _IsMeta(const VJSONValue& inValue) const
	{
		...
		VJSONValue nodeName = inValue.GetProperty("nodeName");
	...
	{
		VJSONValue nodeName = inValue.GetProperty("attributes");
		return !nodeName.IsUndefined();
	}

	VJSONValue _GetAttributes(const VJSONValue& inValue) const
	{
		return inValue.GetProperty("attributes");
	}
	...
	bool _IsDataSourceAttribute(const VJSONValue& inValue) const
	{
		...
		VJSONValue dataType = inValue.GetProperty("data-type");
	...
	bool _IsWidgetAttribute(const VJSONValue& inValue) const
	{
		...
		VJSONValue dataType = inValue.GetProperty("data-type");
		VJSONValue id		= inValue.GetProperty("id");
	...
	{
		VJSONValue dataSourceId = inValue.GetProperty("data-id");
		dataSourceId.GetString(outId);
	}

	void _GetDataSourcesIds(const VJSONValue& inValue, std::vector<XBOX::VString>& outIds) const
	{
		VJSONArray* array = inValue.GetArray();
		if( array )
		{
	...
	{
		VJSONValue widgetId = inValue.GetProperty("id");
		widgetId.GetString(outId);
	}
	
	void _GetWidgetsIds(const VJSONValue& inValue, std::vector<XBOX::VString>& outIds) const
	{
		VJSONArray* array = inValue.GetArray();
		if( array )
		{
			...
			for(int index=0; index<count; index++)
			{
				...
				_GetWidgetsIds(subWidgets, outIds);
				// END WAK0082297
			}
		}
	}

	void _GetDataSourceAndWidgetIds(const VDocumentParserManager::WorkItem* inWorkItem, std::vector<XBOX::VString>& outDataSourceIds, std::vector<XBOX::VString>& outWidgetIds) const
	{
		bool go = false;
		...
		if( go )
		{
			...
			_GetDataSourcesIds(dataSources, outDataSourceIds);
			
			VJSONValue widgets		=	_GetChildren(body);
			_GetWidgetsIds(widgets, outWidgetIds);
		}
	}
	...
	Symbols::ISymbol* _RetainWafSymbol(const VDocumentParserManager::WorkItem* inWorkItem, Symbols::IFile* inOwnerFile) const
	{
		...
		std::vector<Symbols::ISymbol*> syms = inWorkItem->fSymTable->RetainSymbolsByName(NULL, "WAF", inOwnerFile);
		if( syms.size() == 1 )
		{
		...
		symsToAdd.push_back( waf  );
		if( symsToAdd.size() > 0 )
			inWorkItem->fSymTable->AddSymbols(symsToAdd);

		return waf;
	}
	
	Symbols::ISymbol* _RetainWidgetPrototypeSymbol(const VDocumentParserManager::WorkItem* inWorkItem) const
	{
		...
		widgets = inWorkItem->fSymTable->RetainSymbolsByName(NULL, "Widget", NULL);
		if( widgets.size() > 0 )
		{
			for(int index=0; index<widgets.size(); index++)
			{
				...
				{
					prototypes = inWorkItem->fSymTable->RetainSymbolsByName(widgets[index], "prototype", NULL);
	...
	void _CreateDataSourcePropertiesSymbols(const VDocumentParserManager::WorkItem* inWorkItem, Symbols::ISymbol* inOwner, Symbols::IFile* inOwnerFile, const std::vector<XBOX::VString>& inIds, std::vector<Symbols::ISymbol*>& ioSymsToAdd) const
	{
		// Create each data source symbol associated to the parent source
		for(int index=0; index<inIds.size(); index++)
		{
			VSymbol* sym = new VSymbol();
			sym->SetID( inWorkItem->fSymTable->GetNextUniqueID() );
			sym->SetName( inIds[index] );
	...
	void _CreateDataSourceSymbols(const VDocumentParserManager::WorkItem* inWorkItem, Symbols::ISymbol* inWafSym, Symbols::IFile* inOwnerFile, const std::vector<XBOX::VString>& inIds) const
	{
		std::vector<Symbols::ISymbol*> symsToAdd;

		// WAF.sources
		Symbols::ISymbol* wafSourcesSymbol = _CreateDataSourceSymbol(inWorkItem, "sources", Symbols::ISymbol::kKindPublicProperty, inWafSym, inOwnerFile);
		if( wafSourcesSymbol )
		{
			symsToAdd.push_back( wafSourcesSymbol  );
			_CreateDataSourcePropertiesSymbols(inWorkItem, wafSourcesSymbol, inOwnerFile, inIds, symsToAdd);
		}

		// We also need to create two aliases symbols (sources and source)
		// sources
		Symbols::ISymbol* sourcesSymbol = _CreateDataSourceSymbol(inWorkItem, "sources", Symbols::ISymbol::kKindLocalVariableDeclaration, NULL, inOwnerFile);
		if( sourcesSymbol )
		{
			symsToAdd.push_back( sourcesSymbol  );
			_CreateDataSourcePropertiesSymbols(inWorkItem, sourcesSymbol, inOwnerFile, inIds, symsToAdd);
		}
	...
	void _CreateWidgetSymbols(const VDocumentParserManager::WorkItem* inWorkItem, Symbols::ISymbol* inWafSym, Symbols::IFile* inOwnerFile, const std::vector<XBOX::VString>& inIds) const
	{
		std::vector<Symbols::ISymbol*> symsToAdd;
					...
					if (ownerFile)
					{
							 ...
							 (inWorkItem->fParsingIsMandatory) )								// The file has been modified, not saved yet and we discard modifications
						{
							...
							else if( _IsHTMLContent(inWorkItem) && _IsWithinWaPage(inWorkItem) )
							{
								...
								std::vector<XBOX::VString> dataSourceIds, widgetIds;

								_GetDataSourceAndWidgetIds(inWorkItem, dataSourceIds, widgetIds);
								...
								if( wafSym )
								{
									if( dataSourceIds.size() > 0 )
										_CreateDataSourceSymbols(inWorkItem, wafSym, ownerFile, dataSourceIds);
									
									if( widgetIds.size() > 0 )
										_CreateWidgetSymbols(inWorkItem, wafSym, ownerFile, widgetIds);
								}
								
								// Release memory !!!
								ReleaseRefCountable(&wafSym);
...
void WorkerThread::CreateNewInstance( ISymbolTable *inTable, Symbols::ISymbol *inSym, const VString &inSymbolName, const VString &inFileName )
{
	...
	if (!syms.empty()) {
		...
		std::vector< Symbols::ISymbol * > prototypes = inTable->RetainSymbolsByName( temp, "prototype" );
