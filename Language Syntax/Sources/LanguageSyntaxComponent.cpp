...
private:
	...
	void ExecuteTask();
	bool ParseDocument( VDocumentParserManager::WorkItem *inWorkItem, bool& outHasParsingError );
...
bool WorkerThread::ParseDocument( VDocumentParserManager::WorkItem *inWorkItem, bool& outHasParsingError )
{
	...
	do {
		...
		if (fCurrentWorkItem != NULL)
		{
			// Now that we have a work item, it's time to start processing it
			{
				...
				if( !ParseDocument(fCurrentWorkItem, hasParsingErrors) )
				{
#if VERSIONDEBUG
