...
private:
	...
	// Parsing cache
	VCriticalSection fParsingCacheMutex;
	std::map< XBOX::VFilePath, JavaScriptAST::SymbolScopeDeclaration* > fParsingCache;
	
	JavaScriptAST::SymbolScopeDeclaration* _GetSymbolScopeDeclarationFromCache(const XBOX::VFilePath& inPath)
	{
		VTaskLock lock(&fParsingCacheMutex);
		
		JavaScriptAST::SymbolScopeDeclaration* declarations = NULL;
		
		std::map< XBOX::VFilePath, JavaScriptAST::SymbolScopeDeclaration* >::const_iterator it = fParsingCache.find(inPath);
		if( it != fParsingCache.end() )
			declarations = it->second;
		
		return declarations;
	}
	
	void _SetSymbolScopeDeclarationToCache(const XBOX::VFilePath& inPath, JavaScriptAST::SymbolScopeDeclaration* inDeclarations)
	{
		VTaskLock lock(&fParsingCacheMutex);

		std::map< XBOX::VFilePath, JavaScriptAST::SymbolScopeDeclaration* >::iterator it = fParsingCache.find(inPath);
		if( it == fParsingCache.end() )
		{
			fParsingCache[inPath] = inDeclarations;
		}
		else
		{
			// Delete current and add the new
			delete it->second;
			it->second = inDeclarations;
		}
	}
#endif
...
private:
...
#if ACTIVATE_NEXT_JS_AST_WALKER
	void PrepareSymbolScopeDeclarationUpdate(const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
			fCurrentWorkItem->fSymTable->PrepareScopeUpdates(inSourceId, inExecutionContext);
	}
	
	void FinalizeSymbolScopeDeclarationUpdate(const sLONG& inSourceId, const ESymbolFileExecContext& inExecutionContext)
	{
		if( fCurrentWorkItem->fSymTable != NULL )
			fCurrentWorkItem->fSymTable->FinalizeScopeUpdates(inSourceId, inExecutionContext);
	}
		...
		if( fCurrentWorkItem->fSymTable != NULL )
		{
			...
			else
			{
				...
				if( scopeType == eSymbolScopeTypeGlobal )
				{
					...
					if( fCurrentWorkItem->fSymTable->GetGlobalScope(scopeName, scopeType, inExecutionContext, scopeId) == false )
					{
						// Create the scope symbols if can't get it
						fCurrentWorkItem->fSymTable->SetGlobalScope(scopeName, scopeType, inExecutionContext, scopeId);
					}
					else
					{
						fCurrentWorkItem->fSymTable->SetScopeStatus(scopeId, eSymbolScopeStatusUpToDate);
					}
				}
				else
				{
					// We can have only one global symbol per execution context, without regarding the source id
					if( fCurrentWorkItem->fSymTable->GetScope(scopeName, eSymbolScopeTypeAll, inParentScopeId, inSourceId, inExecutionContext, scopeId) == false )
					{
						// Create the scope symbols if can't get it
						fCurrentWorkItem->fSymTable->SetScope(scopeName, scopeType, inParentScopeId, inSourceId, inExecutionContext, fromLine, fromOffset, toLine, toOffset, scopeId);
						
						sLONG nameId;
						if( fCurrentWorkItem->fSymTable->GetName(scopeName, inParentScopeId, inExecutionContext, nameId) == false )
						{
							fCurrentWorkItem->fSymTable->SetName(scopeName,	inParentScopeId, inExecutionContext, nameId);
							
							bool instanceFlag = false;
							bool localVariableFlag = false;
							if( scopeType == eSymbolScopeTypeUndefined )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindUndefined, eSymbolDefinitionTypeUndefined, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
							else if( scopeType == eSymbolScopeTypeFunctionDeclaration )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindFunctionDeclaration, eSymbolDefinitionTypeFunction, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
							else if( scopeType == eSymbolScopeTypeFunctionExpression )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindFunctionExpression, eSymbolDefinitionTypeFunction, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
							else if( scopeType == eSymbolScopeTypeCatch )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindCatch, eSymbolDefinitionTypeFunction, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
							else if( scopeType == eSymbolScopeTypeWith )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindWith, eSymbolDefinitionTypeFunction, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
							else if( scopeType == eSymbolScopeTypeObject )
							{
								fCurrentWorkItem->fSymTable->SetDefinition(inParentScopeId, nameId, inExecutionContext, eSymbolDefinitionKindObject, eSymbolDefinitionTypeObject, eSymbolDefinitionVisibilityPublic, scopeConstructorFlag, instanceFlag, localVariableFlag);
							}
						}
					}
					else
					{
						fCurrentWorkItem->fSymTable->SetScopeStatus(scopeId, eSymbolScopeStatusUpToDate);
						fCurrentWorkItem->fSymTable->SetScopeBondaries(scopeId, fromLine, fromOffset, toLine, toOffset);
					}
				}
...
#if ACTIVATE_NEXT_JS_AST_WALKER
	void _GetName(const VJSONValue& inValue, VString& outName) const
	{
		VJSONValue name = inValue.GetProperty("name");
	...
	bool _IsCalculatedAttribute(const VJSONValue& inValue) const
	{
		bool isCalculated = false;
		
		VString calculatedValue;
		inValue.GetProperty(CVSTR("kind")).GetString(calculatedValue);
		
		if( calculatedValue == CVSTR("calculated") )
			isCalculated = true;
		
		return isCalculated;
	}
	
	void _GenerateSymbolDeclarationsFromDataClassCalculatedAttribute(const VString& inModelName, const VString& inDataClassName, const VJSONValue& inValue, const JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		std::vector<VString> possibleCalculatedMethodNames;
		possibleCalculatedMethodNames.push_back("onGet");
		possibleCalculatedMethodNames.push_back("onSet");
		possibleCalculatedMethodNames.push_back("onQuery");
		possibleCalculatedMethodNames.push_back("onSort");
		
		VIndex count = possibleCalculatedMethodNames.size();
		for(VIndex index=0; index<count; ++index)
		{
			VJSONArray* calculatedMethodArray = inValue.GetProperty(possibleCalculatedMethodNames[index]).GetArray();
			if( calculatedMethodArray != NULL )
			{
				VString attributeName;
				_GetName(inValue, attributeName);

				VString attributeFullName;
				attributeFullName.AppendString(inModelName);
				attributeFullName.AppendString(CVSTR("."));
				attributeFullName.AppendString(inDataClassName);
				attributeFullName.AppendString(CVSTR("."));
				attributeFullName.AppendString(attributeName);
				attributeFullName.AppendString(CVSTR("."));
				attributeFullName.AppendString(possibleCalculatedMethodNames[index]);

#if ACTIVATE_JSLOG
				JSLog log;
				log.SetTitle(CVSTR("ParsingWaModel"));
				log.Append(CVSTR("AttributeCalculated"), attributeFullName);
#endif
			}
		}
	}

	void _GenerateSymbolDeclarationsFromDataClassStorageAttribute(const VString& inModelName, const VString& inDataClassName, const VJSONValue& inValue, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VString attributeName;
		_GetName(inValue, attributeName);
		...
		JSLog log;
		log.SetTitle(CVSTR("ParsingWaModel"));
		log.Append(CVSTR("AttributeStorage"), attributeFullName);
#endif
		
		VectorOfVString attributeTokens;
		attributeFullName.GetSubStrings(CVSTR("."), attributeTokens);
		...
		for(VIndex index=0; index<count; ++index)
		{
			...
			if( index != count-1 )
			{
				...
				JavaScriptAST::SymbolScopeDeclaration* scope = new JavaScriptAST::SymbolScopeDeclaration(currentScope, token, eSymbolScopeTypeUndefined, 0, 0, 0, 0, constructorFlag);
			...
			else
			{
				...
				declaration.SetRhs(new JavaScriptAST::StringLiteralDeclaration());
				declaration.SetInstanceFlag(true);
	...
	void _GenerataSymbolDeclarationsFromDataClassAttributes(const VString& inModelName, const VString& inDataClassName, const VJSONValue& inValue, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VJSONArray* attributesArray = inValue.GetProperty(CVSTR("attributes")).GetArray();
		if( attributesArray != NULL )
		{
			VIndex count = attributesArray->GetCount();
			for(VIndex index=0; index<count; ++index)
			{
				VJSONValue attribute = (*attributesArray)[index];
				if( _IsCalculatedAttribute(attribute) == false )
					_GenerateSymbolDeclarationsFromDataClassStorageAttribute(inModelName, inDataClassName, attribute, ioGlobalScope);
//				else
//					_GenerateSymbolDeclarationsFromDataClassCalculatedAttribute(inModelName, inDataClassName, attribute, ioGlobalScope);
			}
		}
	}
	
	void _GenerateSymbolDeclarationsFromDataClassMethod(const VString& inModelName, const VString& inDataClassName, const VJSONValue& inValue, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VString methodName;
		_GetFrom(inValue, methodName);
		
		VString attributeFullName;
		attributeFullName.AppendString(methodName);
		
#if ACTIVATE_JSLOG
		JSLog log;
		log.SetTitle(CVSTR("ParsingWaModel"));
		log.Append(CVSTR("Method"), attributeFullName);
#endif
	}

	void _GenerataSymbolDeclarationsFromDataClassMethods(const VString& inModelName, const VString& inDataClassName, const VJSONValue& inValue, JavaScriptAST::SymbolScopeDeclaration* ioGlobalScope) const
	{
		VJSONArray* methodsArray = inValue.GetProperty(CVSTR("methods")).GetArray();
		if( methodsArray != NULL )
		{
			VIndex count = methodsArray->GetCount();
			for(VIndex index=0; index<count; ++index)
			{
				VJSONValue method = (*methodsArray)[index];
				_GenerateSymbolDeclarationsFromDataClassMethod(inModelName, inDataClassName, method, ioGlobalScope);
			}
		}
	}
				...
				if( inWorkItem->fSymTable && !IsDying() )
				{
					...
					if (ownerFile)
					{
							...
							(inWorkItem->fParsingIsMandatory) )								// The file has been modified, not saved yet and we discard modifications
						{
							...
							if( _IsJavaScriptContent(inWorkItem) || _IsDataModelContent(inWorkItem) )
							{
								// Notify parsing start
								fManager->GetParsingStartSignal().Trigger(inWorkItem->fFilePath);

								JavaScriptAST::SymbolScopeDeclaration* globalScopeDeclarations = new JavaScriptAST::SymbolScopeDeclaration(NULL, CVSTR("global"), eSymbolScopeTypeGlobal, 0, 0, 0, 0, false);
								
								if( _IsJavaScriptContent(inWorkItem) )
								{
									JavaScriptAST::Visitor* declParser = JavaScriptAST::Visitor::GetDeclarationParser( NULL, NULL, NULL, globalScopeDeclarations );
								...
								{
									VString json = inWorkItem->fFileContents;
									VJSONImporter import(json);
									...
									if( root.IsObject() )
									{
										VJSONValue dataClasses = root.GetProperty(CVSTR("dataClasses"));
										VJSONArray* dataClassesArray = dataClasses.GetArray();
										if( dataClassesArray != NULL )
										{
											VIndex dataClassesCount = dataClassesArray->GetCount();
											for(VIndex dataClassesIndex=0; dataClassesIndex<dataClassesCount; ++dataClassesIndex)
											{
												VJSONValue dataClass = (*dataClassesArray)[dataClassesIndex];
												
												VString dataClassName;
												_GetName(dataClass, dataClassName);
												
												// Attributes
												_GenerataSymbolDeclarationsFromDataClassAttributes(modelName, dataClassName, dataClass, globalScopeDeclarations);
												
												// Methods
//												_GenerataSymbolDeclarationsFromDataClassMethods(modelName, dataClassName, dataClass, globalScopeDeclarations);
											}
										}
								...
								{
									JavaScriptAST::SymbolScopeDeclaration* latest = fManager->_GetSymbolScopeDeclarationFromCache(inWorkItem->fFilePath);
									if( latest != NULL )
									{
#if ACTIVATE_JSLOG
										JSLog compareLog(true);
										compareLog.SetTitle(CVSTR("SymbolScopeDeclarationsComparison"));
#endif
										if( JavaScriptAST::SymbolScopeDeclaration::Compare(globalScopeDeclarations, latest) == false )
										{
#if ACTIVATE_JSLOG
											compareLog.Append(CVSTR("Equals"), CVSTR("false"));
#endif
											fManager->_SetSymbolScopeDeclarationToCache(inWorkItem->fFilePath, globalScopeDeclarations);
											
											PrepareSymbolScopeDeclarationUpdate(ownerFile->GetID(), ownerFile->GetExecutionContext());
											UpdateSymbolScopeDeclaration(globalScopeDeclarations, 0, ownerFile->GetID(), ownerFile->GetExecutionContext());
											FinalizeSymbolScopeDeclarationUpdate(ownerFile->GetID(), ownerFile->GetExecutionContext());
										}
										else
										{
#if ACTIVATE_JSLOG
											compareLog.Append(CVSTR("Equals"), CVSTR("true"));
#endif
											// RELEASE MEMORY
