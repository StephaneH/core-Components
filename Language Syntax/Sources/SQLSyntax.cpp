...
#define SQL_KWORD_TRIGGERS 454
#define SQL_KWORD_TRUE 455
#define SQL_KWORD_TS 456
#define SQL_KW_UNIQUE 457
#define UNLOCK 458
#define UPDATE 459
#define SQL_KWORD_USE 460
#define SQL_KWORD_UTF16 461
#define SQL_KWORD_UTF8 462
#define VALUES 463
#define VIEW 464
#define WHEN 465
#define WHERE 466
#define SQL_KWORD_WITH 467
#define ALPHA_NUMERIC 468
#define TEXT 469
#define TIME 470
#define DURATION 471
#define SQL_KWORD_BOOLEAN 472
#define SQL4D_BYTE 473
#define SQL_KWORD_INT16 474
#define SQL_KWORD_INT32 475
#define SQL_KWORD_INT64 476
#define SQL_KWORD_REAL 477
#define SQL_KWORD_FLOAT 478
#define SQL_KWORD_UUID 479
#define SQL_KWORD_BLOB 480
#define PICTURE 481
#define VARCHAR 482
#define SQL_KWORD_INT 483
#define SMALLINT 484
#define BIT 485
#define VARYING 486
#define NUMERIC 487
#define SQL_KWORD_DOUBLE 488
#define PRECISION 489
#define SQL_KWORD_DATE 490
#define TIMESTAMP 491
#define INTERVAL 492
#define CLOB 493
#define DEBUG 494
#define THREADING 495
#define MODE 496
#define DIRECT 497
#define SYNC 498
#define ASYNC 499
...
#define COMMA 501
#define PERIOD 502
#define LEFT_PARENTHESIS 503
#define RIGHT_PARENTHESIS 504
#define SEMICOLON 505
#define LEFT_CURLY_BRACKET 506
#define RIGHT_CURLY_BRACKET 507
#define DOUBLE_LESS_THAN 508
#define DOUBLE_GREATER_THAN 509
#define QUESTION_MARK 510
#define COLON 511
#define DOLLAR_SIGN 512
...
bool VSQLSyntax::ParseAlterTableStatement( SuggestionList &outSuggestions )
{
	...
	if (!token) {
		...
		outSuggestions.Suggest( "DISABLE TRIGGERS", SuggestionInfo::eKeyword);
		...
		outSuggestions.Suggest( "ENABLE TRIGGERS", SuggestionInfo::eKeyword);
	...
	switch (token->GetValue()) {
		...
		case SQL_KWORD_DISABLE: {
			...
			if (!token) {
				...
				outSuggestions.Suggest( "TRIGGERS", SuggestionInfo::eKeyword);
				return true;
			} else if (token->GetValue() != SQL_KWORD_STAMP &&
						token->GetValue() != SQL_KWORD_REPLICATE &&
						token->GetValue() != SQL_KWORD_TRIGGERS) return false;
		} break;
		case SQL_KWORD_STAMP:
		case SQL_KWORD_REPLICATE:
		case SQL_KWORD_TRIGGERS: {
		...
		switch (token->GetValue()) {
			...
			case ALTER: {
				...
				if (!token) {
					...
					ret = (token->GetValue() == SQL_KWORD_INDEXES || token->GetValue() == SQL_KWORD_CONSTRAINTS || token->GetValue() == SQL_KWORD_TRIGGERS);
...
void VSQLSyntax::SetLine( ICodeEditorDocument* inDocument, sLONG inLineNumber, bool inLoading )
{
	...
	for (vector< ILexerToken *>::iterator iter = tokens.begin(); iter != tokens.end(); iter++)
	{
		...
		switch (current->GetType())
		{
			...
			case ILexerToken::TT_SQL_KEYWORD:
				style = eKeyword;
				break;
				
			case ILexerToken::TT_SQL_COLUMN_FUNCTION:
			case ILexerToken::TT_SQL_SCALAR_FUNCTION:
				style = eFunctionKeyword;
				break;
			...
			case ILexerToken::TT_NAME:
			{
				...
				if (fSymTable)
					sym = fSymTable->GetSymbolByName( current->GetText() );
		...
		inDocument->SetLineStyle( inLineNumber, current->GetPosition(), current->GetPosition() + current->GetLength(), style );
	}
...
void VSQLSyntax::GetSuggestions( ICodeEditorDocument* inDocument, sLONG inLineIndex, sLONG inPos, ITipInfoArray *outSuggestions, sLONG& outStartOffset, bool inAll )
{
	...
	xstr.AppendUniChar( VTask::GetWildChar() );
