...
bool ScriptDocLexer::ConsumeComment( VString &outComment )
{
	...
	while (true)
	{
		...
		if (IsLineEnding( ch ))
		{
			...
			TokenList spaces;
			if (!SkipWhitespaces( toss, &spaces ))
				return false;
	...
	if (nBufferSize > 0)
		outComment.AppendUniChars( szUCharBuffer, nBufferSize );
...
bool ScriptDocLexer::AdvanceOneToken( int &outToken )
{
	...
	if (!SkipWhitespaces( consumedWhitespace ))
		return false;
	...
	else
	{
		...
		// lines.
		if (fFoundComment || fFoundTags)
			return false;
		
		fFoundComment = true;
		VString text;
		if (!ConsumeComment( text ))
			return false;
