...
public:
	 ...
	 */
	virtual bool GetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, ISymbolUpdateParameters* inUpdateParameters, sLONG& outReferenceId) const;
	virtual bool GetReferenceFromScopeId(const sLONG& inFromScopeId, sLONG& outToScopeId) const;

	virtual void SetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, const sLONG& inSourceId);
...
private:
	...
	public:
		 ...
		 *	SYMBOL'S REFERENCES MANAGEMENT
		 */
		virtual bool GetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, ISymbolUpdateParameters* inUpdateParameters, sLONG& outReferenceId, CDB4DBaseContext* inContext) const;
		virtual bool GetReferenceFromScopeId(const sLONG& inFromScopeId, sLONG& outToScopeId, CDB4DBaseContext* inContext) const;

		virtual void SetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, const sLONG& inSourceId, CDB4DBaseContext* inContext);
	...
	private:
		...
		CDB4DField* fSymbolReferenceFieldFromScopeId;
		CDB4DField*	fSymbolReferenceFieldToScopeId;
...
bool VSymbolTable::GetNameFromScope(const XBOX::VString& inSymbolName, const sLONG& inFromScopeId, const ESymbolFileExecContext& inExecutionContext, const sLONG& inSourceId, sLONG& outNameId) const
{
	...
	{
		bool stop = false;
		if( inFromScopeId == 295 && inSymbolName == "event" )
		{
			stop = true;
		}
...
void VSymbolTable::SetName(const XBOX::VString& inSymbolName, const sLONG& inScopeId, const ESymbolFileExecContext& inExecutionContext, const sLONG& inSourceId, sLONG& outSymbolNameId)
{
	...
	{
		bool stop = false;
		if( inScopeId == 295 && inSymbolName == "event" )
		{
			stop = true;
		}
#if ACTIVATE_SYMBOL_TABLE_LIVE_REQUEST
 ...
 *	SYMBOL'S REFERENCES MANAGEMENT
 */
bool VSymbolTable::GetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, ISymbolUpdateParameters* inUpdateParameters, sLONG& outReferenceId) const
{
	bool result = false;
	...
	if( fTable != NULL && fContext != NULL )
	{
		result = fTable->GetReference(inFromScopeId, inToScopeId, inUpdateParameters, outReferenceId, fContext);
...
bool VSymbolTable::GetReferenceFromScopeId(const sLONG& inFromScopeId, sLONG& outToScopeId) const
{
	bool result = false;
	...
	if( fTable != NULL && fContext != NULL )
	{
		result = fTable->GetReferenceFromScopeId(inFromScopeId, outToScopeId, fContext);
		
#if ACTIVATE_SYMBOL_TABLE_LIVE_REQUEST
		VString headerTrace("GetReferenceFromScopeId : ");
		headerTrace.AppendString("inFromScopeId=");
		headerTrace.AppendLong(inFromScopeId);
		headerTrace.AppendString(", outToScopeId=");
		headerTrace.AppendLong(outToScopeId);
...
void VSymbolTable::SetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, const sLONG& inSourceId)
{
	xbox_assert( fTable != NULL && fContext != NULL );
	if( fTable != NULL && fContext != NULL )
	{
		...
		fTable->SetReference(inFromScopeId, inToScopeId, inSourceId, fContext);
...
const char* kSymbolReferenceFieldFromScopeId	=	"FromScopeId";	// Int32, Non-Null
const char*	kSymbolReferenceFieldToScopeId		=	"ToScopeId";	// Int32, Non-Null
 ...
 *	SYMBOL'S REFERENCES MANAGEMENT
 */
bool VSymbolTable::InnerSymbolTable::GetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, ISymbolUpdateParameters* inUpdateParameters, sLONG& outReferenceId, CDB4DBaseContext* inContext) const
{
	bool found = false;
	
	xbox_assert( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL && fSymbolReferenceFieldId != NULL );
	if( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL && fSymbolReferenceFieldId != NULL )
	{
		CDB4DQuery* query = fSymbolReferencesTable->NewQuery();
		if( query != NULL )
		{
			VLong fromScopeId(inFromScopeId);
			query->AddCriteria(fSymbolReferenceFieldFromScopeId, DB4D_Equal, fromScopeId);
			
			query->AddLogicalOperator(DB4D_And);
			VLong toScopeId(inToScopeId);
			query->AddCriteria(fSymbolReferenceFieldToScopeId, DB4D_Equal, toScopeId);
...
bool VSymbolTable::InnerSymbolTable::GetReferenceFromScopeId(const sLONG& inFromScopeId, sLONG& outToScopeId, CDB4DBaseContext* inContext) const
{
	bool found = false;
	
	xbox_assert( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL );
	if( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL )
	{
		CDB4DQuery* query = fSymbolReferencesTable->NewQuery();
		if( query != NULL )
		{
			...
			if( results != NULL )
			{
				...
				if( total == 1 )
				{
					...
					if( record != NULL )
					{
						outToScopeId = GetIntegerField(record, fSymbolReferenceFieldToScopeId, NULL);
...
void VSymbolTable::InnerSymbolTable::SetReference(const sLONG& inFromScopeId, const sLONG& inToScopeId, const sLONG& inSourceId, CDB4DBaseContext* inContext)
{
	xbox_assert( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL && fSymbolReferenceFieldSourceId != NULL && fSymbolReferenceFieldStatus != NULL );
	if( inContext != NULL && fSymbolReferencesTable != NULL && fSymbolReferenceFieldFromScopeId != NULL && fSymbolReferenceFieldToScopeId != NULL && fSymbolReferenceFieldSourceId != NULL && fSymbolReferenceFieldStatus != NULL )
	{
		CDB4DRecord* record = fSymbolReferencesTable->NewRecord(inContext);
		if( record != NULL )
		{
			// Set field values
			record->SetLong(	fSymbolReferenceFieldFromScopeId,	inFromScopeId);
			record->SetLong(	fSymbolReferenceFieldToScopeId,		inToScopeId);
...
fSymbolReferenceFieldFromScopeId(NULL),
fSymbolReferenceFieldToScopeId(NULL),
...
bool VSymbolTable::InnerSymbolTable::InitializeNewDatabase( CDB4DBaseContext *inContext )
{
	...
	if( fUseNextGen == true )
	{
		...
		status &= (VE_OK == fSymbolReferencesTable->AddField( kSymbolReferenceFieldFromScopeId,		DB4D_Integer32,	-1, DB4D_Not_Null,							NULL, inContext ));
		status &= (VE_OK == fSymbolReferencesTable->AddField( kSymbolReferenceFieldToScopeId,		DB4D_Integer32,	-1, DB4D_Not_Null,							NULL, inContext ));
...
void VSymbolTable::InnerSymbolTable::RetainFields( CDB4DBaseContext *inContext )
{
	if( fUseNextGen == true )
	{
		...
		fSymbolReferenceFieldFromScopeId	=	fSymbolReferencesTable->FindAndRetainField(	kSymbolReferenceFieldFromScopeId,	inContext);
		fSymbolReferenceFieldToScopeId		=	fSymbolReferencesTable->FindAndRetainField(	kSymbolReferenceFieldToScopeId,		inContext);
...
void VSymbolTable::InnerSymbolTable::ReleaseFields()
{
	if( fUseNextGen == true )
	{
		...
		ReleaseRefCountable(&fSymbolReferenceFieldFromScopeId);
		ReleaseRefCountable(&fSymbolReferenceFieldToScopeId);
	...
	for(VectorOfVString::const_iterator it=tokens.begin(); it!=tokens.end(); ++it)
	{
		...
		switch(expectedTokenType)
		{
			...
			case 1:	// FIELD NAME
			{
				...
				{
					if( tok==CVSTR("id") || tok==CVSTR("fromscopeid") || tok==CVSTR("toscopeid") || tok==CVSTR("status") || tok==CVSTR("sourceid") )
					{
						fieldName = tok;
						expectedTokenType = 2;
					}
					else
					{
						JSLog error;
						error.SetTitle("Symbol table live requesting");
						error.Append("Error", "Unknown field name for table references (allowed : id, fromscopeid, toscopeid, status, sourceid)");
						return;
					}
				}
			...
			case 3:	// FIELD VALUE
			{
				...
				{
					if( valueString.EndsWith(CVSTR("*")) == true )
					{
						// Remove last char
				...
				if( query )
				{
					...
					else if( tableName==CVSTR("references") )
					{
						...
						else if( fieldName==CVSTR("fromscopeid") )					(valueNULLSet==true) ? query->AddCriteria(fSymbolReferenceFieldFromScopeId, comparator, VLong(0)) : query->AddCriteria(fSymbolReferenceFieldFromScopeId, comparator, VLong(valueLong));
						else if( fieldName==CVSTR("toscopeid") )					(valueNULLSet==true) ? query->AddCriteria(fSymbolReferenceFieldToScopeId, comparator, VLong(0)) : query->AddCriteria(fSymbolReferenceFieldToScopeId, comparator, VLong(valueLong));
		...
		for( RecIDType index=1; index<=total; index++)
		{
			...
			if( record )
			{
				...
				else if( tableName==CVSTR("references") )
				{
					...
					log.Append( CVSTR("FromScopeId"),	GetIntegerField(record, fSymbolReferenceFieldFromScopeId, NULL));
					log.Append( CVSTR("ToScopeId"),		GetIntegerField(record,	fSymbolReferenceFieldToScopeId, NULL));
				}
				else if( tableName==CVSTR("items") )
				{
					JSLog log;
