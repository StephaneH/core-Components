...
//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Update Permission Requested\n");
//#endif

			fAllowedToAddSymbolsEvent.Lock();

//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Updating Permitted\n");
//#endif
		}
	public:
...
//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Lock Requested\n");
//#endif

			fAllowedToAddSymbolsEvent.Reset();
			
//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Locked\n");
//#endif
		}
		
		virtual void UnlockUpdates() {
//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Unlock Requested\n");
//#endif

			fAllowedToAddSymbolsEvent.Unlock();

//#if VERSIONDEBUG
//			DebugMsg("[Symbol Table] Unlocked\n");
//#endif
		}
		// -----
...
#if VERSIONDEBUG
	virtual void DebugDumpSymbol(Symbols::ISymbol* inSymbol)
	{
		// The idea is to get through the prototype chain until the higest level and store each
		// prototype symbol in the vector that will be displayed in a reverse order.
		// It will help us to see and understand what is written in the symbol table
		std::vector<Symbols::ISymbol*> chain;
		
		// Let's add the current symbol
		chain.push_back(inSymbol);

		// We are going to iterate through the chain using its size
		// Be aware that the current chain size may be increased while looping
		for(int index=0; index<chain.size(); index++)
		{
			// Get the prototypes of the current symbol
			std::vector<Symbols::ISymbol*> prototypes = chain[index]->GetPrototypes();
			
			// Append the prototypes to the chain
			chain.insert(chain.end(), prototypes.begin(), prototypes.end());
		}
		
		// Dump !
		for(int index=chain.size()-1; index>=0; index--)
			(chain[index]->GetName() != "prototype") ? chain[index]->DebugDump() : (this->GetSymbolByID(chain[index]->GetOwner()->GetID()))->DebugDump();
	}
#endif
};
...
//	DebugMsg("[Symbol Table AddSymbols] Start\n");
//#endif
...
//	VString msg("[Symbol Table AddSymbols] Stop Count(");
//	msg.AppendLong(inSymbols.size());
...
//#if VERSIONDEBUG
//	uLONG tStart = XBOX::VSystem::GetCurrentTime();
//	DebugMsg("[Symbol Table DeleteFile] Start\n");
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStop = XBOX::VSystem::GetCurrentTime();
//	VString msg("[Symbol Table DeleteFile] Stop Duration(");
//	msg.AppendLong(tStop-tStart);
//	msg.AppendString("ms)\n");
//	DebugMsg(msg);
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStart = XBOX::VSystem::GetCurrentTime();
//	DebugMsg("[Symbol Table DeleteSymbolsForFile] Start\n");
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStop = XBOX::VSystem::GetCurrentTime();
//	VString msg("[Symbol Table DeleteSymbolsForFile] Stop Duration(");
//	msg.AppendLong(tStop-tStart);
//	msg.AppendString("ms)\n");
//	DebugMsg(msg);
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStart = XBOX::VSystem::GetCurrentTime();
//	DebugMsg("[Symbol Table DeleteSymbol] Start\n");
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStop = XBOX::VSystem::GetCurrentTime();
//	VString msg("[Symbol Table DeleteSymbol] Stop Duration(");
//	msg.AppendLong(tStop-tStart);
//	msg.AppendString("ms)\n");
//	DebugMsg(msg);
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStart = XBOX::VSystem::GetCurrentTime();
//	DebugMsg("[Symbol Table AddExtraInformation] Start\n");
//#endif
...
//#if VERSIONDEBUG
//	uLONG tStop = XBOX::VSystem::GetCurrentTime();
//	VString msg("[Symbol Table AddExtraInformation]s Stop Duration(");
//	msg.AppendLong(tStop-tStart);
//	msg.AppendString("ms)\n");
//	DebugMsg(msg);
//#endif
