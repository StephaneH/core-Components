...
private:
	...
	private:
		...
		void RetainSymbolsFromFullName(const VString& inFullName, std::vector<Symbols::ISymbol*>& outSymbols, CDB4DBaseContext* inContext);
			...
			bool	GetBooleanField( CDB4DRecord *inRecord, CDB4DField *inField );
...
bool VSymbolTable::InnerSymbolTable::GetBooleanField(CDB4DRecord* inRecord, CDB4DField* inField)
{
	Boolean temp;
...
//#if ACTIVATE_JSLOG
//	JSLog logSearchAndUpdateSymbol(true);
//	logSearchAndUpdateSymbol.SetTitle( CVSTR("SymbolTable::AddSymbols::SearchAndUpdateSymbols") );
//#endif

	// If we find an existing unique instance of a symbol to add, we update it instead of adding it
	// this way, we don't break any link pointing to that symbol
	//
	// If we don't find any instance of the symbol or if we find multiple instances of the symbol
	// then we add it
	//
	// Symbols that need to be added will be store in the symsToAdd vector
	std::vector<Symbols::ISymbol*> symsToAdd;
	
	// We loop again because we had to compute the full name first
	for(VIndex index=0; index<inSymbols.size(); index++)
	{
		Symbols::ISymbol* current = inSymbols[index];
		
		std::vector<Symbols::ISymbol*> founds;
		RetainSymbolsFromFullName(current->GetFullName(), founds, inContext);
		
		int count = founds.size();
		if( count == 1 )
		{
			// We found one and only one instance of this symbol, we can update it instead of creating it
			
			// Don't forget to reset the edition status of the symbol
			Symbols::ISymbol* symToUpdate = founds[0];
			symToUpdate->SetEditionState(false);
			UpdateSymbol(symToUpdate, inContext);
			
			// !!! Don't forget an important thing !!!
			// Instead of keeping the newest symbol, we keep its latest version with its latest id
			// We must convert the new id to the latest one in each symbols to add
		}
		else
		{
			// We found no instance or more than one instance, we must add the symbol
			symsToAdd.push_back(current);
		}
		
		ReleaseSymbols(founds);
	}

//#if ACTIVATE_JSLOG
//	logSearchAndUpdateSymbol.Append( CVSTR("Count"),		inSymbols.size() -  symsToAdd.size());
//	logSearchAndUpdateSymbol.Print();
//#endif
...
#if ACTIVATE_SMART_PARSING
	SymbolCollectionManager collection( symsToAdd, fSymbolsTable, inContext );
#else
...
#if ACTIVATE_SMART_PARSING
	if( inOnlyEditedSymbols )
	{
		VBoolean state(inOnlyEditedSymbols);
