...
public:
	...
	virtual void SetSymbolsEditionState(const Symbols::IFile* inFile, bool inEditionState);
	virtual void GetSymbolIds(const Symbols::IFile* inFile, bool inEditionState, std::vector<sLONG>& outIds);
...
private:
	...
	public:
		...
		virtual void SetSymbolsEditionState(const Symbols::IFile* inFile, bool inEditionState, CDB4DBaseContext* inContext);
		virtual void GetSymbolIds(const Symbols::IFile* inFile, bool inEditionState, std::vector<sLONG>& outIds, CDB4DBaseContext* inContext);
...
bool VSymbolTable::DeleteFile( Symbols::IFile *inFile )
{
	if (fTable)
	{
...
bool VSymbolTable::DeleteSymbolsForFile( Symbols::IFile *inFile )
{
	if (fTable)
	{
...
void VSymbolTable::SetSymbolsEditionState(const Symbols::IFile* inFile, bool inEditionState)
{
	if (fTable)
		fTable->SetSymbolsEditionState( inFile, inEditionState, fContext );
}




void VSymbolTable::GetSymbolIds(const Symbols::IFile* inFile, bool inEditionState, std::vector<sLONG>& outIds)
{
	if( fTable )
		fTable->GetSymbolIds(inFile, inEditionState, outIds, fContext);
}
...
bool VSymbolTable::InnerSymbolTable::InitializeNewDatabase( CDB4DBaseContext *inContext )
{
	...
	status &= (VE_OK == fSymbolsTable->AddField( kSymbolUndefinedState,	DB4D_Integer32, -1, 0,							NULL, inContext ));
	status &= (VE_OK == fSymbolsTable->AddField( kSymbolInstanceState,	DB4D_Integer32, -1, 0,							NULL, inContext ));
	status &= (VE_OK == fSymbolsTable->AddField( kSymbolReferenceState,	DB4D_Integer32, -1, 0,							NULL, inContext ));
	status &= (VE_OK == fSymbolsTable->AddField( kSymbolEditionState,	DB4D_Integer32, -1, 0,							NULL, inContext ));
	...
	status &= (VE_OK == fFilesTable->AddField( kFileResolvedState,		DB4D_Integer16,	-1,	DB4D_Not_Null,							NULL, inContext ));
	...
	status &= (VE_OK == fVersionTable->AddField( kVersionNumber, DB4D_Integer32, -1, DB4D_Not_Null | kUnique, NULL, inContext ));
...
Symbols::IFile *VSymbolTable::InnerSymbolTable::FileFromRecord( CDB4DTable *inTable, CDB4DRecord *inRecord, CDB4DBaseContext *inContext )
{
	...
	ret->SetResolveState( GetIntegerField(inRecord, fFileResolvedStateField, NULL) );
...
Symbols::ISymbol *VSymbolTable::InnerSymbolTable::SymbolFromRecord( CDB4DTable *inTable, CDB4DRecord *inRecord, CDB4DBaseContext *inContext, bool inFullyLoad )
{
	...
	ret->SetUndefinedState( GetIntegerField(inRecord, fSymbolUndefinedStateField, NULL) );
	ret->SetInstanceState( GetIntegerField(inRecord, fSymbolInstanceStateField, NULL) );
	ret->SetReferenceState(  GetIntegerField(inRecord, fSymbolReferenceStateField, NULL) );
	ret->SetEditionState( GetIntegerField(inRecord, fSymbolEditionStateField, NULL) );
...
bool VSymbolTable::InnerSymbolTable::AddSymbols( std::vector< Symbols::ISymbol * > inSymbols, CDB4DBaseContext *inContext )
{
	...
	log.Append( CVSTR("SymbolsCount"),		inSymbols.size() );
#endif
	...
	for(VIndex index=0; index<inSymbols.size(); index++)
	{
		...
		// Try to find the symbol in database using its FileId and 
...
//#if ACTIVATE_JSLOG
//	JSLog log(true);
//	log.SetTitle( CVSTR("SymbolTable::DeleteFile") );
//	log.Append( CVSTR("Path"),			inFile->GetPath() );
//#endif
...
void VSymbolTable::InnerSymbolTable::SetSymbolsEditionState(const Symbols::IFile* inFile, bool inEditionState, CDB4DBaseContext* inContext)
{
...
void VSymbolTable::InnerSymbolTable::GetSymbolIds(const Symbols::IFile* inFile, bool inEditionState, std::vector<sLONG>& outIds, CDB4DBaseContext* inContext)
{
	if( inFile && fDatabase && fSymbolsTable )
	{
		// Let's get a new query object for us to fill out
		...
		VLong fileId( inFile->GetID() );
		query->AddCriteria( fSymbolFileIDField, DB4D_Equal, fileId );
		
		query->AddLogicalOperator( DB4D_And );
		
		VLong editionState(inEditionState);
		query->AddCriteria( fSymbolEditionStateField, DB4D_Equal, editionState );
