...
XBOX::VError VSecurityManager::ValidateBasicAuthentication(const XBOX::VString& inName, const XBOX::VString& inPassword, bool inHashedPassword, bool* outAuthOk, CUAGSession* &outUAGSession, XBOX::VJSGlobalContext* inContext, XBOX::VJSONObject* requestInfo)
{
    *outAuthOk=false;
	outUAGSession = NULL;
	...
	else
	{
		...
		if (inContext != NULL)
		{
			...
			session = fUAGDirectory->OpenSession( inName, inPassword, &rv, &jsContext, inHashedPassword, requestInfo );
		}
		else
		{
			session = fUAGDirectory->OpenSession( inName, inPassword, &rv, NULL, inHashedPassword, requestInfo );
		}
		XBOX::VTask::GetCurrent()->GetDebugContext().EnableUI();
			...
			if (challenge.EqualToString (inChallenge))
			{
				...
				*/
				...
				if (inContext != NULL)
				{
					// sc 22/06/2012, custom JavaScript authentication support
					XBOX::VJSContext jsContext( inContext);
					outUAGSession = fUAGDirectory->OpenSession(user, &error, &jsContext, requestInfo);
				}
				else
				{
					outUAGSession = fUAGDirectory->OpenSession(user, &error, NULL, requestInfo);
				}
...
void DigestCalcHA1(	uBYTE *pszAlg,
					...
					HASHHEX SessionKey)
{
	// \internal: any update to this routine should be backported to Wakanda Shell
	// \see VShellSession.cpp
	HASH				HA1 = {0};
...
XBOX::VError VSecurityManager::ValidateKerberosAuthentication(const XBOX::VString& inAuthData, bool* outAuthOk, XBOX::VString* outAuthData, CUAGSession* &outUAGSession, XBOX::VJSONObject* requestInfo)
{
    *outAuthOk=false;
	outUAGSession = NULL;
    XBOX::VError rv=VE_OK;
    ...
    if(rv==VE_OK && gssAuthorized)
	{
		...
		else
		{
			...
			XBOX::VTask::GetCurrent()->GetDebugContext().DisableUI();
			session=fUAGDirectory->OpenSession(userAtRealm, &rv, nil, requestInfo);
			XBOX::VTask::GetCurrent()->GetDebugContext().EnableUI();
