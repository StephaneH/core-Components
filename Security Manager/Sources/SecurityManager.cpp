...
XBOX::VError VSecurityManager::ValidateBasicAuthentication(const XBOX::VString& inName, const XBOX::VString& inPassword, bool* outAuthOk, CUAGSession* &outUAGSession, XBOX::VJSGlobalContext* inContext, XBOX::VJSONObject* requestInfo)
{
    *outAuthOk=false;
	outUAGSession = NULL;
	...
	else
	{
		...
		if (inContext != NULL)
		{
			...
			session=fUAGDirectory->OpenSession( inName, inPassword, &rv, &jsContext, false, requestInfo);
		}
		else
		{
			session=fUAGDirectory->OpenSession(inName, inPassword, &rv, NULL, false, requestInfo);
		}
		XBOX::VTask::GetCurrent()->GetDebugContext().EnableUI();
