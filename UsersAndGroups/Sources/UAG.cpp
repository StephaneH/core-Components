...
bool UAGDirectory::TakenByListener(const VString& inUserName, const VString& inPassword, VError& err, UAGSession* &session, VJSContext* inJSContext, bool passIsKey)
{
	bool taken = false;
	err = VE_OK;
	session = nil;
...
CUAGSession* UAGDirectory::MakeDefaultSession(VError* outErr, VJSContext* inJSContext, bool fromLogout)
{
	...
	if (newuser != nil)
	{
		session = new UAGSession(this, xid, newuser, fromLogout);
...
CUAGSession* UAGDirectory::OpenSession(const VString& inUserName, const VString& inPassword, VError* outErr, VJSContext* inJSContext, bool passIsKey)
{
	VError err = VE_OK;
	UAGSession* session = nil;

	if (!TakenByListener(inUserName, inPassword, err, session, inJSContext, passIsKey))
	{
		if (err == VE_OK)
		{
