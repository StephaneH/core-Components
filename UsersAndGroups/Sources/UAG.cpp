...
VUAGManager::~VUAGManager()
{
	VErrorBase::UnregisterLocalizer(CUAGManager::Component_Type);
...
{
	fBase = nil;
	fManager = RetainRefCountable(inManager);
	fUserTable = nil;
	fGroupTable = nil;
	fCoupleUGTable = nil;
	fCoupleGGTable = nil;
...
{
	QuickReleaseRefCountable(fDefaultSession);
	...
	CloseDB();
	QuickReleaseRefCountable(fManager);
}


void UAGDirectory::CloseAndRelease()
{
...
class UAGJSRuntimeDelegate : public VObject, public IJSRuntimeDelegate
{
public:
	...
	virtual	VSystemWorkerNamespace*	RetainRuntimeSystemWorkerNamespace ( )
	{
		xbox_assert ( false );
		...
		xJSContext = VJSGlobalContext::Create(xJSDelegate);
		{
			...
			if (managerResourceFolder != nil)
			{
				...
				if (fileJs.Exists())
				{
					...
					jscontext.EvaluateScript(&fileJs, &result,nil);
					/*fBase refCount++ (4)*/				
					...
					jscontext.EvaluateScript("exportUAG", nil, &result, nil);
					if (!result.IsUndefined() && result.IsObject())
					{
						VJSObject objfunc = result.GetObject();
...
/*fBase refCount++ (3)*/
		...
		if (managerResourceFolder != nil)
		{
			...
			if (fileJs.Exists())
			{
				...
				result = jscontext.GetGlobalObject().GetProperty("importUAG");
...
UAGSession* UAGDirectory::BuildSession(VJSObject resultObj, VJSContext* inJSContext, VJSONObject* requestInfo)
{	
	VError err = VE_OK;
	...
	if (user != nil)
	{
		session = new UAGSession(this, xid, user, false, requestInfo);
		if (regularuser)
...
bool UAGDirectory::TakenByListener(const VString& inUserName, const VString& inPassword, VError& err, UAGSession* &session, VJSContext* inJSContext, bool passIsKey, VJSONObject* requestInfo)
{
	bool taken = false;
	err = VE_OK;
	session = nil;
		...
		if (inJSContext != nil)
		{
			...
			if (inJSContext->GetFunction(fLoginListener, funcobj))
			{
				...
				if (fLoginPromote != nil)
				{
					...
					inJSContext->EvaluateScript(L"currentSession()", nil, &resSession, nil );
					currentSession = resSession.GetObjectPrivateData<VJSSession>();
					if (currentSession != nil)
					{
						CUAGThreadPrivilege* privileges = static_cast<CUAGThreadPrivilege*>(inJSContext->GetGlobalObjectPrivateInstance()->GetSpecific('uagX'));
						promoteToken = currentSession->PromoteIntoGroup(fLoginPromote, privileges);
					}
				}
				...
				{
					CUAGThreadPrivilege* privileges = static_cast<CUAGThreadPrivilege*>(inJSContext->GetGlobalObjectPrivateInstance()->GetSpecific('uagX'));
					currentSession->UnPromoteFromToken(promoteToken, privileges);
				}
				...
				if (result2.IsObject())
				{
					...
					if (err == VE_OK)
					{
						session = BuildSession(resultObj, inJSContext, requestInfo);
#if 0
						bool regularuser = false;
						VString sid, sname, sfullname;
						VJSValue val(resultObj.GetProperty("ID"));
...
CUAGSession* UAGDirectory::MakeDefaultSession(bool RealSession, VError* outErr, VJSContext* inJSContext, bool fromLogout, VJSONObject* requestInfo)
{
	UAGSession* session = nil;
	if (RealSession)
	{
		...
		if (newuser != nil)
		{
			session = new UAGSession(this, xid, newuser, fromLogout, requestInfo);
	...
	{
		VTaskLock lock(&fSessionMutex);
		if (fDefaultSession == nil)
		{
			fDefaultSession = dynamic_cast<UAGSession*>(MakeDefaultSession(true, outErr, inJSContext, false, nil));
			if (fDefaultSession != nil)
			{
				fDefaultSession->SetNeedCookie(false);
				fDefaultSession->DontRetainDirectory();
				UAGUser* defaultuser = dynamic_cast<UAGUser*>(fDefaultSession->RetainUser());
				if (defaultuser != nil)
				{
					defaultuser->DontRetainDirectory();
					defaultuser->Release();
				}

			}
		}
		session = RetainRefCountable(fDefaultSession);
	}
...
CUAGSession* UAGDirectory::OpenSession(const VString& inUserName, const VString& inPassword, VError* outErr, VJSContext* inJSContext, bool passIsKey, VJSONObject* requestInfo)
{
	VError err = VE_OK;
	UAGSession* session = nil;

	if (!TakenByListener(inUserName, inPassword, err, session, inJSContext, passIsKey, requestInfo))
	{
		if (err == VE_OK)
		{
			...
			if (user != nil)
			{
				...
				if (passIsKey)
				{
					VString ha1;
					user->GetHA1(ha1);
					if (!inPassword.EqualToString(ha1, true))
						err = VE_UAG_PASSWORD_DOES_NOT_MATCH;
				}
				else
					err = user->ValidatePassword(inPassword);
				if (err == VE_OK)
				{
					session = new UAGSession(this, userid, user, false, requestInfo);
...
CUAGSession* UAGDirectory::OpenSession(VJSObject* inSessionObj, VError* outErr, VJSContext* inJSContext, VJSONObject* requestInfo)
{
	VError err = VE_OK;
	UAGSession* session = BuildSession(*inSessionObj, inJSContext, requestInfo);
...
CUAGSession* UAGDirectory::OpenSession(const XBOX::VString& inUserAtRealm, XBOX::VError* outErr, VJSContext* inJSContext, VJSONObject* requestInfo)
{
	//jmo - todo : Faire qqchose de toto@SOME_REALM !
	return OpenSession(inUserAtRealm, "kerberized", outErr, nil, false, requestInfo);
}


CUAGSession* UAGDirectory::OpenSession(CUAGUser* inUser, VError* outErr, VJSContext* inJSContext, VJSONObject* requestInfo)
{
	VError err = VE_OK;
	UAGSession* session = nil;
	...
	VUUID userid;
	inUser->GetID(userid);
	session = new UAGSession(this, userid, inUser, false, requestInfo);
	err = session->BuildDependences();
	if (err != VE_OK)
	{
		ReleaseRefCountable(&session);
...
CUAGThreadPrivilege* UAGDirectory::NewThreadPrivilege()
{
	return new UAGThreadPrivilege(this);
}


#if VERSIONDEBUG
CComponent* UAGDirectory::Retain(const char* DebugInfo)
{
	return VComponentImp<CUAGDirectory>::Retain(DebugInfo);
}

void UAGDirectory::Release(const char* DebugInfo)
{
	VComponentImp<CUAGDirectory>::Release(DebugInfo);
}
#endif
