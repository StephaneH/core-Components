...
bool UAGSession::BelongsTo(const XBOX::VUUID& inGroupID, CUAGThreadPrivilege* inThreadPrivileges, bool checkNoAdmin)
{
	bool result = false;

	if (fBelongsTo.find(inGroupID) != fBelongsTo.end())
		result = true;
	else if ((inThreadPrivileges != nil) && inThreadPrivileges->BelongsTo(inGroupID))
		result = true;
...
bool UAGSession::BelongsTo(CUAGGroup* inGroup, CUAGThreadPrivilege* inThreadPrivileges, bool checkNoAdmin)
{
	VUUID id;
	inGroup->GetID(id);
	return BelongsTo(id, inThreadPrivileges, checkNoAdmin);
}
...
sLONG UAGSession::PromoteIntoGroup(CUAGGroup* inGroup, CUAGThreadPrivilege* inThreadPrivileges)
{
	sLONG resultToken = 0;
	VUUID groupid;
	inGroup->GetID(groupid);

	if (!BelongsTo(groupid, inThreadPrivileges, false))
	{
		if (inThreadPrivileges != nil)
			resultToken = inThreadPrivileges->PromoteIntoGroup(inGroup);
	}
...
void UAGSession::UnPromoteFromToken(sLONG promotionToken, CUAGThreadPrivilege* inThreadPrivileges)
{
	if (promotionToken != 0)
	{
		if (testAssert(inThreadPrivileges != nil))
		{
			inThreadPrivileges->UnPromoteFromToken(promotionToken);
		}
	}
}
...
UAGThreadPrivilege::UAGThreadPrivilege(UAGDirectory* inDirectory)
{
	fDirectory = RetainRefCountable(inDirectory);
	fNextToken = 0;
}

UAGThreadPrivilege::~UAGThreadPrivilege()
{
	QuickReleaseRefCountable(fDirectory);
}


void UAGThreadPrivilege::BuildSubPromotions(CUAGGroupVector& groups, IDSet* outGroupsAdded)
{
...
sLONG UAGThreadPrivilege::PromoteIntoGroup(CUAGGroup* inGroup)
{
	sLONG resultToken = 0;
	IDSet* groupsAdded;
	VUUID groupid;
...
void UAGThreadPrivilege::UnPromoteFromToken(sLONG promotionToken)
{
	if (promotionToken != 0)
	{
...
bool UAGThreadPrivilege::BelongsTo(const XBOX::VUUID& inGroupID)
{
	bool result = false;

	if (fPromotedTo.find(inGroupID) != fPromotedTo.end())
		result = true;

	return result;
}

bool UAGThreadPrivilege::BelongsTo(CUAGGroup* inGroup)
{
	VUUID id;
	inGroup->GetID(id);
	return BelongsTo(id);
