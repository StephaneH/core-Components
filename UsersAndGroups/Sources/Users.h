...
class UAGUser : public VComponentImp<CUAGUser>
{
	public:
		...
		void DontRetainDirectory()
		{
			QuickReleaseRefCountable(fDirectory);
			fDirectoryRetained = false;
		}
...
class UAGSession : public VComponentImp<CUAGSession>
{
	public:
		...
		virtual bool BelongsTo(const XBOX::VUUID& inGroupID, CUAGThreadPrivilege* inThreadPrivileges, bool checkNoAdmin = true);

		virtual bool BelongsTo(CUAGGroup* inGroup, CUAGThreadPrivilege* inThreadPrivileges, bool checkNoAdmin = true);
		...
		virtual sLONG PromoteIntoGroup(CUAGGroup* inGroup, CUAGThreadPrivilege* inThreadPrivileges);

		virtual void UnPromoteFromToken(sLONG promotionToken, CUAGThreadPrivilege* inThreadPrivileges);
		...
		virtual bool NeedsCookie() const
		{
			return fNeedsCookie;
		}

		void SetNeedCookie(bool x)
		{
			fSessionID.FromString("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
			fNeedsCookie = x;
		}
	...
	protected:
		...
		*/
		...
		bool fIsDefault, fIsFromLogout, fForcedExpired, fNeedsCookie, fDirectoryRetained;
		DataMap fDataMap;
...
class UAGThreadPrivilege : public VComponentImp<CUAGThreadPrivilege>
{
	public:

		UAGThreadPrivilege(UAGDirectory* inDirectory);
		virtual ~UAGThreadPrivilege();

		virtual bool BelongsTo(const XBOX::VUUID& inGroupID);
		virtual bool BelongsTo(CUAGGroup* inGroup);

		virtual sLONG PromoteIntoGroup(CUAGGroup* inGroup); // returns a promotion token or 0 if not promotion was necessary
